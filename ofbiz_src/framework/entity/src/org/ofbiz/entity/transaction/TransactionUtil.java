            ...
            try {
                ...
                if (currentStatus == Status.STATUS_ACTIVE) {
                    ...
                    if (UtilValidate.isNotEmpty(roc)) {
                        Debug.logError("The current transaction is marked for rollback, not beginning a new transaction and aborting current operation; the rollbackOnly was caused by: " + roc.getCauseMessage() + roc.getCauseThrowable(), module);
                        throw new GenericTransactionException(roc.getCauseMessage(), roc.getCauseThrowable());
                    } else {
                        return false;
                    }
                }
