...
public class MacroFormRenderer implements FormStringRenderer {
    ...
    private String encode(String value, ModelFormField modelFormField, Map<String, Object> context) {
        ...
        SimpleEncoder encoder = (SimpleEncoder) context.get("simpleEncoder");
	...
	public void renderDropDownField(Appendable writer, Map<String, Object> context, DropDownField dropDownField) throws IOException {
        ...
        String selectedAll = dropDownField.getSelectedAll();
        if (UtilValidate.isNotEmpty(modelFormField.getWidgetStyle())) {
            className = modelFormField.getWidgetStyle();
            if (modelFormField.shouldBeRed(context)) {
                alert = "true";
        ...
        if (UtilValidate.isNotEmpty(selectedAll)) {
        	...
        	List<String> currentValueListTempt = new ArrayList<String>();
        	for (String currentValueItemA : currentValueList) {
				currentValueListTempt.add(currentValueItemA.trim());
			}
        	currentValueList = currentValueListTempt;
        } else {
        ...
        while (optionValueIter.hasNext()) {
            ...
            if (UtilValidate.isNotEmpty(currentValueList)) {
                ...
                if (UtilValidate.isNotEmpty(selectedAll)){
                	if (currentValueList.contains(optionValue.getKey()) || currentValueList.contains(optionValue.getKey().trim())) {
                        options.append("selected");
                    } else {
                        options.append("");
                    }
        ...
        StringWriter sr = new StringWriter();
        if (UtilValidate.isNotEmpty(selectedAll)) {
        	sr.append("<@renderDropDownFieldSelectedAll ");
        } else {
    ...
    @Override
	public void renderFormatFieldRowOpenRow(Appendable writer, Map<String, Object> context, ModelForm modelForm, ModelFormField modelFormField) throws IOException {
    	String style = modelFormField.getWidgetStyleRow();
    	StringWriter sr = new StringWriter();
        sr.append("<@renderFormatFieldRowOpenRow ");
        sr.append(" style=\"");
        sr.append(style);
        sr.append("\" />");
        executeMacro(writer, sr.toString());
