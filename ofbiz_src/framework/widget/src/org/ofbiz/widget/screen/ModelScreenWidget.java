...
import com.olbius.widget.html.MenuStringRendererFactory;
...
public abstract class ModelScreenWidget extends ModelWidget {
    ...
    /*OLBIUS first render widget dashboard*/
    ...
    public static class Screenlet extends ModelScreenWidget {
        ...
        protected List<ModelScreenWidget> subWidgets;
        protected boolean findOptions = false;
        protected FlexibleStringExpander findOptionsLabelExdr;
        // Icon style
        ...
        public Screenlet(ModelScreen modelScreen, Element screenletElement) {
            ...
            this.findOptions = "true".equals(screenletElement.getAttribute("findOptions"));
            this.findOptionsLabelExdr = FlexibleStringExpander.getInstance(screenletElement.getAttribute("find-option-label"));
            ...
            String strIconStyle = screenletElement.getAttribute("icon-style");
            if (UtilValidate.isNotEmpty(strIconStyle)) {
            	this.iconStyle = strIconStyle;
            }
        }
        
        public boolean hasFindOptions(){
        	return this.findOptions;
        }
        
        public String getFindOptionsLabel(Map<String, Object> context){
        	 String findOptionsLabel = this.findOptionsLabelExdr.expandString(context);
             SimpleEncoder simpleEncoder = (SimpleEncoder) context.get("simpleEncoder");
             if (simpleEncoder != null) {
            	 findOptionsLabel = simpleEncoder.encode(findOptionsLabel);
             }
             return findOptionsLabel;
        }
...
//                    }
    ...
    public static class Menu extends ModelScreenWidget {
        ...
        public void renderWidgetString(Appendable writer, Map<String, Object> context, ScreenStringRenderer screenStringRenderer) throws IOException {
            ...
            if (menuStringRenderer == null) {
                ...
                HttpServletResponse response = (HttpServletResponse) context.get("response");
                if (request != null && response != null) {
                	ModelMenuEnum mmEnum = ModelMenuEnum.valueOf(modelMenu.getModelMenuName());
            		menuStringRenderer = MenuStringRendererFactory.create(request, response, mmEnum);
                }
            }
            // still null, throw an error
            if (menuStringRenderer == null) {
    ...
    public static class Link extends ModelScreenWidget {
        ...
        public String getTarget(Map<String, Object> context) {
            ...
            SimpleEncoder simpleEncoder = context == null ? null : (SimpleEncoder) context.get("simpleEncoder");
            if (simpleEncoder != null) {
        ...
        public void renderWidgetString(Appendable writer, Map<String, Object> context, ScreenStringRenderer screenStringRenderer) throws GeneralException, IOException {
            try {
                ...
                if (UtilValidate.isNotEmpty(expandedPortalPageId)) {
                    ...
                    // get max row
                    Map<String, Object> cond1 = UtilMisc.toMap("portalPageId", portalPage.getString("portalPageId"));
                    Map<String, Object> cond2 = UtilMisc.toMap("portalPageId", portalPage.getString("portalPageId"), "userLoginId", userLoginId);
                    EntityFindOptions opts = new EntityFindOptions();
                    opts.setLimit(1);
                    opts.setMaxRows(1);
                    List<GenericValue> tmpList = delegator.findList("PortalPagePortlet", EntityCondition.makeCondition(cond2), UtilMisc.toSet("rowSeqId"), UtilMisc.toList("rowSeqId DESC"), opts, false);
                    if(UtilValidate.isEmpty(tmpList)){
                    	userHasData = false;
                    	tmpList = delegator.findList("PortalPagePortlet", EntityCondition.makeCondition(cond1), UtilMisc.toSet("rowSeqId"), UtilMisc.toList("rowSeqId DESC"), opts, false);
                    }
                    if (UtilValidate.isNotEmpty(tmpList) && tmpList.get(0).getString("rowSeqId") != null) {
                        iMaxRow = Integer.parseInt(tmpList.get(0).getString("rowSeqId"));
                    }
                } else {
                ...
                if (iMaxRow != 0) {
					EntityCondition preCond = EntityCondition.makeCondition("portalPageId", portalPage.getString("portalPageId"));
					if (userHasData) preCond = EntityCondition.makeCondition(preCond, EntityOperator.AND, EntityCondition.makeCondition("userLoginId", userLoginId));
					else preCond = EntityCondition.makeCondition(preCond, EntityOperator.AND, EntityCondition.makeCondition("userLoginId", null));
					
                    for (int i = 0; i < iMaxRow; i++) {
                        // render begin of row
                        screenStringRenderer.renderPortalPageRowBegin(writer, context, null, null);
                        
                        /*EntityCondition tmpCond2 = null;
                        int length = 0;
						int remain = 0;
						StringBuilder tmp = new StringBuilder();
                        String strRow = String.valueOf(i + 1);
                        length = strRow.length();
                        remain = 5 - length;
                        tmp = new StringBuilder();
                        tmp.append(StringUtils.repeat("0", remain));
                        tmp.append(strRow);
                        String strRownum = tmp.toString();
                        tmpCond2 = EntityCondition.makeCondition("rowSeqId", strRownum);*/
                        
                        // get list columns of current row
                        String strRownum = UtilFormatOut.formatPaddedNumber(i + 1, 5);
                        
                        // get list portal page portlet columns of current row
                        List<GenericValue> listPPPorletColumns = delegator.findList("PortalPagePortlet", EntityCondition.makeCondition(preCond, EntityCondition.makeCondition("rowSeqId", strRownum)), null, UtilMisc.toList("rowSeqId"), null, false);
                        if (UtilValidate.isEmpty(listPPPorletColumns)) {
                        	continue;
                        }
                        for (int j = 0; j < listPPPorletColumns.size(); j++) {
                            GenericValue columnValue = listPPPorletColumns.get(j);
                            String columnSeqId = columnValue.getString("columnSeqId");
                            
                            // FIXME missing colspan in PortalPageColumn
                            // get PortalPageColumn to pass
                            // GenericValue tmpColumn = delegator.findOne("PortalPageColumn", UtilMisc.toMap("portalPageId", portalPage.getString("portalPageId"), "columnSeqId", columnValue.getString("columnSeqId")), false);
                            ...
                            portalPagePortlets = delegator.findList("PortalPagePortletView", EntityCondition.makeCondition(preCond, EntityCondition.makeCondition(UtilMisc.toMap("portalPortletId", columnValue.getString("portalPortletId"), "columnSeqId", columnSeqId, "rowSeqId", strRownum))), null, UtilMisc.toList("rowSeqId"), null, false);
                            ...
                            if (j != listPPPorletColumns.size() - 1) {
                                nextColumnSeqId = listPPPorletColumns.get(j+1).getString("columnSeqId");
                            }
                            
                            // Iterates through the Portlets in the Column
                            //ListIterator <GenericValue>portletsIterator = portalPagePortlets.listIterator();
                            for(int k = 0; k < portalPagePortlets.size();k++) {
                                GenericValue portletValue = portalPagePortlets.get(k);
                                if(!strRownum.equals(portalPagePortlets.get(k).getString("rowSeqId")) || !columnSeqId.equals(portalPagePortlets.get(k).getString("columnSeqId"))){
                                    continue;
                                }
                                ...
                                if (k < portalPagePortlets.size() - 1) {
                                    nextPortletId = portalPagePortlets.get(k + 1).getString("portalPortletId");
                                    nextPortletSeqId = portalPagePortlets.get(k + 1).getString("portletSeqId");
                                }
                        ...
                        // render end of row
                        screenStringRenderer.renderPortalPageRowEnd(writer, context, null, null);
                    }
                } else {
