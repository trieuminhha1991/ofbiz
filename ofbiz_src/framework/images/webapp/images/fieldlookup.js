...
function lookup_popup1(view_name, form_name, viewWidth, viewheight) {
	var obj_lookupwindow = window.open(getViewNameWithSeparator(view_name)
			...
			+ '&id=' + this.id, '_blank', 'width=' + viewWidth + ',height='
			+ viewheight + ',scrollbars=yes,status=no,resizable=yes,top=' + my
...
function lookup_popup2(view_name) {
	...
	var obj_lookupwindow = window.open(getViewNameWithSeparator(view_name)
			...
			+ argString, '_blank',
			'width=900,height=700,scrollbars=yes,status=no,resizable=yes,top='
...
function initiallyCollapse() {
	...
	var i, j, childEle, childElements, ul, slTitleBar, slTitleBars = LOOKUP_DIV
			.getElementsByClassName('screenlet-title-bar');
...
var Lookup = function(options) {
	...
	var zIndexDialog = options.zIndex;//Edit by kieuanhvu
	options = {
	...
	function _init() {
		...
		this.showDescription = (options.showDescription == "true") ? true
				: false;
	...
	function _modifyContainer() {
		...
		if (document.getElementById(options.inputFieldId) != null) {
	...
	function _createAjaxAutoComplete() {
		if (options.ajaxUrl != "") {
			...
			options.ajaxUrl = options.ajaxUrl
					.replace(options.ajaxUrl.substring(0, options.ajaxUrl
							.indexOf(",")), _newInputBoxId);
			if (typeof options.args == "object" && jQuery.isArray(options.args)) {
				var queryArgs = {};
				for (var i = 0; i < options.args.length; i++) {
					var paramName = "parm" + i;
					if (!(/^\s*$/.test(jQuery(options.args[i]).val()))) {
						queryArgs[paramName] = "" + jQuery(options.args[i]).val();
					}
				}
				new ajaxAutoCompleter2(options.ajaxUrl,
						(options.showDescription == "true") ? true : false,
						options.defaultMinLength, options.defaultDelay,
						options.formName, queryArgs);
			} else {
				new ajaxAutoCompleter(options.ajaxUrl,
						(options.showDescription == "true") ? true : false,
						options.defaultMinLength, options.defaultDelay,
						options.formName);
			}
		...
		jQuery
				.ajax({
					...
					success : function(data) {
						//console.log(zIndex);
						if(zIndexDialog){							
							$("#" + _lookupId).parent().css("zIndex", zIndexDialog);
						}
	...
	function _addCloseEventForClickingOnBackgroundLayer() {
		_backgroundCloseClickEvent = function() {
			if (ACTIVATED_LOOKUP && ACTIVATED_LOOKUP == _lookupId) {
				GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).dialogRef
						.dialog("close");
			}
		}
	...
	function _dialogClose() {
		jQuery(".ui-widget-overlay")
				.unbind("click", _backgroundCloseClickEvent)
...
var ButtonModifier = function(lookupDiv) {
	...
	function _modifyCollapseable() {
		...
		jQuery.each(slTitleBars, function(index) {
			...
			childEle.setAttribute('onclick',
					"javascript:toggleScreenlet(this, 'lec"
							+ COLLAPSE_SEQUENCE_NUMBER
							+ "', 'true', 'Expand', 'Collapse');");
			childEle.href = "javascript:void(0);"
			jQuery(slTitleBar).next('div').attr('id',
					'lec' + COLLAPSE_SEQUENCE_NUMBER);
	...
	function _modifySubmitButton() {
		...
		var input = jQuery("#" + lookupDiv + " button[type=submit]").css({
			display : "block"
		});
		...
		$("#lookupSubmitButton").text("");
		$('<i class="icon-search"/>').text('').append("&nbsp;" + txt).appendTo(
				$("#lookupSubmitButton"));
		input.remove();
	}
	...
	function _modifyPagination() {
		// modify nav-pager
		var navPagers = jQuery("#" + lookupDiv + " .pagination a");
		jQuery.each(navPagers, function(navPager) {
			jQuery(navPagers[navPager]).attr(
					"href",
					"javascript:lookupPaginationAjaxRequest('"
							+ encodeURI(jQuery(navPagers[navPager])
									.attr("href")) + "','link')");
		});

		var navPagersSelect = jQuery("#" + lookupDiv + " .pagination select");
		jQuery.each(navPagersSelect, function(navPager) {
			var onChangeEvent = jQuery(navPagersSelect[navPager]).attr(
					"onchange");
			if ((typeof onChangeEvent) == "function") { // IE6/7 Fix
				onChangeEvent = onChangeEvent.toString();
				var ocSub = onChangeEvent.substring(
						(onChangeEvent.indexOf('=') + 3),
						(onChangeEvent.length - 4));
				...
				if (searchPattern.test(ocSub)) {
					...
					navPagersSelect[navPager].onchange = function() {
						lookupPaginationAjaxRequest(spl[0] + this.value
								+ spl[1], 'select');
					};
					...
					navPagersSelect[navPager].onchange = function() {
						lookupPaginationAjaxRequest(ocSub + this.value,
								'select');
					};
				...
				/*
				 * var ocSub =
				 * onChangeEvent.substring((onChangeEvent.indexOf('=') + 1),
				 * (onChangeEvent.length - 1));
	...
	function _modifyResultTable() {
		var resultTable = jQuery("#" + lookupDiv
				+ " #search-results table:first tbody");
																			...
																			var liSub = link
																					.substring(
																							link
																									.lastIndexOf('/') + 1,
																							(link.length));
																			cellChilds[child].href = "javascript:lookupAjaxRequest('"
																					+ liSub
																					+ "&presentation=layer')";
																		}
																	}
...
function lookupFormAjaxRequest(formAction, form) {
	...
	var screenletTitleBar = jQuery("#" + lookupId
			+ " .loading-image");
	jQuery
			.ajax({
				url : formAction,
				...
				beforeSend : function(jqXHR, settings) {
					// Here we append the spinner to the lookup screenlet and it
					// will
					// shown till the ajax request is processed.
					var indicator = screenletTitleBar.find("span.indicator");
					// Check that if spinner is already in execution then don't
					// add new
					// spinner
					if (indicator.length == 0) {
						screenletTitleBar.html(
								"<span class='indicator'><img src='/images/ajax-loader.gif' alt='' /></span>");
					}
				},
...
var obj_caller = (window.opener && window.opener.lookups ? window.opener.lookups[num_id]
		: null);
...
function set_values(value, value2) {
	...
	var showDescription = GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP) ? GLOBAL_LOOKUP_REF
			.getReference(ACTIVATED_LOOKUP).showDescription
			: false;
	if (showDescription) {
		...
		jQuery.ajax({
			...
			success : function(result) {
				...
				if (result.split("ajaxAutocompleteOptions.ftl -->")[1]) {
					setLookDescription(_fieldId, result
							.split("ajaxAutocompleteOptions.ftl -->")[1].trim()
							.split("<!--")[0].trim(), "", "");
				}
			}
		});
