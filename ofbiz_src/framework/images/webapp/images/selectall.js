...
function ajaxAutoCompleter2(areaCsvString, showDescription, defaultMinLength, defaultDelay, formName, otherQueryArgs){
    var areaArray = areaCsvString.replace(/&amp;/g, '&').split(",");
    var numAreas = parseInt(areaArray.length / 3);
    var queryArgs = {};
    if (otherQueryArgs != "undefined") {
...
function submitFormDisableSubmits(form) {
    ...
    for (var i=0;i<form.length;i++) {
        ...
        if (formel.type == "submit") {
            ...
            var formName = form.getAttribute("name");//origin: var formName = form.name            
...
function expandAll(bool) {
	jQuery(".olbius-extra").each(function() {
		if(bool){
			$(this).attr("class","widget-box olbius-extra");
			$(".icon-chevron-down").attr("class","icon-chevron-up");
		}else{
			$(this).attr("class","widget-box olbius-extra collapsed");
			$(".icon-chevron-up").attr("class","icon-chevron-down");
		}
		var titleBar = $(this).children('.widget-header'), body = $(this).children('.widget-body-inner');
        if (titleBar.children().length > 0 && body.is(':visible') != bool) {
            toggleCollapsiblePanel(titleBar.find('a'), body.attr('id'), 'expand', 'collapse');
        }
	});
