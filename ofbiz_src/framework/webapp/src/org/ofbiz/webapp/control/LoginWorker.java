...
public class LoginWorker {
    ...
    public static GenericValue checkLogout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin != null) {
            ...
            if (!hasBasePermission(userLogin, request) || isFlaggedLoggedOut(userLogin)) {
                ...
                request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(resourceWebapp, "loginservices.user_has_logout", UtilHttp.getLocale(request)));
                request.setAttribute("login", "FALSE"); // thangnv
    ...
    public static String login(HttpServletRequest request, HttpServletResponse response) {
        ...
        // new code by CT: check a user logged in
        GenericValue userLogin1 = checkLogout(request, response);
        if (userLogin1 != null) {
...
//            String visitId = VisitHandler.getVisitId(session);
        ...
        if (ModelService.RESPOND_SUCCESS.equals(result.get(ModelService.RESPONSE_MESSAGE))) {
            ...
            if (userLogin != null && "Y".equals(userLogin.getString("requirePasswordChange"))) {
            	request.setAttribute("_REQ_PASS_CHANGE", "Y"); // thangnv
                return "requirePasswordChange";
            }
    ...
    public static String doMainLogin(HttpServletRequest request, HttpServletResponse response, GenericValue userLogin, Map<String, Object> userLoginSession) {
        ...
        request.setAttribute("_LOGIN_PASSED_", "TRUE");
        request.setAttribute("partyId", userLogin.getString("partyId"));
   ...
   /* public static String checkExternalLoginKey(HttpServletRequest request, HttpServletResponse response) {
    ...
    public static String checkExternalLoginKey(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
 			userLogin = OlbiusLogin.checkExternalLoginKey(request);
 			 ...
 			 Debug.logWarning("Could not find userLogin for external login key", module);
 		}
    ...
    public static boolean hasBasePermission(GenericValue userLogin, HttpServletRequest request) {
    	return Webapp.hasBasePermission(userLogin, request);
    }
