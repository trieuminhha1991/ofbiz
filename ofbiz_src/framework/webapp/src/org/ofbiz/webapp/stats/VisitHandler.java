...
public class VisitHandler {
	...
	public static void setUserLogin(HttpSession session, GenericValue userLogin, boolean userCreated) {
		...
		if (visit != null) {
			...
			String posTerminalId = (String) session.getAttribute("posTerminalId");
			if (UtilValidate.isNotEmpty(posTerminalId)) {
				visit.set("posTerminalId", posTerminalId);
			}
			try {
				visit.store();
			} catch (GenericEntityException e) {
			...
			if (visit == null) {
				synchronized (session) {
					...
					if (visit == null) {
						...
						if (delegator == null) {
							Debug.logError(
									"Could not find delegator with delegatorName [" + delegatorName
											+ "] in session, or a delegator attribute in the ServletContext, not creating Visit entity",
									module);
						} else {
							...
							if (initialRequest != null)
								visit.set("initialRequest", initialRequest.length() > 250
										? initialRequest.substring(0, 250) : initialRequest);
							if (initialReferrer != null)
								visit.set("initialReferrer", initialReferrer.length() > 250
										? initialReferrer.substring(0, 250) : initialReferrer);
							if (initialUserAgent != null)
								visit.set("initialUserAgent", initialUserAgent.length() > 250
										? initialUserAgent.substring(0, 250) : initialUserAgent);
							...
							if (visitor != null) {
								...
								// sometimes these values get stale, so check it
								// before we use it
								try {
									GenericValue checkVisitor = delegator.findOne("Visitor", false, "visitorId",
											visitorId);
				...
				synchronized (session) {
					...
					if (visitor == null) {
						...
						if (delegator == null) {
							...
							// first try to get the current ID from the visitor
							// cookie
							...
							if (UtilValidate.isEmpty(cookieVisitorId)) {
								// no visitor cookie? create visitor and send
								// back cookie too
								...
								try {
									...
									if (visitor == null) {
										// looks like we have an ID that doesn't
										// exist in our database, so we'll
										// create a new one
										visitor = delegator.makeValue("Visitor");
						...
						if (visitor != null) {
							// we got one, and it's a new one since it was null
							// before
							session.setAttribute("visitor", visitor);

							// create the cookie and send it back, this may be
							// done over and over, in effect frequently
