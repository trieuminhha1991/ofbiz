 ...
 */
public class ExportExcelEventHandler implements EventHandler {

    public static final String module = ExportExcelEventHandler.class.getName();
    ...
    public String invoke(Event event, RequestMap requestMap, HttpServletRequest request, HttpServletResponse response) throws EventHandlerException {
        ...
        String eventPathParent = "com.olbius.webapp.event.ExportExcelEvents";
        Class<?> eventClassParent = this.eventClassMap.get(eventPathParent);

        if (eventClassParent == null) {
            synchronized (this) {
            	eventClassParent = this.eventClassMap.get(eventPathParent);
                if (eventClassParent == null) {
                    try {
                        ClassLoader loader = Thread.currentThread().getContextClassLoader();
                        eventClassParent = loader.loadClass(eventPathParent);
                    } catch (ClassNotFoundException e) {
                        Debug.logError(e, "Error loading class with name: " + eventPathParent + ", will not be able to run event...", module);
                    }
                    if (eventClassParent != null) {
                        eventClassMap.put(eventPathParent, eventClassParent);
                    }
                }
            }
        }
        ...
        request.setAttribute("eventPath", event.path);
        Object[] params = new Object[] {request, response};

        return invoke(event.path, eventClassParent, eventClass, paramTypes, params);
    }

    private String invoke(String eventPath, Class<?> eventClassParent, Class<?> eventClass, Class<?>[] paramTypes, Object[] params) throws EventHandlerException {
        if (eventClass == null) {
        ...
        try {
            Method m = eventClassParent.getMethod("exportExcel", paramTypes);
            String eventReturn = null;
            if(Modifier.isStatic(m.getModifiers())) {
            	eventReturn = (String) m.invoke(null, params);
            }else {
            	eventReturn = (String) m.invoke(eventClassParent.newInstance(), params);
            }
