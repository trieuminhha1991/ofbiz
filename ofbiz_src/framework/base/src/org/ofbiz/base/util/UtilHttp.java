...
public class UtilHttp {
    ...
    public static Locale getLocale(HttpServletRequest request, HttpSession session, Object appDefaultLocale) {
        ...
        if (localeObject == null) {
        	if ("true".equals(UtilProperties.getPropertyValue("general.properties", "locale.is.fixxed"))) {
        		localeObject = UtilProperties.getPropertyValue("general.properties", "locale.fixxed");
			} else {
    ...
    public static void setModule(HttpServletRequest request, String module) {
        HttpSession session = request.getSession();
    	session.setAttribute("module", module);
    }
    
    public static String getModule(HttpServletRequest request) {
    	final String MODULE = "HR";
    	HttpSession session = request.getSession();
        ...
        if (module == null) {
            ...
            if (userLogin != null) {
            	if(userLogin.get("lastModule") == null) {
            		module = MODULE;
            	}else {
            		module = userLogin.get("lastModule");
            	}
            }
            session.setAttribute("module", module);
        }
    ...
    public static void setCurrentOrg(HttpServletRequest request, String currentOrg) {
        HttpSession session = request.getSession();
    	session.setAttribute("currentOrg", currentOrg);
    }
    
    public static String getCurrentOrg(HttpServletRequest request) {
    	String ORG = UtilProperties.getPropertyValue("general", "ORGANIZATION_PARTY");
    	HttpSession session = request.getSession();
        ...
        if (currentOrg == null) {
            ...
            Map<String, String> userLogin = UtilGenerics.cast(session.getAttribute("userLogin"));
            if (userLogin != null) {
            	if(userLogin.get("lastOrg") == null) {
            		org = ORG;
            	}else {
            		org = userLogin.get("lastOrg");
            	}
            }
