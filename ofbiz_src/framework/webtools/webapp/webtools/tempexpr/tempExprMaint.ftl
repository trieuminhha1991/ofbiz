...
<h2 class="blue smaller">${title}</h2>
</div>
...
<#if temporalExpression?has_content>
  ...
  <table class="basic-table" cellspacing="0">
    <tr>
      <td class="label arrowed">${uiLabelMap.TemporalExpressionId}</td>
      <td>${temporalExpression.tempExprId}</td>
    </tr>
    <tr>
      <td class="label arrowed">${uiLabelMap.TemporalExpressionType}</td>
      <td>${uiLabelMap.get("TemporalExpression_" + temporalExpression.tempExprTypeId)}</td>
    </tr>
  <#if !"INTERSECTION.UNION.DIFFERENCE.SUBSTITUTION"?contains(temporalExpression.tempExprTypeId)>
    <tr>
      <td class="label arrowed">${uiLabelMap.CommonDescription}</td>
  ...
  <#else>
    ...
    <#elseif "INTERSECTION.UNION.DIFFERENCE.SUBSTITUTION"?contains(temporalExpression.tempExprTypeId)>
      ...
      <#if "INTERSECTION.UNION"?contains(temporalExpression.tempExprTypeId)>
        <tr>
          <td class="label arrowed">${uiLabelMap.TemporalExpressionInclude}</td>
      ...
      <#else>
        ...
        <#if !hasExclude>
          <tr>
            <td class="label arrowed">${uiLabelMap.TemporalExpressionExclude}</td>
        ...
        <#if !hasSubstitution && temporalExpression.tempExprTypeId == "SUBSTITUTION">
          <tr>
            <td class="label arrowed">${uiLabelMap.TemporalExpression_SUBSTITUTION}</td>
    ...
    <#if !"INTERSECTION.UNION.DIFFERENCE.SUBSTITUTION"?contains(temporalExpression.tempExprTypeId)>
        <tr>
          <td>&nbsp;</td>
          <td><button class="btn btn-purple btn-small" type="submit" name="submitBtn" >
          <i class="icon-save"></i>
...
<#macro CreateForm expressionTypeId="" formContents=NullMacro>
  <form name="${expressionTypeId}" method="post" action="<@ofbizUrl>createTemporalExpression</@ofbizUrl>">
    ...
    <table class="basic-table" cellspacing="0">
      ...
      <tr><td colspan="2" class="blue smaller lighter" style="font-size:14px"><h3>${headingText}</h3></td></tr>
      <tr>
        <td class="label arrowed">${uiLabelMap.TemporalExpressionId}</td>
...
<#macro CreateExprAssocForm formName="" exprAssocType="">
  <form name="${formName}" method="post" action="<@ofbizUrl>createTemporalExpressionAssoc</@ofbizUrl>">
    ...
    <td><button style="margin-top: -32px;margin-left: 30px;" class="btn btn-purple btn-small" type="submit" name="submitBtn">
        <i class="icon-save"></i>
         ${uiLabelMap.CommonSave}
        </button>
  </form>
