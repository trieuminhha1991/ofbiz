...
under the License.
-->
    <h4 class="blue">${uiLabelMap.WebtoolsDataFileMessage1}.</h4>
    </div>
    <br />
    <#if security.hasPermission("DATAFILE_MAINT", session)>
      <form method="post" action="<@ofbizUrl>viewdatafile</@ofbizUrl>">
        <table class="basic-table" cellspacing="0">
          <tr>
            <td class="olbius-label">${uiLabelMap.WebtoolsDataDefinitionFileName}</td>
            <td><input name="DEFINITION_LOCATION" type="text" size="60" value="${parameters.DEFINITION_LOCATION?if_exists}" /></td>
            <td><span class="olbius-label">${uiLabelMap.WebtoolsDataIsUrl}</span><input type="checkbox" name="DEFINITION_IS_URL"<#if parameters.DEFINITION_IS_URL?has_content> checked="checked"</#if> /></td>
          </tr>
          <tr>
            <td class="olbius-label">${uiLabelMap.WebtoolsDataDefinitionName}</td>
            <td>
          ...
          <tr>
            <td class="olbius-label">${uiLabelMap.WebtoolsDataFileName}</td>
            <td><input name="DATAFILE_LOCATION" type="text" size="60" value="${parameters.DATAFILE_LOCATION?if_exists}" /></td>
            <td><span class="olbius-label">${uiLabelMap.WebtoolsDataIsUrl}</span><input type="checkbox" name="DATAFILE_IS_URL"<#if parameters.DATAFILE_IS_URL?has_content> checked="checked"</#if> /></td>
          </tr>
          <tr>
            <td class="olbius-label">${uiLabelMap.WebtoolsDataSaveToFile}</td>
            <td><input name="DATAFILE_SAVE" type="text" size="60" value="${parameters.DATAFILE_SAVE?if_exists}"/></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td class="olbius-label">${uiLabelMap.WebtoolsDataSaveToXml}</td>
            <td><input name="ENTITYXML_FILE_SAVE" type="text" size="60" value="${parameters.ENTITYXML_FILE_SAVE?if_exists}" /></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td><button type="submit" class="btn btn-small btn-purple">
            <i class="icon-ok"></i>
    ...
    <#macro displayrecords records>
      ...
      <#if dataFile?has_content && modelDataFile?has_content && (!parameters.ENTITYXML_FILE_SAVE?has_content || parameters.ENTITYXML_FILE_SAVE.length() == 0) && (parameters.DATAFILE_SAVE == null || parameters.DATAFILE_SAVE.length() == 0)>
        ...
        <table class="basic-table" cellspacing="0">
          ...
          <tr>
            <td class="olbius-label">Description</td>
