...
context.sqlCommand = context.request.getParameter("sqlCommand");
...
if (sqlCommand && selGroup) {
    ...
    try {
        ...
        if (sqlCommand.toUpperCase().startsWith("SELECT")) {
			//preprocess sqlCommand with info limit
			if (rowLimit != -1 && sqlCommand.toUpperCase().indexOf("LIMIT") == -1) {
				int indexOfSemiColon = sqlCommand.lastIndexOf(";");
				if (indexOfSemiColon > -1) {
					sqlCommand = sqlCommand.substring(0, indexOfSemiColon) + " limit " + rowLimit + ";";
				} else {
					sqlCommand += " limit " + rowLimit;
				}
			}
		...
		try {
			...
			TransactionUtil.rollback(true, "Failure in operation, rolling back transaction", exc);
		} catch (GenericEntityException e2) {
			String module = "EntitySQLProcessor.groovy";
