...
if ('PURCHASE_ORDER'.equals(shoppingCart.getOrderType())) {
	...
	customerPartyRelList = delegator.findByAnd("PartyRelationship", ["partyIdFrom" : customerPartyId, "roleTypeIdFrom" : "CHILD_MEMBER", "roleTypeIdTo" : "PARENT_MEMBER"], null, false);
	if (customerPartyRelList) {
		for (customerPartyRelItem in customerPartyRelList) {
			customerPartyRoleItem = delegator.findByAnd("PartyRole", ["partyId" : customerPartyRelItem.partyIdTo, "roleTypeId" : "CUSTOMER_GROUP"], null, false);
			if (customerPartyRoleItem) {
				customerPartyIds.add("" + customerPartyRelItem.partyIdTo);
			}
		}
	}
	...
	if ((customerPartyRelList != null) && (customerPartyIds.size() > 0)) {
		...
		agreementCondition = EntityCondition.makeCondition([
			...
			EntityCondition.makeCondition('partyIdFrom', EntityOperator.IN, customerPartyIds)
		], EntityOperator.AND);
	} else {
		// code old
	    // the agreement for a sales order is from the customer to us
