...
if (security.hasPermission("DELYS_ORDER_LIST", session)) {
		...
		if (parameters.resultService.responseMessage == "success") {
			if (isFilterProductStoreId) {
				...
				orderHeaderList = state.getListOrdersPurchaseDis(facilityId, filterDate, delegator, userLogin, parameters.resultService.listAllStore, parameters.productStoreId);
			} else {
				orderHeaderList = state.getListOrdersPurchaseDis(facilityId, filterDate, delegator, userLogin, parameters.resultService.listAllStore, null);
			}
		}
