...
updatePODeliveryInfoWrapper = new HtmlFormWrapper("component://delys/widget/sales/OrderForms.xml", "UpdateDeliveryScheduleInformation", request, response);
...
estimatedReadyDate = "";
if (orderId) {
	listItems = delegator.findByAnd("OrderItem", ["orderId" : orderId], null, false);
	for (item in listItems) {
		if (item.getString("estimatedDeliveryDate")) {
			estimatedReadyDate = item.getString("estimatedDeliveryDate");
			break;
		}
	}
}
context.estimatedReadyDate = estimatedReadyDate;
