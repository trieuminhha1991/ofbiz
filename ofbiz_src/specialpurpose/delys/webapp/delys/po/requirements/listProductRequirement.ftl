...
<#assign columnlistProduct="
				 ...
				 { text: '${uiLabelMap.LogProductId}', dataField: 'productId', width: '150', align: 'center', editable: false, pinned: true},
				 { text: '${uiLabelMap.ProductName}', dataField: 'internalName', align: 'center', width: '150', editable: false },
				 { text: '${uiLabelMap.LogATPTotal}', dataField: 'ATP', align: 'center', width: '100', editable: false,
					 cellsrenderer: function(row, column, value){
                    	 if (value){
				 ...
				 { text: '${uiLabelMap.LogQOHTotal}', dataField: 'QOH', align: 'center', width: '100', editable: false,
					 cellsrenderer: function(row, column, value){
                    	 if (value){
                    		 return '<span style=\"text-align: right\">' + value.toLocaleString(value) +'</span>';
                    	 }
					 ...
					 cellbeginedit: function (row, datafield, columntype) {
						 ...
						 $.ajax({
								...
								success : function(data) {
									listUomTranfer = data['listUomTranfer'];
						        }
						 });
					 },
					 initeditor: function (row, cellvalue, editor) {
						 editor.jqxDropDownList({ source: listUomTranfer, displayMember: 'description', valueMember: 'uomId', placeHolder: '${StringUtil.wrapString(uiLabelMap.LogPleaseSelect)}',
						 });
					 },
				 },
				 "/>
...
<@jqGrid selectionmode="checkbox" idExisted="true" filtersimplemode="true" width="890" viewSize="5" pagesizeoptions="['5', '10', '15', '20', '25', '30', '50', '100']" id="jqxgridProduct" dataField=dataFieldProduct columnlist=columnlistProduct 
	clearfilteringbutton="true" showtoolbar="true" addrow="false" filterable="true" editable="true" customTitleProperties="LogListProductToRequest" rowselectfunction="rowselectfunction(event);"
	url="jqxGeneralServicer?sname=JQGetListProductTotalByProduct" editmode="click" bindresize="false" jqGridMinimumLibEnable="false" autoheight="false" height="275" otherParams="qtyUomIds:S-getProductPackingUoms(productId)<listPackingUoms>" offmode="true"/>
...
<script>  
		...
		if(checkCreateRequirementOrUpdate == 0){
			...
			if(validate != false){
				if(selectedIndexs.length == 0){
					...
					createRequirementPurchaseOrderToPO(selectedIndexs);
				}
			}
		}
	...
	function createRequirementPurchaseOrderToPO(selectedIndexs){
		bootbox.confirm("${uiLabelMap.DAAreYouSureCreate}",function(result){ 
			if(result){	
				...
				row = { 
						requirementTypeId:"PURCHASING_ORDER_REQ", 
				...
				$("#jqxgirdRequirement").jqxGrid('addRow', null, row, "first");
	...
	function showPopupDetail(requirementId){
		...
		var tmpS = $("#jqxgirdRequirement").jqxGrid('source');
