...
<#assign columnlist="{ text: '${uiLabelMap.accProductPromoId}', datafield: 'productPromoId', width: '30%', editable: false, 
					 ...
					 { text: '${uiLabelMap.SequenceId}', width: '20%', datafield: 'sequenceNum',columntype : 'numberinput',createeditor : function(row,column,editor){
					 	editor.jqxNumberInput({width: '210px',height : '25px',digits : 15,spinButtons : false,min : 0 ,max  :999999999999999,decimalDigits : 0});
					 }},
					 "/>
<@jqGrid filtersimplemode="false" addrefresh="true" addType="popup" dataField=dataField columnlist=columnlist clearfilteringbutton="true" showtoolbar="true" addrow="true" deleterow="true" alternativeAddPopup="alterpopupWindow" editable="true"
		 ...
		 editColumns="agreementId[${parameters.agreementId}];agreementItemSeqId[${parameters.agreementItemSeqId}];productPromoId;fromDate(java.sql.Timestamp);thruDate(java.sql.Timestamp);sequenceNum"
		 addColumns="agreementId[${parameters.agreementId}];agreementItemSeqId[${parameters.agreementItemSeqId}];productPromoId;fromDate(java.sql.Timestamp);thruDate(java.sql.Timestamp);sequenceNum"		 
		 />
