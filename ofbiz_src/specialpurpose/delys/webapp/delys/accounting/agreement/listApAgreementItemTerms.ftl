...
<#assign columnlist="{ text: '${uiLabelMap.termTypeId}', datafield: 'termTypeId', editable: false, width: 150,
					 ...
					 { text: '${uiLabelMap.fromDate}', datafield: 'fromDate',cellsformat: 'dd/MM/yyyy', columntype: 'template', width: 150,editable : false,
                     	createeditor: function (row, column, editor) {
                     		...
                     		editor.jqxDateTimeInput({ width: '150px', height: '25px',  formatString: 'yyyy-MM-dd hh:mm:ss' ,allowNullDate : true,value : (data.fromDate !=null)?data.fromDate:null});
                     	},validation : function(cell,value){
							var data = $(\"#jqxgrid\").jqxGrid(\"getrowdata\",cell.row);
							if(data.thruDate <= value || value < (new Date())){
					 ...
					 "/>
<@jqGrid filtersimplemode="false" addType="popup" dataField=dataField columnlist=columnlist clearfilteringbutton="true" showtoolbar="true" addrow="true" deleterow="true" alternativeAddPopup="alterpopupWindow" editable="true" addrefresh="true"
