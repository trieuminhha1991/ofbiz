...
<#assign columnlist="{ text: '${uiLabelMap.SequenceId}', datafield: 'agreementItemSeqId', editable: false, width: '10%',
					 ...
					 { text: '${uiLabelMap.FormFieldTitle_agreementText}', width: '30%', datafield: 'agreementText',cellsrenderer: function (row, column, value) {
        					var regex = /(<([^>]+)>)/ig;
       					 return '<span>' + value.replace(regex, '') + '</span>';
       				  }},
					 { text: '${uiLabelMap.FormFieldTitle_agreementImage}', width: '20%',datafield: 'agreementImage',cellsrenderer : function(row){
					 	var data = $('#jqxgrid').jqxGrid('getrowdata',row);
