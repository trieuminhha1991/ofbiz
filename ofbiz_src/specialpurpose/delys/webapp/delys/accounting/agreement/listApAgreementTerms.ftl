...
<#assign columnlist="{ text: '${uiLabelMap.termTypeId}', datafield: 'termTypeId',filtertype : 'checkedlist',editable: false, width: 150,
					 	cellsrenderer: function(row, colum, value){
					 		...
					 		if(ttData && ttData.length > 0 ){
					 			var filterBox = new $.jqx.dataAdapter(ttData,{autoBind : true});
						 		records = filterBox.records;
						 		records.splice(0, 0, '(${StringUtil.wrapString(uiLabelMap.filterselectallstring)})');
					 		}
							widget.jqxDropDownList({filterable  :true,dropDownHeight : 200,dropDownWidth : 300,source: records, displayMember: 'description', valueMember: 'termTypeId'});
					 	}
					 },
					 { text: '${uiLabelMap.agreementItemSeqId}', datafield: 'agreementItemSeqId', editable: false, width: 180},
					 { text: '${uiLabelMap.invoiceItemTypeId}', datafield: 'invoiceItemTypeId', columntype: 'template',filtertype : 'checkedlist', width: 180, 
					 	createeditor: function (row, column, editor) {
                            editor.jqxDropDownList({width: 180, dropDownWidth: 300, source: iitData, displayMember:\"description\", valueMember: \"invoiceItemTypeId\",
                      ...
                      cellsrenderer: function(row, colum, value){
				 			...
				 			var records = [];
				 			if(iitData && iitData.length  > 0){
				 				var filterBox = new $.jqx.dataAdapter(iitData,{autoBind : true});
						 		records = filterBox.records;
						 		records.splice(0, 0, '(${StringUtil.wrapString(uiLabelMap.filterselectallstring)})');
				 			}
							widget.jqxDropDownList({source: records,filterable : true, displayMember: 'description', valueMember: 'invoiceItemTypeId'});
					 	}
					 },
