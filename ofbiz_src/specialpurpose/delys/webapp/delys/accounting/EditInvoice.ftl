...
<div class="widget-box transparent no-bottom-border">
	...
	<div class="widget-body">
		<div class="row-fluid">
			...
			<div class="span12"><div style="position : absolute;right : 180px;"><button type="submit" class="btn btn-primary btn-small" id="updateButton" name="updateButton"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button></div></div>
		</div>
	</div>			
</div>			
...
<script type="text/javascript">
	...
	var action = (function(){
		var initElement = function(){
			...
			<#if invoice?exists&& (invoice.invoiceTypeId == 'SALES_INVOICE' || invoice.invoiceTypeId == 'PURCHASE_INVOICE')>
				$('#roleTypeId').jqxDropDownList({theme : theme,filterable : true,source : listRole,displayMember : 'description',valueMember : 'roleTypeId',width : '225px',height : '25px'});
			</#if>
		}
		...
		var initRules = function(){
			$('#editInvoiceId').jqxValidator({
				rules : [
					{ input: '#duaDate', message: '${StringUtil.wrapString('Yêu cầu ngày xử lý phải sau ngày tạo hóa đơn'?default(''))}', action: 'change,close', rule: function(){
