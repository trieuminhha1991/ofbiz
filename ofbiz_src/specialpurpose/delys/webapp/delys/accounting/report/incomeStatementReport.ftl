...
<div style="margin-top:5px;margin-bottom:5px;">
	...
	|<a href="IncomeStatementReport.pdf?organizationPartyId=company">>>PDF</a>
	...
	<form action="FinancialStatement" method="POST" name="BalanceSheetReport" id="BalanceSheetReport">
		...
		<input type="hidden" id="reportTypeId" name="reportTypeId" value="9001"/>
		<input type="hidden" id="mType" name="mType" value="incomeStatementFS"/>
	</form>
</div>
<script type="text/javascript">
        $(document).ready(function () {    
        	$.jqx.theme = 'olbius';        
        	<#if context.jsonData?has_content>
        	    var data = ${StringUtil.wrapString(context.jsonData?if_exists)}
        	<#else>
        	    var data = [];
        	</#if>
            // prepare the data
            ...
            $("#treeGrid").jqxTreeGrid(
            {
                ...
                [
                  ...
                  { text: '${context.strCurrentYear?if_exists}', dataField: "value1", cellsAlign: "right", align: 'center', width: '20%', cellsrenderer:
					 	function(row, colum, value){
					 		var data = $('#treeGrid').jqxTreeGrid('getRow', row);
					 		var tmpCurr = formatcurrency(data.value1,'${currencyUomId}');
					 		return "<span>" + tmpCurr + "</span>";
					 	}},
                  { text: '${context.strPreviousYear?if_exists}', dataField: "value2", cellsAlign: "right", align: 'center', width: '20%', cellsrenderer:
					 	function(row, colum, value){
					 		var data = $('#treeGrid').jqxTreeGrid('getRow', row);
            ...
            $(window).bind('resize', function() {
                ...
                if($('#sidebar').css("display") != "none"){
                    ...
                    if($('#sidebar').hasClass("menu-min") != null){
                        $('#treeGrid').jqxTreeGrid({ width: $(window).width() - $('#sidebar').width() - 40 });                     
                    }else{
                        $('#treeGrid').jqxTreeGrid({ width: $(window).width() - $('#sidebar').width() - 40 });
                    }
                    // END COMMENT
                }else{
                    $('#treeGrid').jqxTreeGrid({ width: tmpWidth });
                }
