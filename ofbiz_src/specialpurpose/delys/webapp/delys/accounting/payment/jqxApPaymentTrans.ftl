<script>
	...
	<#assign roleTypes = delegator.findList("RoleType", null, null, null, null, true) />
	...
	<#assign glAccountOACs = delegator.findList("GlAccountOrganizationAndClass", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("organizationPartyId", "company"), null, null, null, true) />
	...
	var fixedAssetTypeData = [
		<#list fixedAssetTypeList as fixedAssetType>
			...
			{
				description: "${fixedAssetType.fixedAssetTypeId}",
...
<#assign columnlisttran="{ text: '${uiLabelMap.acctgTransId}', dataField: 'acctgTransId', width: 200, editable: false,
					 ...
					 { text: '${uiLabelMap.FormFieldTitle_acctgTransTypeId}', dataField: 'acctgTransTypeId', width: 200,columntype: 'dropdownlist',filtertype: 'checkedlist',
						...
						createfilterwidget: function (column, columnElement, widget) {
							var filterBoxAdapter2 = new $.jqx.dataAdapter(acctgTransTypesData, {autobind: true});
				   			...
				   			widget.jqxDropDownList({ theme: theme, source: uniqueRecords2, displayMember: 'description', valueMember: 'acctgTransTypeId', width: '200', dropDownWidth: 300, dropDownHeight: 300});
				   		},
						createeditor: function (row, column, editor) {
                            editor.jqxDropDownList({ theme: theme, source: acctgTransTypesData, displayMember: 'description', valueMember: 'acctgTransTypeId', width: '200', dropDownWidth: 300, dropDownHeight: 300, height: '25'});
                        },
						cellbeginedit: beginedit
					 },
					 { text: '${uiLabelMap.glFiscalTypeId}', dataField: 'glFiscalTypeId', width: 200, columntype: 'dropdownlist',filtertype: 'checkedlist',
						...
						createfilterwidget: function (column, columnElement, widget) {
							var filterBoxAdapter2 = new $.jqx.dataAdapter(glFiscalTypesData, {autobind: true});
				   			var uniqueRecords2 = filterBoxAdapter2._source;
				   			uniqueRecords2.splice(0, 0, '(${StringUtil.wrapString(uiLabelMap.filterselectallstring)})');
				   			widget.jqxDropDownList({ theme: theme, source: uniqueRecords2, displayMember: 'description', valueMember: 'glFiscalTypeId', width: '200', dropDownWidth: 300});
				   		},
						createeditor: function (row, column, editor) {
                            editor.jqxDropDownList({ theme: theme, source: glFiscalTypesData, displayMember: 'description', valueMember: 'glFiscalTypeId', width: '200', dropDownWidth: 300, height: '25'});
                        },
						cellbeginedit: beginedit
					 },
					 { text: '${uiLabelMap.accInvoiceId}', dataField: 'invoiceId', width: 200, columntype: 'template',
						cellsrenderer: function (row, columns, value, defaulthtml, columnproperties, rowdata) {
							var x = value;
							if(rowdata.isPosted == 'Y'){
								return '<div class=\"jqx-cell-disable\"><span><a title=' + id + ' href=' +'/accounting/control/invoiceOverview?invoiceId=' + id + '>' + x + '</a></span></div>'
							}
							return '<span> <a title=' + id + ' href=' +'/accounting/control/invoiceOverview?invoiceId=' + id + '>' + x + '</a> </span>';
						},
					 ...
					 { text: '${uiLabelMap.paymentId}', dataField: 'paymentId', width: 200,columntype: 'template',
						cellsrenderer: function (row, columns, value, defaulthtml, columnproperties, rowdata) {
							var x = value;
							if(rowdata.isPosted == 'Y'){
								return '<div class=\"jqx-cell-disable\"><span><a title=' + id + ' href=' +'/accounting/control/editPayment?paymentId=' + id + '>' + x + '</a></span></div>'
							}
							return '<span> <a title=' + id + ' href=' +'/accounting/control/editPayment?paymentId=' + id + '>' + x + '</a> </span>';
						},
						createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
					 ...
					 { text: '${uiLabelMap.WorkEffortId}', dataField: 'workEffortId', width: 200, columntype: 'template',
						cellsrenderer: function (row, columns, value, defaulthtml, columnproperties, rowdata) {
							var x = value;
							if(rowdata.isPosted == 'Y'){
					 ...
					 { text: '${uiLabelMap.accPostTransaction}', width: 150, dataField:'isPosted',columntype: 'dropdownlist',filtertype: 'checkedlist',
                        ...
                        createfilterwidget: function (column, columnElement, widget) {
                        	var arr = [{description: '${uiLabelMap.accPostted}', value : 'Y'},{description: '${uiLabelMap.accNonePost}', value : 'N'}];
				   			...
				   			var uniqueRecords2 = filterBoxAdapter2._source;
							uniqueRecords2.splice(0, 0, '(${StringUtil.wrapString(uiLabelMap.filterselectallstring)})');
                            widget.jqxDropDownList({ theme: theme, source: uniqueRecords2, displayMember: 'description', valueMember: 'value', width: '200', dropDownWidth: 300});
				   		},
					 	cellbeginedit: beginedit
					 },
...
<@jqGrid filtersimplemode="true" addType="popup" dataField=dataField columnlist=columnlisttran clearfilteringbutton="true" 
	     showtoolbar="true" customTitleProperties="${uiLabelMap.AccountingTransactions}" isShowTitleProperty="true"
		 url="jqxGeneralServicer?sname=JQListTransaction&paymentId=${parameters.paymentId}" id="listTran" initrowdetails="true" initrowdetailsDetail=initrowdetailsDetail rowdetailsheight="400"
	     />
