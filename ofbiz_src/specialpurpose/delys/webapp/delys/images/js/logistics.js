...
$(document).ready(function(){
	...
	if ($("#requirementTypeId").val() == "TRANSFER_REQ"){
		updateCur({
			facilityId: $("select[name='facilityId']").val(),
			contactMechPurposeTypeId: "SHIP_ORIG_LOCATION",
			}, 'getFacilityContactMechs' , 'listFacilityContactMechs', 'contactMechId', 'address1', 'originContactMechId', $("#curOriginContactMechId").val());
		updateCur({
			facilityId: $("select[name='facilityTo']").val(),
			contactMechPurposeTypeId: "SHIPPING_LOCATION",
			}, 'getFacilityContactMechs' , 'listFacilityContactMechs', 'contactMechId', 'address1', 'destContactMechId', $("#curDestContactMechId").val());
		if (!$("#requirementId").val()){
			updateMultiElement({
			...
			}, 'getFacilitiesByStore' , 'listFacilities', 'facilityId', 'facilityName', 'facilityId', $("#facilityIdFrom").val());
			updateCur({
	        	productStoreId: $("select[name='productStoreIdTo']").val(),
	        	facilityId: $("#facilityIdTo").val()
			}, 'getFacilitiesByStore' , 'listFacilities', 'facilityId', 'facilityName', 'facilityTo', $("#facilityIdTo").val());
		}
	} else {
		updateCur({
			facilityId: $("select[name='facilityId']").val(),
...
function updateCur(jsonObject, url, data, key, value, id, curId) {
    jQuery.ajax({
        url: url,
        type: "POST",
        data: jsonObject,
        success: function(res) {
        	var json = res[data];
            renderHtml2(json, key, value, id, curId);
        }
    });
}
...
function renderHtml2(data, key, value, id, curId){
	var y = "";
	if(isNull){
		y = "<option value=''></option>";
	}
