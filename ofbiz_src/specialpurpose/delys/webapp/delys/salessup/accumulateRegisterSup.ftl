...
<@jqGrid filtersimplemode="true" filterable="true" editable="true" addrefresh="true"  showtoolbar="true"  dataField=dataField columnlist=columnlist  clearfilteringbutton="true"  
		 url="jqxGeneralServicer?sname=JQgetListAccumulateRegister" 
		 createUrl="jqxGeneralServicer?sname=accumulateRegisterSUP&jqaction=C" addColumns="customerId;productPromoId;ruleId"
		 />
	    	...
	    	<form id="formAdd" class="form-horizontal">
	    		<div class='row-fluid form-window-content'>
			    		<div class='span12'>
			    			...
			    			<div class='row-fluid margin-bottom10'>
			    				<div class='span5 align-right asterisk'>
			    					${uiLabelMap.DAAccumulateProgram}
			    				</div>
			    				<div class='span7'>
			    					<div id="accumulateAdd"></div>
			    				</div>
		    				</div>
		    				<div class='row-fluid margin-bottom10'>
			    				<div class='span5 align-right asterisk'>
...
<div id="listCustomerRegisted" style="display:none;">
	<div>${uiLabelMap.DAlistAccumulateRegister}</div>
...
<script type="text/javascript">
	...
	(function(){
		$.ajax({
			url : 'JQgetListAccumulateRegister',
			datatype : 'json',
			type : 'POST',
			cache  : false,
			async : false,
			success : function(response,status,xhr){
				if(response.listIterator && response.listIterator.length){
					listAccumulate = response.listIterator;
				}
			},
			error : function(){
			}
	...
	$('#accumulateAdd').on('select',function(event){
		var id = event.args.item.value;
		if(id){
		}
	});
	$('#dropdownlistContentaccumulateAdd').css('margin-top','5px','important');
	$('#dropdownlistContentlevelAdd').css('margin-top','5px','important');
	$('#alterpopupWindow').on('close',function(){
		$('#accumulateAdd').jqxDropDownList('clearSelection');
		$('#customerAdd').val('');
	...
	GridUtils.initDropDownButton({url : 'getListCustomerDelys',filterable : true,showfilterrow : true,width : 500,autoHeight : true,filterable : true}, datafields, columns, null,$('#gridCustomer'),$('#customerAdd'),'partyId');
	...
	$('#alterSave').click(function(){
		...
		var rowdata =  $('#jqxgrid').jqxGrid('getrowdata',index);
			var row = {
					customerId : $("#customerAdd").jqxDropDownButton('val') ? $("#customerAdd").jqxDropDownButton('val') : null,
	        ...
	        $.ajax({
	        	...
	        	cache  : false,
	        	success :  function(response,status,xhr){
	        		if(response.result.duplicate == 'duplicate'){
			        		$('#noti').text('${StringUtil.wrapString(uiLabelMap.DANotiAccumulate)}');
	...
	var initGridListCustomer = function(promoId){
		...
		if(promoId)  $('#accumulateAdd').html('<span style="color:red;">' + getPromoName() + '</span>');
		...
		grid.jqxGrid({
	            ...
	            rendertoolbar : function(toolbar){
		            	if(!toolbar.children().length){
			            	...
			            	var btReject = $('<button class="buttonTheme" id="rejectBt" onclick="reject()" class="grid-action-button"><i class="icon-remove"></i>${uiLabelMap.DACancelRegistration}</button>');
		...
		function reject(){
			...
			var datarow = $('#gridListCustomer').jqxGrid('getrowdata',rowindex);
				if(datarow === 'undefined' || !datarow){
					$('#deleteDialog').text('${StringUtil.wrapString(uiLabelMap.DANotiSelect)}');
			   		$('#deleteDialog').dialog({
			   			resizable : false,
			   			height : 180,
			   			modal : true,
					...
					return;		
				}else if(datarow.registerStatus == 'REG_PROMO_CANCELED') {
