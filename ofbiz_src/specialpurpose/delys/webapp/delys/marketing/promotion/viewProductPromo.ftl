	...
	<h4 class="lighter">${uiLabelMap.DelysPromoViewPromotion}: ${productPromo.promoName?if_exists}
		...
		<span class="widget-toolbar none-content">
			<#if currentStatusId?exists && currentStatusId == "PROMO_CREATED">
				<a href="<@ofbizUrl>EditPromosMarketing?productPromoId=${productPromo.productPromoId}</@ofbizUrl>">
					<i class="icon-pencil open-sans">${uiLabelMap.DelysPromoEditPromotion}</i>
				</a>
			</#if>
			<a href="<@ofbizUrl>EditPromosMarketing</@ofbizUrl>">
	...
	<div class="widget-main">
		<div class="row-fluid">
			...
			<div style="text-align:right">
				...
				<#else>
					<#if hasApproved>
						<#if !(productPromo.thruDate?exists && productPromo.thruDate?has_content)>
							<span class="widget-toolbar none-content">
								<form name="updatePromoThruDate" id="updatePromoThruDate" method="POST" action="<@ofbizUrl>updatePromoThruDate</@ofbizUrl>">
									...
									<button class="btn btn-primary btn-small" type="button" onclick="javascript:updatePromoThruDate2();">${uiLabelMap.DAUpdate}</button>
								</form>
							</span>
						</#if>
					</#if>
