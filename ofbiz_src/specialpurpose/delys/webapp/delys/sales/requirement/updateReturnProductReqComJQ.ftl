...
<script type="text/javascript">
	...
	<#list statusList as statusItem>
		<#if isDistributor>
			<#assign statusIdVar = "REREQCOM_CREATED"/>
			<#if ("REREQCOM_CREATED" == statusItem.statusId || "REREQCOM_NPPAPPROVED" == statusItem.statusId || "REREQCOM_REJECTED" == statusItem.statusId)>
		...
		<#elseif isAsm>
			<#assign statusIdVar = "REREQCOM_NPPAPPROVED"/>
			<#if ("REREQCOM_ASMAPPROVED" == statusItem.statusId || "REREQCOM_NPPAPPROVED" == statusItem.statusId || "REREQCOM_REJECTED" == statusItem.statusId)>
				<#assign description = StringUtil.wrapString(statusItem.get("description", locale)) />
				var row = {};
				row['statusId'] = '${statusItem.statusId}';
				row['description'] = "${description}";
		...
		<#elseif isNbd>
			<#assign statusIdVar = "REREQCOM_ASMAPPROVED"/>
			<#if ("REREQCOM_ASMAPPROVED" == statusItem.statusId || "REREQCOM_NBDAPPROVED" == statusItem.statusId || "REREQCOM_REJECTED" == statusItem.statusId)>
				<#assign description = StringUtil.wrapString(statusItem.get("description", locale)) />
				var row = {};
				row['statusId'] = '${statusItem.statusId}';
				row['description'] = "${description}";
		...
		<#elseif isCeo>
			<#assign statusIdVar = "REREQCOM_NBDAPPROVED"/>
			<#if ("REREQCOM_NBDAPPROVED" == statusItem.statusId || "REREQCOM_CEOAPPROVED" == statusItem.statusId || "REREQCOM_REJECTED" == statusItem.statusId)>
				<#assign description = StringUtil.wrapString(statusItem.get("description", locale)) />
				var row = {};
				row['statusId'] = '${statusItem.statusId}';
				row['description'] = "${description}";
		...
		<#elseif isSalesAdmin>
			<#assign statusIdVar = "REREQCOM_CEOAPPROVED"/>
			<#if ("REREQCOM_APPROVED" == statusItem.statusId || "REREQCOM_CEOAPPROVED" == statusItem.statusId || "REREQCOM_REJECTED" == statusItem.statusId)>
				<#assign description = StringUtil.wrapString(statusItem.get("description", locale)) />
				var row = {};
				row['statusId'] = '${statusItem.statusId}';
				row['description'] = "${description}";
...
<script type="text/javascript">
	...
	<#assign returnReasonList = delegator.findByAnd("ReturnReason", null, null, false)/>
	var reasonData = new Array();
	<#list returnReasonList as reasonItem>
...
<#assign columnlist="{text: '${uiLabelMap.requirementId}', dataField: 'requirementId', width: 150, editable: false,
				 ...
				 {text: '${uiLabelMap.DAReason}', dataField: 'reason', width: 150, editable: false, filtertype: 'checkedlist', 
					cellsrenderer: function(row, column, value){
...
<@jqGrid url="jqxGeneralServicer?sname=JQGetListRequirementAndItemDetail&requirementTypeId=RETURN_PRODCOM_REQ&statusId=${statusIdVar?if_exists}" dataField=dataField columnlist=columnlist filterable="true" clearfilteringbutton="true"
