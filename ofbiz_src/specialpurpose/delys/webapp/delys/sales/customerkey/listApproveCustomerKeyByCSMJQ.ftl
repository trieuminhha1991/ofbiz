...
url="jqxGeneralServicer?sname=JQGetListCustomersKeyByCsm" mouseRightMenu="false"  selectionmode="checkbox" viewSize="10" pagesizeoptions="['5', '15', '30', '50']" 
/> 
...
<script type="text/javascript">
	...
	function approvedCustomerKeysByCsm(){
		var row = $('#jqxgrid').jqxGrid('getselectedrowindexes');
		var listAppCustomerKeysCsm = new Array();
		...
		for(var i = 0; i < row.length; i++){
			var data = $('#jqxgrid').jqxGrid('getrowdata', row[i]);
				...
				listAppCustomerKeysCsm[i] = map;
		}
		if (listAppCustomerKeysCsm.length <= 0){
			$("#notifyMissInfoId").jqxNotification("open");
			return false;
		} else {
			listAppCustomerKeysCsm = JSON.stringify(listAppCustomerKeysCsm);
			jQuery.ajax({
		        url: "approveCustomerKeyByCsm",
		        type: "POST",
		        async: true,
		        data: {
		        		'listAppCustomerKeysCsm': listAppCustomerKeysCsm,
		        		},
