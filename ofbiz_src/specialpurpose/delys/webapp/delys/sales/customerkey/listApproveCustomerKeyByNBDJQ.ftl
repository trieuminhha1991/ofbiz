...
url="jqxGeneralServicer?sname=JQGetListCustomersKeyByNBD" mouseRightMenu="false"  selectionmode="checkbox" viewSize="10" pagesizeoptions="['5', '15', '30', '50']" 
/> 
...
<script type="text/javascript">
	...
	function approvedCustomerKeysByNBD(){
		var row = $('#jqxgrid').jqxGrid('getselectedrowindexes');
		var listAppCustomerKeysNBD = new Array();
		...
		for(var i = 0; i < row.length; i++){
			var data = $('#jqxgrid').jqxGrid('getrowdata', row[i]);
				...
				listAppCustomerKeysNBD[i] = map;
		}
		if (listAppCustomerKeysNBD.length <= 0){
			$("#notifyMissInfoId").jqxNotification("open");
			return false;
		} else {
			listAppCustomerKeysNBD = JSON.stringify(listAppCustomerKeysNBD);
			jQuery.ajax({
		        url: "approveCustomerKeyByNBD",
		        type: "POST",
		        async: true,
		        data: {
		        		'listAppCustomerKeysNBD': listAppCustomerKeysNBD,
		        		},
