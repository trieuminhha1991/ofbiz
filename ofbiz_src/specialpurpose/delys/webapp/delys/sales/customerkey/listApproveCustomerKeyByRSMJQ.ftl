...
url="jqxGeneralServicer?sname=JQGetListCustomersKeyByRsm" mouseRightMenu="false"  selectionmode="checkbox" viewSize="10" pagesizeoptions="['5', '15', '30', '50']" 
/> 
...
<script type="text/javascript">
	...
	function approvedCustomerKeysByRsm(){
		var row = $('#jqxgrid').jqxGrid('getselectedrowindexes');
		var listAppCustomerKeysRsm = new Array();
		...
		for(var i = 0; i < row.length; i++){
			var data = $('#jqxgrid').jqxGrid('getrowdata', row[i]);
				...
				listAppCustomerKeysRsm[i] = map;
		}
		if (listAppCustomerKeysRsm.length <= 0){
			$("#notifyMissInfoId").jqxNotification("open");
			return false;
		} else {
			listAppCustomerKeysRsm = JSON.stringify(listAppCustomerKeysRsm);
			jQuery.ajax({
		        url: "approveCustomerKeyByRsm",
		        type: "POST",
		        async: true,
		        data: {
		        		'listAppCustomerKeysRsm': listAppCustomerKeysRsm,
		        		},
