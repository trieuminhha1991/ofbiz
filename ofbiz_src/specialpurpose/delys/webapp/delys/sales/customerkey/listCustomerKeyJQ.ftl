<#include "../forecast/macroTree.ftl"/>
...
{text: '${StringUtil.wrapString(uiLabelMap.fullName)}', dataField: 'fullName', width: '20%', cellclassname: cellclass},
{text: '${StringUtil.wrapString(uiLabelMap.statusId)}', dataField: 'relStatusId', width: '20%', cellclassname: cellclass},
{text: '${StringUtil.wrapString(uiLabelMap.CommonFromDate)}', dataField: 'fromDate', cellsformat: 'd', width: '20%', filtertype:'range', columntype: 'datetimeinput', cellclassname: cellclass,
	createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
		editor.jqxDateTimeInput({ });
	}
},
{text: '${StringUtil.wrapString(uiLabelMap.CommonThruDate)}', dataField: 'thruDate', cellsformat: 'd', width: '20%', filtertype:'range', columntype: 'datetimeinput',cellclassname: cellclass,
	createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
		editor.jqxDateTimeInput({ });
	}
},
...
<script type="text/javascript">
    ...
    //$("#jqxTreeYear").jqxTree({ width: 200, height: 220});
...
{name: 'partyId', type: 'string'},
{name: 'fromDateNew', type: 'date', other: 'Timestamp'},
{name: 'thruDateNew', type: 'date', other: 'Timestamp'}

]"/>
<#assign columnlistCustomers = "{text: '${StringUtil.wrapString(uiLabelMap.idCustomer)}', dataField: 'partyIdTo', editable: false, width: '17%'},
{text: '${StringUtil.wrapString(uiLabelMap.fullName)}', dataField: 'fullName', editable: false,}, 
...
{text: '${StringUtil.wrapString(uiLabelMap.partyId)}', dataField: 'partyId', hidden : true,},
{text: '${StringUtil.wrapString(uiLabelMap.CommonFromDate)}', dataField: 'fromDateNew', cellsformat: 'd', filtertype:'range',width: '15%', columntype: 'datetimeinput',
	createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
		editor.jqxDateTimeInput({ });
	}
},
{text: '${StringUtil.wrapString(uiLabelMap.CommonThruDate)}', dataField: 'thruDateNew', cellsformat: 'd', filtertype:'range',width: '15%', columntype: 'datetimeinput',
	createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
		editor.jqxDateTimeInput({ });
	}
},
...
<@jqGrid id="jqxgridCustomer" idExisted="true" filtersimplemode="true" viewSize="5" pagesizeoptions="['5', '10', '15', '20', '25', '30', '50', '100']" 
	...
	url="jqxGeneralServicer?sname=JQGetListCustomers" filterable="true" width="700" isShowTitleProperty="false" bindresize="false" alternativeAddPopup="alterpopupWindow" addType="popup" 
...
$('#fromDateNewAdd').on('change', function (event) {
    ...
    for(var i=0; i<= rowindexes.length;i++){
    	$("#jqxgridCustomer").jqxGrid('setcellvalue', rowindexes[i], "fromDateNew", fromDateValue);
    }
});
...
$('#thruDateNewAdd').on('change', function (event) {
    ...
    for(var j=0; j< rowindexes1.length;j++){
    	$("#jqxgridCustomer").jqxGrid('setcellvalue', rowindexes1[j], "thruDateNew", thruDateValue);
    }
});
...
$('#CustomKeyForm').jqxValidator({
	rules : [
		...
		{input: '#fromDateNewAdd', message: '${uiLabelMap.DAFromDateNotBeEmpty}', action: 'blur', rule: 
			function (input, commit) {
				if($('#fromDateNewAdd').jqxDateTimeInput('getDate') == null || 
					$('#fromDateNewAdd').jqxDateTimeInput('getDate') == ''){
					return false;
				}
				return true;
			}
		},
		{input: '#fromDateNewAdd', message: '${uiLabelMap.fromDateValidate}', action: 'blur', rule: 
			function (input, commit) {
				if($('#fromDateNewAdd').jqxDateTimeInput('val', 'date').getTime() < ${nowTimestamp.getTime()}) {
					return false;
				}
				return true;
			}
		...
		{input: '#fromDateNewAdd', message: '${uiLabelMap.fromDateValidate}', action: 'blur', rule: 
			function (input, commit) {
				if($('#fromDateNewAdd').jqxDateTimeInput('val', 'date').getTime() <= $('#thruDateNewAdd').jqxDateTimeInput('val', 'date').getTime()) {
					return false;
				}
				return true;
			}
...
$('#alterSave1').click(function(){
	$('#CustomKeyForm').jqxValidator('validate');
	createCustomerKey();
});
...
function createCustomerKey() {
	...
	if (listCustomerKeys.length <= 0){
		...
		jQuery.ajax({
	        ...
	        async: true,
	        data: {
	        		'listCustomerKeys': listCustomerKeys,
	        		},
...
$('#CustomKeyForm').on('validationSuccess',function(){
...
// select the first row and clear the selection.
	...
	$('#alterpopupWindow1').on('close',function(){
		var rowindexesVali = $('#jqxgridCustomer').jqxGrid('getselectedrowindexes');
		for(var k=0; k <= rowindexesVali.length; k++ ){
	}
