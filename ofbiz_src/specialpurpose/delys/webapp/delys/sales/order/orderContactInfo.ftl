...
<#macro updateOrderContactMech orderHeader contactMechTypeId contactMechList contactMechPurposeTypeId contactMechAddress>
  <#if (!orderHeader.statusId.equals("ORDER_COMPLETED")) && !(orderHeader.statusId.equals("ORDER_REJECTED")) && !(orderHeader.statusId.equals("ORDER_CANCELLED"))>
    <form name="updateOrderContactMech" method="post" action="<@ofbizUrl>updateOrderContactMech</@ofbizUrl>">
      ...
      <select name="contactMechId" class="margin-top11" style="width:auto; max-width:500px">
      ...
      <button type="submit" class="btn btn-primary btn-mini margin-top11" name="submitButton"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button>
    </form>
  </#if>
</#macro>

<#if displayParty?has_content || orderContactMechValueMaps?has_content>
<div id="contactinfo-tab" class="tab-pane">
		                		...
		                		<div>
					                <#if hasCreated>
					                	<#-- /ordermgr/control/orderentry?partyId=${partyId}&amp;orderTypeId=${orderHeader.orderTypeId} -->
					                   	<a href="<@ofbizUrl>newOrder?partyId=${partyId}&amp;orderTypeId=${orderHeader.orderTypeId}</@ofbizUrl>" class="btn btn-mini btn-primary">${uiLabelMap.OrderNewOrder}</a>
					                </#if>
