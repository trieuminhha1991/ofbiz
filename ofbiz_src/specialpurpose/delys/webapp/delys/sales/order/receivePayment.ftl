...
<#if security.hasEntityPermission("ORDERMGR", "_UPDATE", session)>
	...
	<div class="widget-header widget-header-blue widget-header-flat">
		<h4>${uiLabelMap.OrderReceiveOfflinePayments}</h4>
	      		...
	      		<form method="post" action="<@ofbizUrl>receiveOfflinePayments</@ofbizUrl>" name="paysetupform" style="padding-top:15px">
			        ...
			        <#if paymentMethods?has_content>
				        <table class="table table-striped table-bordered table-hover dataTable">
				          	<tr class="header-row">
								<td width="15%" align="right">${uiLabelMap.BankName}</td>
								<td width="20%" align="right">${uiLabelMap.BankOwner}</td>
					            <td width="20%" align="right">${uiLabelMap.BankCode}</td>
					            <td width="15%" align="right">${uiLabelMap.BankType}</td>
					            <td width="15%" class="align-center">${uiLabelMap.OrderAmount}</td>					            
					            <td width="15%">${uiLabelMap.OrderReference}</td>
				          	</tr>
		          			<#list paymentMethods as payMethod>							
				          		<#if payMethod.paymentMethodTypeId?if_exists == "COMPANY_CHECK">
									...
									<#assign bankType = delegator.findOne("Enumeration",Static["org.ofbiz.base.util.UtilMisc"].toMap("enumId", "${payMethod.partyPaymentTypeId}"), false) />	
					          		<tr>
										...
										<td width="15%" align="right">${bankType.get("description",locale)}</td>
							            <td width="15%"><input type="text" size="100%" name="${payMethod.paymentMethodId}_amount" <#if payMethod.partyPaymentTypeId=="MAIN_PAYMENT"> value="${parameters.avo?if_exists}"<#elseif payMethod.partyPaymentTypeId=="SUB_PAYMENT">value="${parameters.avt?if_exists}"</#if>  /></td>
							            <td width="15%"><input type="text" size="100%" name="${payMethod.paymentMethodId}_reference" /></td>
						          	</tr>
								</#if>
				          		<#if payMethod.paymentMethodTypeId?if_exists == "CREDIT_CARD">
									...
									<#assign creditType = delegator.findOne("Enumeration",Static["org.ofbiz.base.util.UtilMisc"].toMap("enumId", "${payMethod.partyPaymentTypeId}"), false) />		
					          		<tr>
										...
										<td width="20%" align="right">${creditType.get("description",locale)}</td>
							            <td width="15%"><input type="text" size="100%" name="${payMethod.paymentMethodId}_amount" <#if payMethod.partyPaymentTypeId=="MAIN_PAYMENT"> value="${parameters.avo?if_exists}"<#elseif payMethod.partyPaymentTypeId=="SUB_PAYMENT">value="${parameters.avt?if_exists}"</#if>  /></td>
							            <td width="15%"><input type="text" size="100%" name="${payMethod.paymentMethodId}_reference" /></td>						          	
									</tr>
								</#if>
								<#if payMethod.paymentMethodTypeId?if_exists == "CASH">
									<#assign partyName = delegator.findOne("PartyNameView",Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", "${payMethod.organizationPartyId}"), false) />	
					          		<tr>
...
<#assign columnlist="
			{ 'text' : '${uiLabelMap.BankName}','datafield' : 'bankName' ,editable : false},
			{'text' : '${uiLabelMap.BankOwner}','datafield' : 'bankOwner',editable : false },
			{'text' : '${uiLabelMap.BankCode}','datafield' : 'bankCode',editable : false },
			{'text' : '${uiLabelMap.BankType}','datafield' : 'bankType' ,editable : false},
			{'text' : '${uiLabelMap.OrderAmount}','datafield' : 'orderAmount',columntype  : 'numberinput',cellsrenderer : function(row){
				var data = $('#jqxgrid').jqxGrid('getrowdata',row);
				return '<span>' + convertLocalNumber(data.orderAmount) + 'đ </span>';
			},createeditor : function(row,column,editor){
				var data = $('#jqxgrid').jqxGrid('getrowdata',row);
				editor.jqxNumberInput({digits : 20,decimalDigits : 2,min : 0,max : 999999999999});	
				editor.jqxNumberInput('val', data.orderAmount);
			} },
			{'text' : '${uiLabelMap.OrderReference}','datafield' : 'orderReference',columntype  : 'numberinput',createeditor : function(row,column,editor){
				var data = $('#jqxgrid').jqxGrid('getrowdata',row);
...
<@jqGrid url="jqxGeneralServicer?sname=getListReceiptPayment&request=${request?if_exists}&orderId=${parameters.orderId?if_exists}&avo=${parameters.avo?if_exists}&avt=${parameters.avt?if_exists}"  columnlist=columnlist 
			  dataField=dataField showtoolbar="true" clearfilteringbutton="true" editable="true" filterable="false"
	/>	
