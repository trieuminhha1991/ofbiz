      			...
      			<table cellspacing="0" cellpadding="1" border="0" class="table table-striped dataTable table-hover table-bordered">
        			...
        			<tbody>
	        			...
	        			<#list shoppingCart.items() as cartLine>
				          	...
				          	<#assign isPayPromoItem = false />
				          	<#if cartLine.associatedOrderId?exists && cartLine.associatedOrderId?has_content && cartLine.associatedOrderItemSeqId?exists && cartLine.associatedOrderItemSeqId?has_content>
				          		<#assign isPayPromoItem = true>
				          	</#if>
			          		<tr valign="top">
	        					<td>
	        						<#if cartLine.getProductId()?exists>
					                    <#-- product item -->
					                    <a href="<@ofbizUrl>editProduct?product_id=${cartLine.getProductId()}</@ofbizUrl>" class="margin-top-nav-10"
					                    	<#if isPayPromoItem> style="color:#d6412b" title="orderId = ${cartLine.associatedOrderId?if_exists}, orderItemSeqId = ${cartLine.associatedOrderItemSeqId?if_exists}"</#if>>
	            				...
	            				<td nowrap="nowrap" align="center">
	            					<div>
	            						<#if !cartLine.getIsPromo() || (cartLine.getIsPromo() && isPayPromoItem)>
