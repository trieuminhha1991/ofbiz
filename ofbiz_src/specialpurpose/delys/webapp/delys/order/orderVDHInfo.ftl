...
<div id="orderinfo-tab" class="tab-pane active">
	...
	<div style="width:100%;height:25px;">
		...
		<#if currentStatus.statusId == "ORDER_CREATED">
			<#if hasSupApproved?exists && hasSupApproved == "TRUE">
		...
		<#if currentStatus.statusId == "ORDER_SUPAPPROVED">
			<#if hasSadApproved?exists && hasSadApproved == "TRUE">
		...
		<#if currentStatus.statusId == "ORDER_NPPAPPROVED">
			<#if hasChiefAccountantApproved?exists && hasChiefAccountantApproved == "TRUE">
				<#if currentStatus.statusId == "ORDER_APPROVED" && orderHeader.orderTypeId == "SALES_ORDER">
	              <a class="btn btn-primary btn-mini floatLeftTableContent" href="javascript:document.PrintOrderPickSheet.submit()">${uiLabelMap.FormFieldTitle_printPickSheet}</a>
	              <form name="PrintOrderPickSheet" method="post" action="<@ofbizUrl>orderPickSheet.pdf</@ofbizUrl>" target="_BLANK">
	                <input type="hidden" name="facilityId" value="${storeFacilityId?if_exists}"/>
	...
	<table class="table table-striped table-bordered dataTable table-hover" cellspacing='0'>
        ...
        <#if productStore?has_content>
          	<tr>
                ...
                <td valign="top">
                  	${productStore.storeName!}&nbsp;<a href="/catalog/control/EditProductStore?productStoreId=${productStore.productStoreId}${externalKeyParam}" target="catalogmgr" class="btn btn-mini btn-primary">(${productStore.productStoreId})</a>
                </td>
      		</tr>  
        </#if>
            ...
            <tr>
              	...
              	<td valign="top">
                  	<#if orderHeader.originFacilityId?has_content>
                    	<a href="/facility/control/EditFacility?facilityId=${orderHeader.originFacilityId}${externalKeyParam}" target="facilitymgr" class="btn btn-mini btn-primary">${orderHeader.originFacilityId}</a>
                  	<#else>
                    	${uiLabelMap.CommonNA}
                  	</#if>
              	</td>
            ...
            <tr>
              	...
              	<td valign="top">
                  	<#if orderHeader.createdBy?has_content>
                    	<a href="/partymgr/control/viewprofile?userlogin_id=${orderHeader.createdBy}${externalKeyParam}" target="partymgr" class="btn btn-mini btn-primary">${orderHeader.createdBy}</a>
                 	<#else>
