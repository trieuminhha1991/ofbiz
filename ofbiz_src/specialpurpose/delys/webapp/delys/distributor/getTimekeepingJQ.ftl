...
<#assign startTime = workingShiftList.get(0).get("startTime").getTime() /> 
<#assign endTime = workingShiftList.get(sizeWorkingShift - 1).get("endTime").getTime() />


<#assign datafield ="[{name: 'partyId', type:'string'},
					  {name: 'fullName', type:'string'},"/>
<#assign columnlist = "{text: '${uiLabelMap.EmployeeName}', datafield: 'fullName', width: 150, cellsalign: 'left', editable: false, pinned: true},
...
<#list dateOfMonth as date>
	...
	<#assign columnlist = columnlist + "{text: '${uiLabelMap.HRCommonInTime}', width: 100, columntype: 'datetimeinput', cellsalign: 'center', filterable: false, datafield: 'startTime_"+ dataFieldGroup + "', columngroup: '" + dataFieldGroup+ "', cellsformat: 'HH:mm:ss',
		createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
			editor.jqxDateTimeInput({width: 99, height: '29px', formatString: 'HH:mm:ss', showCalendarButton: false});
			if(cellvalue){
...
<div class="widget-box transparent no-bottom-border">
	...
	<div class="widget-body">
				...
				<div class="span6">
					<div class="row-fluid">
						...
						<div class="span2" style="margin-top: 2px">
							<button class="btn btn-primary btn-mini icon-search" id="btnSearch"></button>
						</div>
					</div>	
				</div>	
			</div>
		...
		<div class="row-fluid">
			<@jqGrid url="jqxGeneralServicer?month=${month}&year=${year}&hasrequest=Y&sname=JQGetSalesmanListTimekeeping" 
				...
				updateRowFunction="updateRowCustom" selectionmode="singlecell" viewSize="10" pagesizeoptions="['10', '20', '30', '50']"
...
$(document).ready(function () {
		...
		while(fromDate.getTime() < thruDate.getTime()){
			...
			columnlistEdit.push({"text": "${uiLabelMap.HRCommonInTime}", "width": 100, "cellsalign": "center", "filterable": false, 
				"datafield": "startTime_"+ columnGroup, "columngroup": columnGroup , "cellsformat": "HH:mm:ss",
					createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
						editor.jqxDateTimeInput({width: 99, height: 29, formatString: 'HH:mm:ss', showCalendarButton: false});
						if(cellvalue){
							editor.val(cellvalue);
						}
					},
			...
			columnlistEdit.push({"text": "${uiLabelMap.HRCommonOutTime}", "width": 100, "cellsalign": "center", "filterable": false, 
				...
				createeditor: function (row, cellvalue, editor, celltext, cellwidth, cellheight) {
					editor.jqxDateTimeInput({width: 99, height: 19, formatString: 'HH:mm:ss', showCalendarButton: false});
					if(cellvalue){
						editor.val(cellvalue);
					}
				},
	...
	//	source._source.url = "jqxGeneralServicer?month=" + (month - 1) +"&year=" + year + "&hasrequest=Y&sname=JQGetSalesmanListTimekeeping;
