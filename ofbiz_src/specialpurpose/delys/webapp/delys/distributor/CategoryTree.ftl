...
<script type="text/javascript">
        ...
        <#macro fillTree rootCat>
            <#if (rootCat?has_content)>
                <#list rootCat as root>
                    {
                    "data": {
                    	"title" : unescapeHtmlText("<#if root.categoryName?exists>${root.categoryName?js_string} [${root.productCategoryId}]<#else>${root.productCategoryId?js_string}</#if>"), 
                    	"attr" : {
                    		"href" : "#", <#--<@ofbizUrl>/EditProdCatalog?prodCatalogId=${root.productCategoryId}</@ofbizUrl>-->
                    		"onClick" : "callDocument('${root.productCategoryId}', 0, 'catalog');"
                		}
                    },
        	...
        	jQuery("#tree").jstree({
            	...
            	"types" : {
	         		...
	         		"types" : {
	             		"CATEGORY" : {
		                 	"icon" : { 
		                 		"image" : "/images/jquery/plugins/jsTree/themes/apple/group.png",
		                     	"position" : "10px40px"
	                 		}
	         			}
	     			}
	...
	function callDocument(id, count, type) {
    	// jQuerry Ajax Request
    	var dataSet = {};
    	if(type == "catalog") {
        	// URL = 'EditProdCatalog?prodCatalogId='+id;
        	$("#tree").jstree('toggle_node',$("#" + id));
    	} else {
        	// URL = 'EditCategory?productCategoryId='+id;
        	jQuery.ajax({
				url: 'getProductCategoryMembersAjax',
				data: "categoryId=" + id,
				type: 'post',
				async: false,
				...
				error: function(data) {
					alert("Error during get list product in category selected");
				}
			});
    	}
		...
		<div class="row-fluid">
			<div class="form-horizontal basic-custom-form form-decrease-padding" id="updateQuotation" name="updateQuotation" style="display: block;">
				<div class="row margin_left_10 row-desc">
					<div id="tree" class="span5" style="background: #ffffff;"></div>
					<div id="lst-products" class="span7"></div>
				</div>
			</div>
		</div>
	</div>
