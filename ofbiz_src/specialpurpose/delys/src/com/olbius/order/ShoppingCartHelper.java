...
public class ShoppingCartHelper {
        ...
        for (String parameterName : context.keySet()) {
            ...
            if (underscorePos >= 0 && (!parameterName.endsWith("_i18n"))) {
                try {
                    ...
                    } else if (parameterName.startsWith("fromInventoryItemId")) {
                    	if (UtilValidate.isNotEmpty(quantString)) {
                    		item.setAttribute("fromInventoryItemId", quantString);
                    	}
                    } else {
    ...
    /** Update the items in the shopping cart for pay promo. */
    public Map<String, Object> modifyCartPayPromo(Security security, GenericValue userLogin, Map<String, ? extends Object> context, boolean removeSelected, String[] selectedItems, Locale locale) {
        Map<String, Object> result = null;
        if (locale == null) {
            locale = this.cart.getLocale();
        }
        ...
        for (ShoppingCartItem item : deleteList) {
            ...
            try {
                this.cart.removeCartItem(itemIndex, dispatcher);
                // TODOCHANGE change method remove item: 
                // this.cart.removeCartItemPayPromo(itemIndex, dispatcher);
            } catch (CartItemModifyException e) {
                result = ServiceUtil.returnError(new ArrayList<String>());
                errorMsgs.add(e.getMessage());
            }
