...
invItemTypePrefix += "_%";
organizationPartyId = parameters.parameters.organizationPartyId[0];
exprBldr = new EntityConditionBuilder();
listAllConditions = parameters.listAllConditions;
listAllConditions.add(exprBldr.LIKE(invoiceItemTypeId: invItemTypePrefix));
invoiceItemTypes = delegator.findList("InvoiceItemType", exprBldr.AND(listAllConditions), null, null, null, false);
result = ServiceUtil.returnSuccess();
result.listIterator = invoiceItemTypes.collect { invoiceItemType ->
    defaultAccount = true
    glAccount = null;
