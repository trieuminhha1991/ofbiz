 ...
 */
olbius.directive('googlemap', ["$rootScope", "$compile", "$timeout", 'DialogFactory', 'LanguageFactory', 'uiGmapGoogleMapApi', "GPS", 'NumberFactory',
function(root, $compile, $timeout, DialogFactory, LanguageFactory, GoogleMapApi, GPS, NumberFactory) {
	function init(scope, element, attrs) {
		...
		element.ready(function() {
			...
			} else if (scope.height == 'auto') {
				var content = $(".page-content");
				var container = $('.main-content');
				var wh = 2 * container.height() - container.innerHeight();
				var inn = content.innerHeight() - content.height();
				var cth = $('#control-map').height();
				if (cth) {
					cth += 15;
				}
				var mh = wh - inn - cth - 10;
				mapel.height(mh);
			}
		...
		// if (!scope.marker) {
		// scope.marker = scope.createMarker(0, null, '', null, null, '', null, '');
		// }
		...
		scope.updateCurrentLocation = function() {
			var location = scope.getCurrentLocation();
			if (location) {
		...
		scope.markerClickEvent = function(marker) {
			marker.onClicked = function() {
				...
				str += "<center><button ng-click='chooseSelected(marker)' class='btn-action-primary btn-action-primary-small margin-right-10'>";
				str += "<i class='fa fa-map-pin'></i></button>" + "<button ng-click='onClicked(clicked)' class='btn-action-primary btn-action-primary-small'>";
				str += "<i class='fa fa-motorcycle'></i></button></center></div>";
				marker.title = str;
				setTimeout(function() {
