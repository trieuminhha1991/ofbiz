app.factory('GPS', ['$rootScope', '$q', 'geolocation', '$state', 'StorageFactory', 'Popup', function($rootScope, $q, geolocation, $state, StorageFactory, Popup) {
	var self = this;
	...
	self.getCurrentLocation = function(scope, isWatch, callback) {
		...
		if(self.requireRelocate() || !position){
			...
			var diag = window.cordova && cordova.plugins ? cordova.plugins.diagnostic : null;
			if(diag){
				var func = diag.isLocationEnabled;
				if(func){
					var err = diag.switchToLocationSettings;
					var errf = function(){
						callback();
