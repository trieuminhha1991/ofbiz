app.controller('CartController', function($rootScope, $scope, $controller, $interval, $timeout, OrderService, CartService, CartFactory) {
	var self = $scope;
	var root = $rootScope;
	self.self = self;
	...
	self.checkoutAction = function() {
		try {
			...
			if (products.length) {
				...
				data = angular.extend(data, self.currentLocation);
