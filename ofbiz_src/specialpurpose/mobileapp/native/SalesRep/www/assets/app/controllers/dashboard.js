app.controller('DashboardController', function($rootScope, $scope, $location, $timeout, $controller, DashboardService, EmployeeService, CalendarFactory, Chart) {
    ...
    self.$watch('kpi.target', function(){
	if(self.currentTotalProfit){
		self.currentTotalProfitPercent = self.currentTotalProfit / self.kpi.target * 100;
	}
    });
    self.$watch('currentTotalProfit', function(){
	if(self.kpi && self.kpi.target){
		self.currentTotalProfitPercent = self.currentTotalProfit / self.kpi.target * 100;
	}
    });
    self.hide = function(){
	...
	if(!force){
        ...
        DashboardService.getOrderTotal(obj, before, self.hide).then(function(res) {
            ...
            for (var x in yAxis) {
                ...
                var size = yAxis[x].length;
                var end = size - 1;
                for(var y in yAxis[x]){
					total += yAxis[x][y];
					if(report.dateType == 'MONTH' && y == end){
				self.currentTotalProfit = yAxis[x][y];
		            }
                }
            }
