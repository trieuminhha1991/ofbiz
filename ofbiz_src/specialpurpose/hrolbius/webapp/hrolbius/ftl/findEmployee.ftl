...
<#assign extInfo = parameters.extInfo?default("N")>
    <#if partyList?exists>
    <div id="findEmployeeResults" class="widget-box transparent no-bottom-border">
        <div class="widget-header widget-header-blue widget-header-flat wi1dget-header-large">
        <h3><i class="icon-table"></i>${uiLabelMap.EmployeeFound}</h3>
    ...
    <#if partyList?has_content>
        <table class="table table-hover table-striped table-bordered dataTable" cellspacing="0">
            <tr class="header-row">
                <td class="header-table">${uiLabelMap.EmployeeId}</td>
            ...
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                ...
                <td class="button-col align-float">
                    <a class="btn btn-primary btn-mini" href="<@ofbizUrl>EmployeeProfile?partyId=${partyRow.partyId}</@ofbizUrl>">${uiLabelMap.CommonDetails}&nbsp;<i class="icon-info-sign"></i></a>
                </td>
            </tr>
    ...
    <#else>
        <div>
            <p class="alert alert-info">${uiLabelMap.NoEmployeeFound}</p>
        </div>
    </#if>
    </div>
