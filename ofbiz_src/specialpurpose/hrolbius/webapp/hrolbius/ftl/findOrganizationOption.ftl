...
<div id="FindOrganizationalUnit" class="widget-box transparent no-bottom-border">
    <#-- <div class="widget-header">
   <h4><i class="icon-search"></i>${uiLabelMap.CommonFind} ${uiLabelMap.organization}</h4>
        <span class="widget-toolbar">
            <#if parameters.hideFields?default("N") == "Y">
                <li><a class="btn btn-mini btn-primary" href="<@ofbizUrl>FindOrganizationalUnit?hideFields=N${paramList}</@ofbizUrl>"><i class="icon-double-angle-up"></i>&nbsp;${uiLabelMap.CommonShowLookupFields}&nbsp;</a></li>
            <#else>
            <#if partyList?exists><li><a class="btn btn-mini btn-primary" href="<@ofbizUrl>FindOrganizationalUnit?hideFields=Y${paramList}</@ofbizUrl>"><i class="icon-double-angle-down"></i>&nbsp;${uiLabelMap.CommonHideFields}</a></li></#if>
    ...
    <div class="widget-body">
      <#-- NOTE: this form is setup to allow a search by partial partyId or userLoginId; to change it to go directly to
       ...
       <div class="margin-left30" style="margin-top:5px;">
        <form method="post" name="lookupparty" action="<@ofbizUrl>FindOrganizationalUnit</@ofbizUrl>" class="basic-form">
                ...
                <tr><td class="arrowed">${uiLabelMap.PartyContactInformation}</td>
                    <td>
                    	...
                    	<label style="float:left;">
                    		<input type="radio" name="extInfo" value="N" onclick="javascript:refreshInfo();" <#if extInfo == "N">checked="checked"</#if>><span class="lbl">${uiLabelMap.CommonNone}</span>
                		</label>
                		<label style="float:left;margin-left:5px;">
                    		<input type="radio" name="extInfo" value="P" onclick="javascript:refreshInfo();" <#if extInfo == "P">checked="checked"</#if>><span class="lbl">${uiLabelMap.PartyPostal}</span>
                		</label>
                		<label style="float:left;margin-left:5px;">
                    		<input type="radio" name="extInfo" value="T" onclick="javascript:refreshInfo();" <#if extInfo == "T">checked="checked"</#if>><span class="lbl">${uiLabelMap.PartyTelecom}</span>
                		</label>
                		<label style="float:left;margin-left:5px;">
                    		<input type="radio" name="extInfo" value="O" onclick="javascript:refreshInfo();" <#if extInfo == "O">checked="checked"</#if>><span class="lbl">${uiLabelMap.CommonOther}</span>
                		</label>
                    	</div>
                    </td>
                </tr>
                <tr><td class="arrowed">${uiLabelMap.OrgUnitId}</td>
                    <td>
                      <@htmlTemplate.lookupField value='${requestParameters.partyId?if_exists}' formName="lookupparty" name="partyId" id="partyId" fieldFormName="LookupOrganizationNew"/>
                    </td>
                </tr>
                <tr>
                	<td>
                	...
                	<td>
                		<input type="text" name="groupName" value="${parameters.groupName?if_exists}"/>
                	</td>
                </tr>
                <tr>
                	<td><input type="hidden" name="roleTypeId" value="INTERNAL_ORGANIZATIO"/></td>
                </tr>
            <#if extInfo == "P">
            ...
            <div align="center">
                    ...
                    <a class="btn btn-primary btn-small" href="<@ofbizUrl>FindOrganizationalUnit?roleTypeId=INTERNAL_ORGANIZATIO&amp;hideFields=Y&amp;lookupFlag=Y</@ofbizUrl>" >${uiLabelMap.CommonShowAllRecords}&nbsp;<i class="icon-lightbulb"></i></a>
            </div>
        </form>
        </div>
    	</div>
