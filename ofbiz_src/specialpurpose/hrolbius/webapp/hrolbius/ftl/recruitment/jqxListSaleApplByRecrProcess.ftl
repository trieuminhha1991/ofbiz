...
<script>
	...
	<#assign listStatus = delegator.findList("StatusItem", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("statusTypeId", Static["org.ofbiz.entity.condition.EntityOperator"].IN , ["APPLICANT_STATUS"]), null, null, null, false)>
...
<#assign columnlist="{ text: '${uiLabelMap.CommonId}', datafield: 'partyId', width: 100, editable: false,
                     ...
                     { text: '${uiLabelMap.comments}', datafield: 'comments'},
...
<#if Static['com.olbius.util.SecurityUtil'].hasRole("DELYS_SALESSUP_GT", userLogin.getString("partyId"), delegator)>
 	<@jqGrid id="jqxgrid" filtersimplemode="true" addrefresh="true" addType="popup" alternativeAddPopup="alterpopupNewApplicant" addrow="true" showtoolbar="true" clearfilteringbutton="true"
		 ...
		 mouseRightMenu="true" contextMenuId="contextMenu" otherParams="workEffortName:S-getRecruitmentRound(workEffortId,partyId)<workEffortName>"
		/>
<#else>
	 <@jqGrid id="jqxgrid" filtersimplemode="true" addrefresh="true" addType="popup" alternativeAddPopup="alterpopupNewApplicant" addrow="false" showtoolbar="true" clearfilteringbutton="true"
		 url="jqxGeneralServicer?sname=JQGetListApplicant&workEffortId=${parameters.workEffortId}" dataField=dataField columnlist=columnlist
		 mouseRightMenu="true" contextMenuId="contextMenu" otherParams="workEffortName:S-getRecruitmentRound(workEffortId,partyId)<workEffortName>"
		/>
</#if>
...
<div id='contextMenu' style="display: none;">
	<ul>
	    <li id="sendEmail">${uiLabelMap.sendEmail}</li>
	    <li id="agreePreliminary">${uiLabelMap.agreePreliminary}</li>
	    <li id="proposeProbation">${uiLabelMap.proposeProbation}</li>
	</ul>
</div>	
	        ...
	        <div style="overflow: hidden; padding: 5px; margin-left: 10px" id="windowContentNewApplicant">
			    <div id='jqxTabs' style="position: relative;">
		            ...
		            <div id="newGeneralInfo">
		                <#include "jqxEditSaleApplGeneralInfo.ftl" />
		            ...
		            <div id="newOverview" style="margin:10px">
		            	...
		            	<div class="row-fluid jqx-tabs-button-olbius">
		                	<div class="span12" style="text-align: right">
			                	<button type="button" class="btn btn-primary btn-success back back btn-small"><i class="icon-arrow-left"></i>${uiLabelMap.CommonPrevious}</button>
		                		<button type="button" class="btn btn-primary btn-small" id="submit">${uiLabelMap.CommonCreate}&nbsp;<i class="icon-arrow-right"></i></button>
		                	</div>
	                	</div>
		            </div>
		        </div>
...
$(document).ready(function () {
	...
	<#include "jsEditSaleApplGeneralInfo.ftl" />
