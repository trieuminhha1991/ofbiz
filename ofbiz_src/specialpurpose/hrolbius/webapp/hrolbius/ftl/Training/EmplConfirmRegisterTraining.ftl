<script type="text/javascript">
	...
	<#if trainingCoursePartyRegister?exists>
		<div class="clearfix">
			<div class="pull-left alert alert-success inline no-margin">
				<i class="bigger-120 blue"></i>
				${uiLabelMap.CommonStatus}: &nbsp;
				<#if trainingCoursePartyRegister.statusId?exists>
					<#assign statusItem = delegator.findOne("StatusItem", Static["org.ofbiz.base.util.UtilMisc"].toMap("statusId", trainingCoursePartyRegister.statusId), false)>
	...
	<#if trainingCoursePartyRegister?exists && trainingCoursePartyRegister.statusId=="COURS_REGIS_WAIT_CON">	
	<hr/>
	<div class="row-fluid">
		<div class="span12">
			<div class="span6">
				<form action="<@ofbizUrl>updateEmplResgisterTrainingCourse</@ofbizUrl>" method="post">
					<input type="hidden" name="statusId" value="COURS_REGIS_PARTCE">
					<input type="hidden" name="trainingCourseId" value="${parameters.trainingCourseId}">
					<button style="float: right;" type="submit" class="btn btn-primary btn-small icon-ok">${uiLabelMap.RegistedAttendance}</button>
				</form>
			</div>
			<div class="span6">
				<form action="<@ofbizUrl>updateEmplResgisterTrainingCourse</@ofbizUrl>" method="post">
					<input type="hidden" name="statusId" value="COURS_REGIS_REJECT">
					<input type="hidden" name="trainingCourseId" value="${parameters.trainingCourseId}">
					<button type="submit" class="btn btn-danger btn-small icon-remove">${uiLabelMap.NotRegistedAttendance}</button>
				</form>
			</div>
		</div>
	</div>
