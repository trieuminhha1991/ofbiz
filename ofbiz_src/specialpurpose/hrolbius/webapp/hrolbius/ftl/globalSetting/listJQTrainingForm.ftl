...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'trainingFormTypeId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListTrainingForm"
		 createUrl="jqxGeneralServicer?sname=createTrainingForm&jqaction=C" addColumns="trainingFormTypeId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteTrainingForm&jqaction=D" deleteColumn="trainingFormTypeId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateTrainingForm"  editColumns="trainingFormTypeId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.TrainingForm}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="trainingFormAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#trainingFormAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#trainingFormAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		trainingFormTypeId : $('#trainingFormAdd').val(),
