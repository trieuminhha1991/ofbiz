...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'enumId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListExamTypes"
		 createUrl="jqxGeneralServicer?sname=createExamType&jqaction=C" addColumns="enumId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteExamType&jqaction=D" deleteColumn="enumId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateExamType"  editColumns="enumId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.HROlbiusAddExamType}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="enumIdAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#enumIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#enumIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
