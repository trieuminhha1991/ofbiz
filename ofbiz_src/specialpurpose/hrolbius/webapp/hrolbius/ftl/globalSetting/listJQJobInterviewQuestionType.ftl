...
<#assign dataField="[
	{name : 'questionTypeId', type : 'string'},
	{name : 'description',type : 'string'}
]"/>
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'questionTypeId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListJobQuestionType"
		 createUrl="jqxGeneralServicer?sname=creatJobInterviewQuestionType&jqaction=C" addColumns="questionTypeId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteJobInterViewQuestionType&jqaction=D" deleteColumn="questionTypeId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateJobInterviewQuestionType"  editColumns="questionTypeId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.HumanResAddJobInterviewQuestionType}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="questionTypeAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#questionTypeAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#questionTypeAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		questionTypeId : $('#questionTypeAdd').val(),
