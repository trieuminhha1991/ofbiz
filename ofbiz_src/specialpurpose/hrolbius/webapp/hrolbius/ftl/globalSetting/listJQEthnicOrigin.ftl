...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'ethnicOriginId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListEthnicOrigin"
		 createUrl="jqxGeneralServicer?sname=createEthnicOrigin&jqaction=C" addColumns="ethnicOriginId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteEthnicOrigin&jqaction=D" deleteColumn="ethnicOriginId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateEthnicOrigin"  editColumns="ethnicOriginId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.EthnicOrigin}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="ethnicOriginIdAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#ethnicOriginIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#ethnicOriginIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		ethnicOriginId : $('#ethnicOriginIdAdd').val(),
