...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'partyQualTypeId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListQualificationTypes"
		 createUrl="jqxGeneralServicer?sname=createQualificationType&jqaction=C" addColumns="partyQualTypeId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteQualificationType&jqaction=D" deleteColumn="partyQualTypeId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateQualificationType"  editColumns="partyQualTypeId;description"
		/>	
	...
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				...
				<div class="controls">
					<input type="text" id="partyQualTypeIdAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#partyQualTypeIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#partyQualTypeIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		partyQualTypeId : $('#partyQualTypeIdAdd').val(),
