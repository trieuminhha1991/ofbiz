...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'maritalStatusId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListMarialStatus"
		 createUrl="jqxGeneralServicer?sname=createMaritalStatus&jqaction=C" addColumns="maritalStatusId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteMaritalStatus&jqaction=D" deleteColumn="maritalStatusId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateMaritalStatus"  editColumns="maritalStatusId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.MaritalStatus}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="maritalStatusIdAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#maritalStatusIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#maritalStatusIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		maritalStatusId : $('#maritalStatusIdAdd').val(),
