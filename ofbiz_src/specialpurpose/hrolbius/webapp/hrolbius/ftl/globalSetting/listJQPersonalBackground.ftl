...
<#assign dataField="[
	{name : 'personalBackgroundId', type : 'string'},
	{name : 'description',type : 'string'}
]"/>
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'personalBackgroundId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListPersonalBackground"
		 createUrl="jqxGeneralServicer?sname=createPersonalBackground&jqaction=C" addColumns="personalBackgroundId;description"
		 removeUrl="jqxGeneralServicer?sname=deletePersonalBackground&jqaction=D" deleteColumn="personalBackgroundId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updatePersonalBackground"  editColumns="personalBackgroundId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.PersonalBackground}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="personalBackgroundIdAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#personalBackgroundIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#personalBackgroundIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		personalBackgroundId : $('#personalBackgroundIdAdd').val(),
