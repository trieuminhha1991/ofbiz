...
<#assign dataField="[
	{name : 'jobInterviewTypeId', type : 'string'},
	{name : 'description',type : 'string'}
]"/>
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'jobInterviewTypeId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListJobInterviewType"
		 createUrl="jqxGeneralServicer?sname=createJobInterviewType&jqaction=C" addColumns="jobInterviewTypeId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteJobInterviewType&jqaction=D" deleteColumn="jobInterviewTypeId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateJobInterviewType"  editColumns="jobInterviewTypeId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.HumanResAddJobInterviewType}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="jobInterviewAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#jobInterviewAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#jobInterviewAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		jobInterviewTypeId : $('#jobInterviewAdd').val(),
