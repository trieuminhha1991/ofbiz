...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'majorId',width : '250px',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListMajor"
		 createUrl="jqxGeneralServicer?sname=createMajor&jqaction=C" addColumns="majorId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteMajor&jqaction=D" deleteColumn="majorId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateMajor"  editColumns="majorId;description"
		/>	
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#majorIdAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#majorIdAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
