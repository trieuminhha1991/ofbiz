...
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'trainingTypeId',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListTrainingType"
		 createUrl="jqxGeneralServicer?sname=createTrainingType&jqaction=C" addColumns="trainingTypeId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteTrainingType&jqaction=D" deleteColumn="trainingTypeId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateTrainingType"  editColumns="trainingTypeId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.HROlbiusTrainingType}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="trainingTypeAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#trainingTypeAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#trainingTypeAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		trainingTypeId : $('#trainingTypeAdd').val(),
