...
<#assign dataField="[
	{name : 'trainingLevelId', type : 'string'},
	{name : 'description',type : 'string'}
]"/>
<#assign columnlist="
			{text : '${uiLabelMap.CommonId}',dataField : 'trainingLevelId',editable : false},
...
<@jqGrid filtersimplemode="true"  deleterow="true" filterable="true" addrow="true"   addType="popup" addrefresh="true" alternativeAddPopup="alterpopupWindow" dataField=dataField columnlist=columnlist  clearfilteringbutton="true" 
		editable="true" 
		 url="jqxGeneralServicer?sname=JQgetListTrainingLevel"
		 createUrl="jqxGeneralServicer?sname=createTrainingLevel&jqaction=C" addColumns="trainingLevelId;description"
		 removeUrl="jqxGeneralServicer?sname=deleteTrainingLevel&jqaction=D" deleteColumn="trainingLevelId"
		 updateUrl="jqxGeneralServicer?jqaction=U&sname=updateTrainingLevel"  editColumns="trainingLevelId;description"
		/>	
<div id="alterpopupWindow" style="display : none;">
	<div>${uiLabelMap.CommonAdd} ${uiLabelMap.TrainingLevel}</div>
	<div style="overflow: hidden;">
		<form id="formAdd" class="form-horizontal">
			<div class="control-group no-left-margin">
				<label class="control-label asterisk">${uiLabelMap.CommonId}</label>
				<div class="controls">
					<input type="text" id="trainingLevelAdd"/>
				</div>
			</div>
			<div class="control-group no-left-margin">
...
$('#alterpopupWindow').jqxWindow({ width: 500, height : 230,resizable: true,  isModal: true, autoOpen: false, cancelButton: $("#alterCancel"), modalOpacity: 0.7 });
	...
	$('#trainingLevelAdd').jqxInput({width : '200px',height : '25px'});
	$('#descriptionAdd').jqxInput({width : '200px',height : '25px'});
	$('#formAdd').jqxValidator({
			rules : [
				{input : '#trainingLevelAdd',message : '${StringUtil.wrapString(uiLabelMap.FieldRequired?default(''))}' , action : 'blur',rule : 'required'},
	...
	var row = {};
	row = {
		trainingLevelId : $('#trainingLevelAdd').val(),
