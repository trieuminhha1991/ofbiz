<div id="${dataToggleModalId}" class="modal hide fade" tabindex="-1" >
	<div class="modal-header no-padding">
		<div class="table-header">
			...
			${uiLabelMap.HROlbiusTrainingClassType}
		</div>
	</div>	
		...
		<form action="<@ofbizUrl>${linkUrl}</@ofbizUrl>" id="${formId}" class="form-horizontal" method="post" name="${formId}">
			...
			<div class="row-fluid">
				<div class="control-group no-left-margin">
					...
					<div class="controls">
						<input type="text" name="trainingClassTypeId" class="required">
					</div>
				</div>
				<div class="control-group no-left-margin">
					<label class="control-label asterisk">${uiLabelMap.HRolbiusPreRequisiteSkill}</label>
					<div class="controls">
						<@htmlTemplate.lookupField formName="${formId}" name="parentTypeId" id="parentTypeId" fieldFormName="LookupTraining" zIndex="2000"/>
					</div>
				</div>	
				<div class="control-group no-left-margin">
