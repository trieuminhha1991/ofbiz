...
<#assign payrollCharacteristicId = parameters.payrollCharacteristicId?if_exists>
<div class="row-fluid">
	<div class="span12">
		<div class="widget-box transparent no-bottom-border">
			<div id="salary-calculate" class="widget-body">
				<form action="<@ofbizUrl>createFormula</@ofbizUrl>" class="basic-form form-horizontal" method="post" id="createFormula">
					<#assign step = 3>
					<div class="row-fluid">
						<div class="salary-calculate-element">
						<div class="row-fluid">
							<div class="span6">	
								<div class="control-group no-left-margin ">
									<label class="">
										<label for="EditFormula_code" <#if !payrollFormulaGV?has_content>class="asterisk" </#if> id="EditFormula_code_title">${uiLabelMap.formulaCode}</label>  
									</label>
									<div class="controls">
										<#if payrollFormulaGV?has_content>
											${payrollFormulaGV.code}
											<#assign functionStr = payrollFormulaGV.function> 
											<input type="hidden" name="code" value="${payrollFormulaGV.code}">
										<#else>
											<input type="text" name="code" size="25" id="EditFormula_code" style="margin-left: 10px">
										</#if>  
								 	</div>
								</div>
							</div>
							<div class="span5">
								<div class="control-group no-left-margin">
								    ...
								    <div class="controls">
										<input type="text" name="name" value="<#if payrollFormulaGV?exists>${payrollFormulaGV.name?if_exists}</#if>" size="25" id="EditFormula_name">  
								    </div>
							    </div>
						    </div>
						    <#--<!-- <div class="span4">
							    <div class="control-group no-left-margin">
							    	<label class="">
										<label for="EditFormula_invoiceItemTypeId" id="EditFormula_invoiceItemTypeId_title">${uiLabelMap.AccountingInvoicePurchaseItemType}</label>  
									</label>
								    <div class="controls">
									    <span class="ui-widget">
										    <select name="invoiceItemTypeId" id="EditFormula_invoiceItemTypeId" size="1">
										    	<#list invoiceItemType as type>
										    		<option value="${type.invoiceItemTypeId}" <#if payrollFormulaGV?exists && type.invoiceItemTypeId == payrollFormulaGV.invoiceItemTypeId> selected="selected" </#if>>${type.description?if_exists}</option>
										    	</#list>
										    </select>
									  	</span>
								    </div>
						 ...
						 <#if parameters.payrollCharacteristicId?has_content>
							<div class="row-fluid">
								 <div class="span6">
									<div class="control-group no-left-margin">
										...
										<div class="controls">
											<div class="row-fluid">
												<div class="span12">
													<div id="payrollCharacteristic" class="span6"></div>
													<#if parameters.payrollCharacteristicId == "INCOME">
														<#assign listPayrollCharacteristic = payrollIncome>
														<div id="TaxableType" class="span6"></div>
													<#elseif parameters.payrollCharacteristicId == "DEDUCTION">
														<#assign listPayrollCharacteristic = payrollDeduction>
														<div id="Exempted" class="span6" style="margin-left: 20px !important; margin-top: 7px;">${uiLabelMap.IsExempted}</div>
													</#if>
												</div>
											</div>
										</div>
								...
								<div class="span6">
									<#if parameters.payrollCharacteristicId == "INCOME">
										<div class="control-group no-left-margin" id="incomeTaxRate" style="display: none;">
											<label class="control-label">${uiLabelMap.HrTaxRate}</label>
											<div class="controls">
												<div id="taxRate"></div>
											</div>
										</div>
								...
								<#if parameters.payrollCharacteristicId == "INCOME">
									<div class="row-fluid">
										<div class="span6">
											<div class="control-group no-left-margin">
												<label class="control-label">${uiLabelMap.PayrollItemType}</label>
												<div class="controls">
										...
										$(document).ready(function () {
											var listPayrollItemTypeArr = new Array();
											<#list payrollItemType as item>
												var row = {};
												row["payrollItemTypeId"] = "${item.payrollItemTypeId}";
												row["description"] = "${item.description}";
												listPayrollItemTypeArr[${item_index}] = row;
											</#list>
											var sourcePayrollItemType =
									        {
									            localdata: listPayrollItemTypeArr,
									            datatype: "array"
									        };
									        ...
									        $("#payrollItemType").jqxDropDownList({ selectedIndex: 0,  source: dataAdapterPayrollItemType, 
								        		...
								        		itemHeight: 25, height: 25, width: 220,
								        		renderer: function (index, label, value) {
													for(var i=0; i < listPayrollItemTypeArr.length; i++){
														if(listPayrollItemTypeArr[i].payrollItemTypeId == value){
															return listPayrollItemTypeArr[i].description;
														}
													}
												    return value;
												}
							...
							$(document).ready(function () { 
								var listPayrollCharArr = new Array();
								<#list listPayrollCharacteristic as characteristic>
									var row = {};
									row["payrollCharacteristicId"] = "${characteristic.payrollCharacteristicId}";
									row["description"] = "${characteristic.description}";
									listPayrollCharArr[${characteristic_index}] = row;
								</#list>
								var sourcePayrollCharType =
						        {
						            localdata: listPayrollCharArr,
						            datatype: "array"
						        };
								
						        var dataAdapterPayrollChar = new $.jqx.dataAdapter(sourcePayrollCharType);
						        $("#payrollCharacteristic").jqxDropDownList({ selectedIndex: 0,  source: dataAdapterPayrollChar, 
						        	displayMember: "description", valueMember: "payrollCharacteristicId", theme: 'olbius',
					        		itemHeight: 25, height: 25, width: 100, autoDropDownHeight: true,
					        		renderer: function (index, label, value) {
										for(var i=0; i < listPayrollCharArr.length; i++){
											if(listPayrollCharArr[i].payrollCharacteristicId == value){
												return listPayrollCharArr[i].description;
											}
										}
									    return value;
									}
					            });
						        var listTaxableTypeArr = new Array();
						        <#list taxableType as tax>
						        	var row = {};
						        	row["taxableTypeId"] = "${tax.taxableTypeId}";
						        	row["description"] = "${tax.description}";
						        	listTaxableTypeArr[${tax_index}] = row;
						        </#list>
						        var sourceTaxableType =
						        {
						            localdata: listTaxableTypeArr,
						            datatype: "array"
						        };
								
						        var dataAdapterTaxable = new $.jqx.dataAdapter(sourceTaxableType);
						        if($("#TaxableType").length > 0){
						        	$("#TaxableType").jqxDropDownList({ selectedIndex: 0,  source: dataAdapterTaxable, 
						        		...
						        		itemHeight: 25, height: 25, width: 190, autoDropDownHeight: true, 
						        		renderer: function (index, label, value) {
											for(i=0; i < listTaxableTypeArr.length; i++){
												if(listTaxableTypeArr[i].taxableTypeId == value){
													return listTaxableTypeArr[i].description;
												}
											}
										    return value;
										}
						            });
						        	$("#taxRate").jqxNumberInput({ width: 220, height: '25px', digits: 3, symbolPosition: 'right', symbol: '%',  spinButtons: false , theme: 'olbius'});
						        ...
						        if($("#Exempted").length > 0){
						        	$("#Exempted").jqxCheckBox({width: 50, height: 25, checked: false, theme: 'olbius'});
						        }
						...
						<div class="control-group no-left-margin">
							...
							<#assign payrollFormulaSize = payrollFormula?size>
							<#assign nbrRows = (payrollFormulaSize / step)?ceiling>    
								...
								<#list 1..nbrRows as i>
									<tr>
										<#list ((i-1)*step + 1)..(i*step) as j>		
											<#if (j <= payrollFormulaSize)>
												<td style="background-color: #f5f5f5" id="${payrollFormula.get(j-1).code}_payroll">
													<a href="javascript:void(0)"  
														onclick="cal_add('${payrollFormula.get(j-1).code}','${payrollFormula.get(j-1).name}', 1, true)" >
															${payrollFormula.get(j-1).code}</a>
												</td>
												<td>
													${payrollFormula.get(j-1).name}
												</td>
											<#else>
						...
						<div class="control-group no-left-margin">
							<#if payrollParameters?has_content>
								<#assign payrollParameterSize = payrollParameters?size>
								<#assign payrollParameterRows = (payrollParameterSize/step)?ceiling>
								<h4>${uiLabelMap.parameters}</h4>
								<div class="space-4"></div>
								<table class="table table-bordered dataTable">
									<#list 1..payrollParameterRows as i>
										<tr>
											<#list ((i-1)*step + 1)..(i*step) as j>		
												<#if (j <= payrollParameterSize)>
													<td style="background-color: #f5f5f5">
														<a href="javascript:void(0)" onclick="cal_add('${payrollParameters.get(j-1).code}', '${payrollParameters.get(j-1).name}', 1, false)">${payrollParameters.get(j-1).code}</a>					
													</td>
													<td>
														${payrollParameters.get(j-1).name}
													</td>
												<#else>
													<td></td>
													<td></td>
							...
							<div class="row-fluid">
								...
								<div class="span7">
									<div class="operator-editor">									
										<span>${uiLabelMap.formulaFunction}&nbsp;
										<#if payrollFormulaGV?exists><a href="javascript:void(0)" id="viewCurrFormula">(${uiLabelMap.ReviewFormula})</a></span></#if>
										<div id="normalCode">
											...
											<!-- <textarea style="height:100px;" class="autosize-transition span12"></textarea> -->	
											<textarea name="function" style="height:80px;" class="autosize-transition span12"  id="calc_des" readonly="readonly" onfocus="FocusColor2(this);fn_Selectitem(this);"></textarea>
										...
										<button type="button" class="btn btn-primary btn-small" name="submitButton" id="submitButton">
											
											<#if !payrollFormulaGV?exists>
...
<#if payrollFormulaGV?exists>
	<div id="notificationFormula">
		<div>
			${functionStr?if_exists}
		</div>
	</div>
</#if>
...
jQuery(document).ready(function () {
	<#list payrollFormula as formula>
		$("#${formula.code}_payroll").jqxTooltip({ content: '${formula.function?if_exists}', position: 'mouse', name: 'payrollTooltip', width: 'auto', theme: 'olbius'});
	</#list>
	<#if payrollFormulaGV?exists>
		$("#viewCurrFormula").click(function(event){
			$("#notificationFormula").jqxNotification('open');
		});
		$("#notificationFormula").jqxNotification({
	        width: 250, position: "top-right", opacity: 1, icon:{width: 0, height: 0},
	        autoOpen: false, autoClose: false, template: "info"
	    });
	</#if>
	jQuery("#EditFormula_code").focus();
	var validator = jQuery("#createFormula").validate({
		errorElement: 'span',
	...
	jQuery("#submitButton").click(function(){
		...
		var functionRelated = "";
		//console.log(allFunctionAndParam);
		...
		<#if payrollCharacteristicId == "INCOME">
			var payrollCharacteristicId = $("#payrollCharacteristic").val();
			var taxableTypeId = $("#TaxableType").val();
			var taxRate = $("#taxRate").val();
			var payrollItemTypeId = $("#payrollItemType").val();
			params.push({name: "payrollCharacteristicId", value: payrollCharacteristicId});
			params.push({name: "taxableTypeId", value: taxableTypeId});
			params.push({name: "payrollItemTypeId", value: payrollItemTypeId});
			if(taxRate){
				params.push({name: "taxRate", value: taxRate});	
			}
		<#elseif payrollCharacteristicId == "DEDUCTION">
			var payrollCharacteristicId = $("#payrollCharacteristic").val();
			var checked = $("#Exempted").jqxCheckBox('checked');
			var exempted = "N";
			if(checked){
				exempted = "Y";
			}
			params.push({name: "payrollCharacteristicId", value: payrollCharacteristicId});
			params.push({name: "exempted", value: exempted});
		</#if>
		<#if payrollFormulaGV?has_content>
			<#assign url = "updateFormula">
		<#else>
			<#assign url = "createFormula">
		</#if>
		jQuery.ajax({
			...
			success: function(data){
				...
				if(data._EVENT_MESSAGE_){
					...
					bootbox.dialog({
						  ...
						  title: "${StringUtil.wrapString(uiLabelMap.PayrollError)}",
						  buttons: {
						    danger: {
						      label: "${StringUtil.wrapString(uiLabelMap.CommonClose)}!",
...
function getFalseCond(trRootId, condNo){
	...
	if(jQuery("#trFalse" + condNo).css('display') != 'none'){
		//console.log("#trFalse" + condNo);
		//console.log(jQuery("#ipColFalse" + condNo).val());
...
function fn_SetCondition(pos, val_yn, del_id, tr_parent){ 
    ...
    if(pos == "n" && val_yn == "n"){
        ...
        strTag += '<td class="info"><button class="btn btn-mini btn-danger icon-reply open-sans marginTop-10" onclick="fn_removeword(\'ipColDes1\');" >'+strBtnDel+' </button>';
        strTag += '<button class="btn btn-mini btn-primary icon-remove open-sans marginTop-10" onclick="fn_Conditionyn(\'n\')" >'+strBtnCC+'</button></td></tr>';     
        ...
        strTag += '<td class="info"><button type="button" class="btn btn-mini btn-danger icon-reply open-sans marginTop-10" onclick="fn_removeword(\'ipColTrue1\');">'+strBtnDel+'</button>';
        strTag += '<button class="btn btn-mini btn-primary icon-code-fork open-sans marginTop-10" onclick="fn_SetCondition(\'trTrue1\',\'y\',\'trTrue1\', \'trDefine1_True\');">'+ strBtnCon +'</button></td></tr>';
        ...
        strTag += '<td class="info"><button class="btn btn-mini btn-danger icon-reply open-sans marginTop-10" onclick="fn_removeword(\'ipColFalse1\');">'+strBtnDel+'</button>';
        strTag += '<button class="btn btn-mini btn-primary icon-code-fork open-sans marginTop-10" onclick="fn_SetCondition(\'trFalse1\',\'y\',\'trFalse1\', \'trDefine1_False\')">'+ strBtnCon +'</button></td></tr>';
