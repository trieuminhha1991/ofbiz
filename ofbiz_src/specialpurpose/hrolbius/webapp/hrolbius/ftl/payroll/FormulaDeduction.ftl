...
<#assign columnlist="{ text: '${uiLabelMap.formulaCode}', datafield: 'code', width: 180},
					 ...
					 {text: '${uiLabelMap.IsExemptedTax}', datafield: 'exempted',   cellsalign: 'center',
					 	cellsrenderer: function(row, column, value){
					 		if(value == 'Y'){
...
<@jqGrid url="jqxGeneralServicer?sname=JQGetFormulaDeduction" dataField=dataField columnlist=columnlist
	...
	addrow="true" alternativeAddPopup="settingFormula" addType="popup" id="jqxgrid"
	deleteColumn="code"
	editrefresh ="true"
	editmode="click"
	showtoolbar = "true"
	autorowheight = "true"
/>
<!-- customcontrol1="icon-plus-sign open-sans@${uiLabelMap.NewFormula}@EditFormula?payrollCharacteristicId=DEDUCTION" -->
<script type="text/javascript">
var periodTypes = [
	...
	function initJqxDropdownlist(){
		...
		$("#newPayrollCharacId").jqxDropDownList({ selectedIndex: 0,  source: dataAdapterPayrollChar, 
    		...
    		renderer: function (index, label, value) {
				for(var i=0; i < characDeductionArr.length; i++){
					if(characDeductionArr[i].payrollCharacteristicId == value){
						return characDeductionArr[i].description;
					}
				}
			    return value;
			}
