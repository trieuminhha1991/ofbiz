    ...
    <#else>
      <form method="post" action="<@ofbizUrl>updateGiftCard?DONE_PAGE=${donePage}</@ofbizUrl>" name="editgiftcardform" style="margin: 0;">
       ...
       <!-- <div class="button-bar">
          ...
          <a href="javascript:document.editgiftcardform.submit()" class="btn btn-info btn-small margin-top8 margin-left8 margin-bottom8">${uiLabelMap.CommonSave}</a>
        </div>-->
        <table class="basic-table" style="margin-top:30px" cellspacing="0">
        <tr>
          <td class="olbius-label">${uiLabelMap.AccountingCardNumber}</td>
          <td>
            <input type="text" size="20" maxlength="60" name="cardNumber" value="${giftCardData.cardNumber?if_exists}" />
          </td>
        </tr>
        <tr>
          <td class="olbius-label">${uiLabelMap.AccountingPinNumber}</td>
          <td>
            <input type="text" size="10" maxlength="60" name="pinNumber" value="${giftCardData.pinNumber?if_exists}" />
          </td>
        </tr>
        <tr>
          <td class="olbius-label">${uiLabelMap.CommonExpireDate}</td>
          <td>
            <#assign expMonth = "">
            <#assign expYear = "">
      ...
      <div class="button-bar">
        <a href="<@ofbizUrl>${donePage}?partyId=${partyId}</@ofbizUrl>" class="icon-remove btn btn-info btn-small margin-top8 margin-left8 margin-bottom8">${uiLabelMap.CommonCancelDone}</a>
        <a href="javascript:document.editgiftcardform.submit()" class="icon-save btn btn-info btn-small margin-top8 margin-left8 margin-bottom8">${uiLabelMap.CommonSave}</a>
      </div>
  </div>
</div>
