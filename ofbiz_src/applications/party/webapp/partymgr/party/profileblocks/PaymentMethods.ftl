...
<#macro maskSensitiveNumber cardNumber>
  ...
  <div id="partyPaymentMethod" class="widget-box transparent no-bottom-border">
    <div class="widget-header">
        <h4>${uiLabelMap.PartyPaymentMethodInformation}</h4>
        <div class="widget-toolbar">
        <#if security.hasEntityPermission("PAY_INFO", "_CREATE", session) || security.hasEntityPermission("ACCOUNTING", "_CREATE", session)>
          <li><a class=" margin-top5 open-sans icon-plus-sign" style="text-decoration: blink; font-size:14px;"  href="<@ofbizUrl>editeftaccount?partyId=${partyId}</@ofbizUrl>">&nbsp${uiLabelMap.AccountingCreateNewEftAccount}</a></li>
          <li><a class=" margin-top5 open-sans icon-plus-sign" style="text-decoration: blink; font-size:14px;" href="<@ofbizUrl>editgiftcard?partyId=${partyId}</@ofbizUrl>">&nbsp${uiLabelMap.AccountingCreateNewGiftCard}</a></li>
          <li><a class=" margin-top5 open-sans icon-plus-sign" style="text-decoration: blink; font-size:14px;" href="<@ofbizUrl>editcreditcard?partyId=${partyId}</@ofbizUrl>">&nbsp${uiLabelMap.AccountingCreateNewCreditCard}</a></li>
          <li><a class=" margin-top5 open-sans icon-plus-sign" style="text-decoration: blink; font-size:14px;" href="<@ofbizUrl>EditBillingAccount?partyId=${partyId}</@ofbizUrl>">&nbsp${uiLabelMap.AccountingCreateBillingAccount}</a></li>
        </#if>
        </div>
      <br class="clear" />
    </div>
      ...
      <#if paymentMethodValueMaps?has_content || billingAccounts?has_content>
          ...
          <#list paymentMethodValueMaps as paymentMethodValueMap>
            ...
            <tr>
              <#if "CREDIT_CARD" == paymentMethod.paymentMethodTypeId && paymentMethodValueMap.creditCard?has_content>
                ...
                <td class="button-col">
                  <#if security.hasEntityPermission("MANUAL", "_PAYMENT", session)>
                    <a class="btn btn-info btn-mini" href="/accounting/control/manualETx?paymentMethodId=${paymentMethod.paymentMethodId}${externalKeyParam}">${uiLabelMap.PartyManualTx}</a>
                  </#if>
                  <#if security.hasEntityPermission("PAY_INFO", "_UPDATE", session) || security.hasEntityPermission("ACCOUNTING", "_UPDATE", session)>
                    <a class="btn btn-info btn-mini icon-refresh open-sans" href="<@ofbizUrl>editcreditcard?partyId=${partyId}&amp;paymentMethodId=${paymentMethod.paymentMethodId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                  </#if>
                <#-- </td> -->
              <#elseif "GIFT_CARD" == paymentMethod.paymentMethodTypeId>
                ...
                <td class="button-col">
                  <#if security.hasEntityPermission("PAY_INFO", "_UPDATE", session) || security.hasEntityPermission("ACCOUNTING", "_UPDATE", session)>
                    <a class="btn btn-mini btn-info icon-refresh open-sans" href="<@ofbizUrl>editgiftcard?partyId=${partyId}&amp;paymentMethodId=${paymentMethod.paymentMethodId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                  </#if>
                <#-- </td> -->
              <#elseif "EFT_ACCOUNT" == paymentMethod.paymentMethodTypeId>
                ...
                <td class="button-col">
                  <#if security.hasEntityPermission("PAY_INFO", "_UPDATE", session) || security.hasEntityPermission("ACCOUNTING", "_UPDATE", session)>
                    <a class="btn btn-mini btn-info icon-refresh open-sans" href="<@ofbizUrl>editeftaccount?partyId=${partyId}&amp;paymentMethodId=${paymentMethod.paymentMethodId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                  </#if>
                <#-- </td> -->
              ...
              <#if security.hasEntityPermission("PAY_INFO", "_DELETE", session) || security.hasEntityPermission("ACCOUNTING", "_DELETE", session)>
                <a class="btn btn-mini btn-warning icon-remove open-sans" href="<@ofbizUrl>deletePaymentMethod/viewprofile?partyId=${partyId}&amp;paymentMethodId=${paymentMethod.paymentMethodId}</@ofbizUrl>">${uiLabelMap.CommonExpire}</a>
              <#else>
                &nbsp;
              </#if>
        ...
        <#if billingAccounts?has_content>
            ...
            <tr>
              ...
              <td class="button-col">
                <a class="btn btn-mini btn-info icon-refresh open-sans" href="<@ofbizUrl>EditBillingAccount?billingAccountId=${billing.billingAccountId}&amp;partyId=${partyId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                <a class="btn btn-mini btn-warning icon-remove open-sans" href="<@ofbizUrl>deleteBillingAccount?partyId=${partyId}&amp;billingAccountId=${billing.billingAccountId}</@ofbizUrl>">${uiLabelMap.CommonExpire}</a>
              </td>
          </tr>
          </#list>
        </#if>
