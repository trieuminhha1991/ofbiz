...
-->
  ...
  <div id="partyContactInfo" class="widget-box transparent no-bottom-border">
    <div class="widget-header">
      <h4>${uiLabelMap.PartyContactInformation}</h4> 
        <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session) || userLogin.partyId == partyId>
       <span class="widget-toolbar">
          <li><a class="open-sans icon-plus-sign" style="text-decoration: blink; font-size:14px;" href="<@ofbizUrl>editcontactmech?partyId=${partyId}</@ofbizUrl>">&nbsp${uiLabelMap.CommonCreateNew}</a></li>
          </span> 
        </#if>
      <br class="clear" />
    </div>
            ...
            <tr>
              ...
              <td>
                ...
                <#elseif "EMAIL_ADDRESS" = contactMech.contactMechTypeId>
                  <div>
                    ...
                    </form><a class=" open-sans icon-arrow-right" href="javascript:document.createEmail${contactMech.infoString?replace("&#64;","")?replace(".","")}.submit()">${uiLabelMap.CommonSendEmail}</a>
                  </div>
              ...
              <td class="button-col">
                <#if security.hasEntityPermission("PARTYMGR", "_UPDATE", session) || userLogin.partyId == partyId>
                  <a style="margin-left: 0px; margin-bottom: 10px;" class="btn btn-mini btn-info icon-ok open-sans" href="<@ofbizUrl>editcontactmech?partyId=${partyId}&amp;contactMechId=${contactMech.contactMechId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                </#if>
