...
public class PartyServices {
     ...
     * @return Map with the result of the service, the output parameters.
     */
    public static Map<String, Object> perpareCreateCustomer(DispatchContext ctx, Map<String, Object> context){
    	Map<String, Object> result = FastMap.newInstance();
    	 GenericValue userLogin = (GenericValue) context.get("userLogin");
		 String partyId = (String) userLogin.get("partyId");
		 String groupName = "DELYS_CSM";
		 String entityName = "PartyGroup";
		 context.put("entityName", entityName);
		 Map<String, Object> inputFields = checkMap(context.get("inputFields"),
					String.class, Object.class);
		 inputFields.put("partyId", partyId);
		 inputFields.put("groupName", groupName);
		 context.put("inputFields", inputFields);
		 result = FindServices.performFind(ctx, context);
    ...
    public static Map<String, Object> updatePerson(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        Date idIssueDate = (Date) context.get("idIssueDate");
        java.util.Date now = UtilDateTime.nowDate();
        java.sql.Date nowDate = new java.sql.Date(now.getTime());
        if(birthDate != null){
        	if (idIssueDate != null && (idIssueDate.before(birthDate)|| idIssueDate.after(nowDate))){
            	return ServiceUtil.returnError(UtilProperties.getMessage(resourceError, 
                        "error.idissuedate", locale));
            }
        }
        /*end edit by dunglv*/
	...
	public static Map<String, Object> jqGetPartyByRole(DispatchContext ctx, Map<String, Object> context) {
		Delegator delegator = ctx.getDelegator();
		...
		if (parameters.containsKey("roleTypeId") && parameters.get("roleTypeId").length > 0) {
			if (UtilValidate.isNotEmpty(parameters.get("roleTypeId"))) {
		...
		try {
			...
			listOrders = EntityMiscUtil.processIteratorToList(parameters, successResult, delegator, "PartyAndRoleFullNameSimple", 
					EntityCondition.makeCondition(listAllConditions), null, null, listSortFields, opts);

		} catch (Exception e) {
			String errMsg = "Fatal error calling jqGetPartyByRole service: " + e.toString();
			Debug.logError(e, errMsg, module);
