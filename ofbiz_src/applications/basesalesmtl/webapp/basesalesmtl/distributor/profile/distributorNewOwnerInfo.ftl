...
<script type="text/javascript">
	...
	$(function(){
		OlbDistributorNewOwner.init();
	});
	
	var OlbDistributorNewOwner = (function(){
		var init = function(){
			initElement();
			initEvent();
			initValidateForm();
		...
		var initValidateForm = function(){
			var extendRules = [
                ...
                {input : "#wn_own_birthDate", message : multiLang.BSBirthDateBeforeToDay, action : 'change, blur',
                    rule : function(input, commit){
                        var now = new Date();
