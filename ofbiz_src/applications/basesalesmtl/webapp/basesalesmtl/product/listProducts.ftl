...
<#assign columnlists = "
                ...
                <#assign columnlists = columnlists + "
                { text: '${uiLabelMap.ExpireDate}', dataField: 'expiredDate', columntype: 'custom', cellsformat: 'dd/MM/yyyy', width: 100, editable: true, sortable: false, filterable: false, cellclassname: productGridCellclass,
					cellsrenderer: function (row, column, value){
						var data = $('#jqxgridProduct').jqxGrid('getrowdata', row);
						if (glEditorId != null){
							var key = 'row' + data.productId;
					...
					cellendedit: function (row, b, c ,o,n){
				 	...
				 	initeditor: function (row, column, editor) {
						editor.jqxDateTimeInput({clearString: '${uiLabelMap.Clear}', width: 100, height: '25px', formatString: 'dd/MM/yyyy'});
						var data = $('#jqxgridProduct').jqxGrid('getrowdata', row);
						var key = 'row' + data.productId;
						if (editor[0].id){
                ...
                <#if displayDescription?exists && displayDescription = "Y">
					<#assign columnlists = columnlists + "{text: '${StringUtil.wrapString(uiLabelMap.CommonNote)}', dataField: 'description', width: '14%', filterable:false, sortable:false, cellclassname: productGridCellclass},"/>
				</#if>
