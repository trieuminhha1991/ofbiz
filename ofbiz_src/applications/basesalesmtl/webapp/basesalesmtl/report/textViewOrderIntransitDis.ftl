...
<script>
	...
	$("#alterPopupWindowIntransitDetail").jqxWindow({ maxWidth: 700, minWidth: 550, height:270, width:500, minHeight: 100, maxHeight: 500, resizable: false,  isModal: true, autoOpen: false, cancelButton: $("#addButtonCancel"), modalOpacity: 0.7, theme:'olbius'});
	...
	function prepareIntransitDataForGrid(){
		$.ajax({
			...
			for(var i in listIntransitOrderToday){
				var deliveryDate = listIntransitOrderToday[i].deliveryDate;
				if(deliveryDate){
					listIntransitOrderToday[i].deliveryDate = deliveryDate.time;
				}
			}
