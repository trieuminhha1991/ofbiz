...
<script type="text/javascript" id="TopCustomerColumnChart">
$(function(){
	...
	var config = {
		...
		title: {
			text: '${StringUtil.wrapString(uiLabelMap.BSMTopCustomerChart)}',
			x: -20 //center
		},
		xAxis: {
			type: 'category',
    ...
    var columnChart = OLBIUS.oLapChart('TopCustomerColumnChart', config, configPopup, 'evaluateTopCustomerChartv2', true, true, OLBIUS.defaultColumnFunc, 0.39);

    columnChart.funcUpdate(function (oLap) {
        oLap.update({
        	'yearr': oLap.val('yearr'),
            'monthh': oLap.val('monthh'),
            'topCustomer': oLap.val('topCustomer'),
        });
    });
