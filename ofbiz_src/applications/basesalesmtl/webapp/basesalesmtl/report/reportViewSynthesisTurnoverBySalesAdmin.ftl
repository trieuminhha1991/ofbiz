...
<script type="text/javascript">
	...
	var gOrgan; var gOrderStatus; var gFromDate; var gThruDate; var gChannel;
	var testGrid; var testGrid2; var gCustomTime;
...
$(function(){
	...
	$.ajax({url: 'getStoreListColumn',
	    ...
	    success: function(data) {
	    	...
	    	for (var i = 0; i < listDatafield.length; i++){
	    		...
	    		var full_title = code;
	    		var field = {text: full_title, datafield:listDatafield[i].product_id+"_q", type: 'string', width: '10%', cellsalign: 'right', cellsformat: 'n2', align: 'center'};
	...
	column.push({text: "${StringUtil.wrapString(uiLabelMap.BSStaffId)}", datafield:'staffId', align: 'center', cellsalign: 'left', type: 'string', width: '10%', pinned : true});
	column.push({text: "${StringUtil.wrapString(uiLabelMap.BSFullName)}", datafield:'staffName', cellsalign: 'left', type: 'string', width: '15%', pinned : true, align: 'center' });
	column.push({text: "${StringUtil.wrapString(uiLabelMap.BSVolumeTotal)}", datafield:'volumeTotal', cellsalign: 'right', type: 'number', width: '10%', pinned : true, cellsformat: 'n2', });
	...
	$.ajax({url: 'getStoreListColumn',
	    ...
	    success: function(data) {
	    	...
	    	for (var i = 0; i < listDatafield.length; i++){
	    		...
	    		var field3 = {text: full_title, datafield:listDatafield[i].product_id+"_t", type: 'string', width: '10%', cellsalign: 'right', cellsformat: 'n2'};
	    		column.push(field);
	    		column.push(field3);
	    	}
	    },
