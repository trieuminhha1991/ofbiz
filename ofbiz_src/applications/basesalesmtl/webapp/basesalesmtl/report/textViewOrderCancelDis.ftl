...
<div id="alterPopupWindowCancelDetail" style="display : none;">
	<div>
		${uiLabelMap.BSMOrderCancelToday}
	</div>
	<div>
...
<script>
	var position_type = "DISTRIBUTORR";
	$("#alterPopupWindowCancelDetail").jqxWindow({ maxWidth: 700, minWidth: 550, height:270, width:500, minHeight: 100, maxHeight: 500, resizable: false,  isModal: true, autoOpen: false, cancelButton: $("#addButtonCancel"), modalOpacity: 0.7, theme:'olbius'});
	...
	function prepareCancelDataForGrid(){
		$.ajax({
			...
			for(var i in listCancelOrderToday){
				var deliveryDate = listCancelOrderToday[i].deliveryDate;
				if(deliveryDate){
					listCancelOrderToday[i].deliveryDate = deliveryDate.time;
				}
			}
