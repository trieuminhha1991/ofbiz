...
<#assign productStore = delegator.findList("ProductStore", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition(Static["org.ofbiz.base.util.UtilMisc"].toMap("payToPartyId", ownerPartyId)), null, null, null, false)>
	...
	var date_type_source_custom = [
        {text:'${StringUtil.wrapString(uiLabelMap.olap_month)}', value: 'MONTH'}, 
	];
...
$(function(){
        ...
        var config = {
			title: '${StringUtil.wrapString(uiLabelMap.BSMNewAgency)}',
            service: 'salesOrder',
            columns: [
                ...
                { text: '${StringUtil.wrapString(uiLabelMap.BSMAgencyCode)}', datafield: 'agency', type: 'string', width: '15%'},
                { text: '${StringUtil.wrapString(uiLabelMap.BSMAgency)}', datafield: 'agency_name', type: 'string', width: '20%'},
                { text: '${StringUtil.wrapString(uiLabelMap.BSSalesmanCode)}', datafield: 'salesman', type: 'string', width: '16%'},
                { text: '${StringUtil.wrapString(uiLabelMap.BSSalesman)}', datafield: 'salesman_name', type: 'string', width: '20%'},
                { text: '${StringUtil.wrapString(uiLabelMap.BSDistributor)}', datafield: 'distributor', type: 'string', width: '13%'},
                { text: '${StringUtil.wrapString(uiLabelMap.BSValueTotal2)}', datafield: 'value_total', type: 'number', width: '13%', cellsformat: 'n2', cellsalign: 'right'},
            ]
        };
		...
		},
		{
       	...
       	var gGrid = OLBIUS.oLapGrid('test', config, configPopup, 'getNewAgencyListGrid', true);
