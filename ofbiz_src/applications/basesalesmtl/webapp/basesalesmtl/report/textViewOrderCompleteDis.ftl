...
<script>
	$("#alterPopupWindowDetail").jqxWindow({ maxWidth: 700, minWidth: 550, height:270, width:500, minHeight: 100, maxHeight: 500, resizable: false,  isModal: true, autoOpen: false, cancelButton: $("#addButtonCancel"), modalOpacity: 0.7, theme:'olbius'});
	function viewDetail(){
		prepareDataForGrid();
	};
	...
	function prepareDataForGrid(){
		$.ajax({
			...
			for(var i in listCompleteOrderToday){
				var deliveryDate = listCompleteOrderToday[i].deliveryDate;
				if(deliveryDate){
					listCompleteOrderToday[i].deliveryDate = deliveryDate.time;
				}
			}
