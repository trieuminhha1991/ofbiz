...
<#assign columnlist="{text: '${StringUtil.wrapString(uiLabelMap.DmsSequenceId)}', datafield: '', pinned: true, groupable: false, filterable: false, draggable: false, resizable: false, width: 50,
					...
					{ text: '${StringUtil.wrapString(uiLabelMap.DmsAgreementId)}', datafield: 'agreementCode', width: 120,
						cellsrenderer: function (row, column, value) {
							var data = $('#listAgreementAgent').jqxGrid('getrowdata', row);
							return '<div style=margin:4px;><a href=AgreementDetail?sub=AgreementWithAgent&agreementId=' + data.agreementId + '>' + value + '</div>';
						}
					},
					{ text: '${StringUtil.wrapString(uiLabelMap.BSAgentId)}', datafield: 'partyIdFrom', width: 150},
...
<@jqGrid id="listAgreementAgent" dataField=dataField columnlist=columnlist clearfilteringbutton="true" showtoolbar="true"
	url="jqxGeneralServicer?sname=JQGetListAgreementAgent&partyIdFrom=${parameters.partyIdFrom?if_exists}" editable="false"
...
<#assign listStatusItem = delegator.findList("StatusItem", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("statusTypeId", "AGREEMENT_STATUS"), null, null, null, true) />
<script>
	...
	$(document).ready(function() {
		var mainGrid = $("#listAgreementAgent");
