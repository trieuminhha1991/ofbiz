...
$(document).ready(function() {
	...
	Validator.init();
	if (partyIdPram) {
		AddMTConsignee.letUpdate(partyIdPram);
...
if (typeof (AddMTConsignee) == "undefined") {
	var AddMTConsignee = (function() {
			...
			$('#fuelux-wizard').ace_wizard().on('change' , function(e, info) {
		    	...
		    	bootbox.dialog(mesConfirm,
					[{
		    		    ...
		    		    "callback": function() {
		    		    	Loading.show();
							var url = UpdateMode?"updateMTConsignee":"createMTConsignee";
							var logoImageUrl;
							if ($('#logoImageUrl').prop('files')[0]) {
								logoImageUrl = DataAccess.uploadFile($('#logoImageUrl').prop('files')[0]);
							}
							var data = AddMTConsignee.getValue();
							data.logoImageUrl = logoImageUrl;
							DataAccess.execute({ url: url, data: data }, AddMTConsignee.notify);
		    		    }
					},
					{
		...
		var notify = function(res) {
			...
			if(res["_ERROR_MESSAGE_"] || res["_ERROR_MESSAGE_LIST_"]){
				...
				setTimeout(function() {
					if (res) {
						if (res['partyId']) {
							location.href = "MTConsigneeDetail?partyId=" + res['partyId'];
						}
					}
				}, 2000);
			}
