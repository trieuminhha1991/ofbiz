$(function() {  
	PurDlvProductDate.init();
});

var PurDlvProductDate = (function() {
	var rowSelectedId = null;
	var grid = $('#jqxGridProductDate');
				...
				if (dataField == 'quantity'){
					if (value != oldvalue){
						if (items && items.length > 0){
							...
							if (!check){
								...
								newItem["orderItemSeqId"] = rowData.orderItemSeqId;
					...
					if (rowData.quantity > 0){
						...
						if (items && items.length > 0){
							...
							items.push(y);
							listProductMap[rowData.orderItemSeqId] = groupAndSum(items);
						}
					}
				}
			}
	...
	var getColumns = function(grid){
		var columns = [
			...
			{ text: uiLabelMap.ProductId, dataField: 'productCode', width: '12%', minwidth: 100, editable: false, pinned: true, cellclassname:productGridPromoclass},
			{ text: uiLabelMap.ProductName, dataField: 'productName', width: '25%', minwidth: 120, editable:false, cellclassname: productGridPromoclass},
			...
			{ text: uiLabelMap.ActualReceivedQuantitySum, datafield: 'quantity', sortable: false,  width: '15%', editable: true, filterable: false, sortable: false, cellclassname: productGridCellclass,
				...
				cellsrenderer: function(row, column, value) {
				    ...
				    if (listProductSelected.length > 0){
				    	$.each(listProductSelected, function(i){
			   				var olb = listProductSelected[i];
			   				if (olb.orderItemSeqId == rowData.orderItemSeqId && olb.orderItemSeqId == rowData.orderItemSeqId){
			   					value = olb.quantity;
			   					return false;
			   				}
			   			});
	...
	var rendertoolbarInfo = function (toolbar){
        ...
        container.append('<a href="javascript:PurDlvProductDate.deleteRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="red fa fa-times"></i></a>');
        container.append('<a href="javascript:PurDlvProductDate.addRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="fa fa-plus"></i></a>');
        container.append('<a href="javascript:PurDlvProductDate.addMultiRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="fa fa-plus-square"></i></a>');
	...
	var productGridPromoclass = function (row, column, value, data) {
    	if (data.isPromo == 'Y'){
	...
	var generaterow = function (){
		  ...
		  if (productSelected){
			  ...
			  row['deliveryItemSeqId'] = productSelected.deliveryItemSeqId;
			  row['orderItemSeqId'] = productSelected.orderItemSeqId;
			  row['expireDate'] = null;
	...
	var deleteRow = function (){
		...
		if (data){
			if (data.quantity > 0){
				if (productSelected){
					var items = listProductMap[productSelected.orderItemSeqId];
					if (items){
						$.each(items, function(i){
			   				var olb = items[i];
						...
						if (items.length > 0){
							listProductMap[productSelected.orderItemSeqId] = items;
						} else {
							delete listProductMap[productSelected.orderItemSeqId];
						}
					}
				}
			}
	...
	return {
		...
		updateGridProductLocalData: updateGridProductLocalData,
		addRow:addRow,
