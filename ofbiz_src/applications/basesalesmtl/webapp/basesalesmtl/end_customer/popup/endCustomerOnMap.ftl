...
<script>
    ...
    function updateAgentLocation() {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                OlbAgentDetailOnMap.notify("success")
                setTimeout(function(){$("#button-close").click() }, 500);
            },
            error: function(err){OlbAgentDetailOnMap.notify(err); console.log(err)}
        })
    }
    ...
    var OlbAgentDetailOnMap = (function() {
        var init = function () {
            initJqxElements()
            if (!editable) {
                $("#button-update").hide();
        ...
        var openMapDetail = function(partyId, geoPointId, address, latitude, longitude) {
            var agent = {};
            var lat = null;
            var lon = null;
            agent["address1"] = address;
            agent["partyId"] = partyId;
            agent["geoPointId"] = geoPointId;
