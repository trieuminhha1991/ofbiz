<script>
    ...
    var cellclassname = function (row, columnfield, value) {
 			...
 			if (typeof(data) != 'undefined') {
 				...
 				} else if ("PARTY_CREATED" == data.statusId) {
 					return "background-important-nd";
 				} else if ("PARTY_APPROVED" == data.statusId) {
...
<#assign columnlist="
				...
				{ text: '${StringUtil.wrapString(uiLabelMap.BSUserLoginId)}', datafield: 'userLoginId', width: 150, cellclassname: cellclassname},
...
<@jqGrid id=id dataField=dataField columnlist=columnlist clearfilteringbutton="true" selectionmode="singlerow"
	showtoolbar="true" url="jqxGeneralServicer?sname=JQGetCustomerRegistration&isSelfReg=Y&isAppAccount=Y" contextMenuId="Context${id}"  mouseRightMenu="true"
...
<#include "requestNewRegisterCustomerUpdatePopup.ftl"/>
	...
	var ContexMenuCustomer = (function(){
		...
		var approveCustomer = function(rowIndexs){
			if (rowIndexs && rowIndexs.length) {
				...
				if (flagApprove) {
                    Request.post('approveRequestRegisterMobileCustomer', obj, function(res){
                        if (res.partyId) {
                            reloadGrid();
