<script type="text/javascript">
    ...
    if (partyIdPram) {
        UpdateMode = true;
        document.title = "${StringUtil.wrapString(uiLabelMap.BSUpdateEndCustomer)}";
        $($(".breadcrumb").children()[2]).html("<span class='divider'><i class='icon-angle-right'></i></span>${StringUtil.wrapString(uiLabelMap.BSUpdateEndCustomer)}");
        $($(".widget-header").children("h4")).html("${StringUtil.wrapString(uiLabelMap.BSUpdateEndCustomer)}");
    }
</script>
...
<div class="row-fluid">
    ...
    <div class="step-content row-fluid position-relative" id="step-container">
		<#include "endCustomerNewInfo.ftl"/>

		<#include "endCustomerNewContactInfo.ftl"/>

		<#include "endCustomerNewOwnerInfo.ftl"/>
    </div>
                    ...
                    success: function(data){
                        jOlbUtil.processResultDataAjax(data, function(data, errorMessage){
                                    ...
                                    if (data.partyId != undefined && data.partyId != null) {
                                        window.location.href = "EndCustomerDetail?partyId=" + data.partyId;
                                    }
                                }
                        );
                    },
