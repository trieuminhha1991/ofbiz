...
<#if !endCustomerInfo?exists><#assign endCustomerInfo = {}/></#if>
<div class="row-fluid">
    <div class="span12">
                ...
                <div style="width:100%; border-bottom: 1px solid #c5d0dc"><#--widget-toolbar no-border-->
                    <div class="row-fluid">
                        ...
                        <div class="span2" style="height:34px; text-align:right">
                            <#if endCustomerInfo.partyId?exists>
                                <a href="javascript:void(0)" data-rel="tooltip"
                                   onclick="OlbAgentDetailOnMap.openMapDetail('${endCustomerInfo.partyId?default("")}','${endCustomerInfo.geoPointId?default("")}','${endCustomerInfo.address1?default("")}', '${endCustomerInfo.latitude?default('')}', '${endCustomerInfo.longitude?default('')}')"
                                   title="${uiLabelMap.BSTitleLocaltionAgent}" data-placement="left" class="button-action">
                                    <i class="fa fa-globe open-sans"></i>
                                </a>
							</#if>
							<#if endCustomerInfo.partyId?exists && hasOlbPermission("MODULE", "${permission}", "UPDATE")>
								<a href="<@ofbizUrl>${linkEdit}?partyId=${endCustomerInfo.partyId}<#if parameters.sub?has_content>&sub=${parameters.sub?if_exists}</#if></@ofbizUrl>" data-rel="tooltip"
            ...
            <div class="widget-body" style="margin-top: -12px !important">
                <div class="widget-main padding-4">
                    <div class="tab-content overflow-visible" style="padding:8px 0">
						...
						<#include "endCustomerViewInfo.ftl"/>

						<#include "endCustomerViewOwner.ftl">

                        <#if endCustomerInfo.representativeOfficeId?has_content>
							<#include "endCustomerViewRep.ftl">
						</#if>

						<#include "endCustomerViewPartyRel.ftl">
...
<#include "popup/endCustomerOnMap.ftl">

<#if endCustomerInfo.statusId == "PARTY_DISABLED" && security.hasEntityPermission("CUSTOMER_MT", "_APPROVE", session)>
...
<#if endCustomerInfo.statusId == "PARTY_ENABLED" && security.hasEntityPermission("CUSTOMER_MT", "_APPROVE", session)>
