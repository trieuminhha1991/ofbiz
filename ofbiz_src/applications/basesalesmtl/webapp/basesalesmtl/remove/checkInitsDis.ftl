<style type="text/css">
	...
	[id^=horizontalScrollBarjqxParty]{
	...
	<div class="span6">
		<form class="form-horizontal basic-custom-form form-size-mini" id="initOrderEntry" name="initOrderEntry" method="post" action="<@ofbizUrl>initSalesOrderEntryDis</@ofbizUrl>" style="display: block;">
			...
			<div class="control-group">
				...
				<div class="controls">
					<div class="span12">
						<input type="text" name="orderId" id="orderId" class="span12 input-small" style="width:250px" maxlength="20" value="<#if cart?exists && cart?has_content && cart.getOrderId()?exists>${cart.getOrderId()?exists}<#else>${parameters.orderId?if_exists}</#if>">
					</div>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label required" for="salesmanId">${uiLabelMap.DASalesman}</label>
				<div class="controls">
					<div class="span12">
						<div id="salesmanId">
		 					<div id="jqxPartySalesmanGrid"></div>
		 				</div>
					</div>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<div class="span12">
						<div id="partyId">
		 					<div id="jqxPartyCustomerGrid"></div>
		 				</div>
					</div>
				</div>
			</div>
...
<#assign columnlist="{ text: '${uiLabelMap.DAProductId}', dataField: 'productId', width: '14%', editable:false},
                 	...
                 	{ text: '${uiLabelMap.DAQOHTotal}', dataField: 'qohTotal', width: '10%', editable:false, filterable:false, sortable:false, cellsformat :'n'},
                 	{ text: '${uiLabelMap.DAATPTotal}', dataField: 'atpTotal', width: '10%', editable:false, filterable:false, sortable:false, cellsformat : 'n'},
...
<script type="text/javascript">
	...
	$("#desiredDeliveryDate2").jqxDateTimeInput({width: '248px', height: '25px', allowNullDate: true, value: null, formatString: 'dd/MM/yyyy HH:mm:ss'});
	...
	initDropdownButton($("#salesmanId"), $("#jqxPartySalesmanGrid"), "JQGetListSalesmanByDist", "${userLogin.partyId?if_exists}");
	$("#jqxPartySalesmanGrid").on('rowselect', function (event) {
        var args = event.args;
        var row = $("#jqxPartySalesmanGrid").jqxGrid('getrowdata', args.rowindex);
        var dropDownContent = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['partyId'] + '</div>';
        $("#salesmanId").jqxDropDownButton('setContent', dropDownContent);
        $("#partyId").val("");
        var salesmanId = $("#salesmanId").val();
        var tmpAddress = $("#jqxPartyCustomerGrid").jqxGrid('source');
	 	tmpAddress._source.url = "jqxGeneralServicer?sname=JQGetListCustomerBySalesman&partyId="+salesmanId;
	 	$("#jqxPartyCustomerGrid").jqxGrid('source', tmpAddress);
    });
    <#if salesmanId?exists>
    	$("#salesmanId").jqxDropDownButton('setContent', '<div style="position: relative; margin-left: 3px; margin-top: 5px;">${salesmanId}</div>');
    </#if>
    
    initDropdownButton($("#partyId"), $("#jqxPartyCustomerGrid"), "JQGetListCustomerBySalesman", "");
    $("#jqxPartyCustomerGrid").on('rowselect', function (event) {
        var args = event.args;
        var row = $("#jqxPartyCustomerGrid").jqxGrid('getrowdata', args.rowindex);
        var dropDownContent = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['partyId'] + '</div>';
        $("#partyId").jqxDropDownButton('setContent', dropDownContent);
        updateCheckoutArea();
    });
    
    <#if currentPartyId?exists>
    	$("#partyId").jqxDropDownButton('setContent', '<div style="position: relative; margin-left: 3px; margin-top: 5px;">${currentPartyId?string}</div>');
    </#if>
	...
	function initDropdownButton(dropDownButtonObj, jqxGridObj, urlSName, partyIdCond) {
		dropDownButtonObj.jqxDropDownButton({ width: 248, height: 25});
		var sourcePartySalesman = {
					...
					beforeprocessing: function (data) {
					    sourcePartySalesman.totalrecords = data.TotalRows;
					},
					filter: function () {
					   	// update the grid and send a request to the server.
					   	jqxGridObj.jqxGrid('updatebounddata');
					},
		...
		var dataAdapterPF = new $.jqx.dataAdapter(sourcePartySalesman, {
	    	autoBind: true,
	    	formatData: function (data) {
	        ...
	        downloadComplete: function (data, status, xhr) {
	                if (!sourcePartySalesman.totalRecords) {
	                    sourcePartySalesman.totalRecords = parseInt(data['odata.count']);
	                }
	        }
	    });	
		jqxGridObj.jqxGrid({
			...
			columns:[{text: '${uiLabelMap.DAPartyId}', datafield: 'partyId', width: 160 },
						{text: '${uiLabelMap.DAFullName}', datafield: 'fullName', width: 160 },
						{text: '${uiLabelMap.DAFirstName}', datafield: 'firstName', width: 160 },
						{text: '${uiLabelMap.DAMiddleName}', datafield: 'middleName', width: 160 },
						{text: '${uiLabelMap.DALastName}', datafield: 'lastName', width: 160 }]
		});
	}
</script>
