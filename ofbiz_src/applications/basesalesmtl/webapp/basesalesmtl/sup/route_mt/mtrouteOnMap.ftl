<div id="PopupRouteOnMap" class='hide'>
    <div>
    ...
    <div class="form-window-container">
        <div class='row-fluid'>
            <div class='span10'>
                <div id="mapRoute" style="height: 500px;"></div>
            </div>
...
<script>
    if (typeof (OlbRouteOnMap) == "undefined") {
        var OlbRouteOnMap = (function() {
            var self = {};
            self.popup;
            ...
            self.bindEvent = function(){
                ...
                $("#dropdownSalesmanRouteMap").on('close', function (event) {
                    ...
                    if (OlbCore.isNotEmpty(self.currentSalesmanId)) {
                        var url = 'jqxGeneralServicer?sname=JQGetListRouteSimpleMT&pagesize=0' + '&salesmanId=' + self.currentSalesmanId;
            ...
            self.setMarker = function (routeMap) {
                ...
                if (OlbCore.isNotEmpty(bounds.b)) {
                    if (bounds.b.b === 180) {
            ...
            self.open = function(row) {
                ...
                routeCBB.clearAll();
                var rowIndexSelected = $("#ListRoute").jqxGrid('getSelectedRowindex');
                var rowData = $("#ListRoute").jqxGrid('getrowdata', rowIndexSelected);
                var url = 'jqxGeneralServicer?sname=JQGetListRouteSimpleMT&pagesize=0';
                if (OlbCore.isNotEmpty(rowData)) {
                    salesmanDDB.selectItem([rowData.salesmanId]);
                    url = 'jqxGeneralServicer?sname=JQGetListRouteSimpleMT&pagesize=0' + '&salesmanId=' + rowData.salesmanId;
                }
                self.updateSource(url);
                if (OlbCore.isNotEmpty(rowData)) {
                    routeCBB.selectItem([rowData.routeId]);
                }
            };
    ...
    $(function () {
        // ensure that OlbRouteOnMap init one time.
        if (flagPopupLoadRouteOnMap) {
            OlbRouteOnMap.init();
            flagPopupLoadRouteOnMap = false;
            setTimeout(function(){ flagPopupLoadRouteOnMap = true }, 300);
        }
    });
</script>
