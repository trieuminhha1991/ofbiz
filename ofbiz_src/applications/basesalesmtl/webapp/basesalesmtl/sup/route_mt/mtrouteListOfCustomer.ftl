<div id="AddGoogleMapForm" class='hide'>
    <div>
    ${uiLabelMap.BSAddMTCustomerToRoute}
    </div>
    ...
    var CustomerRoute = (function () {
        ...
        self.initMap = function () {
            ...
            $('#pick-day').on('close', function (event) {
                var items = event.owner.listBox.items.filter(function (item) {
                    return item.checked
                }).map(function (item) {
                    return item.value
                });
        ...
        self.removeCustomerAction = function () {
            ...
            Request.post("removeRouteStores", bin, function (res) {
                if (!!res._ERROR_MESSAGE_) {
                    ...
                    var index = custionmerInRoute.markers.findIndex(function (mk) {
                        return mk.customerId == self.storeSelected.customerId
                    });
        ...
        self.addCustomerAction = function () {
            ...
            Request.post("createRouteStores", bin, function (res) {
                if (!!res._ERROR_MESSAGE_) {
                    ...
                    newMarker.customerId = self.storeSelected.customerId;
                    var index = custionmerOutRoute.markers.findIndex(function (mk) {
                        return mk.customerId == self.storeSelected.customerId;
                    });
                    ...
                    $("#table-visiting-routes-of-customer").jqxGrid(
                            {
                                ...
                                columns: [
                                    {
                                        text: '${StringUtil.wrapString(uiLabelMap.BsRouteId)}',
                                        datafield: 'routeCode',
                                        width: 140,
                                        cellsrenderer: cellsrenderer
                                    },
                                    {
                                        text: '${StringUtil.wrapString(uiLabelMap.BSRouteName)}',
                                        datafield: 'routeName',
                                        width: 170,
                                        cellsrenderer: cellsrenderer
                                    },
                                    {
                                        text: '${StringUtil.wrapString(uiLabelMap.BSDescription)}',
                                        datafield: 'description',
                                        width: 160,
                                        cellsrenderer: cellsrenderer
                                    },
                                    {
                                        text: '${StringUtil.wrapString(uiLabelMap.BSScheduleDescription)}',
                                        datafield: 'scheduleRoute',
                                        width: 'auto',
                                        cellsrenderer: cellsrenderer
                                    }
                                ]
                ...
                if (res.results) {
                    ...
                    if (!!self.map) {
                        ...
                        if (self.data.salesmanId) {
                            Request.post("getCustAvailableBySalesmanMT", {
                                salesmanId: self.data.salesmanId,
        ...
        self.drawStoreMarker = function (stores, color, type) {
            ...
            stores.forEach(function (store, i) {
                ...
                if (!!latitude && typeof latitude == "number" && latitude > -360 && latitude < 360 &&
                    ...
                    var name = !!store.fullName ? store.fullName : store.customerName;
                    var address = !!store.address1 ? store.address1 : store.postalAddressName;
                    var customerIdStr = !!store.customerId ? store.customerId : store.partyId;
                    var html = "<i class='fa fa-tag'></i>&nbsp;" + customerIdStr + "</br>"
            ...
            $("#table-store-no-location").jqxGrid(
                    {
                        ...
                        columns: [
                            ...
                            {
                                text: '${uiLabelMap.Address}',
                                datafield: 'postalAddressName',
                                width: 'auto',
                                cellsrenderer: cellsrenderer
                            }
                        ]
        ...
        self.updateStoreMarker = function (store, color, type) {
            ...
            if (!!latitude && typeof latitude == "number" && latitude > -360 && latitude < 360 &&
                    !!longitude && typeof longitude == "number" && longitude > -360 && longitude < 360) {
                var index = self.markers.findIndex(function (mk) {
                    return mk.customerId == store.customerId
                })
                        ...
                        if (!viewOutNoneRoute.selected) {
                            if (!!self.customerNoneRoute) {
                                view.markers.filter(function (marker) {
                                    return !!self.customerNoneRoute.find(function (customer) {
                                        return customer.customerId === marker.customerId
                                    });
    ...
    $(function () {
        ...
        if (typeof (CustomerRoute) != "undefined") {
            if (flagPopupLoadListOfCustomer) {
                ...
                setTimeout(function () {
                    flagPopupLoadListOfCustomer = true
                }, 300);
            }
        }
    });
