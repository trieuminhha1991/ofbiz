<div id="PopupCustomerOnMap" class='hide'>
    <div>
    ...
    <div class="form-window-container">
        ...
        <div class="form-action">
            ...
            <button id="updateLocation" class="btn btn btn-success form-action-button pull-right" onClick="OlbCustomerOnMap.updateCustomerLocation()"><i class="fa-floppy-o"></i>${uiLabelMap.BSUpdateLocation}</button>
        </div>
    </div>
</div>

<script>
    if (typeof (OlbCustomerOnMap) == "undefined") {
        var OlbCustomerOnMap = (function() {
            var self = {};
            ...
            self.updateCustomerLocation = function(){
                $.ajax({
                    ...
                    complete: function(data){
                        ...
                        self.close();
                        OlbCustomerRoute.updateSource(self.currCustomer.routeId);
                    }
                });
            };
            ...
            self.open = function(data) {
                ...
                if (OlbCore.isNotEmpty(data)) {
                    ...
                    var addressMatch = data.postalAddressName.match(/[^(null)\s,].+/); //exclude null,
    ...
    $(function () {
        // ensure that OlbCustomerOnMap init one time.
        if (flagPopupLoadCustomerOnMap) {
            OlbCustomerOnMap.init();
            flagPopupLoadCustomerOnMap = false;
            setTimeout(function(){ flagPopupLoadCustomerOnMap = true }, 300);
        }
    });
</script>
