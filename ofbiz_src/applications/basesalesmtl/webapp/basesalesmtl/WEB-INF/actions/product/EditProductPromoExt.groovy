...
if (productPromoId) {
	...
	if (productPromo != null) {
		...
		// TODO: not use
		List<GenericValue> productStorePromoAppl = delegator.findList("ProductStorePromoExtAppl", EntityCondition.makeCondition(listCond, EntityOperator.AND), null, null, null, false);
		...
		if (productPromo && productPromo.productPromoTypeId) {
			GenericValue productPromoType = delegator.findOne("ProductPromoExtType", UtilMisc.toMap("productPromoTypeId", productPromo.productPromoTypeId), false);
			if (productPromoType) context.channelId = productPromoType.productPromoTypeGroupId;
		}
		
		// Get list product promotion rule
		List<GenericValue> listProductPromoRule = delegator.findByAnd("ProductPromoExtRule", UtilMisc.toMap("productPromoId", productPromo.productPromoId), UtilMisc.toList("productPromoRuleId"), false);
		// Get list product promotion condition
		List<GenericValue> listProductPromoCond = delegator.findByAnd("ProductPromoExtCond", UtilMisc.toMap("productPromoId", productPromo.productPromoId), UtilMisc.toList("productPromoRuleId", "productPromoCondSeqId"), false);
		// Get list product promotion action
		List<GenericValue> listProductPromoAction = delegator.findByAnd("ProductPromoExtAction", UtilMisc.toMap("productPromoId", productPromo.productPromoId), UtilMisc.toList("productPromoRuleId", "productPromoActionSeqId"), false);
		// Get list product promo product
		List<GenericValue> listProductPromoProduct = delegator.findByAnd("ProductPromoExtProduct", UtilMisc.toMap("productPromoId", productPromo.productPromoId), null, false);
		// Get list product promo category
		List<GenericValue> listProductPromoCategory = delegator.findByAnd("ProductPromoExtCategory", UtilMisc.toMap("productPromoId", productPromo.productPromoId), null, false);
