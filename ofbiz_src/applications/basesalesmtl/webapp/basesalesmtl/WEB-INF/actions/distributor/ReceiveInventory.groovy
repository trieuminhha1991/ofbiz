...
end out of the box code*/
// start new code
ownerParty = null;
if (facility) {
	owner = facility.getRelatedOne("OwnerParty", false);
	if (owner) {
		ownerParty = delegator.findOne("Party", ["partyId" : owner.partyId], false);
	}
}
//end new code
...
if (purchaseOrder && facility) {
    if (ownerParty) {
        ownerCurrencyUomId = ownerParty.preferredCurrencyUomId;
...
if (ownerParty) {
    ...
    if (purchaseOrderItems) {
        purchaseOrderItems.each { orderItem ->
            ...
            if (productId) {
                result = dispatcher.runSync("getProductCost", [productId : productId, currencyUomId : ownerParty.preferredCurrencyUomId,
