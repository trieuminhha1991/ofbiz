...
public class ProductPromoEvents {
	public static final String module = ProductPromoEvents.class.getName();
	...
	public static String createProductPromoExtAdvance(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!SecurityUtil.getOlbiusSecurity(security).olbiusHasPermission(session, null, "MODULE", "PRODPROMOTION_EXT_NEW")) {
        ...
        try {
        	...
        	String organizationPartyId = SalesUtil.getCurrentOrganization(delegator, userLogin.getString("partyId"));
        	...
        	Map<String, Object> result0 = dispatcher.runSync("createProductPromoExt", contextMap);
	        ...
	        if (rowCount < 1) {
	            ...
	            for (int i = 0; i < rowCount; i++) {
	                ...
	                Map<String, Object> result1 = dispatcher.runSync("createProductPromoExtRule", 
	                		UtilMisc.<String, Object>toMap("productPromoId", productPromoIdSuccess, "ruleName", ruleName, "ruleText", ruleText, "userLogin", userLogin, "locale", locale));
	                // no values for price and paramMap (a context for adding attributes)
	    	        ...
	    	        if (rowCountSeqCond < 1) {
	    	        	...
	    	        	for (int j = 0; j < rowCountSeqCond; j++) {
	    	                ...
	    	                if (UtilValidate.isNotEmpty(condValue)) {
	    	                	Map<String, Object> result2 = dispatcher.runSync("createProductPromoExtCond", 
	    	        ...
	    	        if (rowCountSeqAction < 1) {
	    	        	...
	    	        	for (int j = 0; j < rowCountSeqAction; j++) {
	    	                ...
	    	                //if (productPromoActionOperEnumId == null) productPromoActionOperEnumId = "PPAOP_AND";
	    	                if (quantity != null || amount != null) {
	    	                    //Debug.logInfo("Attempting to add to cart with productId = ", module);
	    	                	Map<String, Object> result3 = dispatcher.runSync("createProductPromoExtAction", 
	    	                    		UtilMisc.<String, Object>toMap("productPromoId", productPromoIdSuccess, 
	...
	@SuppressWarnings("unchecked")
	public static String updateProductPromoExtAdvance(HttpServletRequest request, HttpServletResponse response) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Locale locale = UtilHttp.getLocale(request);
        HttpSession session = request.getSession();
        ...
        if (!SecurityUtil.getOlbiusSecurity(security).olbiusHasPermission(session, null, "MODULE", "PRODPROMOTION_EXT_EDIT")) {
        ...
        try {
        	...
        	contextMap.put("userLogin", userLogin);
        	contextMap.put("locale", locale);
        	Map<String, Object> result0 = dispatcher.runSync("updateProductPromoExt", contextMap);
        	// no values for price and paramMap (a context for adding attributes)
            controlDirective = SalesUtil.processResult(result0, request);
            if (controlDirective.equals("error")) {    // if the add to cart failed, then get out of this loop right away
            	try {
            ...
            List<GenericValue> listProductStorePromoAppl = delegator.findByAnd("ProductStorePromoExtAppl", 
            ...
            if (listProductStorePromoAppl != null) {
            	...
            	for (GenericValue productStorePromoAppl : listProductStorePromoAppl) {
            		if (productStoreIds != null && productStoreIds.contains(productStorePromoAppl.getString("productStoreId"))) {
            	...
            	if (productStoreIds != null) {
            		for (String productStoreId : productStoreIds) {
                		if (productStoreIdAppls.contains(productStoreId)) {
                			...
                			// create new
                			GenericValue newProductStorePromoAppl = delegator.makeValue("ProductStorePromoExtAppl");
            ...
            List<GenericValue> listRoleTypePromoAppl = delegator.findByAnd("ProductPromoExtRoleTypeAppl", 
            		UtilMisc.toMap("productPromoId", productPromoId), null, false);
            if (listRoleTypePromoAppl != null) {
            	...
            	for (GenericValue roleTypePromoAppl : listRoleTypePromoAppl) {
            		if (roleTypeIds != null && roleTypeIds.contains(roleTypePromoAppl.getString("roleTypeId"))) {
            			if (fromDate.equals(roleTypePromoAppl.getTimestamp("fromDate"))) {
            				...
            				GenericValue newProductPromoRoleTypeAppl = delegator.makeValue("ProductPromoExtRoleTypeAppl");
            ...
            List<GenericValue> listProductPromoProduct = delegator.findByAnd("ProductPromoExtProduct", UtilMisc.toMap("productPromoId", productPromoId), null, false);
            List<GenericValue> listProductPromoCategory = delegator.findByAnd("ProductPromoExtCategory", UtilMisc.toMap("productPromoId", productPromoId), null, false);
	        ...
	        if (rowCount < 1) {
	            ...
	            for (int i = 0; i < rowCount; i++) {
	                ...
	                GenericValue productPromoRule = delegator.findOne("ProductPromoExtRule", UtilMisc.toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId), false);
	                ...
	                if ("Y".equals(isRemoveRule)) {
	                	if (isRuleExisted) {
	                		// delete condition
	    	        		Map<String, Object> result2 = dispatcher.runSync("deleteProductPromoExtRule", UtilMisc.<String, Object>toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId, "userLogin", userLogin, "locale", locale));
	    	                // no values for price and paramMap (a context for adding attributes)
	    	                controlDirective = SalesUtil.processResult(result2, request);
	    	                if (controlDirective.equals("error")) {    // if the add to cart failed, then get out of this loop right away
	    	                	try {
	                ...
	                if (isRuleExisted) {
	                	result1 = dispatcher.runSync("updateProductPromoExtRule", 
		                ...
		                result1 = dispatcher.runSync("createProductPromoExtRule", 
	    	        ...
	    	        if (rowCountSeqCond < 1) {
	    	        	...
	    	        	for (int j = 0; j < rowCountSeqCond; j++) {
	    	                ...
	    	                GenericValue productPromoCond = delegator.findOne("ProductPromoExtCond", UtilMisc.toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId, "productPromoCondSeqId", productPromoCondSeqId), false);
	    	                if (productPromoCond != null) isCondExisted = true;
	    	    	        
	    	    	        if ("Y".equals(isRemoveCond)) {
	    	    	        	if (isRuleExisted && isCondExisted) {
		    	                	// delete condition
	    	    	        		Map<String, Object> result2 = dispatcher.runSync("deleteProductPromoExtCond", UtilMisc.<String, Object>toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId, "productPromoCondSeqId", productPromoCondSeqId, "userLogin", userLogin, "locale", locale));
			    	                // no values for price and paramMap (a context for adding attributes)
			    	                controlDirective = SalesUtil.processResult(result2, request);
			    	                if (controlDirective.equals("error")) {    // if the add to cart failed, then get out of this loop right away
			    	                	try {
	    	                ...
	    	                if (isRuleExisted && isCondExisted) {
	    	                	if (UtilValidate.isNotEmpty(condValue)) {
	    	                		Map<String, Object> result2 = dispatcher.runSync("updateProductPromoExtCond", 
			    	                ...
			    	                if (listProductPromoProductCond != null) {
			    	                	...
			    	                	for (String productId : productIdListCond) {
			    	                		if (productIdAppls.contains(productId)) {
			    	                			...
			    	                			// create new
			    	                			GenericValue newProductPromoProduct = delegator.makeValue("ProductPromoExtProduct");
			    	                ...
			    	                if (listProductPromoCategoryCond != null) {
			    	                	...
			    	                	for (String productCategoryId : productCatIdListCond) {
			    	                		if (categoryIdAppls.contains(productCategoryId)) {
			    	                			...
			    	                			// create new
			    	                			GenericValue newProductPromoCategory = delegator.makeValue("ProductPromoExtCategory");
	    	        ...
	    	        if (rowCountSeqAction < 1) {
	    	        	...
	    	        	for (int j = 0; j < rowCountSeqAction; j++) {
	    	                ...
	    	                GenericValue productPromoAction = delegator.findOne("ProductPromoExtAction", UtilMisc.toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId, "productPromoActionSeqId", productPromoActionSeqId), false);
	    	                if (productPromoAction != null) isActionExisted = true;
	    	                
	    	    	        if ("Y".equals(isRemoveAction)) {
	    	    	        	if (isRuleExisted && isActionExisted) {
		    	                	// delete condition
	    	    	        		Map<String, Object> result2 = dispatcher.runSync("deleteProductPromoExtAction", UtilMisc.<String, Object>toMap("productPromoId", productPromoId, "productPromoRuleId", productPromoRuleId, "productPromoActionSeqId", productPromoActionSeqId, "userLogin", userLogin, "locale", locale));
			    	                // no values for price and paramMap (a context for adding attributes)
			    	                controlDirective = SalesUtil.processResult(result2, request);
			    	                if (controlDirective.equals("error")) {    // if the add to cart failed, then get out of this loop right away
			    	                	try {
	    	                ...
	    	                if (isRuleExisted && isActionExisted) {
		    	                if (quantity != null || amount != null) {
		    	                    //Debug.logInfo("Attempting to add to cart with productId = ", module);
		    	                	Map<String, Object> result3 = dispatcher.runSync("updateProductPromoExtAction", 
		    	                    		UtilMisc.<String, Object>toMap("productPromoId", productPromoId, 
