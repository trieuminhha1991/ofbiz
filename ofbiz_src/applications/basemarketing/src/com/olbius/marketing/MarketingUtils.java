...
//		[(birthDate >= '2015-08-18 00:00:00.0' AND birthDate <= '2015-08-31 23:59:59.0')]
	...
	public static Map<String, Object> listCompact(List<Map<String, Object>> listOriginal, Map<String, ? extends Object> context) {
	...
	@SuppressWarnings("unchecked")
	public static Map<String, Object> listCompact(List<Map<String, Object>> listOriginal, Map<String, ? extends Object> context, int totalRows) {
		Map<String, Object> successResult = ServiceUtil.returnSuccess();
		List<Map<String, Object>> listReturn = FastList.newInstance();
		List<EntityCondition> listAllConditions = (List<EntityCondition>) context.get("listAllConditions");
		List<String> listSortFields = (List<String>) context.get("listSortFields");
	...
	public static Map<String, Object> listCompact(List<Map<String, Object>> listOriginal, List<EntityCondition> listAllConditions, List<String> listSortFields, Map<String, String[]> parameters) {
		Map<String, Object> successResult = ServiceUtil.returnSuccess();
		List<Map<String, Object>> listReturn = FastList.newInstance();
		int pagesize = Integer.parseInt(parameters.get("pagesize")[0]);
		int pageNum = Integer.parseInt(parameters.get("pagenum")[0]);
	...
	public static Map<String, Object> getPartiesByCampaign(Delegator delegator, String partyIdFrom, String marketingCampaignId) throws GenericEntityException{
		...
		if ("any".equals(marketingCampaignId)) {
			...
			conditions.add(EntityCondition.makeCondition(UtilMisc.toMap("partyIdFrom", partyIdFrom, "marketingCampaignId", marketingCampaignId)));
		}
		...
		List<String> listContactId = EntityUtil.getFieldListFromEntityList(partyCampaignRelationships, "partyIdTo", true);
	...
	public static List<EntityCondition> makeCondition(Map<String, ? extends Object> context, Delegator delegator) throws GenericEntityException {
		...
		for (EntityCondition e : listAllConditions) {
			...
			if ("LIKE".equals(operator)) {
				...
				switch (fieldName) {
				case "partyId":
						conditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyId"), EntityJoinOperator.LIKE,  "%" + value.replaceAll("\\s+","%") + "%"));
					break;
				case "groupName":
					conditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("groupName"), EntityJoinOperator.LIKE,  "%" + value.replaceAll("\\s+","%") + "%"));
					break;
				case "partyFullName":
					conditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyFullName"), EntityJoinOperator.LIKE,  "%" + value.replaceAll("\\s+","%") + "%"));
					break;
				case "contactNumber":
						if (value.length() < 4) {
							continue;
						}
						conditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyId"), EntityJoinOperator.IN, listOwnerTelecomNumber(delegator, value)));
					break;
				case "emailAddress":
						conditions.add(EntityCondition.makeCondition("partyId", EntityJoinOperator.IN, listOwnerEmail(delegator, value)));
					break;
				case "representativeMember":
						...
						if (mappingPartyName.containsKey("firstName")) {
							dummyConditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("firstName"), EntityJoinOperator.LIKE, "%" + mappingPartyName.get("firstName").toString().replaceAll("\\s+","%") + "%"));
						}
						if (mappingPartyName.containsKey("middleName")) {
							dummyConditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("middleName"), EntityJoinOperator.LIKE, "%" + mappingPartyName.get("middleName").toString().replaceAll("\\s+","%") + "%"));
						}
						if (mappingPartyName.containsKey("lastName")) {
							dummyConditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("lastName"), EntityJoinOperator.LIKE, "%" + mappingPartyName.get("lastName").toString().replaceAll("\\s+","%") + "%"));
						}
						List<String> partyGourpIds = searchGroupOfParty(delegator,
								EntityCondition.makeCondition(dummyConditions, (EntityJoinOperator)mappingPartyName.get("JoinOperator")));
				...
				case "saler":
					...
					partyIds = searchSalerOfParty(delegator,
							EntityCondition.makeCondition(dummyConditions, (EntityJoinOperator)mappingPartyName.get("JoinOperator")));
...
//							simple search for address1
							partyIds = searchAddressOfParty(delegator,
									EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("address1"), EntityJoinOperator.LIKE, "%" + value + "%"));
							conditions.add(EntityCondition.makeCondition("partyId", EntityJoinOperator.IN, partyIds));
						}
					break;
				case "districtGeoName":
					partyIds = searchAddressOfParty(delegator,
							makeConditionByDistrict(delegator, value));
					conditions.add(EntityCondition.makeCondition("partyId", EntityJoinOperator.IN, partyIds));
					break;
				default:
					conditions.add(e);
				...
				case "resultEnumTypeId":
				    ...
				    EntityCondition ep = resultCondition(delegator, userLogin, e, pagesize, pageNum, isPerson);
				    if(ep != null){
					conditions.add(ep);
				    }
					break;
...
//					(entryDate >= '2015-11-08 00:00:00.0' AND entryDate <= '2015-11-18 23:59:59.0')
				if (condition.contains("AND") && condition.contains("entryDate")) {
//						is range
					List<EntityCondition> conditionsRange = FastList.newInstance();
					conditionsRange.add(EntityCondition.makeCondition(UtilMisc.toMap("communicationEventTypeId", "PHONE_COMMUNICATION",
							"statusId", "COM_SCHEDULED")));
					conditionsRange.add(e);
					List<GenericValue> communicationEvents = delegator.findList("CommunicationEvent",
							EntityCondition.makeCondition(conditionsRange), UtilMisc.toSet("partyIdTo"), null, null, false);
					List<String> partyIds = EntityUtil.getFieldListFromEntityList(communicationEvents, "partyIdTo", true);
					conditions.add(EntityCondition.makeCondition("partyId", EntityJoinOperator.IN, partyIds));
				}
			} else {
				conditions.add(e);
	...
	private static EntityCondition resultCondition(Delegator delegator, GenericValue userLogin, EntityCondition e, int pagesize, int pageNum, boolean isPerson){
...
//			List<EntityCondition> reslist = FastList.newInstance();
			List<String> reslist = FastList.newInstance();
			...
			for(String curCond : listCond){
				...
				if(UtilValidate.isNotEmpty(tmp) && !curCond.contains(default_string)){
					...
					re = re.replace(")", "");
					reslist.add(re);
//					reslist.add(EntityCondition.makeCondition("resultEnumTypeId", re));
					reCond.add(EntityCondition.makeCondition("resultEnumTypeId", EntityOperator.LIKE, "%" + re + "%"));
					reCond.add(EntityCondition.makeCondition("resultEnumTypeId", re));
				}else if(curCond.contains(default_string)){
					reCond.add(EntityCondition.makeCondition("resultEnumTypeId", EntityOperator.EQUALS, null));
					reslist.add(default_string);
				}
			}
			if(UtilValidate.isNotEmpty(reCond)){
				EntityFindOptions opts = new EntityFindOptions();
				opts.setResultSetType(EntityFindOptions.TYPE_SCROLL_SENSITIVE);
				EntityListIterator tmpList = null;
				try {
					List<EntityCondition> listEnCond = UtilMisc.toList(EntityCondition.makeCondition(reCond, EntityOperator.OR),
							EntityCondition.makeCondition("partyIdFrom", partyIdFrom),
//							EntityCondition.makeCondition("partyTypeId", "PERSON"),
							EntityCondition.makeCondition("statusId", EntityJoinOperator.NOT_EQUAL, "CONTACT_REJECTED"),
							EntityCondition.makeCondition("campaignStatusId", EntityJoinOperator.EQUALS, "MKTG_CAMP_INPROGRESS"),
							EntityCondition.makeCondition(
									UtilMisc.toList(EntityCondition.makeCondition("thruDate", EntityOperator.GREATER_THAN_EQUAL_TO,
											new Timestamp(System.currentTimeMillis())),
											EntityCondition.makeCondition("thruDate", null)), EntityOperator.OR));
					listEnCond.add(EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr("fromDatePTR", "thruDatePTR")));
					listEnCond.add(EntityCondition.makeCondition("partyType", EntityJoinOperator.IN, UtilMisc.toList("CUSTOMER", "CONTACT", "INDIVIDUAL_CUSTOMER")));
					EntityCondition endCond = EntityCondition.makeCondition(listEnCond);

					if (isPerson) {
						tmpList = delegator.find("PartyCampaignRelationshipAndPerson", endCond, null,
								UtilMisc.toSet("partyIdFrom","partyIdTo", "communicationEventId", "resultEnumTypeId"), null, opts);
					} else {
						tmpList = delegator.find("PartyCampaignRelationshipAndPartyGroup", endCond, null,
								UtilMisc.toSet("partyIdFrom","partyIdTo", "communicationEventId", "resultEnumTypeId"), null, opts);
					}
//					int start = pagesize * pageNum;
					GenericValue e1 = null;
//					int length = 0;
//					int i = 0;
					List<String> condParty = FastList.newInstance();
					while((e1 = tmpList.next()) != null){
//						if(i < start){
//							continue;
//						}
						if(isPartyHasLatestResult(delegator, e1, reslist)){
//							length++;
							condParty.add(e1.getString("partyIdTo"));
//							if(length >= pagesize){
//								break;
//							}
						}
//						i++;
					...
					if(UtilValidate.isNotEmpty(condParty)){
						return EntityCondition.makeCondition("partyIdTo", EntityOperator.IN, condParty);
					}
	...
	public static boolean isPartyHasLatestResult(Delegator delegator, GenericValue e, List<String> keys){
		...
		if(UtilValidate.isNotEmpty(communicationEventId)){
			...
			for(String entry : tmpEntryList){
				try {
					event = delegator.findOne("CommunicationEvent", UtilMisc.toMap("communicationEventId", entry), false);
	...
	@SuppressWarnings("unchecked")
	public static List<EntityCondition> makeCondition(Map<String, ? extends Object> context, Delegator delegator, boolean forAgreement) throws GenericEntityException {
		List<EntityCondition> conditions = FastList.newInstance();
		List<EntityCondition> listAllConditions = (List<EntityCondition>) context.get("listAllConditions");
		for (EntityCondition e : listAllConditions) {
			String condition = e.toString();
			...
			if ("LIKE".equals(operator)) {
				...
				case "partyFrom":
					conditions.add(EntityCondition.makeCondition(UtilMisc.toList(
							EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("groupName"), EntityJoinOperator.LIKE,  "%" + value.replaceAll("\\s+","%") + "%"),
							EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyFullName"), EntityJoinOperator.LIKE,  "%" + value.replaceAll("\\s+","%") + "%")
	...
	private static List<String> listOwnerEmail(Delegator delegator, String infoString) throws GenericEntityException {
		List<GenericValue> listContactMech = delegator.findList("ContactMech",
				EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("infoString"), EntityJoinOperator.LIKE, "%" + infoString + "%"), UtilMisc.toSet("contactMechId"), null, null, false);
	...
	private static List<String> listOwnerTelecomNumber(Delegator delegator, String contactNumber) throws GenericEntityException {
		List<GenericValue> listTelecomNumber = delegator.findList("TelecomNumber",
				EntityCondition.makeCondition("contactNumber", EntityJoinOperator.LIKE, "%" + contactNumber + "%"), UtilMisc.toSet("contactMechId"), null, null, false);
		List<String> listContactMechId = EntityUtil.getFieldListFromEntityList(listTelecomNumber, "contactMechId", true);
		List<EntityCondition> conditions = FastList.newInstance();
		conditions.add(EntityCondition.makeCondition("contactMechId", EntityJoinOperator.IN, listContactMechId));
	...
	private static List<String> searchGroupOfParty(Delegator delegator, EntityCondition condition) throws GenericEntityException {
		List<GenericValue> persons = delegator.findList("Person",
				condition, UtilMisc.toSet("partyId"), null, null, false);
		...
		conditions.add(EntityCondition.makeCondition(UtilMisc.toMap("roleTypeIdFrom", "REPRESENTATIVE", "roleTypeIdTo", "ORGANIZATION_UNIT")));
	...
	private static List<String> searchSalerOfParty(Delegator delegator, EntityCondition condition) throws GenericEntityException {
		List<GenericValue> persons = delegator.findList("Person",
				condition, UtilMisc.toSet("partyId"), null, null, false);
		List<String> partyIds = EntityUtil.getFieldListFromEntityList(persons, "partyId", true);
		List<EntityCondition> conditions = FastList.newInstance();
		conditions.add(EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr()));
		conditions.add(EntityCondition.makeCondition("partyIdFrom", EntityJoinOperator.IN, partyIds));
		conditions.add(EntityCondition.makeCondition(UtilMisc.toMap("roleTypeIdFrom", "SALES_EXECUTIVE", "partyRelationshipTypeId", "CUSTOMER_REL")));
		List<GenericValue> partyIdTos = delegator.findList("PartyRelationship",
				EntityCondition.makeCondition(conditions), UtilMisc.toSet("partyIdTo"), null, null, false);
		partyIds = EntityUtil.getFieldListFromEntityList(partyIdTos, "partyIdTo", true);
		return partyIds;
	}
	private static List<String> searchAddressOfParty(Delegator delegator, EntityCondition condition) throws GenericEntityException{
		List<GenericValue> postalAddresses = delegator.findList("PostalAddress",
				condition, UtilMisc.toSet("contactMechId"), null, null, false);
	...
	private static EntityCondition makeConditionByGeoType(Delegator delegator, String geoTypeId, String value, String geoId) throws GenericEntityException {
		List<EntityCondition> dummyConditions = FastList.newInstance();
		dummyConditions.clear();
		dummyConditions.add(EntityCondition.makeCondition("geoTypeId", EntityJoinOperator.EQUALS, geoTypeId));
		dummyConditions.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("geoName"), EntityJoinOperator.LIKE, "%" + value + "%"));
		List<GenericValue> geos = delegator.findList("Geo",
				EntityCondition.makeCondition(dummyConditions), UtilMisc.toSet("geoId"), null, null, false);
