...
List listDebtSales = new ArrayList();
lastTimePeriodHistory.each{ period->
	listCycle.add(period.get("periodName"));
	longTermDebt = Fomular.evalueTargetValue(delegator, "9732", reportId1, reportId1, reportIdT1, period.get("customTimePeriodId"), period.getString("isClosed"), parameters.organizationPartyId);
	...
	if(netSales.compareTo(BigDecimal.ZERO) != 0){
		listDebtSales.add(longTermDebt.divide(netSales, 2, RoundingMode.DOWN));
	} else {
		listDebtSales.add(BigDecimal.ZERO);
	}
}
context.listCycle = listCycle;
context.listDebtSales = listDebtSales;