...
List listCashSales = new ArrayList();
lastTimePeriodHistory.each{ period->
	listCycle.add(period.get("periodName"));
	cash = Fomular.evalueTargetValue(delegator, "9656", reportId1, reportIdM1, reportId1, period.get("customTimePeriodId"), period.getString("isClosed"), parameters.organizationPartyId);
	...
	System.out.println("Sale " + netSales);
	System.out.println("Cash " + cash);
	if(netSales.compareTo(BigDecimal.ZERO) != 0){
		listCashSales.add(cash.divide(netSales, 2, RoundingMode.DOWN));
	} else {
		listCashSales.add(BigDecimal.ZERO);
	}
}
context.listCycle = listCycle;
context.listCashSales = listCashSales;