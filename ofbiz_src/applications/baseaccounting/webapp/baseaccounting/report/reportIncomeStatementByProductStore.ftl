...
<#--GRID-->
<div id="incomeStatementReportByProductStore"></div>
...
<script type="text/javascript">
	...
	$(function () {
		var config = {
			title: '${StringUtil.wrapString(uiLabelMap.BACCProductStoreIncomeStatement)}',
			button: true,
			service: "acctgTransTotal",
			id: 'incomeStatementReportByProductStore',
			...
			columns: [
				{ 
					...
					text: '${StringUtil.wrapString(uiLabelMap.BSSaleChanelId)}',
					datafield: {name: 'productStoreId', type: 'string'},
					width: 150, filterable: true, pinned: true
				}, {
					text: '${StringUtil.wrapString(uiLabelMap.BSSaleChanelName)}',
			...
			exportFileName: 'BAO_CAO_DOANH_THU_THEO_CUA_HANG_' + (new Date()).formatDate("ddMMyyyy")
		};
