<script type="text/javascript" id="grid">
	...
	function bindingDataToReport(){
		var config = {
                title: ' ${uiLabelMap.BACCProductIncomeGrowthStm}',
                service: 'acctgTransTotal',
                columns: [
                          { text: '${uiLabelMap.BACCProductName}', datafield: 'productId', width: '28%',
            			  ...
            			  { text: '${uiLabelMap.BACCSaleIncomeRate}', datafield: 'saleIncomeRate', width: '12%',
            				  cellsrenderer: function (row, columnfield, value, defaulthtml, columnproperties) {
        						  return '<span class=align-right>' + (value ? value : '0') + '%' + '</span>';
            				  }  
            			  }
        ...
        var grid = OLBIUS.oLapGrid('grid', config, configPopup, 'getProductIncomeGrowthStm', true);
