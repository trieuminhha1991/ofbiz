...
<#include "liabilityReceivableTotalFilter.ftl">
...
<#assign columnlist="{ text: '${uiLabelMap.BACCCustomerId}', dataField: 'partyCode', width: 120, cellsformat: 'dd/MM/yyyy'},
					 ...
					 { text: '${uiLabelMap.BACCDebitAmount}', dataField: 'openingDebitAmount', width: 150, columngroup: 'openingAmount',
						 cellsrenderer: function(row, column, value){
							...
							var data = $('#liabilityReceivableTotalGrid').jqxGrid('getrowdata',row);
						 ...
						 aggregatesrenderer: function (aggregates, column, element) {
                        	...
                        	var total = 0;
                          	...
                          	var data = $('#liabilityReceivableTotalGrid').jqxGrid('getrows');
...
<@jqGrid id="liabilityReceivableTotalGrid" filtersimplemode="true" filterable="false" addrefresh="true" editable="false" addType="popup" showtoolbar="true" clearfilteringbutton="false"
		 url="jqxGeneralServicer?sname=JqxGetLiabilityReceivableTotal&glAccountId=${receipGlAccount.glAccountId}&fromDate=${fromDate}&thruDate=${thruDate}" sortable="false"
...
<script>
	...
	$('#liabilityReceivableTotalGrid').on("bindingcomplete", function (event) {
        ...
        jQuery.ajax({
            url: 'getTotalLiabilityReceivableTotal',
            async: false,
            type: 'POST',
            data: {
	...
	var exportExcel = function(){
		var allData = $('#liabilityReceivableTotalGrid').jqxGrid('getrows');
		if(allData.length > 0){
            ...
            window.location.href = "exportLiabilityReceivableTotalExcel?glAccountId=" + glId + "&fromDate=" + fromDate + "&thruDate=" + thruDate;
		} else {
    ...
    function getLastUpdated() {
        jQuery.ajax({
            ...
            complete: function(){
            	if (_updating) {
                    _updating = false;
                    $("#liabilityReceivableTotalGrid").jqxGrid('updatebounddata');
