...
<#assign columnlist="{ text: '${uiLabelMap.BACCTransDate}', dataField: 'transDate', width: 150, cellsformat: 'dd/MM/yyyy'},
					 { text: '${uiLabelMap.BACCVoucherID}', dataField: 'voucherID', width: 150, columngroup: 'glVoucher'},
					 { text: '${uiLabelMap.BACCVoucherDate}', dataField: 'voucherDate', width: 150, columngroup: 'glVoucher', cellsformat: 'dd/MM/yyyy'},
					 ...
					 { text: '${uiLabelMap.BACCDebitAmount}', dataField: 'debitAmount', width: 150, columngroup: 'amount',
						 cellsrenderer: function(row, column, value){
							 var data = $('#moneyInBankGrid').jqxGrid('getrowdata',row);
        		  			 if(data != undefined && data){
        		  				return '<span class=align-right>' + formatcurrency(value, data.currencyId) + '</span>';
        		  			 }
						 }
					 },
					 { text: '${uiLabelMap.BACCCreditAmount}', dataField: 'creditAmount', width: 150, columngroup: 'amount',
						 cellsrenderer: function(row, column, value){
							 var data = $('#moneyInBankGrid').jqxGrid('getrowdata',row);
        		  			 if(data != undefined && data){
                		  		return '<span class=align-right>' + formatcurrency(value, data.currencyId) + '</span>';
					 ...
					 { text: '${uiLabelMap.BACCCashBalance}', dataField: 'balAmount', width: 150, columngroup: 'amount',
						 cellsrenderer: function(row, column, value){
							 var data = $('#moneyInBankGrid').jqxGrid('getrowdata',row);
        		  			 if(data != undefined && data){
        		  				return '<span class=align-right>' + formatcurrency(value, data.currencyId) + '</span>';
						 ...
						 aggregatesrenderer: function (aggregates, column, element) {
                        	...
                        	var data = $('#moneyInBankGrid').jqxGrid('getrows');
 ...
 <#assign columngrouplist=" { text: '${uiLabelMap.BACCGLVoucher}', name: 'glVoucher', align: 'center'},
  							{ text: '${uiLabelMap.BACCAmount}', name: 'amount', align: 'center'}
						">
 ...
 <#assign url = "jqxGeneralServicer?sname=JqxGetMoneyInBank&glAccountId=${glAccount.glAccountId}&fromDate=" + '${fromDate}' + "&thruDate=" + '${thruDate}' />
 
 <@jqGrid id="moneyInBankGrid" filtersimplemode="true" filterable="false" addrefresh="true" editable="false" addType="popup" showtoolbar="true" clearfilteringbutton="true"
 ...
 <script>
	$( document ).ready(function(){
		moneyFilter.initFilter();	
		moneyFilter.bindEvent();
	});
	
	$('#moneyInBankGrid').on("bindingcomplete", function (event) {
		...
		jQuery.ajax({
            url: 'getEndingAmountMoneyInBank',
            async: false,
            type: 'POST',
            data: {
            	glAccountId: glId,
	...
	var exportExcel = function(){
		if($('#moneyInBankGrid').jqxGrid('getrows').length > 0){
			var glId = $("#glAccountId").attr('data-value');
			glId = glId != null && glId !== undefined && glId ? glId : '${glAccount.glAccountId?if_exists}';
            ...
            window.location.href = "exportMoneyInbankExcel?glAccountId=" + glId + "&fromDate=" + fromDate + "&thruDate=" + thruDate;
		} else {
