...
<#include "analysisReportFilter.ftl">
...
<#assign columnlist="{ text: '${uiLabelMap.BACCTransDate}', dataField: 'transDate', width: 150, cellsformat: 'dd/MM/yyyy'},
					 ...
					 { text: '${uiLabelMap.BACCDebitAmount}', dataField: 'debitAmount', width: 150, columngroup: 'arisingAmount',
						 cellsrenderer: function(row, column, value){
							 var data = $('#analysisReportGrid').jqxGrid('getrowdata',row);
						 ...
						 aggregatesrenderer: function (aggregates, column, element) {
                        	...
                        	var data = $('#analysisReportGrid').jqxGrid('getrows');
                        	...
                        	if (data && data.length > 0) {
                          	...
                          	renderstring += '<div style=\"position: relative; margin: 6px; text-align: right; overflow: hidden;\"><b>' + '-' + '</b></div>';
                          	renderstring += \"</div>\";
                          	return renderstring;
                      	}
					 },
					 ...
					 { text: '${uiLabelMap.BACCCreditAmount}', dataField: 'creditBalAmount', width: 150, columngroup: 'balAmount',
						 ...
						 aggregatesrenderer: function (aggregates, column, element) {
                        	...
                        	var totalEnding = '-';
                        	if (totalMap && totalMap.totalEnding >= 0) {
                          		total = formatcurrency(totalMap.totalEnding, currencyUomId);
                          	}
                        	var data = $('#analysisReportGrid').jqxGrid('getrows');
                        	var currencyUomId = 'VND';
                        	if (data && data.length > 0) {
...
<#assign url = "jqxGeneralServicer?sname=JqxGetAnalysisReport&glAccountId=${glAccount.glAccountId}&fromDate=" + '${fromDate}' + "&thruDate=" + '${thruDate}' />
<@jqGrid id="analysisReportGrid" filtersimplemode="true" filterable="false" addrefresh="true" editable="false" addType="popup" showtoolbar="true" clearfilteringbutton="true"
...
<script>
	...
	$('#analysisReportGrid').on("bindingcomplete", function (event) {
	...
	var exportExcel = function(){
		var dataGrid = $("#analysisReportGrid").jqxGrid('getrows');
		if (dataGrid.length == 0) {
			jOlbUtil.alert.error("${uiLabelMap.ReportCheckNotData}");
			return false;
		}
		
		var winURL = "exportAnalysisReportExcelNew";
