...
<script>
	...
	var cellclassname = function (row, column, value, data) {
		...
		if (data['fixedAssetId'] == undefined) {
	        return 'highlight';
	    }
	}
	...
	var columnlist = [{ text: '${uiLabelMap.BACCSeqId}', datafield: 'seqId', width: 100,
	                  ...
	                  { text: '${uiLabelMap.BACCFixedAssetName}', datafield: 'fixedAssetName', filtertype : 'number', width: 150,
	                	  cellclassname: cellclassname  
	                  },
	                  ...
	                  { text: '${uiLabelMap.BACCMonthlyAllocAmount}', datafield: 'monthlyAllocAmount', filtertype : 'number', width: 180,
	                	  cellsrenderer: function(row, columns, value){
	                		  var data = $('#costAllocGrid').jqxGrid('getrowdata',row);
	                		  return '<span>'+formatcurrency(value, data.uomId)+'</span>';
	                	  },
	                  ...
	                  { text: '${uiLabelMap.BACCPreAccumulatedAllocAmount}', datafield: 'preAccumulatedAllocAmount', filtertype : 'number', width: 220,
	                	  cellsrenderer: function(row, columns, value){
	                		  var data = $('#costAllocGrid').jqxGrid('getrowdata',row);
	                		  return '<span>'+formatcurrency(value, data.uomId)+'</span>';
	                	  },
	                  ...
	                  { text: '${uiLabelMap.BACCAccumulatedAllocAmount}', datafield: 'accumulatedAllocAmount', filtertype : 'number', width: 220,
	                	  cellsrenderer: function(row, columns, value){
	                		  var data = $('#costAllocGrid').jqxGrid('getrowdata',row);
	                		  return '<span>'+formatcurrency(value, data.uomId)+'</span>';
	                	  },
	...
	{
		Grid.initGrid({source : {localdata: data,cache : false},virtualmode : false,width : '100%',
			rendertoolbar : rendertoolbar,showtoolbar : true,}, datafields, columnlist, null, $("#costAllocGrid"));
	}
</script>