...
<#assign columnlist="{ text: '${uiLabelMap.BACCInvoiceId}', dataField: 'invoiceId', width: '9%', pinned: true,
						cellsrenderer: function(row, column, value){
							var data = $('#jqxgridInvoice').jqxGrid('getrowdata', row);
							if(data != undefined && data){
                                return '<span><a href=ViewARInvoice?selectedItemMenu=ARInvoiceListDueReceivable&invoiceId=' + data.invoiceId + ' style=\"color: blue !important;\">' + data.invoiceId + '</a></span>';
							}}
						},"/>
<#assign columnlist = columnlist + "{ text: '${uiLabelMap.BACCInvoiceToParty}', width: '18%', dataField:'fullNameTo',  pinned: true, cellclassname: cellclass,
					    cellsrenderer: function (row, column, value){
					        var data = $('#jqxgridInvoice').jqxGrid('getrowdata', row);
					        return '<span><a href=\"MTCustomerDetail?partyId=' + data.partyId + '\">' + value + '</a></span>';
					    }},"/>
...
<@jqGrid id="jqxgridInvoice" filtersimplemode="true" addrefresh="true" editable="false" addType="popup" showtoolbar="true" clearfilteringbutton="true"
		 url="jqxGeneralServicer?sname=jqxGetListInvoiceByDueDate&invoiceType=${businessType}&fromDueDate=${monthStart}&toDueDate=${monthEnd}&isNullDueDate=Y" dataField=dataField columnlist=columnlist
...
<script>
	...
	var exportExcel = function(){
        ...
        statusField.setAttribute("value", "INVOICE_PAID,INVOICE_CANCELLED");
        form.appendChild(statusField);

        var isNullDueDateField = document.createElement("input");
        var isNullDueDate = $('input[name=switch-field-1]').is(":checked");
        isNullDueDateField.setAttribute("type", "hidden");
        isNullDueDateField.setAttribute("name", "isNullDueDate");
        isNullDueDateField.setAttribute("value", isNullDueDate);
        form.appendChild(isNullDueDateField);
