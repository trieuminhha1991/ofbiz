...
<#assign columnlist="{ text: '${uiLabelMap.BACCPeriod}', datafield: 'customTimePeriodId', width: 100,
						cellsrenderer: function(row, column, value){
							return '<span><a href=ListAllocatedEquips?customTimePeriodId=' + value + '>' + value + '</a></span>';
						},
					 },
					 { text: '${uiLabelMap.BACCDescription}', dataField: 'periodName' },
					 { text: '${uiLabelMap.FromDate}', dataField: 'fromDate', width: 150, filtertype: 'range', cellsformat: 'dd/MM/yyyy' },
					 { text: '${uiLabelMap.ThruDate}', dataField: 'thruDate', width: 150, filtertype: 'range', cellsformat: 'dd/MM/yyyy' },
...
<@jqGrid id="equipAllocGrid" customTitleProperties="BACCListEquipAllocPeriods" customcontrol1="icon-plus open-sans@${uiLabelMap.accAddNewRow}@javascript: void(0);@OLBNewAllocPeriod.openWindow()" customcontrol2="fa-cog open-sans@${uiLabelMap.BACCReady}@javascript: void(0);@postTrans()" filtersimplemode="true" filterable="true" editable="false" addType="popup" showtoolbar="true"
		 url="jqxGeneralServicer?sname=JqxGetListEquipAllocs" dataField=dataField columnlist=columnlist clearfilteringbutton="true"
	/>
<script>
	postTrans = function(){
		var rowindex = $('#equipAllocGrid').jqxGrid('getselectedrowindex');
		var data = $('#equipAllocGrid').jqxGrid('getrowdata', rowindex);
		if(data !== undefined){
			if(data.isClosed == 'Y'){
