...
public class ApInvoiceList implements InvoiceList {
	...
	public Map<String, Object> getListInvoice(DispatchContext dispatcher, Map<String, Object> context) throws GenericEntityException, GenericServiceException {
		...
		UtilServices.getAllInvoiceType(delegator, INVOICE_TYPE, "PURCHASE_INVOICE");
    	...
    	if(context.containsKey("status") && UtilValidate.isNotEmpty((String)context.get("status")) ){
    		String tmp = (String) context.get("status");String[] _status = null; 
    		if(tmp.indexOf(",") != -1){
    			_status = tmp.split(",");
    		}
    		if(_status != null){
		...
		for(GenericValue item : listTmpInvs){
			...
			//List<String> _list = FastList.newInstance();
			//getTotal
