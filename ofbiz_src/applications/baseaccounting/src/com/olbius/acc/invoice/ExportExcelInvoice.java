...
public class ExportExcelInvoice extends ExportExcelAbstract {
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "bang-ke-hoa-don";
		SimpleDateFormat format = new SimpleDateFormat("dd-MM-YYYY");
		String dateTime = format.format(nowTimestamp);
		fileName += "-" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "SGCBangKeHoaDon", locale).toUpperCase());
		
		setRunServiceName("JqxGetListInvoices");
		setModuleExport("ACC");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "SGCBangKeHoaDon", locale));
		setSplitSheet(false);
		...
		addColumn(12, UtilProperties.getMessage("AccountingUiLabels", "AccountingInvNr", locale), "invoiceId");
		addColumn(20, UtilProperties.getMessage(RESOURCE, "SGCNgayHD", locale), "invoiceDate");
		addColumn(20, UtilProperties.getMessage(RESOURCE, "SGCTongTienHD", locale), "total", ExportExcelStyle.STYLE_CELL_CURRENCY);
		addColumn(20, UtilProperties.getMessage(RESOURCE, "SGCNgayGioValidate", locale), "verifiedDate");
		if ("AR".equals(invoiceType)) {
			addColumn(12, UtilProperties.getMessage(RESOURCE_ACC, "BACCCustomerId", locale), "partyCode");
			addColumn(25, UtilProperties.getMessage(RESOURCE_ACC, "BACCCustomerName", locale), "fullNameTo");
		} else {
			addColumn(12, UtilProperties.getMessage(RESOURCE_LOG, "SupplierId", locale), "partyCodeFrom");
			addColumn(30, UtilProperties.getMessage(RESOURCE_LOG, "SupplierName", locale), "fullNameFrom");
		}
		addColumn(20, UtilProperties.getMessage(RESOURCE, "SGCTinhTrangTT", locale), "newStatusId");
        addColumn(20, UtilProperties.getMessage(RESOURCE_ACC, "BACCDueDate", locale), "dueDate");
        addColumn(20, UtilProperties.getMessage(RESOURCE_ACC, "BACCAmountToApply", locale), "amountToApply", ExportExcelStyle.STYLE_CELL_CURRENCY);
	}
