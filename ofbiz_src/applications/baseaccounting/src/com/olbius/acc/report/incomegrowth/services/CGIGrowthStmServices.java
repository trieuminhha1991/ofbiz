...
import com.olbius.acc.report.incomegrowth.entity.CustGroupIncomeGrowth;
import com.olbius.acc.report.incomegrowth.entity.IncomeGrowth;
import com.olbius.acc.utils.EntityUtils;

public class CGIGrowthStmServices implements ReportServiceInterface{
	
	public static final String module = CGIGrowthStmServices.class.getName();
	...
	public Map<String, Object> getData(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			getIncomeStmCtx.put("groupId", context.get("groupId"));
			...
			Map<String, Object> getIncomeStm1Rs = dispatcher.runSync("getCustGroupIncomeStm", getIncomeStmCtx);
			...
			getIncomeStmCtx.put("init", context.get("init"));
			getIncomeStmCtx.put("hasTransTime", "N");
			Map<String, Object> getIncomeStm2Rs = dispatcher.runSync("getCustGroupIncomeStm", getIncomeStmCtx);
			...
			for(Map<String, Object> item1 : data1) {
				for(Map<String, Object> item2 : data2) {
					if(((String)item1.get("groupId")).equals(((String)item2.get("groupId")))) {
						CustGroupIncomeGrowth income = new CustGroupIncomeGrowth();
						...
						income.setGroupId((String)item1.get("groupId"));
	...
	@SuppressWarnings("unchecked")
	public Map<String, Object> evaluateCGGColumnChart(DispatchContext dctx, Map<String, Object> context) {
		LocalDispatcher dispatcher = dctx.getDispatcher();
		...
		try {
			mapCtx = ServiceUtil.setServiceFields(dispatcher, "getCustGroupIncomeGrowthStm", context, userLogin, timeZone, locale);
			mapRs = dispatcher.runSync("getCustGroupIncomeGrowthStm", mapCtx);
		...
		adapter.convertData(yAxis, xAxis, UtilMisc.toList("grossProfitRate", "saleIncomeRate"), "groupId", data);
		Map<String, Object> result = ServiceUtil.returnSuccess();
