...
public class HSSFBuilder {
	...
	protected void buildSheet(Workbook wb, SheetConfig config) throws ParseException {
		Sheet sheet = wb.createSheet(config.getSheetName() + Math.random());
	...
	protected void createTitle(Sheet sheet, SheetConfig config, Map<String, CellStyle> styles,
		...
		sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, columnConfs.size()));
				...
				if (column.getDataType() != null) {
					...
					case STRING:
						...
						if (row.containsKey("parent")) {
							...
							cellData.setCellStyle(styles.get("cell_normal_centered_border_full_10"));
						break;
					case NUMBERIC:
