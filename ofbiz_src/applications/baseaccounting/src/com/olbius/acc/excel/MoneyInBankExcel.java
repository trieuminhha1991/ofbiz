...
public class MoneyInBankExcel {
	...
	public static void export(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCMoneyInBank", locale).toUpperCase());
			...
			titles.addAll(UtilMisc.toList(UtilProperties.getMessage(RESOURCE, "BACCCreditAmount", locale),
			...
			for (String t : titles) {
				...
				if (UtilMisc.toList(0, 3, 4, 8, 9).contains(index)) {
					...
					if (index == 1) {
						...
						cell.setCellType(ExcelUtil.CELL_TYPE_STRING);
						cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCAmount", locale));
						cell.setCellStyle(styles.get("cell_bold_centered_wrap_text_bordered_10"));

						cell_dummy = row.createCell((index + 1));
						cell_dummy.setCellStyle(styles.get("cell_bold_centered_wrap_text_bordered_10"));

						cell_dummy = row.createCell((index + 2));
						cell_dummy.setCellStyle(styles.get("cell_bold_centered_wrap_text_bordered_10"));
					}
			...
			Map<String, Object> rs = dispatcher.runSync("JqxGetMoneyInBank",
			...
			ExcelUtil.responseWrite(response, wb, "so-tien-gui-ngan-hang-");
