...
public class LiabilityBalanceExcel {
	...
	public static void export(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCLiabilityBalance", locale).toUpperCase());
			...
			if (partyId != null) {
				...
				if (UtilValidate.isNotEmpty(party)) {
					...
					cellParty.setCellValue(UtilProperties.getMessage("BaseLogisticsUiLabels", "Supplier", locale) + ": "
							+ partyId + " - " + partyName);
				}
			} else {
				cellParty.setCellValue(UtilProperties.getMessage("BaseLogisticsUiLabels", "Supplier", locale) + ": ");
			...
			Map<String, Object> rs = dispatcher.runSync("JqxGetLiabilityBalance",
			...
			ExcelUtil.responseWrite(response, wb, "so-chi-tiet-cong-no-phai-tra-");
