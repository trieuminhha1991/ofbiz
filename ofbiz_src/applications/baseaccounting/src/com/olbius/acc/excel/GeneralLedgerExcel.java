...
public class GeneralLedgerExcel {
	...
	public static void export(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCGeneralLedger", locale).toUpperCase());
			...
			for (String t : titles) {
				...
				if (UtilMisc.toList(0, 3, 4, 7, 8).contains(index)) {
					...
					if (index == 1) {
						...
						sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, index, index + 1));
						cell.setCellType(ExcelUtil.CELL_TYPE_STRING);
						cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCGLAmount", locale));
						cell.setCellStyle(styles.get("cell_bold_centered_wrap_text_bordered_10"));
			...
			Map<String, Object> rs = dispatcher.runSync("JqxGetGeneralLedger",
			...
			ExcelUtil.responseWrite(response, wb, "so-cai-tai-khoan-");
