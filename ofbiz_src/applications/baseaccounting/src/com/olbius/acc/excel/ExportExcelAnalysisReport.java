...
public class ExportExcelAnalysisReport extends ExportExcelAbstract {
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "bang-chi-tiet-tai-khoan";
		SimpleDateFormat format = new SimpleDateFormat("dd-MM-YYYY");
		String dateTime = format.format(nowTimestamp);
		fileName += "-" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE_ACC, "BACCAnalysisReport", locale).toUpperCase());
		
		setRunServiceName("JqxGetAnalysisReport");
		setModuleExport("BASEACCOUTING");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE_ACC, "BACCAnalysisReport", locale));
		setSplitSheet(false);
		...
		addColumn(20, UtilProperties.getMessage(RESOURCE_ACC, "BACCArisingAmount", locale) + " " +
		...
		addColumn(20, UtilProperties.getMessage(RESOURCE_ACC, "BACCBalanceAmount", locale) + " " +
				UtilProperties.getMessage(RESOURCE_ACC, "BACCDebitAmount", locale), "debitBalAmount", ExportExcelStyle.STYLE_CELL_CURRENCY);
		addColumn(20, UtilProperties.getMessage(RESOURCE_ACC, "BACCBalanceAmount", locale) + " " +
				UtilProperties.getMessage(RESOURCE_ACC, "BACCCreditAmount", locale), "creditBalAmount", ExportExcelStyle.STYLE_CELL_CURRENCY);
