...
public class LiabilitySupplierExcel {
	...
	public static void export(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			cell.setCellValue(UtilProperties.getMessage(RESOURCE, "BACCLiabilitySupplier", locale).toUpperCase());
			...
			Map<String, Object> rs = dispatcher.runSync("JqxGetLiabilitySupplier",
			...
			ExcelUtil.responseWrite(response, wb, "so-tong-hop-cong-no-phai-tra-");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
