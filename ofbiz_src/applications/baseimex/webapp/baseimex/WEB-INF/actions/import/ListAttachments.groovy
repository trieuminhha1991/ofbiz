...
List<GenericValue> listOrder = new ArrayList<GenericValue>();
if(billId != null){
	listOrder = delegator.findList("OrderAndContainerAndAgreementAndOrderDetail", EntityCondition.makeCondition(UtilMisc.toMap("billId", billId)), fieldToSelects, null, null, false);
}else if(containerId != null){
	listOrder = delegator.findList("OrderAndContainerAndAgreementAndOrderDetail", EntityCondition.makeCondition(UtilMisc.toMap("containerId", containerId)), fieldToSelects, null, null, false);
}

System.out.println("AAA: " +listOrder);
...
for (GenericValue x : listOrderItems) {
	...
	internalName = (String)thisProduct.get("productName");
