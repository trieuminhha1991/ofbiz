...
<script>
	...
	productEvent.agreementId = '${StringUtil.wrapString(productEvent.agreementId?if_exists)}';
	productEvent.packingListId = '${StringUtil.wrapString(productEvent.packingListId?if_exists)}';
...
<script type="text/javascript" src="/imexresources/js/product/detailProductEventBegin.js?v=1.0.0"></script>
					...
					<div class="row-fluid">
							...
							<div class="tabbable">
								<ul class="nav nav-tabs" id="recent-tab"><#-- nav-tabs-menu-->
									<#if productEvent?has_content>
										...
										<li <#if activeTab?exists && activeTab == "requirement-tab"> class="active"</#if>>
											<a data-toggle="tab" href="#requirement-tab">${uiLabelMap.Requirement}</a>
										</li>
									</#if>
								</ul>
							</div><!--.tabbable-->
						...
						<div class="span2" style="height:34px; text-align:right">
							...
							<#if productEvent.statusId == "PRODUCT_EVENT_CREATED">
								<#if hasOlbPermission("MODULE", "IMEX_TESTING", "ADMIN")>
								...
								<#if hasOlbPermission("MODULE", "IMEX_TESTING", "ADMIN") || productEvent.createdByUserLogin == userLogin.userLoginId>
				    		...
				    		<#if productEvent.statusId == "PRODUCT_EVENT_APPROVED">	
								<a style="cursor: pointer;" href="javascript:EventDetailBegin.createExportRequirement('${productEvent.eventId?if_exists}')" data-rel="tooltip" title="${uiLabelMap.BIECreateExportRequirement}" data-placement="bottom" class="button-action"><i class="fas fa-bullhorn"></i></a>
							</#if>
						</div>
					</div>
				</div>
