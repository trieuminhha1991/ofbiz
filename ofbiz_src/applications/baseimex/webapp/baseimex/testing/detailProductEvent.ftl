	...
	<div style="position:relative"><!-- class="widget-body"-->
		...
		<div><!--class="widget-main"-->
			<h3 style="text-align:center;font-weight:bold; padding:0;line-height:20px">
				${StringUtil.wrapString(uiLabelMap.BIEDetailExportTesting)?upper_case} ${StringUtil.wrapString(productEvent.eventName?if_exists)?upper_case}
			</h3>
			<div class="row-fluid">
				<div class="form-horizontal form-window-content-custom span-text-left content-description">
					<div class="row-fluid margin-top20">
						<div class="span6">
							...
							<div class="row-fluid">
								<div class="span4">
									<span>${StringUtil.wrapString(uiLabelMap.BIEAgreementId)}:</span>
								</div>
								<div class="span8 align-left">
									<#if productEvent.agreementId?has_content>
										<span>${productEvent.agreementId?if_exists}</span>
									</#if>
								</div>
							</div>
							<div class="row-fluid">
								<div class="span4">
									<span>${StringUtil.wrapString(uiLabelMap.BIEPackingList)}:</span>
								</div>
								<div class="span8 align-left">
									<#if productEvent.packingListId?has_content>
										<#assign packing = delegator.findOne("PackingListHeader", {"packingListId" : productEvent.packingListId?if_exists}, false)!>
										<span>${packing.packingListNumber?if_exists}</span>
									</#if>
								</div>
							</div>
						</div>
			...
			<thead>
				<tr style="font-weight: bold;">
					...
					<td width="7%">${StringUtil.wrapString(uiLabelMap.Unit)}</td>
				  	<td width="8%">${StringUtil.wrapString(uiLabelMap.BLQuantityRegistered)}</td>
				  	<td width="8%">${StringUtil.wrapString(uiLabelMap.BLQuantityUse)}</td>
				</tr>
			</thead>
			<tbody>
				<#assign i = 0>
					...
					<#list products as item>
						...
						<tr>
				            ...
				            <td class="align-right">
	   							<#if item.quantity?exists>
	   								<#if item.requireAmount?has_content && item.requireAmount == 'Y' && item.amountUomTypeId == 'WEIGHT_MEASURE'>
		   								${Static["org.ofbiz.base.util.UtilFormatOut"].formatDecimalNumber(item.registeredQuantity?if_exists, "#,##0.00", locale)}
		   							<#else>
		   								${Static["org.ofbiz.base.util.UtilFormatOut"].formatDecimalNumber(item.registeredQuantity?if_exists, "#,##0", locale)}
		   							</#if>
		   						</#if>
				            </td>
				            <td class="align-right">
