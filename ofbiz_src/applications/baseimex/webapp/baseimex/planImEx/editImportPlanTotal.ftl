...
<h4 class="smaller lighter blue" style="margin: 5px 0px 10px 0px !important;font-weight:500;line-height:20px;font-size:18px;">
	${uiLabelMap.BIEEditImportPlan}
</h4>
<script>
	<#assign productPlanHeader = delegator.findOne("ProductPlanHeader", {"productPlanId" : parameters.productPlanId?if_exists}, false)!>
	
	var productPlanCode = '${StringUtil.wrapString(productPlanHeader.productPlanCode?if_exists)}';
	var productPlanName = '${StringUtil.wrapString(productPlanHeader.productPlanName?if_exists)}';
	var customTimePeriodId = '${productPlanHeader.customTimePeriodId?if_exists}';
	var description = '${StringUtil.wrapString(productPlanHeader.description?if_exists)}';
	
	<#assign supplier = delegator.findOne("PartyFullNameDetail", {"partyId" : productPlanHeader.supplierPartyId?if_exists}, false)!>
	<#assign period = delegator.findOne("CustomTimePeriod", {"customTimePeriodId" : productPlanHeader.customTimePeriodId?if_exists}, false)!>
		...
		<div class="wizard-steps-steps">
			<ul class="wizard-steps wizard-steps-square">
				...
				<li data-target="#step2" style="pointer-events: none;">
					<span class="step">2. ${uiLabelMap.BIEConfirmInfo}</span>
				</li>
			</ul>
		</div>
	...
	<div class="step-content row-fluid position-relative" id="step-container">
		...
		<div class="step-pane active" id="step1">
			<div style="position:relative">
				<div class="row-fluid">
					<div class="span12">
						<#include "editImportPlanProduct.ftl"/>
					</div>
				</div>
			</div>
		</div>
		<div class="step-pane" id="step2">
			<#include "editImportPlanConfirm.ftl"/>
		</div>
	</div><!--.step-content-->
...
<script type="text/javascript" src="/imexresources/js/import/plan/editImportPlanTotal.js?v=1.0.0"></script>