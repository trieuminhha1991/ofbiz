...
var ObjProduct = (function() {
	...
	var initInput = function (){
		...
		$("#description").jqxInput({ width: 300, height: 50, theme: theme});
		
		if (productPlanCode){
			$("#productPlanCode").jqxInput('val', productPlanCode);
		}
		if (productPlanName){
			$("#productPlanName").jqxInput('val', productPlanName);
		}
		if (description){
	...
	var updateQuantityPeriod = function (listProducts){
		for (var i in listProducts){
			...
			listProducts[i].listData = listDataNew;
		}
		return listProducts;
	}
