 ...
 *
 */
public class SalesOrder extends AbstractOlbiusBusEvent {
	...
	public void busHandle(MessageData messageData, String user) throws Exception {
		...
		List<GenericValue> genericValues = delegator.findByAnd("BusConvert", UtilMisc.toMap("busId", order.getGroupFrom().getBusId(), "own", "Y"), null,
				false);
		...
		if (partyId != null) {
			...
			value.set("partyFromId", partyId);
			value.set("partyToId", user);
			value.set("orderType", "SALES");
	...
	protected void end(String id, String user, String type) {
		try {
			...
			Debug.logError(e, SalesOrder.class.getName());
		}
	}
