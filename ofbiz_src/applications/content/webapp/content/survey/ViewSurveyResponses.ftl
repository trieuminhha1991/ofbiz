...
-->
  ...
  <table class="table table-hover table-striped table-bordered dataTable" cellspacing="0">
                  ...
                  <#list answers as answer>
                    ...
                    <#elseif question.surveyQuestionTypeId == "CONTENT">
                       <#if answer.contentId?has_content>
                         ...
                         <a href="<@ofbizUrl>img?imgId=${content.dataResourceId}</@ofbizUrl>" class="btn btn-mini btn-info">${answer.contentId}</a>&nbsp;-&nbsp;${content.contentName?if_exists}
                       </#if>
                    </#if>
                  </#list>
                </#if>
    ...
    <#else>
      <tr>
        <td class="alert alert-info">${uiLabelMap.ContentSurveyNoQuestions}</div></td>
      </tr>
    </#if>
  </table>
