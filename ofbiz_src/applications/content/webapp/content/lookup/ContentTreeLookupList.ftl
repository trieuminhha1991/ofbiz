...
<#if contentAssoc?has_content>  
      ...
      <#if tabButtonItem=="ListContentTree">
        <#--Form ListContentTree-->
          <tr <#if alt_row> class="alternate-row"</#if>> 
              <td><a class="btn btn-mini btn-info" href="javascript:set_value('${contentData.contentId?if_exists}')">${contentData.contentId?if_exists}</a></td>
      ...
      <#elseif tabButtonItem=="ListDocument">
          <#--Form ListDocument-->
          <tr <#if alt_row> class="alternate-row"</#if>>
              <td><a class="btn btn-mini btn-info" href="/content/control/editContent?contentId=${contentData.contentId?if_exists}">${contentData.contentName?if_exists}[${contentData.contentId?if_exists}]</a></td>
              <td><a class="btn btn-mini btn-info" href="/content/control/showContent?contentId=${contentData.contentId?if_exists}" target="_blank">${uiLabelMap.CommonView}</a></td>
             ...
             <#assign caFromDate = Static["org.ofbiz.base.util.UtilDateTime"].toDateString(contentData.caFromDate, "dd/MM/yyyy")/>
              ...
              <td><a class="btn btn-mini btn-danger icon-trash open-sans" href="javascript:document.listDocumentForm_${listcount}.submit()" >${uiLabelMap.CommonDelete}</a></td>
          </tr>
