...
public class PartyMember extends OlbiusSecurityCache<Map<String, List<OlbiusParty>>> {

	private PartyMemberType memberType;
	
	public PartyMember(PartyMemberType memberType) {
	...
	public Map<String, List<OlbiusParty>> loadCache(Delegator delegator, String key) throws Exception {
		...
		List<GenericValue> values = delegator.findList("PartyRelationship", condition, null, null, null, false);
		...
		for (GenericValue value : values) {
			...
			if (!type.isEmpty()) {
				if (!key.equals(value.getString(type))) {
					OlbiusParty party = new OlbiusParty(value.getString(type));
