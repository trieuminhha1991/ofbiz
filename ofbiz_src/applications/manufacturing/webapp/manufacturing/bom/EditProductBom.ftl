...
<div class="widget-box no-border-bottom transparent">
  <div class="widget-header">
      <h4>${uiLabelMap.PageTitleEditProductBom} <#if product?exists>${(product.internalName)?if_exists}</#if>&nbsp;[${uiLabelMap.CommonId}&nbsp;${productId?if_exists}]</h4>
    <br class="clear"/>
  </div>
  <div class="widget-body">
    <#if product?has_content>
    <div class="padding-top8 padding-bottom8 padding-left8">
        <a href="<@ofbizUrl>BomSimulation</@ofbizUrl>?productId=${productId}&amp;bomType=${productAssocTypeId}" class="btn btn-mini btn-primary open-sans">${uiLabelMap.ManufacturingBomSimulation}</a>
    </div>
    </#if>
    <br />
    <br />
    <form style="" name="searchform" action="<@ofbizUrl>UpdateProductBom</@ofbizUrl>#topform" method="post">
    <input type="hidden" name="UPDATE_MODE" value=""/>

    <table  class="basic-table" cellspacing="0" style="margin-top: -25px;">
        <tr >
        ...
        <tr>
            ...
            <td>
                ...
                <span><a href="javascript:document.searchform.submit();" class="btn btn-mini btn-info open-sans icon-eye-open">${uiLabelMap.ManufacturingShowBOMAssocs}</a></span>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <span><a href="javascript:document.searchform.UPDATE_MODE.value='COPY';document.searchform.submit();" class="btn btn-mini btn-info icon-copy open-sans">${uiLabelMap.ManufacturingCopyBOMAssocs}</a></span>
            </td>
        </tr>
    </table>
    </form>
    <hr />
    <form style="" action="<@ofbizUrl>UpdateProductBom</@ofbizUrl>" method="post" name="editProductAssocForm">
    ...
    <div class="span12 align-center" style="margin-top: 20px;">
        <button type="submit" class="btn btn-primary btn-small open-sans" >
        	<i class="icon-ok" ></i>
        	<#if !(productAssoc?exists)>${uiLabelMap.CommonAdd}<#else>${uiLabelMap.CommonEdit}</#if>
        </button>
    </div>
    </div>
    </form>
...
<div class="widget-box">
  <div class="widget-header">
       <h3>${uiLabelMap.ManufacturingProductComponents}</h3>
    <br class="clear"/>
  </div>
  <div class="widget-body">
    <a name="components"></a>
    ...
    <#assign curProductAssocType = assocFromProduct.getRelatedOne("ProductAssocType", true)>
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><a href="<@ofbizUrl>EditProductBom?productId=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn btn-info btn-mini">${(assocFromProduct.productIdTo)?if_exists}</a></td>
        <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProductBom?productId=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn btn-info btn-mini">${(listToProduct.internalName)?if_exists}</a></#if>&nbsp;</td>
        ...
        <td>
        <a href="<@ofbizUrl>UpdateProductBom?UPDATE_MODE=DELETE&amp;productId=${productId}&amp;productIdTo=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}&amp;fromDate=${(assocFromProduct.fromDate)?if_exists}&amp;useValues=true</@ofbizUrl>" class="btn btn-danger btn-mini">${uiLabelMap.CommonDelete}</a>
        </td>
        <td>
        <a href="<@ofbizUrl>EditProductBom?productId=${productId}&amp;productIdTo=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}&amp;fromDate=${(assocFromProduct.fromDate)?if_exists}&amp;useValues=true</@ofbizUrl>" class="btn btn-info btn-mini">${uiLabelMap.CommonEdit}</a>
        </td>
      </tr>
...
<div class="widget-box">
  <div class="widget-header">
      <h3>${uiLabelMap.ManufacturingProductComponentOf}</h3>
    <br class="clear"/>
  </div>
  <div class="widget-body">
      <table class="table table-hover table-striped table-bordered dataTable" cellspacing="0">
        ...
        <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
            <td><a href="<@ofbizUrl>EditProductBom?productId=${(assocToProduct.productId)?if_exists}&amp;productAssocTypeId=${(assocToProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn btn-mini btn-info">${(assocToProduct.productId)?if_exists}</a></td>
<!--                <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProduct?productId=${(assocToProduct.productId)?if_exists}</@ofbizUrl>" class="buttontext">${(listToProduct.internalName)?if_exists}</a></#if></td> -->
            <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProductBom?productId=${(assocToProduct.productId)?if_exists}&amp;productAssocTypeId=${(assocToProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn btn-mini btn-info">${(listToProduct.internalName)?if_exists}</a></#if></td>
      ...
      <br />
     	${uiLabelMap.CommonNote}: <b style="color: red;">${uiLabelMap.CommonRed}</b> ${uiLabelMap.ManufacturingNote1} <b style="color: red;">${uiLabelMap.CommonRed}</b>${uiLabelMap.ManufacturingNote2} <b style="color: red;">${uiLabelMap.CommonRed}</b>${uiLabelMap.ManufacturingNote3}
  </div>
</div>
</#if>
