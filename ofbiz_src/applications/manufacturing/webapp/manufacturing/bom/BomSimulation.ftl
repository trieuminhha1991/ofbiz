...
<div class="widget-box no-border-bottom transparent">
  <div class="widget-header">
      <h3>${uiLabelMap.ManufacturingSelectedFeatures}</h3>
    <br class="clear"/>
  </div>
...
<div class="widget-box no-border-bottom transparent">
  <div class="widget-header">
      <h3 style="padding: 0px !important;">${uiLabelMap.ManufacturingBomSimulation}</h3>
    <br class="clear"/>
  </div>
  <div class="widget-body">
</#if>
        ...
        <#if tree?has_content>
          ...
          <#list tree as node>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              ...
              <td align="right"><a href="<@ofbizUrl>EditProductBom?productId=${(node.product.productId)?if_exists}&amp;productAssocTypeId=${(node.bomTypeId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-info">${uiLabelMap.CommonEdit}</a></td>
            </tr>
        ...
        <#else>
            <tr>
              <td colspan="6" class="alert alert-info">
              	<p>${uiLabelMap.CommonNoElementFound}.</p>
              </td>
            </tr>
        </#if>
      </table>
      ...
      <table class="table table-striped table-bordered table-hover dataTable" cellspacing="0">
        ...
        <#if productsData?has_content>
          ...
          <#list productsData as productData>
            ...
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a href="/catalog/control/EditProduct?productId=${node.product.productId}${externalKeyParam}" class="btn btn-mini btn-info">${node.product.productId}</a></td>
              <td>${node.product.internalName?default("&nbsp;")}</td>
              <td align="right">${node.quantity}</td>
              ...
              <#else>
              <td align="right"><a href="/catalog/control/EditProductCosts?productId=${node.product.productId}${externalKeyParam}" class="btn btn-mini btn-info">NA</a></td>
              </#if>
