...
List<GenericValue> returnItems = new ArrayList<GenericValue>();
returnItems = delegator.findByAnd("ReturnItem", [returnId : returnId], null, false);
boolean checkEmpty = true;
if(returnItems.size() > 0){ checkEmpty = false;}
else {checkEmpty = true;}
context.checkEmpty = checkEmpty;
...
listStatusItemReturnHeader = delegator.findList("StatusItem", EntityCondition.makeCondition(EntityCondition.makeCondition("statusTypeId", EntityOperator.EQUALS, "PORDER_RETURN_STTS"), EntityOperator.OR,
                                EntityCondition.makeCondition("statusTypeId", EntityOperator.EQUALS, "ORDER_RETURN_STTS")), null, null, null, false);
