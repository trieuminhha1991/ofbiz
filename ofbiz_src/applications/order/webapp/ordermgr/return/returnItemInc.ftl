...
under the License.
-->
<h4>${uiLabelMap.OrderReturnFromOrder} ${uiLabelMap.CommonNbr} ${orderId}</h4>
<#if orh?exists>
...
<input type="checkbox" style="opacity: 1 !important; position: initial !important; margin-bottom: 0.5% !important;" name="selectAll" value="Y" onclick="javascript:toggleAll(this, '${selectAllFormName}');highlightAllRows(this, 'returnItemId_tableRow_', 'selectAllForm');"/>
...
<table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
      ...
      <#list returnableItems.keySet() as orderItem>
        <#if orderItem.getEntityName() == "OrderAdjustment">
            ...
            <input type="hidden" name="orderAdjustmentId_o_${rowCount}" value="${orderItem.orderAdjustmentId}"/>
              ...
              <td></td>
              <td>
                <input type="checkbox" style="opacity: 1 !important; position: initial !important;" name="_rowSubmit_o_${rowCount}" value="Y" onclick="javascript:checkToggle(this, '${selectAllFormName}');highlightRow(this,'returnItemId_tableRow_${rowCount}');"/>
              </td>
            </tr>
        <#else>
            ...
            <input type="hidden" name="description_o_${rowCount}" value="${orderItem.itemDescription?if_exists}"/>
              ...
              <td>
                <input type="text" class="width100px" size="6" name="returnQuantity_o_${rowCount}" value="${returnableItems.get(orderItem).get("returnableQuantity")}"/>
              </td>
              <td align='left'>
              ...
              <td>
                <#if orderItem.productId?exists>
                  ...
                  <#if product.productTypeId == "ASSET_USAGE_OUT_IN">
                    <input type="text" size="8" name="returnPrice_o_${rowCount}" class="width100px" value="0.00"/>
                  <#else>
                    <input type="text" size="8" name="returnPrice_o_${rowCount}" class="width100px" value="${returnableItems.get(orderItem).get("returnablePrice")?string("##0.00")}"/>
                  </#if>
                </#if>
              </td>
              <td>
    ...
    <div>
    <h4>${uiLabelMap.OrderReturnAdjustments} ${uiLabelMap.CommonNbr} ${orderId}</h4>
    <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
    <#if orderHeaderAdjustments?has_content>
      ...
      <#list orderHeaderAdjustments as adj>
        ...
        <input type="hidden" name="description_o_${rowCount}" value="${description}"/>
          ...
          <td>
            <input type="checkbox" style="opacity: 1 !important; position: initial !important;" name="_rowSubmit_o_${rowCount}" value="Y" onclick="javascript:checkToggle(this, '${selectAllFormName}');"/>
          </td>
        </tr>
        <#assign rowCount = rowCount + 1>
      </#list>
    ...
    <div>
    <h4>${uiLabelMap.OrderReturnManualAdjustment} ${uiLabelMap.CommonNbr} ${orderId}</h4>
    <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
    <tr>
      <td>
    ...
    <tr>
      <td colspan="3" style="opacity: 1 !important; position: initial !important;" class="tooltip">*&nbsp;${uiLabelMap.OrderReturnPriceNotIncludeTax}</td>
      <td colspan="6">&nbsp;</td>
    </tr>
</table>
</div>
