...
<div class="widget-box olbius-extra no-border-bottom transparent">
      ...
      <table cellspacing="0" cellpadding="1" border="0" class="table table-striped dataTable table-hover table-bordered">
        ...
        <#list shoppingCart.items() as cartLine>
          ...
          <table border="0">
            ...
            <#if cartLine.getProductId()?exists>
              <#assign itemProductAssocList = cartLine.getProduct().getRelated("AssocProductAssoc", null, Static["org.ofbiz.base.util.UtilMisc"].toList("productAssocTypeId", "sequenceNum"), false)?if_exists/>
            </#if>
            ...
            <td nowrap="nowrap" align="center"><div><#if !cartLine.getIsPromo()>
					<input type="checkbox" name="selectedItem" onclick="javascript:checkToggle(this);"/><#else>&nbsp;</#if><span class="lbl">${cartLineIndex}</span>
            	</div>
        	</td>
          </tr>
        </#list>
  ...
  <#else>
    <div class="alert alert-info open-sans">${uiLabelMap.OrderNoOrderItemsToDisplay}</div>
  </#if>
</div>
</div>
    </div>
