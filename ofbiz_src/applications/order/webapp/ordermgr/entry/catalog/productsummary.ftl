...
<#if product?exists>
    ...
    <div class="productsummary">
        <div class="smallimage">
            <a href="${productUrl}" class="buttontext">
        ...
        <div class="productbuy">
          ...
          <#elseif product.productTypeId?if_exists == "ASSET_USAGE">
            <a href="${productUrl}" class="btn btn-mini btn-primary">${uiLabelMap.OrderMakeBooking}...</a>
          <#-- check to see if it is an aggregated or configurable product; will enter parameters on the detail screen-->
          <#elseif product.productTypeId?if_exists == "AGGREGATED" || product.productTypeId?if_exists == "AGGREGATED_SERVICE" || product.productTypeId?if_exists == "AGGR_DIGSERV">
            <a href="${productUrl}" class="btn btn-mini btn-primary">${uiLabelMap.OrderConfigure}...</a>
          <#-- check to see if the product is a virtual product -->
          <#elseif product.isVirtual?exists && product.isVirtual == "Y">
            <a href="${productUrl}" class="btn btn-mini btn-primary">${uiLabelMap.OrderChooseVariations}...</a>
          <#-- check to see if the product requires an amount -->
          <#elseif product.requireAmount?exists && product.requireAmount == "Y">
            <a href="${productUrl}" class="btn btn-mini btn-primary">${uiLabelMap.OrderChooseAmount}...</a>
          <#elseif product.productTypeId?if_exists == "ASSET_USAGE_OUT_IN">
            <a href="${productUrl}" class="btn btn-mini btn-primary">${uiLabelMap.OrderRent}...</a>
          <#else>
            <form method="post" action="<@ofbizUrl>additem</@ofbizUrl>" name="the${requestAttributes.formNamePrefix?if_exists}${requestAttributes.listIndex?if_exists}form" style="margin: 0;">
              ...
              <a href="javascript:document.the${requestAttributes.formNamePrefix?if_exists}${requestAttributes.listIndex?if_exists}form.submit()" class="btn btn-mini btn-primary margin-top-nav-10">${uiLabelMap.OrderAddToCart}</a>
            <#if mainProducts?has_content>
              ...
              <#if prodCatMem?exists && prodCatMem.quantity?exists && 0.00 < prodCatMem.quantity?double>
                <form method="post" action="<@ofbizUrl>additem</@ofbizUrl>" name="the${requestAttributes.formNamePrefix?if_exists}${requestAttributes.listIndex?if_exists}defaultform" style="margin: 0;">
                  ...
                  <input type="hidden" name="clearSearch" value="N"/>
                  <input type="hidden" name="mainSubmitted" value="Y"/>
                  <a href="javascript:document.the${requestAttributes.formNamePrefix?if_exists}${requestAttributes.listIndex?if_exists}defaultform.submit()" class="btn btn-mini btn-primary">${uiLabelMap.CommonAddDefault}(${prodCatMem.quantity?string.number}) ${uiLabelMap.OrderToCart}</a>
                </form>
        ...
        <div class="productinfo">
          <div>
            <a href="${productUrl}" class="btn btn-primary btn-mini margin-top5">${productContentWrapper.get("PRODUCT_NAME")?if_exists}</a>
          </div>
          ...
          <a href="javascript:document.addToCompare${requestAttributes.listIndex?if_exists}form.submit()" class="btn btn-mini btn-primary">${uiLabelMap.ProductAddToCompare}</a>
        </div>
    </div>
    <hr/>
<#else>
&nbsp;<p class="alert alert-info">${uiLabelMap.ProductErrorProductNotFound}.</p><br />
</#if>
