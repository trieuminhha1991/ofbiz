...
<div class="widget-box olbius-extra">
    ...
    <tr>
      <td colspan="2" align="left">
        <#if previousProductId?exists>
          <a href="<@ofbizUrl>product/~category_id=${categoryId?if_exists}/~product_id=${previousProductId?if_exists}</@ofbizUrl>" class="btn btn-small btn-info margin-left1 icon-arrow-left  icon-on-left"> ${uiLabelMap.CommonPrevious}</a>&nbsp;|&nbsp;
        </#if>
        <a href="<@ofbizUrl>category/~category_id=${categoryId?if_exists}</@ofbizUrl>" class="btn btn-small btn-info">${(category.categoryName)?default(category.description)?if_exists}</a>
        <#if nextProductId?exists>
          &nbsp;|&nbsp;<a href="<@ofbizUrl>product/~category_id=${categoryId?if_exists}/~product_id=${nextProductId?if_exists}</@ofbizUrl>" class="btn btn-small btn-info margin-left1"> ${uiLabelMap.CommonNext} <i class="icon-arrow-right  icon-on-right"> </i></a>
        </#if>
      </td>
    </tr>
  </#if>
  ...
  <tr>
	<td align="left" valign="top">
      <h2 class="blue smaller">${productContentWrapper.get("PRODUCT_NAME")?if_exists}</h2>
        ...
        <#if product.isVirtual?if_exists?upper_case == "Y">
          <#if product.virtualVariantMethodEnum?if_exists == "VV_FEATURETREE" && featureLists?has_content>
            ...
            <div id="addCart1" style="display:none;>
              ...
              <a href="javascript:javascript:addItem();" class="btn btn-small btn-info margin-top-nav-10"><span style="white-space: nowrap;">${uiLabelMap.OrderAddToCart}</span></a>
              &nbsp;
            </div>
            <div id="addCart2" style="display:block;>
              <span style="white-space: nowrap;"><b>${uiLabelMap.CommonQuantity}:</b></span>&nbsp;
              <input type="text" size="5" value="1" disabled="disabled"/>
              <a href="javascript:showErrorAlert("${uiLabelMap.CommonErrorMessage2}","${uiLabelMap.CommonPleaseSelectAllFeaturesFirst}");" class="btn btn-small btn-info margin-top-nav-10"><span style="white-space: nowrap;">${uiLabelMap.OrderAddToCart}</span></a>
              &nbsp;
            </div>
          </#if>
        ...
        <#elseif product.virtualVariantMethodEnum?if_exists != "VV_FEATURETREE">
          <#if inStock>
            ...
            <#if product.productTypeId?if_exists == "ASSET_USAGE" || product.productTypeId?if_exists == "ASSET_USAGE_OUT_IN">
                <table>
                <tr>
                <td>Reserve Start</td> <!-- ten nay Thangnv lay tu name="reservStart" trong <td> duoi -->
                <td>
				...
				<tr>
				<td>Reserve End</td> <!-- ten nay Thangnv lay tu name="reservEnd" trong <td> duoi -->
				<td>     
                ...
                <tr>
                <#--td nowrap="nowrap" align="left">Number<br />of days</td><td><input type="textt" size="4" name="reservLength"/></td></tr><tr><td>&nbsp;</td><td align="left" nowrap>&nbsp;</td-->
               <td nowrap="nowrap" align="left">Number of persons</td><td><input type="text" size="4" name="reservPersons" value="2"/></td>
                </tr>
                <tr>
                <#--td nowrap="nowrap" align="right">Number<br />of days</td><td><input type="textt" size="4" name="reservLength"/></td></tr><tr><td>&nbsp;</td><td align="right" nowrap>&nbsp;</td-->
                 <td nowrap="nowrap" align="left">Number of rooms</td><td><input type="text" size="5" name="quantity" value="1"/></td>
                </tr>
                </table>
            <#else/>
            ...
            <a href="javascript:addItem()" class="btn btn-small btn-info margin-top-nav-10" ><span style="white-space: nowrap;"> ${uiLabelMap.OrderAddToCart}</span></a>&nbsp;
          </#if>
    ...
    <div>
      <#if sessionAttributes.userLogin?has_content && sessionAttributes.userLogin.userLoginId != "anonymous">
        ...
        <form name="addToShoppingList" method="post" action="<@ofbizUrl>addItemToShoppingList<#if requestAttributes._CURRENT_VIEW_?exists>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>">
          ...
          &nbsp;&nbsp;
          <#if product.productTypeId?if_exists == "ASSET_USAGE" || product.productTypeId?if_exists == "ASSET_USAGE_OUT_IN">
              <table><tr><td>&nbsp;</td><td align="left">${uiLabelMap.CommonStartDate} (yyyy-mm-dd)</td><td><input type="text" size="10" name="reservStartStr" /></td><td>Number of&nbsp;days</td><td><input type="text" size="4" name="reservLength" /></td><td>&nbsp;</td><td align="left">Number of&nbsp;persons</td><td><input type="text" size="4" name="reservPersons" value="1" /></td><td align="left">Qty&nbsp;</td><td><input type="text" size="5" name="quantity" value="1" /></td></tr></table>
          <#else>
              <input type="text" size="5" name="quantity" value="1"/>
              <input type="hidden" name="reservStartStr" value= ""/>
          </#if>
          <a href="javascript:addShoplistSubmit();" class="btn btn-small btn-info margin-top-nav-10">${uiLabelMap.OrderAddToShoppingList}</a>
        </form>
      <#else> <br />
        ${uiLabelMap.OrderYouMust} <a href="<@ofbizUrl>checkLogin/main</@ofbizUrl>" class="btn btn-small btn-info">${uiLabelMap.CommonBeLogged}</a>
            ...
            <#list imageKeys as key>
              ...
              <#if swatchProduct?has_content && indexer < maxIndex>
                ...
                <td align="center" valign="bottom">
                  ...
                  <br />
                  <a href="javascript:getList('FT${featureOrderFirst}','${indexer}',1);" class="btn btn-small btn-info">${key}</a>
                </td>
              </#if>
  ...
  <div id="long-description">
      <div><p class="alert alert-info">${productContentWrapper.get("LONG_DESCRIPTION")?if_exists}</p></div>
      <div><p class="alert alert-warning">${productContentWrapper.get("WARNINGS")?if_exists}</p></div>
  </div>
  ...
  <#if assocProducts?has_content>
    ...
    <#list assocProducts as productAssoc>
        <div>
          <a href="<@ofbizUrl>${targetRequest}/<#if categoryId?exists>~category_id=${categoryId}/</#if>~product_id=${productAssoc.productIdTo?if_exists}</@ofbizUrl>" class="btn btn-small btn-info">
