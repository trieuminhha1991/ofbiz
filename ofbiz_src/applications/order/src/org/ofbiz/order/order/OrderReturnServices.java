...
public class OrderReturnServices {
    ...
    public static Map<String, Object> getReturnableQuantity(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (returnable && (itemStatus.equals("ITEM_APPROVED") || itemStatus.equals("ITEM_COMPLETED") || itemStatus.equals("ITEM_IN_TRANSIT"))) {
        ...
        BigDecimal returnablePrice = orderItem.getBigDecimal("unitPrice");
        // TODOCHANGE return 
        Delegator delegator = dctx.getDelegator();
        if (orderItem.containsKey("alternativeUnitPrice") && orderItem.containsKey("quantityUomId")) {
        	// convert quantity to default quantity of product
        	String orderQuantityUomId = orderItem.getString("quantityUomId");
        	String defaultQuantityUomId = product.getString("quantityUomId");
        	BigDecimal alternativePrice = orderItem.getBigDecimal("alternativeUnitPrice");
        	if (alternativePrice != null && UtilValidate.isNotEmpty(defaultQuantityUomId) && UtilValidate.isNotEmpty(orderQuantityUomId)) {
        		BigDecimal quantityTmp = UomWorker.customConvertUom(product.getString("productId"), orderQuantityUomId, defaultQuantityUomId, BigDecimal.ONE, delegator);
            	if (quantityTmp != null) {
        			returnablePrice = alternativePrice.divide(quantityTmp, decimals, rounding).setScale(decimals, rounding);
            	}
        	}
        }
        // end
        result.put("returnablePrice", returnablePrice);
        return result;
    }
    
    // get the returnable amount for an order item
    public static Map<String, Object> getReturnableAmount(DispatchContext dctx, Map<String, ? extends Object> context) {
    	GenericValue orderItem = (GenericValue) context.get("orderItem");
    	GenericValue product = null;
    	BigDecimal returnableAmount = BigDecimal.ZERO;
    	Locale locale = (Locale) context.get("locale");
    	if (orderItem.get("productId") != null) {
    		try {
    			product = orderItem.getRelatedOne("Product", false);
    	...
    	BigDecimal returnablePrice = orderItem.getBigDecimal("unitPrice");
    	Map<String, Object> result = ServiceUtil.returnSuccess();
    	if (UtilValidate.isEmpty(product.get("requireAmount")) || "N".equals(product.getString("requireAmount"))){
    		result.put("returnablePrice", returnablePrice);
        	result.put("returnableAmount", returnableAmount);
        	return result;
    	}
    	// check returnable status
    	boolean returnable = true;
    	...
    	String itemStatus = orderItem.getString("statusId");
    	BigDecimal orderAmount = orderItem.getBigDecimal("selectedAmount");
    	
    	// get the returnable amount
    	...
    	if (returnable && (itemStatus.equals("ITEM_APPROVED") || itemStatus.equals("ITEM_COMPLETED") || itemStatus.equals("ITEM_IN_TRANSIT"))) {
    		...
    		if (UtilValidate.isEmpty(returnedItems)) {
    			...
    			for (GenericValue returnItem : returnedItems) {
    				...
    				String returnStatus = returnHeader.getString("statusId");
    				if (UtilValidate.isNotEmpty(returnItem.get("returnAmount"))) { // Accounting process fail!!!
    				if (!returnStatus.equals("RETURN_CANCELLED")) {
    					returnedAmount = returnedAmount.add(returnItem.getBigDecimal("returnAmount"));
    				}
    				}
    			}
    			if (returnedAmount.compareTo(orderAmount) < 0) {
    				returnableAmount = orderAmount.subtract(returnedAmount);
    			}
    		}
    	}
    	...
    	if (orderItem.containsKey("alternativeUnitPrice") && orderItem.containsKey("weightUomId")) {
    		...
    		if (alternativeUnitPrice != null && UtilValidate.isNotEmpty(selectedAmount) && selectedAmount.compareTo(BigDecimal.ZERO) > 0) {
				returnablePrice = alternativeUnitPrice.divide(selectedAmount, decimals, rounding).setScale(decimals, rounding);
    		}
    	}
    	// end
    	result.put("returnablePrice", returnablePrice);
    ...
    public static Map<String, Object> getReturnedAmountPromo(DispatchContext ctx, Map<String, ? extends Object> context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Delegator delegator = ctx.getDelegator();
        GenericValue promoItem = (GenericValue) context.get("promoItem");
        String orderAdjustmentId = promoItem.getString("orderAdjustmentId");
        String productPromoId = promoItem.getString("productPromoId");
        String productPromoRuleId = promoItem.getString("productPromoRuleId");
        String productPromoActionSeqId = promoItem.getString("productPromoActionSeqId");
        Map<String, Object> successResult = ServiceUtil.returnSuccess();
        BigDecimal returnedAmount = BigDecimal.ZERO;
        try {
            List<EntityCondition> conds = FastList.newInstance();
            conds.add(EntityCondition.makeCondition("orderAdjustmentId", orderAdjustmentId));
            conds.add(EntityCondition.makeCondition("productPromoId", productPromoId));
            conds.add(EntityCondition.makeCondition("productPromoRuleId", productPromoRuleId));
            conds.add(EntityCondition.makeCondition("productPromoActionSeqId", productPromoActionSeqId));
            List<GenericValue> returns = delegator.findList("ReturnAdjustment", EntityCondition.makeCondition(conds), null, null, null, false);
            for(GenericValue returnItem : returns) {
                    returnedAmount = returnedAmount.add(returnItem.getBigDecimal("amount"));
            }
            successResult.put("returnedAmount", returnedAmount);
        } catch (GenericEntityException e) {
            e.printStackTrace();
            return ServiceUtil.returnError(e.getMessage());
        }
                    ...
                    if (serviceResult.containsKey(ModelService.ERROR_MESSAGE)) {
                        ...
                        if (item.get("productId") != null) {
                            try {
                                ...
                                returnInfo.put("productId", product.get("productId"));
                                returnInfo.put("productCode", product.get("productCode"));
                            } catch (GenericEntityException e) {
                                Debug.logError(e, module);
                                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                        "OrderErrorUnableToGetOrderItemInformation", locale));
                        ...
                        returnInfo.put("orderItemSeqId", item.getString("orderItemSeqId"));
                        returnInfo.put("itemDescription", item.getString("itemDescription"));
                        ...
                        if (UtilValidate.isNotEmpty(itemAdjustments)) {
                            for (GenericValue itemAdjustment : itemAdjustments) {
                                ...
                                returnInfo.put("itemTypeKey", itemTypeKey);
                                if(itemAdjustment.getString("orderAdjustmentTypeId").equals("SALES_TAX")){
                                	returnInfo.put("orderAdjustmentTypeId", itemAdjustment.getString("orderAdjustmentTypeId"));
                                	returnInfo.put("orderItemSeqId", orderItemQuantityIssued.getString("orderItemSeqId"));
                                }
                                if(itemAdjustment.getString("comments") != null){
                                	returnInfo.put("comments", itemAdjustment.getString("comments"));
                                }
                        ...
                        while (pmtppit.hasNext() && amountLeftToRefund.compareTo(ZERO) == 1) {
                            ...
                            if (electronicTypes.contains(paymentMethodTypeId)) {
                                ...
                                try {
                                    ...
                                    Map<String, Object> input = UtilMisc.<String, Object>toMap("userLogin", userLogin, "amount", amountLeftToRefund, "statusId", statusId);
                                    //end edit by dunglv
    ...
    public static Map<String, Object> createReturnAdjustment(DispatchContext dctx, Map<String, Object> context) {
        ...
        if (orderAdjustmentId != null) {
            ...
            try {
    			GenericValue objProduct = delegator.findOne("Product", false, UtilMisc.toMap("productId", returnItem.getString("productId")));
    			requireAmount = objProduct.getString("requireAmount");
    		} catch (GenericEntityException e) {
    			return ServiceUtil.returnError("OLBIUS: find product error ! " + e.toString());
    		}
        }
        ...
        //String requireAmount = "";
        if (returnItem != null) { // TODOCHANGE check case create record "order adjustment"
        try {
			GenericValue objProduct = delegator.findOne("Product", false, UtilMisc.toMap("productId", returnItem.getString("productId")));
			if (objProduct != null) requireAmount = objProduct.getString("requireAmount");
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError("OLBIUS: find product error ! " + e.toString());
		}
        }
        ...
        if (returnItem != null) {  // returnAdjustment for returnItem
            if (needRecalculate(returnAdjustmentTypeId)) {
				...
				if (UtilValidate.isNotEmpty(requireAmount) && "Y".equals(requireAmount)) {
					quantity = quantity.multiply(returnItem.getBigDecimal("returnAmount"));
				}
                ...
                if (UtilValidate.isNotEmpty(requireAmount) && "Y".equals(requireAmount)) {
                	returnTotal = returnItem.getBigDecimal("returnPrice").multiply(quantity);
                	orderTotal = orderItem.getBigDecimal("quantity").multiply(orderItem.getBigDecimal("unitPrice")).multiply(orderItem.getBigDecimal("selectedAmount"));
                }
                else
                {
                	returnTotal = returnItem.getBigDecimal("returnPrice").multiply(quantity);
                	orderTotal = orderItem.getBigDecimal("quantity").multiply(orderItem.getBigDecimal("unitPrice"));
                }                
    ...
    public static Map<String, Object> updateReturnAdjustment(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            if (returnItem != null) {  // returnAdjustment for returnItem
                ...
                BigDecimal originalReturnAmount = (context.get("originalReturnAmount") != null) ? ((BigDecimal) context.get("originalReturnAmount")) : returnItem.getBigDecimal("returnAmount");
                ...
                if (needRecalculate(returnAdjustmentTypeId)) {
                    ...
                    if (UtilValidate.isNotEmpty(requireAmount) && "Y".equals(requireAmount)) {
                    	 originalReturnTotal = originalReturnPrice.multiply(originalReturnAmount).multiply(originalReturnQuantity);
                    	 returnTotal = returnItem.getBigDecimal("returnPrice").multiply(returnItem.getBigDecimal("returnQuantity")).multiply(returnItem.getBigDecimal("returnAmount"));
					}
            ...
            result.put("returnId", returnAdjustment.get("returnId"));
            return result;
        } catch (GenericEntityException e) {
            Debug.logError(e, "Failed to store returnAdjustment", module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resource,
