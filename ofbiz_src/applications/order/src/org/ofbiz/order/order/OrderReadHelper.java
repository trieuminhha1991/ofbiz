...
public class OrderReadHelper {
    ...
    public String getCurrentStatusString(Locale locale) {
    	GenericValue statusItem = null;
    	try {
    		statusItem = orderHeader.getRelatedOne("StatusItem", true);
    	} catch (GenericEntityException e) {
    	...
    	if (statusItem != null) {
    		return statusItem.get("description", locale).toString();
    	} else {
    		return orderHeader.getString("statusId");
    	}
    }
   ...
   public List<Map<String, Object>> getShippableItemInfo(String shipGroupSeqId) {
    ...
    // thangnv add
    public BigDecimal getOrderCancelGrandTotal() {
    	if (totalPrice == null) {
    		totalPrice = getOrderGrandTotal(getOrderItemsCancel(), getAdjustments());
    	}// else already set
    	return totalPrice;
    }
    ...
    // thangnv add
    public List<GenericValue> getOrderItemsCancel() {
    	List<EntityExpr> exprs = UtilMisc.toList(
    ...
    */
    ...
    public BigDecimal getOrderReturnedTotal(boolean includeAll) {
        ...
        return getOrderReturnedTotalByTypeBd(returnTypeId, includeAll);
    }
    ...
    public BigDecimal getNewOrderItemSubTotal(GenericValue orderItem) {
        return getNewOrderItemSubTotal(orderItem, getAdjustments());
    }
    ...
    public static BigDecimal getOrderItemQuantity(GenericValue orderItem) {
    	...
    	if (alternativeOrderQty != null) {
    		...
    		if (ZERO.compareTo(cancelQty) != 0 && ZERO.compareTo(orderQty) != 0) {
    			alternativeCancelQty = cancelQty.multiply(alternativeOrderQty).divide(orderQty, 2, RoundingMode.HALF_UP);
    		}
    ...
    // Extend by VietTB for amount
    public static BigDecimal getOrderItemQuantityExtend(GenericValue orderItem) throws GenericEntityException {
    	GenericValue product = null;
        if (orderItem.get("productId") != null) {
        ...
        if (UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount"))) {
        	orderQty = orderItem.getBigDecimal("selectedAmount").multiply(orderItem.getBigDecimal("quantity"));
        }        
        if (cancelQty == null) cancelQty = ZERO;
        if (orderQty == null) orderQty = ZERO;
        ...
        //TODOCHANGE quantityUomId
        BigDecimal alternativeOrderQty = orderItem.getBigDecimal("alternativeQuantity");
    	if (alternativeOrderQty != null && !(UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount")))) {
    		BigDecimal diffQty = orderQty.subtract(cancelQty).setScale(scale, rounding);
    		if (ZERO.compareTo(diffQty) == 0) {
    			return ZERO;
    		}
...
//                    Debug.logInfo("Item : " + orderItem.getString("orderId") + " / " + orderItem.getString("orderItemSeqId") + " = " + itemTotal, module);
    ...
    /** The passed adjustments can be all adjustments for the order, ie for all line items */
    //Create by VIETTB
    public static BigDecimal getNewOrderItemSubTotal(GenericValue orderItem, List<GenericValue> adjustments) {
        return getNewOrderItemSubTotal(orderItem, adjustments, false, false);
    }
    ...
    public static BigDecimal getOrderItemSubTotal(GenericValue orderItem, List<GenericValue> adjustments, boolean forTax, boolean forShipping) {
        ...
        if (unitPrice == null || quantity == null) {
            ...
            // TODOCHANGE quantityUomId
            if (orderItem.getBigDecimal("alternativeUnitPrice") != null) {
    ...
    /** The passed adjustments can be all adjustments for the order, ie for all line items */
    public static BigDecimal getNewOrderItemSubTotal(GenericValue orderItem, List<GenericValue> adjustments, boolean forTax, boolean forShipping) {
        BigDecimal unitPrice = orderItem.getBigDecimal("unitPrice");
        BigDecimal quantity = getOrderItemQuantity(orderItem);
        BigDecimal result = ZERO;
        ...
        // subtotal also includes non tax and shipping adjustments; tax and shipping will be calculated using this adjusted value
        result = result.add(getNewOrderItemAdjustmentsTotal(orderItem, adjustments, true, false, false, forTax, forShipping));
    ...
    public static BigDecimal calcOrderPromoAdjustmentsBd(List<GenericValue> allOrderAdjustments) {
        ...
        // TODOCHANGE tax discount adjustment
        List<EntityCondition> listCondOr = FastList.newInstance();
        listCondOr.add(EntityCondition.makeCondition("orderAdjustmentTypeId", "DISCOUNT_ADJUSTMENT"));
        listCondOr.add(EntityCondition.makeCondition("orderAdjustmentTypeId", "PROMOTION_ADJUSTMENT"));
        List<GenericValue> promoAdjustments = EntityUtil.filterByCondition(allOrderAdjustments, EntityCondition.makeCondition(listCondOr, EntityOperator.OR));
        //Old: List<GenericValue> promoAdjustments = EntityUtil.filterByAnd(allOrderAdjustments, UtilMisc.toMap("orderAdjustmentTypeId", "PROMOTION_ADJUSTMENT"));
    ...
    // By VietTB
    public static Map<String, Object> calcOrderPromoAdjustmentsBdNotAmount(Delegator delegator, List<GenericValue> allOrderAdjustments) {
        BigDecimal promoAdjTotal = ZERO;
        List<GenericValue> listOrderAmountAdjusment = new FastList<GenericValue>();
        // TODOCHANGE tax discount adjustment
        List<EntityCondition> listCondOr = FastList.newInstance();
        listCondOr.add(EntityCondition.makeCondition("orderAdjustmentTypeId", "DISCOUNT_ADJUSTMENT"));
        listCondOr.add(EntityCondition.makeCondition("orderAdjustmentTypeId", "PROMOTION_ADJUSTMENT"));
        ...
        if (UtilValidate.isNotEmpty(promoAdjustments)) {
            ...
            while (promoAdjIter.hasNext()) {
                ...
                if (promoAdjustment != null) {
                	try {
	        			Map<String, Object> findProductPromoAction = new FastMap<String, Object>();
	        			findProductPromoAction.put("productPromoId", promoAdjustment.getString("productPromoId"));
	        			findProductPromoAction.put("productPromoRuleId", promoAdjustment.getString("productPromoRuleId"));	
	        			findProductPromoAction.put("productPromoActionSeqId", promoAdjustment.getString("productPromoActionSeqId"));	                	
						
	        			GenericValue productPromoAction = delegator.findByPrimaryKey("ProductPromoAction", findProductPromoAction);
	                    if (!"PROMO_ORDER_AMOUNT".equals(productPromoAction.getString("productPromoActionEnumId"))){
	                    	BigDecimal amount = promoAdjustment.getBigDecimal("amount").setScale(taxCalcScale, taxRounding);
	                    	promoAdjTotal = promoAdjTotal.add(amount);
	                    } else {
	                    	listOrderAmountAdjusment.add(promoAdjustment);
	                    }
        ...
        result.put("listOrderAmountAdjusment", listOrderAmountAdjusment);
        result.put("promoAdjTotal", promoAdjTotal);
...
//        Debug.logInfo("rental parameters....Nbr of persons:" + persons + " extra% 2nd person:" + secondPersonPerc + " extra% Nth person:" + nthPersonPerc + " Length: " + length + "  total rental adjustment:" + rentalAdjustment ,module);
    ...
    /** The passed adjustments can be all adjustments for the order, ie for all line items */
    public static BigDecimal getNewOrderItemAdjustmentsTotal(GenericValue orderItem, List<GenericValue> adjustments, boolean includeOther, boolean includeTax, boolean includeShipping, boolean forTax, boolean forShipping) {
    	List<GenericValue> adjustmentss = FastList.newInstance();
    	return calcItemAdjustments(getOrderItemQuantity(orderItem), orderItem.getBigDecimal("unitPrice"),
    			adjustmentss,
                includeOther, includeTax, includeShipping, forTax, forShipping);
    }
    ...
    public static BigDecimal newCalcItemAdjustments(BigDecimal quantity, BigDecimal unitPrice, List<GenericValue> adjustments, boolean includeOther, boolean includeTax, boolean includeShipping, boolean forTax, boolean forShipping) {
        BigDecimal adjTotal = ZERO;

        if (UtilValidate.isNotEmpty(adjustments)) {
            List<GenericValue> filteredAdjs = filterOrderAdjustments(adjustments, includeOther, includeTax, includeShipping, forTax, forShipping);
            for (GenericValue orderAdjustment : filteredAdjs) {
            	// not total DISCOUNT_ADJUSTMENT by VietTB
	            if ( !"DISCOUNT_ADJUSTMENT".equals(orderAdjustment.getString("orderAdjustmentTypeId"))){ 	            	
	                adjTotal = adjTotal.add(OrderReadHelper.calcItemAdjustment(orderAdjustment, quantity, unitPrice));
	            }
            }
        }
