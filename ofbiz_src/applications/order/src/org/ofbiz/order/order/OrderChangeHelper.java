                ...
                if (UtilValidate.isNotEmpty(orderItems)) {
                    for (GenericValue orderItem : orderItems) {
                        ...
                        if (product != null) {
                            ...
                            String orderItemTypeParentId = null; // TODOCHANGE new orderItemType PRODPROMO_ORDER_ITEM
                            if (!"PRODUCT_ORDER_ITEM".equals(orderItemType)) {
	                            try {
									GenericValue orderItemTypeParentGV = delegator.findOne("OrderItemType", UtilMisc.toMap("orderItemTypeId", orderItemType), true);
									if (orderItemTypeParentGV != null) orderItemTypeParentId = orderItemTypeParentGV.getString("parentTypeId");
								} catch (GenericEntityException e) {
									Debug.logError("Problems with find parent order item type : " + orderItemType, module);
								}
                            }
                            if (!"PRODUCT_ORDER_ITEM".equals(orderItemType) && !(orderItemTypeParentId != null && "PRODUCT_ORDER_ITEM".equals(orderItemTypeParentId))) {
