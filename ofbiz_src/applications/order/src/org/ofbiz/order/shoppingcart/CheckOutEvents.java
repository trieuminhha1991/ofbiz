...
public class CheckOutEvents {
    ...
    public static String createOrder(HttpServletRequest request, HttpServletResponse response) {
        ...
        //Debug.log(module + "::createOrder, PQD TEST");
        
        return cart.getOrderType().toLowerCase();
    }
    
    //create order for calculate PO (change last price) by dunglv
    public static String createOrderForPO(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        ShoppingCart cart = ShoppingCartEvents.getCartObject(request);
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        ...
        callResult = checkOutHelper.createOrderChangeSuppPrice(userLogin, distributorId, affiliateId, trackingCodeOrders, areOrderItemsExploded, visitId, webSiteId, "false");
        if (callResult != null) {
            ServiceUtil.getMessages(request, callResult, null);
            if (ServiceUtil.isError(callResult)) {
                // messages already setup with the getMessages call, just return the error response code
