...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
    ...
    protected List<GenericValue> adjustments = FastList.newInstance();
    // OrderTerms
    ...
    private Map<GenericPK, BigDecimal> quantityUsedPerPromoCandidate = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> countTimesUsedPerPromoCandidate = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> promoQuantityUsed = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> amountUsedPerPromoCandidate = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> promoAmountUsed = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> quantityUsedPerPromoFailed = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> quantityUsedPerPromoActual = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> amountUsedPerPromoFailed = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private Map<GenericPK, BigDecimal> amountUsedPerPromoActual = new HashMap<GenericPK, BigDecimal>(); // TODOCHANGE new promotion condition
    private boolean cartContainedReturn = false; //pos add new
    private boolean wholeCartIsReturn = false; //pos add new
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
    	...
    	return addOrIncreaseItem(productId, selectedAmount, quantity, reservStart, reservLength, reservPersons, accommodationMapId, 
    			accommodationSpotId, shipBeforeDate, shipAfterDate, features, attributes, orderItemAttributes, prodCatalogId, configWrapper, 
    			itemType, itemGroupNumber, parentProductId, dispatcher, Boolean.TRUE);
    }
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            ...
            Boolean triggerExternalOpsBool) throws CartItemModifyException, ItemNotFoundException {
        if (isReadOnlyCart()) {
           throw new CartItemModifyException("Cart items cannot be changed");
        }
        ...
        for (int i = 0; i < this.cartLines.size(); i++) {
            ...
            if (sci.equals(productId, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, features, attributes, orderItemAttributes, prodCatalogId,selectedAmount, configWrapper, itemType, itemGroup, false)) {
                ...
                if (getOrderType().equals("PURCHASE_ORDER")) {
                	//hoandv add new process for saleschanneltype is pos_sale_channnel
                	if(!getChannelType().equalsIgnoreCase("POS_SALES_CHANNEL")){
        ...
        if (getOrderType().equals("PURCHASE_ORDER")) {
        	...
        	if(getChannelType().equalsIgnoreCase("POS_SALES_CHANNEL")){
        		item = ShoppingCartItem.makePurchaseOrderItem(Integer.valueOf(0), productId, selectedAmount, quantity, features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, this, supplierProduct, shipBeforeDate, shipAfterDate, cancelBackOrderDate, triggerExternalOpsBool);
        	}else{
        		if (attributes != null && attributes.containsKey("quantityUomId") && UtilValidate.isNotEmpty(attributes.get("quantityUomId"))){
        			String quantityUomId = (String) attributes.get("quantityUomId");
        			GenericValue product;
					try {
						product = delegator.findOne("Product", false, UtilMisc.toMap("productId", productId));
						BigDecimal convert = ProductUtil.getConvertPackingNumber(delegator, productId, quantityUomId, product.getString("quantityUomId"));
						supplierProduct = getSupplierProduct(productId, quantity.multiply(convert), dispatcher);
					} catch (GenericEntityException e) {
						return -1;
					}
        		} else {
            ...
            item = ShoppingCartItem.makeItem(Integer.valueOf(0), productId, selectedAmount, quantity, null,
                    reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, shipBeforeDate, shipAfterDate,
                    features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher,
                    this, triggerExternalOpsBool, Boolean.TRUE, parentProductId, Boolean.FALSE, Boolean.FALSE);
        }
    ...
    public ShoppingCartItem findCartItem(String productId, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, 
    		String accommodationMapId, String accommodationSpotId, Map<String, GenericValue> features, Map<String, Object> attributes, Map<String, String> orderItemAttributes, 
    		String prodCatalogId, BigDecimal selectedAmount, ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber) {
    	ShoppingCart.ShoppingCartItemGroup itemGroup = this.getItemGroupByNumber(itemGroupNumber);
    	...
    	for (int i = 0; i < this.cartLines.size(); i++) {
            ...
            if (cartItem.equals(productId, reservStart, reservLength, reservPersons, 
            		accommodationMapId, accommodationSpotId, features, attributes, orderItemAttributes, 
    ...
    //pos add new
    public void deleteCartItem(int index, LocalDispatcher dispatcher) throws CartItemModifyException {
        if (isReadOnlyCart()) {
           throw new CartItemModifyException("Cart items cannot be changed");
        }
        if (index < 0) return;
    ...
    //pos add new
    public boolean haveContainReturnItem(){
    	if(!wholeCartIsReturned()){
    		this.cartContainedReturn = false;
    		return false;
    	}else{
    		if(UtilValidate.isNotEmpty(cartLines) && cartLines.size() > 0){
        		for (ShoppingCartItem cartItem : cartLines) {
    				BigDecimal quantity = cartItem.getQuantity();
    				if(quantity.compareTo(BigDecimal.ZERO) < 0){
    					this.cartContainedReturn = true;
    					return true;
    				}
    			}
        		this.cartContainedReturn = false; 
    ...
    //pos add new
    public boolean wholeCartIsReturned(){
    	boolean hasReturnedOrder = false;
    	//if hasReturnedOrder = false, the cart 
    	if(UtilValidate.isNotEmpty(cartLines) && cartLines.size() > 0){
    		for (ShoppingCartItem cartItem : cartLines) {
				BigDecimal quantity = cartItem.getQuantity();
				if(quantity.compareTo(BigDecimal.ZERO) >0){
    		...
    		if(hasReturnedOrder){
    			this.wholeCartIsReturn = false;
    		}else{
    			this.wholeCartIsReturn = true;
    		}
    		return hasReturnedOrder;
    	}else{
    		if(hasReturnedOrder){
    			this.wholeCartIsReturn = false;
    		}else{
    ...
    //pos add new
    public boolean isCartContainedReturn() {
		return cartContainedReturn;
	}
    //pos add new
	public void setCartContainedReturn(boolean cartContainedReturn) {
		this.cartContainedReturn = cartContainedReturn;
	}
	 //pos add new
	public boolean isWholeCartIsReturn() {
		return wholeCartIsReturn;
	}
	 //pos add new
	public void setWholeCartIsReturn(boolean wholeCartIsReturn) {
		this.wholeCartIsReturn = wholeCartIsReturn;
	}
    ...
    public ShoppingCartItem findCartItem(String associatedOrderId, String associatedOrderItemSeqId) {
        if (associatedOrderId != null && associatedOrderItemSeqId != null) {
            for (int i = 0; i < this.cartLines.size(); i++) {
                ShoppingCartItem cartItem = cartLines.get(i);
                String itemOrderId = cartItem.getAssociatedOrderId();
                String itemOrderSeqId = cartItem.getAssociatedOrderItemSeqId();
                if (itemOrderId != null && associatedOrderId.equals(itemOrderId) 
                		&& itemOrderSeqId != null && associatedOrderItemSeqId.equals(itemOrderSeqId)) {
                    return cartItem;
                }
            }
        }
    ...
    /** TODOCHANGE add code: extends form removeCartItem method. Remove an item from the cart object. */
    public void removeCartItemPayPromo(int index, LocalDispatcher dispatcher) throws CartItemModifyException {
        if (isReadOnlyCart()) {
           throw new CartItemModifyException("Cart items cannot be changed");
        }
        if (index < 0) return;
        ...
        // ShoppingCartItem item = cartLines.remove(index);
        // set quantity to 0 to trigger necessary events
        // item.setQuantityPayPromo(BigDecimal.ZERO, dispatcher, this);
    }
...
//    protected String billFromVendorPartyId = null;
  //  protected String shipFromVendorPartyId = null;
    ...
    /** TODOCHANGE Returns the item's customer desired delivery date. */
    public Timestamp getDefaultItemDeliveryDateTime() {
        String ddDate = getDefaultItemDeliveryDate();

        if (ddDate != null) {
            try {
                return Timestamp.valueOf(ddDate);
            } catch (IllegalArgumentException e) {
                Debug.logWarning(e, "Order problem getting desired delivery date");
                return null;
            }
        }
        return null;
    ...
    //add by dunglv for pos
    public BigDecimal getTotalSalesTaxPos() {
        BigDecimal totalTax = ZERO;
        for (int i = 0; i < shipInfo.size(); i++) {
            CartShipInfo csi = this.getShipInfo(i);
            totalTax = totalTax.add(csi.getTotalTaxPos(this)).setScale(taxCalcScale, taxRounding);
        }
        return totalTax.setScale(taxFinalScale, taxRounding);
    }
    ...
    /** Returns the sub-total in the cart (item-total - discount).
     * TODOCHANGE price with tax */
    public BigDecimal getSubTotalAndTax() {
    	BigDecimal itemsTotal = BigDecimal.ZERO;
    	for (ShoppingCartItem cartItem : this.cartLines) {
    		itemsTotal = itemsTotal.add(cartItem.getItemSubTotalAndTax(this));
    	}
    	return itemsTotal;
    }
    ...
    //pos add new
    public BigDecimal getSubTotalNoReturn(){
   	 BigDecimal itemsTotal = BigDecimal.ZERO;
        for (ShoppingCartItem cartItem : this.cartLines) {
       	 BigDecimal quantity = cartItem.getQuantity();
       	 if(quantity.compareTo(BigDecimal.ZERO) > 0){
   ...
   //pos add new
   public BigDecimal getSubTotalReturn(){
  	 BigDecimal itemsTotal = BigDecimal.ZERO;
       for (ShoppingCartItem cartItem : this.cartLines) {
      	 BigDecimal quantity = cartItem.getQuantity();
      	 if(quantity.compareTo(BigDecimal.ZERO) <  0){
   ...
   /** Returns the sub-total in the cart (item-total - not discount). */
   public BigDecimal getSubTotalNotAdj() {
       BigDecimal itemsTotal = BigDecimal.ZERO;
       for (ShoppingCartItem cartItem : this.cartLines) {
           itemsTotal = itemsTotal.add(cartItem.getItemSubTotalNotAdj());
       }
       return itemsTotal;
   }
    //pos add new
    public BigDecimal getGrandTotalNoReturn() {
    	BigDecimal grandTotal = this.getSubTotalNoReturn().add(this.getTotalShipping()).add(this.getTotalSalesTaxPos()).add(this.getOrderOtherAdjustmentTotal());
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
        return grandTotal;
    }
    //pos add new
    public BigDecimal getGranTotalReturn(){
    	BigDecimal grandTotalNoReturn = getGrandTotalNoReturn();
    	BigDecimal grandTotal = getGrandTotal();
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
    	return grandTotalNoReturn.subtract(grandTotal);
    }
    //pos add new
    public BigDecimal getGrandTotalNotAdjusment(){
    	 BigDecimal grandTotal = this.getSubTotal();
    	 grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
         return grandTotal;
    }
    //pos add new
    //remove all tax on cart
    public void removeAllTax(){
    	List<GenericValue> adjustmentList = getAdjustments();
    	if(UtilValidate.isNotEmpty(adjustmentList)){
    		for (GenericValue adjustment : adjustmentList) {
				if(adjustment.get("orderAdjustmentTypeId").equals("SALES_TAX")){
					adjustmentList.remove(adjustment);
				}
			}
    	}
    ...
    /** Returns the sub-total in the cart (item-total - discount). 
     * TODOCHANGE usePriceWithTax promo condition */
    public BigDecimal getSubTotalForPromotions(boolean usePriceWithTax) {
        BigDecimal itemsTotal = BigDecimal.ZERO;
        for (ShoppingCartItem cartItem : this.cartLines) {
            GenericValue product = cartItem.getProduct();
            if (product != null && "N".equals(product.getString("includeInPromotions"))) {
            ...
            // TODOCHANGE
            BigDecimal itemSubTotal = null;
            if (usePriceWithTax) {
            	itemSubTotal = cartItem.getItemSubTotalAndTax(this);
            } else {
            	itemSubTotal = cartItem.getItemSubTotal();
            }
            if (itemSubTotal != null) itemsTotal = itemsTotal.add(itemSubTotal);
        }
        return itemsTotal;
    }
    
    /** Returns the sub-total in the cart (item-total - discount). */
    public List<ShoppingCartItem> getCartLines() {        
    ...
    public void addProductPromoUse(String productPromoId, String productPromoCodeId, BigDecimal totalDiscountAmount, BigDecimal quantityLeftInActions, Map<GenericPK, String> statusUsePerCondActual) {
    	if (UtilValidate.isNotEmpty(productPromoCodeId) && !this.productPromoCodes.contains(productPromoCodeId)) {
    		throw new IllegalStateException("Cannot add a use to a promo code use for a code that has not been entered.");
    	}
    	if (Debug.verboseOn()) Debug.logVerbose("Used promotion [" + productPromoId + "] with code [" + productPromoCodeId + "] for total discount [" + totalDiscountAmount + "] and quantity left in actions [" + quantityLeftInActions + "]", module);
    	this.productPromoUseInfoList.add(new ProductPromoUseInfo(productPromoId, productPromoCodeId, totalDiscountAmount, quantityLeftInActions, statusUsePerCondActual));
    }
    ...
    public void clearCartItemUseInPromoInfo() {
        ...
        this.clearPromoRuleUseInfo(); // TODOCHANGE usePriceWithTax promo condition
    }
    ...
    // TODOCHANGE change code: extend from "clearAllPromotionAdjustments" method
    public void clearAllPromotionAdjustments() {
        ...
        // end old
        clearAllPromotionAdjustmentsHasPayPromo();
    }
    
	// TODOCHANGE new orderItemType PRODPROMO_ORDER_ITEM
    public void clearAllPromotionAdjustmentsHasPayPromo() {
        // remove cart adjustments from promo actions
        List<GenericValue> cartAdjustments = this.getAdjustments();
        if (cartAdjustments != null) {
            Iterator<GenericValue> cartAdjustmentIter = cartAdjustments.iterator();
        ...
        while (cartItemIter.hasNext()) {
            ShoppingCartItem checkItem = cartItemIter.next();
            if (checkItem != null && checkItem.getIsPromo()) {
            	// TODOCHANGE add check if item is item pay promotion then not remove
            	if (!"PRODPROMO_ORDER_ITEM".equals(checkItem.getItemType())) {
            		this.clearItemShipInfo(checkItem);
                    cartItemIter.remove();
            	}
            } else {
    ...
    /** TODOCHANGE add new 
     * Remove a promotion code in the cart, 
     * @param productPromoCodeId The promotion code to check and add
     * @return 
     */
    public String removeProductPromoCode(String productPromoCodeId, LocalDispatcher dispatcher) {
        if (!this.productPromoCodes.contains(productPromoCodeId)) {
            return UtilProperties.getMessage(resource_error, "productpromoworker.promotion_code_has_not_yet_been_used", UtilMisc.toMap("productPromoCodeId", productPromoCodeId), locale);
        }
        this.productPromoCodes.remove(productPromoCodeId);
        // new promo code, re-evaluate promos
        ProductPromoWorker.doPromotions(this, dispatcher);
        return null;
    }
    ...
    public String getAggregatedInstanceId (ShoppingCartItem item, LocalDispatcher dispatcher) {
        ...
        try {
            ...
            else if(ProductWorker.isAggregateDigitalService(delegator, item.getProductId())) {
                serviceContext.put("productTypeId", "AGGR_DIGSERV_CONF");
            }
            else {
    ...
    private void explodeItems(LocalDispatcher dispatcher) {
        ...
        synchronized (cartLines) {explodeItems(dispatcher);
    ...
    /*public List<GenericValue> makeAllShipGroupInfos(boolean isSetupFacility) {
    	// check "fromInventoryItemId"
    	if (this.shipInfo.size() <= 1) {
    		setupFacilityIdForShipInfo(); // TODOCHANGE
    	}
        List<GenericValue> groups = new LinkedList<GenericValue>();
        long seqId = 1;
        for (CartShipInfo csi : this.shipInfo) {
    ...
    // TODOCHANGE count facility in shipItemInfo
    private void setupFacilityIdForShipInfo() {
    	List<String> listFacility = new ArrayList<String>();
    	Map<ShoppingCartItem, String> listCartItemAndFacilityId  = FastMap.newInstance(); // List (item, facilityId) of item_fromInventoryItemId
    	boolean hasNullValue = false;
    	int sizeShipInfo = 0;
    	
    	if (this.shipInfo != null) {
    		sizeShipInfo = this.shipInfo.size();
    	}
    	
    	for (CartShipInfo csi : this.shipInfo) {
    		for (ShoppingCartItem item : csi.shipItemInfo.keySet()) {
    			// CartShipInfo.CartShipItemInfo csii = csi.shipItemInfo.get(item);
    			GenericValue facilityItem = null;
                String facilityId = "";
				try {
					facilityItem = delegator.findOne("InventoryItem", UtilMisc.toMap("inventoryItemId", item.getAttribute("fromInventoryItemId")), false);
				} catch (GenericEntityException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
                if (UtilValidate.isNotEmpty(facilityItem)) {
                	facilityId = facilityItem.getString("facilityId");
                	if (UtilValidate.isNotEmpty(facilityId) && !listFacility.contains(facilityId)) {
                		listFacility.add(facilityId);
                	}
                } else {
            		hasNullValue = true;
            	}
                listCartItemAndFacilityId.put(item, facilityId);
    		}
		}
    	
    	boolean added = false;
    	if (listFacility != null && listFacility.size() > 0) {
    		if (listFacility.size() > sizeShipInfo) {
    			int minus = listFacility.size() - sizeShipInfo;
    			for (int i = 0; i < minus; i++) {
    				int newShipInfo = addShipInfo();
    				if (newShipInfo > -1) {
    					this.shipInfo.get(newShipInfo).copyCartShipInfo(this.shipInfo.get(0));
    				}
    				added = true;
				}
    		}
    	...
    	if (added) {
    		...
    		// this.shipInfo.size() - listFacility.size() = 0 or 1
        	for (int i = 0; i < listFacility.size(); i++) {
				CartShipInfo csi = this.shipInfo.get(i);
				if (csi.getFacilityId() != null && listFacility.get(i).equals(csi.getFacilityId())) {
					continue;
				}
				this.setShipGroupFacilityId(i, listFacility.get(i));
    		}
        	
        	if (sizeShipInfo > 1) {
        		// change all item of lastShipInfo to firstShipInfo
        		CartShipInfo lastShipInfo = this.shipInfo.get(sizeShipInfo - 1);
        		if (UtilValidate.isNotEmpty(lastShipInfo)) {
        			for (ShoppingCartItem item : lastShipInfo.shipItemInfo.keySet()) {
        				String facilityId = listCartItemAndFacilityId.get(item);
        				if ((lastShipInfo.getFacilityId() != null && facilityId != null && !lastShipInfo.getFacilityId().equals(facilityId)) 
    							|| (lastShipInfo.getFacilityId() == null && facilityId != null)) {
    						positionItemToGroup(item, item.getQuantity(), sizeShipInfo - 1, 0, false);
    					}
        			}
        		}
        		
        		// change items to every shipInfo by facilityId
        		for (int i = 0; i < (sizeShipInfo - 1); i++) {
            		CartShipInfo csi = this.shipInfo.get(i);
            		for (ShoppingCartItem item : csi.shipItemInfo.keySet()) {
            			String facilityId = listCartItemAndFacilityId.get(item);
            			if ((csi.getFacilityId() != null && facilityId != null && !csi.getFacilityId().equals(facilityId))) {
            				int intFinded = sizeShipInfo - 1;
            				for (int j = 0; j < sizeShipInfo; j++) {
								CartShipInfo cartItemFinded = this.shipInfo.get(j);
            					if (facilityId.equals(cartItemFinded.getFacilityId())) {
									intFinded = j;
									break;
								}
							}
            				positionItemToGroup(item, item.getQuantity(), this.shipInfo.indexOf(csi), intFinded, false);
            			} else if (csi.getFacilityId() == null && facilityId != null && !"".equals(facilityId)) {
            				positionItemToGroup(item, item.getQuantity(), this.shipInfo.indexOf(csi), sizeShipInfo - 1, false);
            			}
            		}
            	}
        	}
    ...
    public boolean removeShipInfo(int index) {
    	if (index < this.shipInfo.size()) {
    		CartShipInfo csi = this.shipInfo.get(index);
    		if (csi != null && csi.shipItemInfo.size() == 0) {
    			this.shipInfo.remove(index);
    			return true;
    		}
    	}
    	return false;
        ...
        for (CartShipInfo csi : shipInfo) {
            ...
            for (ShoppingCartItem item : items) {
                ...
                if (item.getAssociatedOrderId() != null && item.getAssociatedOrderItemSeqId() != null) {
                    ...
                    orderItemAssociation.set("quantity", item.getQuantity());
                    allOrderItemAssociations.add(orderItemAssociation);
                }
            }
        }
    ...
    public Map<String, Object> makeCartMap(LocalDispatcher dispatcher, boolean explodeItems) {
        ...
        // result.put("orderItemShipGroupInfo", this.makeAllShipGroupInfos(true));
        ...
        // TODOCHANGE add new attribute: "salesMethodChannelEnumId", "isFavorDelivery", "agreementId", "salesExecutiveId", 
        // "favorDistributorPartyId"
        result.put("salesMethodChannelEnumId", this.getSalesMethodChannelEnumId());
        result.put("agreementId", this.getAgreementId());
        result.put("salesExecutiveId", this.getAttribute("salesExecutiveId"));
    	result.put("isFavorDelivery", this.getAttribute("isFavorDelivery"));
    	result.put("favorDistributorPartyId", this.getAttribute("favorDistributorPartyId"));
    	result.put("priority", this.getAttribute("priority"));
    	result.put("estimatedDeliveryDate", this.getDefaultItemDeliveryDateTime());
    	result.put("shipAfterDate", this.getDefaultShipAfterDate());
    	result.put("shipBeforeDate", this.getDefaultShipBeforeDate());
        // END CHANGE
    ...
    // TODOCHANGE new method in ShoppingCart
    public String getSalesMethodChannelEnumId(){
    	String salesMethodChannelEnumId = null;
        String productStoreId = this.getProductStoreId();
        if (UtilValidate.isNotEmpty(productStoreId)) {
        	try {
	        	GenericValue productStore = this.getDelegator().findOne("ProductStore", UtilMisc.toMap("productStoreId", productStoreId), true);
	        	salesMethodChannelEnumId = productStore.getString("salesMethodChannelEnumId");
	        } catch (GenericEntityException e) {
	            Debug.logError(e, "Cannot find product store with id: " + productStoreId, module);
	        }
        }
        return salesMethodChannelEnumId;
    }
    ...
    public static class ProductPromoUseInfo implements Serializable {
        ...
        public Map<GenericPK, String> statusUsePerCondActual = null; // TODOCHANGE manual promotion
        ...
        public ProductPromoUseInfo(String productPromoId, String productPromoCodeId, BigDecimal totalDiscountAmount, 
        		BigDecimal quantityLeftInActions, Map<GenericPK, String> statusUsePerCondActual) {
        	this.productPromoId = productPromoId;
        	this.productPromoCodeId = productPromoCodeId;
        	this.totalDiscountAmount = totalDiscountAmount;
        	this.quantityLeftInActions = quantityLeftInActions;
        	this.statusUsePerCondActual = statusUsePerCondActual;
        }
        ...
        public Map<GenericPK, String> getStatusUsePerCondActual() { return this.statusUsePerCondActual; }
    }
    ...
    public static class CartShipInfo implements Serializable {
        ...
        public void copyCartShipInfo(CartShipInfo paramCartShipInfo) {
        	this.orderTypeId = paramCartShipInfo.getOrderTypeId();
        	this.internalContactMechId = paramCartShipInfo.getContactMechId();
        	this.telecomContactMechId = paramCartShipInfo.telecomContactMechId;
        	this.shipmentMethodTypeId = paramCartShipInfo.getShipmentMethodTypeId();
        	this.supplierPartyId = paramCartShipInfo.getSupplierPartyId();
        	this.carrierRoleTypeId = paramCartShipInfo.getCarrierPartyId();
        	this.carrierPartyId = paramCartShipInfo.carrierPartyId;
        	this.facilityId = paramCartShipInfo.getFacilityId();
        	this.giftMessage = paramCartShipInfo.giftMessage;
        	this.shippingInstructions = paramCartShipInfo.shippingInstructions;
        	this.maySplit = paramCartShipInfo.maySplit;
        	this.isGift = paramCartShipInfo.isGift;
        	this.shipEstimate = paramCartShipInfo.shipEstimate;
        	this.shipBeforeDate = paramCartShipInfo.shipBeforeDate;
        	this.shipAfterDate = paramCartShipInfo.shipAfterDate;
        	this.vendorPartyId = paramCartShipInfo.vendorPartyId;
        	this.productStoreShipMethId = paramCartShipInfo.productStoreShipMethId;
        	this.attributes = paramCartShipInfo.attributes;
        }
        ...
        public List<GenericValue> makeItemShipGroupAndAssoc(Delegator delegator, ShoppingCart cart, long groupIndex) {
            ...
            for (ShoppingCartItem item : shipItemInfo.keySet()) {
                ...
                //edit by dunglv for pos
                //check quantity > 0 add to item ship group
                if (itemInfo.quantity.compareTo(BigDecimal.ZERO) > 0){
                	values.add(assoc);
                }
                ...
                for (GenericValue taxAdj : itemInfo.itemTaxAdj) {
                    ...
                    taxAdj.set("shipGroupSeqId", shipGroupSeqId);
                    //edit by thaint: comment by thaint, because dms not used pos, For further information please contact dunglv
                    //edit by dunglv
//                    if (taxAdj.getBigDecimal("amount").compareTo(BigDecimal.ZERO) > 0){
                    	values.add(taxAdj);
//                    }
                }
            }
        ...
        public CartShipItemInfo setItemInfo(ShoppingCartItem item, BigDecimal quantity, List<GenericValue> taxAdj) {
            ...
            //edit by dunglv for return no order pos
            if ((quantity.compareTo(BigDecimal.ZERO) >= 0) || (item.getQuantity().compareTo(BigDecimal.ZERO) < 0)) {
        ...
        //add by dunglv for pos
        public BigDecimal getTotalTaxPos(ShoppingCart cart) {
            List<GenericValue> taxAdjustments = FastList.newInstance();
            taxAdjustments.addAll(shipTaxAdj);
            for (CartShipItemInfo info : shipItemInfo.values()) {
            	for (GenericValue item : info.itemTaxAdj){
            		if (item.getBigDecimal("amount").compareTo(BigDecimal.ZERO) > 0){
            			 taxAdjustments.add(item);
            		}
            	}
            }
            Map<String, Object> taxByAuthority = OrderReadHelper.getOrderTaxByTaxAuthGeoAndParty(taxAdjustments);
            ...
            if (valueObj != null) {
                ...
                if ("Y".equals(splitPayPrefPerShpGrp)  && cart.paymentInfo.size() == 1) {
                    for (CartShipInfo csi : cart.getShipGroups()) {
                    	...
                    	/* by PQD for logging */
                    	//BigDecimal total = csi.getTotal();
                    	//BigDecimal totalAdjustment = cart.getOrderOtherAdjustmentTotal();
                    	//int sz = cart.getShipGroupSize();
                    	//BigDecimal shipEstimate = csi.getShipEstimate();
                    	//BigDecimal tax = csi.getTotalTax(cart);
                        ...
                        //Debug.logInfo("Creating OrderPaymentPreference, origin maxAmount = " + maxAmount
                        //		+ ", total = " + total + ", totalAdjustment = " + totalAdjustment + ", sz = " + sz + 
                        //		", shipEstimate = " + shipEstimate + ", tax = " + tax 
                        //		,module);
                        
                        maxAmount = maxAmount.setScale(scale, rounding);

                        //Debug.logInfo("Creating OrderPaymentPreference, ajust maxAmount = " + maxAmount,module);
                        /* end of PQD */
    ...
    // TODOCHANGE new promotion condition
    public synchronized void resetPromoRuleUse(String productPromoId, String productPromoRuleId, BigDecimal valueRollback) {
        for (ShoppingCartItem cartItem : this) {
            cartItem.resetPromoRuleUse(productPromoId, productPromoRuleId);
        }
        resetPromoRuleUseCart(productPromoId, productPromoRuleId, valueRollback);
    }
    
    // TODOCHANGE new promotion condition
    public BigDecimal getPromoQuantityAvailable(GenericPK productPromoCondActionPK, BigDecimal totalAvailabel) {
    	BigDecimal quantity = BigDecimal.ZERO;
    	if (totalAvailabel == null) {
    		...
    		quantity = totalAvailabel;
    	}
    	if (quantity.compareTo(BigDecimal.ZERO) < 1) return BigDecimal.ZERO;
    	BigDecimal promoQuantityUsed = this.promoQuantityUsed.get(productPromoCondActionPK);
    	if (promoQuantityUsed != null) {
    		return quantity.subtract(promoQuantityUsed);
    	}
        return quantity;
    }
    
    // TODOCHANGE new promotion condition
    public synchronized BigDecimal addPromoQuantityCandidateUse(BigDecimal quantityDesired, GenericValue productPromoCondAction, boolean checkAvailableOnly, BigDecimal totalAvailabel) {
        ...
        BigDecimal promoQuantityAvailable = this.getPromoQuantityAvailable(productPromoCondActionPK, totalAvailabel);
        ...
        if (promoQuantityAvailable.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (!checkAvailableOnly) {
        		BigDecimal countTimesApply = promoQuantityAvailable.divide(promoQuantityToUse, 0, BigDecimal.ROUND_DOWN);
        		countTimesUsedPerPromoCandidate.put(productPromoCondActionPK, countTimesApply);
        		promoQuantityToUse = promoQuantityToUse.multiply(countTimesApply);
                ...
                BigDecimal existingValueTotalUsed = this.promoQuantityUsed.get(productPromoCondActionPK);
                if (existingValueTotalUsed == null) {
                    this.promoQuantityUsed.put(productPromoCondActionPK, promoQuantityToUse);
                } else {
                    this.promoQuantityUsed.put(productPromoCondActionPK, promoQuantityToUse.add(existingValueTotalUsed));
                }
            }
    ...
    // TODOCHANGE new promotion condition
    public BigDecimal getPromoAmountAvailable(GenericPK productPromoCondActionPK, BigDecimal totalAvailabel, boolean usePriceWithTax) {
    	...
    	if (totalAvailabel == null) {
    		...
    		for (ShoppingCartItem cartItem : this.cartLines) {
    			if (!cartItem.getIsPromo()) {
                	...
                	if (usePriceWithTax) {
                		List<CartShipInfo> csis = this.shipInfo;
                		for (CartShipInfo csi : csis) {
                			CartShipInfo.CartShipItemInfo itemInfo = csi.shipItemInfo.get(cartItem);
                			if (itemInfo == null) {
                				continue;
                			}
                			// create the item tax adjustment
    				...
    				amount = amount.add(itemSubTotal);
    			}
            }
    	} else {
    		amount = totalAvailabel;
    	}
    	if (amount.compareTo(BigDecimal.ZERO) < 1) return BigDecimal.ZERO;
    	BigDecimal promoAmountUsed = this.promoAmountUsed.get(productPromoCondActionPK);
    	if (promoAmountUsed != null) {
    		return amount.subtract(promoAmountUsed);
    	}
        return amount;
    }
    
    // TODOCHANGE new promotion condition
    public synchronized BigDecimal addPromAmountCandidateUse(BigDecimal amountDesired, GenericValue productPromoCondAction, boolean checkAvailableOnly, 
    		BigDecimal totalAvailabel, boolean usePriceWithTax) {
        if (amountDesired.compareTo(BigDecimal.ZERO) == 0) return BigDecimal.ZERO;
        GenericPK productPromoCondActionPK = productPromoCondAction.getPrimaryKey();
        BigDecimal promoAmountAvailable = this.getPromoAmountAvailable(productPromoCondActionPK, totalAvailabel, usePriceWithTax);
        BigDecimal promoAmountToUse = amountDesired;
        if (promoAmountAvailable.compareTo(BigDecimal.ZERO) > 0) {
            if (promoAmountToUse.compareTo(promoAmountAvailable) > 0) {
            	promoAmountToUse = promoAmountAvailable;
            }

            if (!checkAvailableOnly) {
            	BigDecimal countTimesApply = promoAmountAvailable.divide(promoAmountToUse, 0, BigDecimal.ROUND_DOWN);
        		countTimesUsedPerPromoCandidate.put(productPromoCondActionPK, countTimesApply);
        		promoAmountToUse = promoAmountAvailable.multiply(countTimesApply);
            	
                // keep track of candidate promo uses on cartItem
                BigDecimal existingValue = this.amountUsedPerPromoCandidate.get(productPromoCondActionPK);
                if (existingValue == null) {
                    this.amountUsedPerPromoCandidate.put(productPromoCondActionPK, promoAmountToUse);
                } else {
                    this.amountUsedPerPromoCandidate.put(productPromoCondActionPK, promoAmountToUse.add(existingValue));
                }

                BigDecimal existingValueTotalUsed = this.promoAmountUsed.get(productPromoCondActionPK);
                if (existingValueTotalUsed == null) {
                    this.promoAmountUsed.put(productPromoCondActionPK, promoAmountToUse);
                } else {
                    this.promoAmountUsed.put(productPromoCondActionPK, promoAmountToUse.add(existingValueTotalUsed));
                }
            }

            return promoAmountToUse;
        } else {
            return BigDecimal.ZERO;
        }
    }
    ...
    // TODOCHANGE new promotion condition
    public synchronized void resetPromoRuleUseCart(String productPromoId, String productPromoRuleId, BigDecimal valueRollback) {
        ...
        while (entryIter.hasNext()) {
            ...
            if (productPromoId.equals(productPromoCondActionPK.getString("productPromoId")) && productPromoRuleId.equals(productPromoCondActionPK.getString("productPromoRuleId"))) {
                if (valueRollback != null && valueRollback.compareTo(quantityUsed) <= 0) {
                	entryIter.remove();
                    quantityUsed = valueRollback;
                    ...
                    BigDecimal promoQuantityUsed = this.promoQuantityUsed.get(productPromoCondActionPK);
                    this.promoQuantityUsed.put(productPromoCondActionPK, promoQuantityUsed.subtract(quantityUsed));
                }
            }
        }
        
        Iterator<Map.Entry<GenericPK, BigDecimal>> entryIter2 = this.amountUsedPerPromoCandidate.entrySet().iterator();
        while (entryIter2.hasNext()) {
            Map.Entry<GenericPK, BigDecimal> entry = entryIter2.next();
            GenericPK productPromoCondActionPK = entry.getKey();
            BigDecimal amountUsed = entry.getValue();
            if (productPromoId.equals(productPromoCondActionPK.getString("productPromoId")) && productPromoRuleId.equals(productPromoCondActionPK.getString("productPromoRuleId"))) {
            	if (valueRollback != null && valueRollback.compareTo(amountUsed) <= 0) {
            		entryIter2.remove();
            		amountUsed = valueRollback;
                    BigDecimal existingValue = this.amountUsedPerPromoFailed.get(productPromoCondActionPK);
                    if (existingValue == null) {
                        this.amountUsedPerPromoFailed.put(productPromoCondActionPK, amountUsed);
                    } else {
                        this.amountUsedPerPromoFailed.put(productPromoCondActionPK, amountUsed.add(existingValue));
                    }
                    BigDecimal promoQuantityUsed = this.promoAmountUsed.get(productPromoCondActionPK);
                    this.promoAmountUsed.put(productPromoCondActionPK, promoQuantityUsed.subtract(amountUsed));
            	}
            }
        }
    }
    ...
    public synchronized void clearPromoRuleUseInfo() {
        ...
        this.promoQuantityUsed.clear();
        this.amountUsedPerPromoActual.clear();
        this.amountUsedPerPromoCandidate.clear();
        this.amountUsedPerPromoFailed.clear();
        this.promoAmountUsed.clear();
        this.countTimesUsedPerPromoCandidate.clear();
    }

	public Map<GenericPK, BigDecimal> getCountTimesUsedPerPromoCandidate() {
		return countTimesUsedPerPromoCandidate;
	}
	
	public void makeNextSeqCartItems() {
        // now build the lines
        synchronized (cartLines) {
            for (ShoppingCartItem item : cartLines) {
                if (UtilValidate.isEmpty(item.getOrderItemSeqId())) {
