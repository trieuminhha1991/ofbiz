...
public class CheckOutHelper {
    ...
    protected List<String> setCheckOutShippingAddressInternal(String shippingContactMechId) {
        List<String> errorMessages = new ArrayList<String>();
        String errMsg = null;
    ...
    private List<String> setCheckOutShippingOptionsInternal(String shippingMethod, String shippingInstructions, String orderAdditionalEmails,
        ...
        if (UtilValidate.isNotEmpty(shipBeforeDate)) {
            if (UtilValidate.isDate(shipBeforeDate)) {
                cart.setShipBeforeDate(UtilDateTime.toTimestamp(shipBeforeDate));
            } else if (UtilValidate.isNotEmpty(shipBeforeDate)) {
            	try {
            		if (UtilValidate.isNotEmpty(shipBeforeDate)) {
        	        	Long shipBeforeDateL = Long.parseLong(shipBeforeDate);
        	        	cart.setShipBeforeDate(new Timestamp(shipBeforeDateL));
        	        }
                } catch (Exception e) {
                	errMsg = UtilProperties.getMessage(resource_error, "checkhelper.specify_if_shipBeforeDate_is_date", (cart != null ? cart.getLocale() : Locale.getDefault()));
                    errorMessages.add(errMsg);
        ...
        if (UtilValidate.isNotEmpty(shipAfterDate)) {
            if (UtilValidate.isDate(shipAfterDate)) {
                cart.setShipAfterDate(UtilDateTime.toTimestamp(shipAfterDate));
            } else if (UtilValidate.isNotEmpty(shipAfterDate)) {
            	try {
            		if (UtilValidate.isNotEmpty(shipAfterDate)) {
        	        	Long shipAfterDateL = Long.parseLong(shipAfterDate);
        	        	cart.setShipAfterDate(new Timestamp(shipAfterDateL));
        	        }
                } catch (Exception e) {
                	errMsg = UtilProperties.getMessage(resource_error, "checkhelper.specify_if_shipAfterDate_is_date", (cart != null ? cart.getLocale() : Locale.getDefault()));
                    errorMessages.add(errMsg);
    ...
    public Map<String, Object> setCheckOutOptions(String shippingMethod, String shippingContactMechId, Map<String, Map<String, Object>> selectedPaymentMethods,
            ...
            String orderAdditionalEmails, String maySplit, String giftMessage, String isGift, String internalCode, String shipBeforeDate, String shipAfterDate) {
    	return setCheckOutOptions(shippingMethod, shippingContactMechId, selectedPaymentMethods, 
    			singleUsePayments, billingAccountId, shippingInstructions, 
    			orderAdditionalEmails, maySplit, giftMessage, isGift, internalCode, shipBeforeDate, shipAfterDate, Boolean.TRUE, Boolean.TRUE);
    }

    public Map<String, Object> setCheckOutOptions(String shippingMethod, String shippingContactMechId, Map<String, Map<String, Object>> selectedPaymentMethods,
            List<String> singleUsePayments, String billingAccountId, String shippingInstructions,
            String orderAdditionalEmails, String maySplit, String giftMessage, String isGift, String internalCode, String shipBeforeDate, String shipAfterDate, Boolean runPromo, Boolean runCalcTax) {
        List<String> errorMessages = new ArrayList<String>();
        Map<String, Object> result = null;
        String errMsg = null;
    ...
    public Map<String, Object> createOrderChangeSuppPrice(GenericValue userLogin, String modifySupplierPrice) {
        return createOrderChangeSuppPrice(userLogin, null, null, null, false, null, cart.getWebSiteId(), modifySupplierPrice);
    }
    ...
    public Map<String, Object> createOrder(GenericValue userLogin, String distributorId, String affiliateId,
        ...
        Debug.log(module + ":: order/shoppingcart/CheckOutHelper.createOrder PQD TEST");
        ...
        if (party != null) {
          if (!"_NA_".equals(party.getString("partyId"))) {
    ...
    //overide by dat
    public Map<String, Object> createOrderChangeSuppPrice(GenericValue userLogin, String distributorId, String affiliateId,
            List<GenericValue> trackingCodeOrders, boolean areOrderItemsExploded, String visitId, String webSiteId, String modifySupplierPrice) {
        if (this.cart == null) {
            return null;
        }
        String orderId = this.cart.getOrderId();
        ...
        //get the TrackingCodeOrder List
        context.put("trackingCodeOrders", trackingCodeOrders);
        //modify last price supplier product
        context.put("modifySupplierPrice", modifySupplierPrice);
        if (distributorId != null) context.put("distributorId", distributorId);
        if (affiliateId != null) context.put("affiliateId", affiliateId);
        ...
        try {
            storeResult = dispatcher.runSync("storeOrderChangeSuppPrice", context);
            orderId = (String) storeResult.get("orderId");
            if (UtilValidate.isNotEmpty(orderId)) {
                this.cart.setOrderId(orderId);
                if (this.cart.getFirstAttemptOrderId() == null) {
    ...
    /* TODOCHANGE default method call to new method is added moment time */
    public void calcAndAddTax(GenericValue shipAddress, boolean skipEmptyAddresses) throws GeneralException {
    	calcAndAddTax(shipAddress, skipEmptyAddresses, null);
    }

    /* TODOCHANGE add new attribute moment time */
    public void calcAndAddTax(GenericValue shipAddress, boolean skipEmptyAddresses, Timestamp moment) throws GeneralException {
        ...
        for (int i = 0; i < shipGroups; i++) {
            ...
            Map<String, Object> serviceContext = this.makeDivTaxContext(i, shipAddress, shoppingCartItemIndexMap, cart.getFacilityId(), skipEmptyAddresses);
            if (skipEmptyAddresses && serviceContext == null) {
                csi.clearAllTaxInfo();
                continue;
            }
            serviceContext.put("momentTime", moment); //TODOCHANGE new moment time
            List<List<? extends Object>> taxReturn = this.getTaxAdjustments(dispatcher, "calcDivTax", serviceContext);
    ...
    private Map<String, Object> makeTaxContext(int shipGroup, GenericValue shipAddress, Map<Integer, ShoppingCartItem> shoppingCartItemIndexMap, String originFacilityId, boolean skipEmptyAddresses) {
        ...
        List<BigDecimal> alternativeQuantity = new ArrayList<BigDecimal>(totalItems);
        List<BigDecimal> alternativePrice = new ArrayList<BigDecimal>(totalItems);
        List<BigDecimal> alternativeAmount = new ArrayList<BigDecimal>(totalItems);
        List<String> quantityUomId = new ArrayList<String>(totalItems);
        ...
        for (int i = 0; i < totalItems; i++) {
            ...
            // TODOCHANGE quantityUomId
            if (cartItem.getAttribute("alternativeQuantity") != null && cartItem.getAttribute("alternativeUnitPrice") != null && cartItem.getAttribute("quantityUomId") != null) {
            	BigDecimal tmpAlternativePrice = cartItem.getAlternativeUnitPrice();
            	BigDecimal tmpAlternativeQuantity = cartItem.getAlternativeQuantity();
            	alternativeQuantity.add(tmpAlternativeQuantity);
            	alternativePrice.add(tmpAlternativePrice);
            	//alternativeAmount.add(cartItem.getItemSubTotal(tmpAlternativeQuantity));
            	alternativeAmount.add(tmpAlternativePrice.multiply(tmpAlternativeQuantity).multiply(cartItem.getRentalAdjustment()).add(cartItem.getOtherAdjustments()));
            	quantityUomId.add((String) cartItem.getAttribute("quantityUomId"));
            } else {
            	alternativeQuantity.add(new BigDecimal(-1));
            	alternativePrice.add(new BigDecimal(-1));
            	alternativeAmount.add(new BigDecimal(-1));
            	quantityUomId.add("");
            }
        }
        ...
        // TODOCHANGE quantityUomId
        serviceContext.put("itemAlternativeQuantityList", alternativeQuantity);
        serviceContext.put("itemAlternativePriceList", alternativePrice);
        serviceContext.put("itemAlternativeAmountList", alternativeAmount);
        serviceContext.put("itemQuantityUomIdList", quantityUomId);
        
        return serviceContext;
    }
    // create by VietTB
    protected Map<String, Object> makeDivTaxContext(int shipGroup, GenericValue shipAddress, Map<Integer, ShoppingCartItem> shoppingCartItemIndexMap, String originFacilityId, boolean skipEmptyAddresses) {
        ShoppingCart.CartShipInfo csi = cart.getShipInfo(shipGroup);
        int totalItems = csi.shipItemInfo.size();
        ...
        List<BigDecimal> alternativeAmount = new ArrayList<BigDecimal>(totalItems);
        List<String> quantityUomId = new ArrayList<String>(totalItems);
        List<List<BigDecimal>> itemDivTaxList =  new ArrayList <List<BigDecimal>>(totalItems);
        ...
        for (int i = 0; i < totalItems; i++) {
            ...
            if (cartItem.getAttribute("alternativeQuantity") != null && cartItem.getAttribute("alternativeUnitPrice") != null && cartItem.getAttribute("quantityUomId") != null) {
            	...
            	alternativePrice.add(tmpAlternativePrice);
            	//alternativeAmount.add(cartItem.getItemSubTotal(tmpAlternativeQuantity));            	
            	List<BigDecimal> orderAdjustmentValueList = new ArrayList<BigDecimal>();
            	List<GenericValue> adjustments = cartItem.getAdjustments();
            	BigDecimal orderAdjustmentValueCheck = BigDecimal.ZERO;
                if (UtilValidate.isNotEmpty(adjustments)) { 
                    List<GenericValue> filteredAdjs = OrderReadHelper.filterOrderAdjustments(adjustments, true, false, false, false, false);
                    for (GenericValue orderAdjustment : filteredAdjs) {
                    	BigDecimal calcItemAdjustment = OrderReadHelper.calcItemAdjustment(orderAdjustment, tmpAlternativeQuantity, tmpAlternativePrice);
                    	//Edit by VietTB
                    	if ( !"DISCOUNT_ADJUSTMENT_NOTAX".equals(orderAdjustment.getString("orderAdjustmentTypeId")))
                    	orderAdjustmentValueList.add(calcItemAdjustment);
                    	orderAdjustmentValueCheck = orderAdjustmentValueCheck.add(calcItemAdjustment);
                    }
                }
                BigDecimal alternativeAmountCheck = tmpAlternativePrice.multiply(tmpAlternativeQuantity).multiply(cartItem.getRentalAdjustment());
                orderAdjustmentValueCheck = orderAdjustmentValueCheck.add(alternativeAmountCheck);                
                if (orderAdjustmentValueCheck.compareTo(BigDecimal.ZERO) == 0)
                		{
                			alternativeAmount.add(BigDecimal.ZERO);
                		}
                else alternativeAmount.add(tmpAlternativePrice.multiply(tmpAlternativeQuantity).multiply(cartItem.getRentalAdjustment()));
                itemDivTaxList.add(i, orderAdjustmentValueList);
            	quantityUomId.add((String) cartItem.getAttribute("quantityUomId"));
            } else {
            	alternativeQuantity.add(new BigDecimal(-1));
            	alternativePrice.add(new BigDecimal(-1));
        ...
        serviceContext.put("itemAlternativeAmountList", alternativeAmount);
        serviceContext.put("itemQuantityUomIdList", quantityUomId);
        serviceContext.put("itemDivTaxList", itemDivTaxList);
        serviceContext.put("cartSubTotal", cart.getSubTotal()); // TODOCHANGE tax add attribute cartSubTotal
        
        return serviceContext;
    }    
    // Calc the tax adjustments.
    protected List<List<? extends Object>> getTaxAdjustments(LocalDispatcher dispatcher, String taxService, Map<String, Object> serviceContext) throws GeneralException {
