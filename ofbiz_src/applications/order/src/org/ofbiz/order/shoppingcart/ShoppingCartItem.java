...
public class ShoppingCartItem implements java.io.Serializable {
    ...
    // TODOCHANGE add new attribute: "quantityUomId", "alternativeQuantity"
    public static String[] attributeNames = { "shoppingListId", "shoppingListItemSeqId", "surveyResponses",
                                              ...
                                              "quantityUomId", "weightUomId", "alternativeQuantity", "alternativeUnitPrice", "expireDate", "idEAN",
                                              "atpQuantity", "alternativeAtpQuantity", "price"};

    public static final MathContext generalRounding = new MathContext(10);

    protected transient Delegator delegator = null;
    /** the actual or variant product */
    protected transient GenericValue _product = null;
    /** the virtual product if _product is a variant */
    protected transient GenericValue _parentProduct = null;

    protected String delegatorName = null;
    protected String prodCatalogId = null;
    protected String productId = null;
    protected String supplierProductId = null;
    ...
    protected String productCategoryId = null;
    ...
    protected BigDecimal displayPrice = null;
    ...
    protected Map<String, String> contactMechIdsMap = FastMap.newInstance();
    ...
    protected Map<GenericPK, BigDecimal> quantityUsedPerPromoCandidate = new HashMap<GenericPK, BigDecimal>();
    protected Map<GenericPK, BigDecimal> quantityUsedPerPromoFailed = new HashMap<GenericPK, BigDecimal>();
    protected Map<GenericPK, BigDecimal> quantityUsedPerPromoActual = new HashMap<GenericPK, BigDecimal>();
    protected Map<String, GenericValue> additionalProductFeatureAndAppls = new HashMap<String, GenericValue>();
    ...
    public static ShoppingCartItem makePurchaseOrderItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity,
        ...
        if (UtilValidate.isNotEmpty(selectedAmount) && selectedAmount.compareTo(BigDecimal.ZERO) > 0){
        	String quantityUomId = product.getString("quantityUomId");
        	newItem.setAttribute("quantityUomId", quantityUomId);
        	BigDecimal convert = BigDecimal.ONE;
        	if (attributes.containsKey("weightUomId")){
        		newItem.setAttribute("weightUomId", attributes.get("weightUomId"));
        		if (UtilValidate.isNotEmpty(attributes.get("weightUomId")) && UtilValidate.isNotEmpty(product.getString("weightUomId"))) {
        			convert = ProductUtil.getConvertWeightNumber(delegator, productId, (String)attributes.get("weightUomId"), product.getString("weightUomId"));
				}
        	}
        	if (UtilValidate.isNotEmpty(attributes.get("price"))) {
        		BigDecimal price = (BigDecimal)attributes.get("price");
        		newItem.setAttribute("alternativeUnitPrice", price);
        		newItem.setBasePrice(price.divide(convert, decimals, rounding));
			} else {
				newItem.setAlternativeUnitPrice(supplierProduct.getBigDecimal("lastPrice"));
			}
        } else if (attributes != null && attributes.containsKey("quantityUomId") && UtilValidate.isNotEmpty(attributes.get("quantityUomId"))){
        	newItem.setAttribute("quantityUomId", attributes.get("quantityUomId"));
        	newItem.setAttribute("alternativeQuantity", quantity);
        	BigDecimal convert = ProductUtil.getConvertPackingNumber(delegator, productId, (String)attributes.get("quantityUomId"), product.getString("quantityUomId"));
        	if (UtilValidate.isNotEmpty(attributes.get("price"))) {
        		BigDecimal price = (BigDecimal)attributes.get("price");
        		newItem.setAttribute("alternativeUnitPrice", price);
        		newItem.setBasePrice(price.divide(convert, decimals, rounding));
			} else {
				newItem.setAttribute("alternativeUnitPrice", supplierProduct.getBigDecimal("lastPrice"));
			}
        } else {
        	if (UtilValidate.isNotEmpty(attributes.get("price"))) {
        		BigDecimal price = (BigDecimal)attributes.get("price");
...
//        if ("AGGREGATED".equals(product.getString("productTypeId")) || "AGGREGATED_SERVICE".equals(product.getString("productTypeId")) || "AGGR_DIGSERV".equals(product.getString("productTypeId"))) {
...
//        }
        ...
        try {
            newItem.setQuantity(quantity, dispatcher, cart, triggerExternalOpsBool, true, false);
//            cart.setItemShipGroupQty(newItem, quantity, 0);
        ...
        if (attributes != null && attributes.containsKey("itemComment") && UtilValidate.isNotEmpty(attributes.get("itemComment"))){
        	newItem.setItemComment((String)attributes.get("itemComment"));
        }
...
//			}
            ...
            newItem.setName(product.getString("productName"));
        }
        return newItem;

    }
    
    public static ShoppingCartItem makePurchaseOrderItem(Integer cartLocation, String productId, BigDecimal selectedAmount, 
    		BigDecimal quantity, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, 
            ...
            Timestamp cancelBackOrderDate) throws CartItemModifyException, ItemNotFoundException{
    	
    	return makePurchaseOrderItem(cartLocation, productId, selectedAmount, quantity, additionalProductFeatureAndAppls, 
    			attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, cart, supplierProduct, 
    			shipBeforeDate, shipAfterDate, cancelBackOrderDate, Boolean.TRUE);
    ...
    */
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
        ...
        if (attributes == null || UtilValidate.isEmpty(attributes.get("itemDesiredDeliveryDate"))) {
        	Timestamp ddDate = null;
            try {
    	        if (UtilValidate.isNotEmpty(cart.getDefaultItemDeliveryDate())) {
    	        	ddDate = Timestamp.valueOf(cart.getDefaultItemDeliveryDate());
    	        }
            } catch (Exception e) {
            	Debug.logError("Error when format date time order item desiredDeliveryDate", module);
            }
            if (ddDate != null) newItem.setDesiredDeliveryDate(ddDate);
        }
        ...
        // TODOCHANGE set quantityUomId to ShoppingCartItem
        String quantityUomId = product.getString("quantityUomId");
        if (UtilValidate.isNotEmpty(quantityUomId) && UtilValidate.isEmpty(newItem.getAttribute("quantityUomId"))) {
        	newItem.setAttribute("quantityUomId", quantityUomId);
        	newItem.setAttribute("alternativeQuantity", quantity);
        }
    ...
    protected void setQuantity(BigDecimal quantity, LocalDispatcher dispatcher, ShoppingCart cart, boolean triggerExternalOps, boolean resetShipGroup, boolean updateProductPrice, boolean skipInventoryChecks) throws CartItemModifyException {
        ...
        // not check error for pay promotion item.
        if (this.isPromo && !"PRODPROMO_ORDER_ITEM".equals(this.itemType)) {
        ...
        this.setAttribute("alternativeQuantity", quantity);
        String quantityUomId = (String) this.getAttribute("quantityUomId");
        String defaultQuantityUomId = this.getProduct() != null ? this.getProduct().getString("quantityUomId") : null;
        if (UtilValidate.isEmpty(quantityUomId)) {
    		quantityUomId = defaultQuantityUomId;
    		this.setAttribute("quantityUomId", quantityUomId);
        }
        // Changed by thangnv to check if product has amount 
        if (UtilValidate.isNotEmpty(this.getSelectedAmount()) && this.getSelectedAmount().compareTo(BigDecimal.ZERO) > 0 && "PURCHASE_ORDER".equals(cart.getOrderType())) {
        	quantity = BigDecimal.ONE;
        	this.setAttribute("alternativeQuantity", quantity);
        } else if (UtilValidate.isNotEmpty(quantityUomId) && UtilValidate.isNotEmpty(defaultQuantityUomId)) {
        	// Check by quantity uom
        	BigDecimal quantityTmp = UomWorker.customConvertUom(productId, quantityUomId, defaultQuantityUomId, quantity, delegator);
        	if (quantityTmp != null) {
    ...
    public void updatePrice(LocalDispatcher dispatcher, ShoppingCart cart) throws CartItemModifyException {
        ...
        if (_product != null && isModifiedPrice == false) {
            try {
                ...
                priceContext.put("quantity", this.getAlternativeQuantity());
                priceContext.put("amount", this.getSelectedAmount());
                
                if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                    priceContext.put("currencyUomId", cart.getCurrency());
                    String quantityUomIdDefault = this.getProduct().getString("quantityUomId");
                    String quantityUomId = (String) this.getAttribute("quantityUomId");
                    
                    // TODOCHANGE calculate purchase price by alternative quantity
                    /*Map<String, Object> priceResult = null;
                	if (UtilValidate.isNotEmpty(quantityUomIdDefault)) {
                		Map<String, Object> priceContextCustom = FastMap.newInstance();
                		priceContextCustom.putAll(priceContext);
                		priceContextCustom.put("quantityUomId", quantityUomIdDefault);
                		priceResult = dispatcher.runSync("calculatePurchasePrice", priceContextCustom);
                	} else {
                		priceResult = dispatcher.runSync("calculatePurchasePrice", priceContext);
                	}
                	Map<String, Object> priceResultAlter = null;
                	if (quantityUomIdDefault != null && !quantityUomIdDefault.equals(quantityUomId)) {
                		Map<String, Object> priceContextAlterCustom = FastMap.newInstance();
                		priceContextAlterCustom.putAll(priceContext);
                		priceContextAlterCustom.put("quantityUomId", quantityUomId);
                		priceResultAlter = dispatcher.runSync("calculatePurchasePrice", priceContextAlterCustom);
                	} else {
                		priceResultAlter = priceResult;
                	}*/
                    Map<String, Object> priceContextAlterCustom = FastMap.newInstance();
            		priceContextAlterCustom.putAll(priceContext);
            		priceContextAlterCustom.put("quantityUomId", quantityUomId);
            		Map<String, Object> priceResultAlter = dispatcher.runSync("calculatePurchasePrice", priceContextAlterCustom);
            		
            		// process alternative price
                    if (ServiceUtil.isError(priceResultAlter)) {
                        throw new CartItemModifyException("There was an error while calculating the price: " + ServiceUtil.getErrorMessage(priceResultAlter));
                    }
                    Boolean validPriceFoundAlter = (Boolean) priceResultAlter.get("validPriceFound");
                    if (!validPriceFoundAlter.booleanValue()) {
                    ...
                    BigDecimal alternativeUnitPrice = (BigDecimal) priceResultAlter.get("price");
                    this.setAlternativeUnitPrice(alternativeUnitPrice);
                    
                    // process price
                    BigDecimal quantityConvert = null;
                    if (quantityUomIdDefault != null && !quantityUomIdDefault.equals(quantityUomId)) {
                    	quantityConvert = UomWorker.customConvertUom(this.getProductId(), quantityUomId, quantityUomIdDefault, BigDecimal.ONE, delegator);
                    }
                    if (quantityConvert == null) quantityConvert = BigDecimal.ONE;
                    
                    BigDecimal price = null;
                    if (alternativeUnitPrice != null) price = alternativeUnitPrice.divide(quantityConvert, decimals, RoundingMode.HALF_UP);
                    if (isAlternativePacking) {
                        this.setBasePrice((price).divide(pieces, RoundingMode.HALF_UP));
                    } else {
                        this.setBasePrice((price));
                    }
                    
                    this.setDisplayPrice(this.basePrice);
                    this.orderItemPriceInfos = UtilGenerics.checkList(priceResultAlter.get("orderItemPriceInfos"));
                    // end change
                    ...
                    */
                    ...
                    if (UtilValidate.isNotEmpty(quantityUomId)) {
                    	...
                    	if (quantityUomIdDefault != null && !quantityUomIdDefault.equals(quantityUomId)) {
                    		...
                    		priceResultAlternative = priceResult;
                    	}
                    } else {
                    	// Old code
                    	priceResult = dispatcher.runSync("calculateProductPrice", priceContext);
                    ...
                    if (Boolean.FALSE.equals(validPriceFound)) {
                    	...
                    	if (this.getProduct() != null) productCode = this.getProduct().getString("productCode");
                    	if (productCode == null) productCode = this.getProductId();
                    	String errorMessage = UtilProperties.getMessage(resource_error, "OrderCouldNotFindValidPriceForTheProductWithIdNotAddingToCart", UtilMisc.toMap("productId", productCode), locale);
                    ...
                    if(isAlternativePacking){
                        ...
                        if (priceResultAlternative != null && priceResultAlternative.get("basePrice") != null) {
                        	this.setAttribute("alternativeUnitPrice", priceResultAlternative.get("basePrice"));
                        }
                    ...
                    if (configWrapper != null) {
                        ...
                        this.setAttribute("alternativeUnitPrice", configWrapper.getTotalPrice());
    ...
    /** TODOCHANGE new attribute alternativeQuantity Returns the quantity. */
    public BigDecimal getAlternativeQuantity() {
    	return (BigDecimal) this.getAttribute("alternativeQuantity");
    }
    ...
    public String getName() {
    	if (itemDescription != null) {
    		...
    		// TODOCHANGE add info selectedAmount
	  		if (selectedAmount != null && !selectedAmount.equals(BigDecimal.ZERO)) {
	  			itemName += " [A: " + selectedAmount.doubleValue() + "]";
	  		}
	  		return itemName;
        }
    }
   ...
   public List<GenericValue> getFeaturesForSupplier(LocalDispatcher dispatcher,String partyId) {
    ...
    /** TODOCHANGE Sets the alternative unit price for the item; use with caution */
    public void setAlternativeUnitPrice(BigDecimal alternativeUnitPrice) {
        this.setAttribute("alternativeUnitPrice", alternativeUnitPrice);
    }
    
    /** TODOCHANGE alternativeUnitPrice Returns the alternative base price. */
    public BigDecimal getAlternativeUnitPrice() {
        BigDecimal curBasePrice = null;
        BigDecimal alternativeUnitPrice = (BigDecimal) this.getAttribute("alternativeUnitPrice");
        if (alternativeUnitPrice != null) {
            if (selectedAmount.compareTo(BigDecimal.ZERO) > 0) {
                curBasePrice = alternativeUnitPrice.multiply(selectedAmount);
            } else {
                curBasePrice = alternativeUnitPrice;
            }
        }
        return curBasePrice;
    }
    
    public BigDecimal getAlterPriceWithTax(ShoppingCart cart, LocalDispatcher dispatcher) {
    	BigDecimal price = null;
    	if (UtilValidate.isEmpty(this.getAttribute("alternativePrice"))) {
    		price = this.getBasePrice();
    	} else {
    		price = (BigDecimal) this.getAttribute("alternativePrice");
    	}
    	BigDecimal priceRetult = price;
		try {
			Map<String, Object> totalPriceWithTaxMap = dispatcher.runSync("calcTaxForDisplay", UtilMisc.toMap("basePrice", price, "productId", this.productId, "productStoreId", cart.getProductStoreId()));
			if (ServiceUtil.isSuccess(totalPriceWithTaxMap)) {
				priceRetult = (BigDecimal) totalPriceWithTaxMap.get("priceWithTax");
			}
		} catch (GenericServiceException e) {
			Debug.logWarning("Error when calculate tax for price", module);
		}
        return priceRetult;
    }
    ...
    /** Edit by VietTB */
    public BigDecimal getNewOtherAdjustments() {
        return OrderReadHelper.newCalcItemAdjustments(quantity, getBasePrice(), this.getAdjustments(), true, false, false, false, false);
    }
...
//        Debug.logInfo("Price" + getBasePrice() + " quantity" +  quantity + " Rental adj:" + getRentalAdjustment() + " other adj:" + getOtherAdjustments(), module);
    	// TODOCHANGE quantityUomId
    	BigDecimal alternativeQuantity = this.getAlternativeQuantity();
    	BigDecimal alternativeUnitPrice = this.getAlternativeUnitPrice();
    	if (alternativeQuantity != null && alternativeUnitPrice != null) {
    		
    		// by PQD
    		//Debug.logInfo("getItemSubTotal, alternativeUnitPrice = " + alternativeUnitPrice
    		//		+ ", alternativeQuantity = " + alternativeQuantity + ", rentalAdjustment = " + getRentalAdjustment()
    		//		+ ", otherAdjustment = " + getOtherAdjustments()
    		//		,module);
    		return alternativeUnitPrice.multiply(alternativeQuantity).multiply(getRentalAdjustment()).add(getOtherAdjustments());
    	} else {
    		// Old code: 
    		// by PQD
    		//Debug.logInfo("getItemSubTotal, basePrice = " + getBasePrice()
    		//		+ ", quantity = " + quantity + ", rentalAdjustment = " + getRentalAdjustment()
    		//		+ ", otherAdjustment = " + getOtherAdjustments()
    		//		, module);
    ...
    /** Edit by VietTB */
    public BigDecimal getNewItemSubTotal(BigDecimal quantity) {
//        Debug.logInfo("Price" + getBasePrice() + " quantity" +  quantity + " Rental adj:" + getRentalAdjustment() + " other adj:" + getOtherAdjustments(), module);
    	// TODOCHANGE quantityUomId
    	BigDecimal alternativeQuantity = this.getAlternativeQuantity();
    	BigDecimal alternativeUnitPrice = this.getAlternativeUnitPrice();
    	if (alternativeQuantity != null && alternativeUnitPrice != null) {
    		return alternativeUnitPrice.multiply(alternativeQuantity).multiply(getRentalAdjustment()).add(getNewOtherAdjustments());
    	} else {
    		// Old code: 
    		return getBasePrice().multiply(quantity).multiply(getRentalAdjustment()).add(getOtherAdjustments());
    	}
     ...
     * @param cart
     * @return item sub total of cart item
     */
    public BigDecimal getItemSubTotalAndTax(ShoppingCart cart) {
    	BigDecimal itemSubTotal = this.getItemSubTotal();
    	if (cart != null) {
    		List<ShoppingCart.CartShipInfo> csis = cart.getShipGroups();
    		if (UtilValidate.isNotEmpty(csis)) {
    			for (ShoppingCart.CartShipInfo csi : csis) {
        			ShoppingCart.CartShipInfo.CartShipItemInfo itemInfo = csi.shipItemInfo.get(this);
        			if (UtilValidate.isNotEmpty(itemInfo)){
    ...
    //Edit by VietTB
    public BigDecimal getNewItemSubTotal() {
        return this.getNewItemSubTotal(this.getQuantity());
    }
    ...
    //pos add new
    public BigDecimal getItemSubTotalNotAdj(BigDecimal quantity) {
      	if (UtilValidate.isEmpty(this.getAttribute("quantityUomId")))
            return getBasePrice().multiply(quantity);
      	else{
      		BigDecimal alternativeUnitPrice = this.getAlternativeUnitPrice();
      		return alternativeUnitPrice.multiply(quantity);
      		}
     }
    //pos add new
    public BigDecimal getItemSubTotalNotAdj() {
      	if (UtilValidate.isEmpty(this.getAttribute("quantityUomId")))
              return this.getItemSubTotalNotAdj(this.getQuantity());
          else{
        	  BigDecimal alternativeProductQuantity = this.getAlternativeQuantity();
        	  return this.getItemSubTotalNotAdj(alternativeProductQuantity); 
          }
     }
    //pos add new
    public List<GenericValue> getListQuantityUom(){
    	...
    	if(productId != null && !productId.equalsIgnoreCase("")){
    		...
    		try {
    		List<GenericValue>	listConvertUom = delegator.findList("ConfigPacking", condition, null, null, null, false);
				for (GenericValue convertUom : listConvertUom) {
					String uomFromId = convertUom.getString("uomFromId");
					GenericValue quantityUom = delegator.findOne("Uom", UtilMisc.toMap("uomId", uomFromId), false);
					listReturn.add(quantityUom);
				}
			GenericValue product = delegator.findOne("Product", UtilMisc.toMap("productId", productId), false);
			String basicUomId = product.getString("quantityUomId");
			if(basicUomId != null && !basicUomId.equalsIgnoreCase("")){
				GenericValue basicUom = delegator.findOne("Uom", UtilMisc.toMap("uomId", basicUomId), false);
				if(UtilValidate.isNotEmpty(basicUom)){
					listReturn.add(basicUom);
				}
			}
			String uomSelected = (String) this.getAttribute("quantityUomId");
			if(uomSelected != null && !uomSelected.equalsIgnoreCase("")){
				GenericValue quantityUomSelected = delegator.findOne("Uom", UtilMisc.toMap("uomId", uomSelected), false);
				if(UtilValidate.isNotEmpty(quantityUomSelected)){
					for (GenericValue uom : listReturn) {
						
						if(uom.equals(quantityUomSelected)){
							listReturn.remove(uom);
						}
					}
				}
			}
    ...
    public BigDecimal getAdjustment(){
      	List<GenericValue> listAdjustments = this.getAdjustments();
      	
      	BigDecimal amountDiscount = BigDecimal.ZERO;
      	if(listAdjustments.size() > 0){
      		GenericValue cartItemAdj = listAdjustments.get(0);
      		String amount = cartItemAdj.getString("sourcePercentage");
      		if(UtilValidate.isNotEmpty(amount)){
      			
      			amountDiscount = cartItemAdj.getBigDecimal("sourcePercentage");
      			amountDiscount = amountDiscount.negate();
      			BigDecimal total = new BigDecimal("100");
      			BigDecimal itemTotalNoAdj = this.getDisplayItemSubTotalNoAdj();
      			amountDiscount = amountDiscount.divide(total, 4, BigDecimal.ROUND_HALF_UP);
      			amountDiscount = amountDiscount.multiply(itemTotalNoAdj);
      			
      			
      		}else{
      			amountDiscount = cartItemAdj.getBigDecimal("amount");
      			amountDiscount = amountDiscount.negate();
      		}
    ...
    //pos add new
    public BigDecimal getATPInfo(String uomFromId, String uomToId,  LocalDispatcher dispatcher, ShoppingCart cart) throws CartItemModifyException{
    	GenericValue product = this.getProduct();
    	String productId = product.getString("productId");
    	String basicUomId = product.getString("quantityUomId");
    	String facilityId = cart.getFacilityId();
    	BigDecimal quantityATP = getInventoryItemSummary(delegator, productId, facilityId);
		BigDecimal quantityConverted = quantityATP;
		setAttribute("atpQuantity", quantityATP);
		setAttribute("alternativeAtpQuantity", quantityConverted);
    	if(UtilValidate.isNotEmpty(productId) && UtilValidate.isNotEmpty(facilityId)){
    		if(uomToId == null){
        		uomToId = basicUomId;
        	}
        	if(uomFromId.equals(basicUomId)){
        		if(!uomToId.equals(basicUomId)){
        			quantityConverted = UomWorker.customConvertUom(productId, uomToId, basicUomId, quantityATP, "revert", delegator);
        			setAttribute("atpQuantity", quantityATP);
        			setAttribute("alternativeAtpQuantity", quantityConverted);
        		}
        	}else{
        		if(uomToId.equals(basicUomId)){
					if(!uomFromId.equals(basicUomId)){
						quantityConverted = UomWorker.customConvertUom(productId, uomFromId, basicUomId, quantityATP, "revert", delegator);
					}
					setAttribute("atpQuantity", quantityConverted);
        			setAttribute("alternativeAtpQuantity", quantityConverted);
        		}else{
        			try {
        				GenericValue convertFrom = null;
        				List<GenericValue> listConfigs = delegator.findList("ConfigPacking", EntityCondition.makeCondition(UtilMisc.toMap("productId", productId, "uomFromId", uomFromId, "uomToId", basicUomId)), null, null, null, false);
        				listConfigs = EntityUtil.filterByDate(listConfigs);
        				if (!listConfigs.isEmpty()){
        					convertFrom = listConfigs.get(0);
        				} else {
        					return new BigDecimal(-1);
        				}
        				
        				GenericValue convertTo = null;
        				List<GenericValue> listConfigTos = delegator.findList("ConfigPacking", EntityCondition.makeCondition(UtilMisc.toMap("productId", productId, "uomFromId", uomToId, "uomToId", basicUomId)), null, null, null, false);
        				listConfigTos = EntityUtil.filterByDate(listConfigTos);
        				if (!listConfigTos.isEmpty()){
        					convertTo = listConfigTos.get(0);
        				} else {
        					return new BigDecimal(-1);
        				}
        				
						BigDecimal quanConvertFrom = convertFrom.getBigDecimal("quantityConvert");
						BigDecimal quanConvertTo= convertTo.getBigDecimal("quantityConvert");
						if(quanConvertFrom != null && quanConvertTo != null && quanConvertTo.compareTo(BigDecimal.ZERO) != 0){
							BigDecimal quantityConvert = quanConvertFrom.divide(quanConvertTo, 3, RoundingMode.HALF_UP);
							BigDecimal newQuantity = UomWorker.customConvertUom(productId, uomFromId, basicUomId, quantityATP, "convert", delegator);
							quantityConverted = quantityConvert.multiply(quantityConverted);
							setAttribute("atpQuantity", newQuantity);
		        			setAttribute("alternativeAtpQuantity", quantityConverted);
						}
					} catch (GenericEntityException e) {
						// TODO Auto-generated catch block
    	...
    	BigDecimal atpQuantity = (BigDecimal) getAttribute("atpQuantity");
    	return atpQuantity;
    	
    }
    //pos add new
    public BigDecimal getInventoryItemSummary(Delegator delegator, String productId, String facilityId){
    	BigDecimal atpQuantity = BigDecimal.ZERO;
    	Map<String, String> inventoryItemSummaryConds = FastMap.newInstance();
    	inventoryItemSummaryConds.put("productId", productId);
    	inventoryItemSummaryConds.put("facilityId", facilityId);
    	EntityCondition inventoryItemSummaryCond = EntityCondition.makeCondition(inventoryItemSummaryConds, EntityOperator.AND);
    	List<GenericValue> inventoryItemList = FastList.newInstance();
    	try {
			inventoryItemList = delegator.findList("InventoryItemProductTotalFacility", inventoryItemSummaryCond, null, null, null, false);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	for (GenericValue inventoryItem : inventoryItemList) {
			BigDecimal quantityTmp = inventoryItem.getBigDecimal("availableToPromiseTotal");
			if(UtilValidate.isNotEmpty(quantityTmp)){
				atpQuantity = atpQuantity.add(quantityTmp);
			}
		}
    	return atpQuantity;
    }
    //pos add new
    public void updateUom(String uomFromId, String uomToId,  LocalDispatcher dispatcher, ShoppingCart cart) throws CartItemModifyException{
      	GenericValue product = this.getProduct();
      	String productId = product.getString("productId");
      	String basicUomId = product.getString("quantityUomId");
      	BigDecimal quantity = this.getQuantity();
      	BigDecimal quantityATP = (BigDecimal) this.getAttribute("atpQuantity");
      	String quantityUomId = (String) this.getAttribute("quantityUomId");
      	if(UtilValidate.isNotEmpty(quantityUomId)){
      		quantity = this.getAlternativeQuantity();
      		quantityATP = (BigDecimal) this.getAttribute("alternativeAtpQuantity");
      	}
      	
      	BigDecimal quantityConverted = quantity;
      	BigDecimal quantityATPConverted = quantityATP;
      	if(UtilValidate.isNotEmpty(productId)){
          	...
          	if(uomFromId.equals(basicUomId)){
          		if(!uomToId.equals(basicUomId)){
          			quantityConverted = UomWorker.customConvertUom(productId, uomToId, basicUomId, quantity, "revert", delegator);
          			quantityATPConverted = UomWorker.customConvertUom(productId, uomToId, basicUomId, quantityATP, "revert", delegator);
          			this.setAttribute("alternativeQuantity", quantityConverted);
                  	this.quantity = quantity;
                  	this.setAttribute("atpQuantity", quantityATP);
                  	this.setAttribute("alternativeAtpQuantity", quantityATPConverted);
          		}
          	}else{
          		if(uomToId.equals(basicUomId)){
          			quantityConverted = UomWorker.customConvertUom(productId, uomFromId, uomToId, quantity, "convert", delegator);
          			quantityATPConverted = UomWorker.customConvertUom(productId, uomFromId, uomToId, quantityATP, "convert", delegator);
          			this.setAttribute("alternativeQuantity", quantityConverted);
                  	this.quantity = quantityConverted;
                  	this.setAttribute("atpQuantity", quantityATPConverted);
                  	this.setAttribute("alternativeAtpQuantity", quantityATPConverted);
          			
          		}else{
          			try {
          				GenericValue convertFrom = null;
        				List<GenericValue> listConfigFroms = delegator.findList("ConfigPacking", EntityCondition.makeCondition(UtilMisc.toMap("productId", productId, "uomFromId", uomFromId, "uomToId", basicUomId)), null, null, null, false);
        				listConfigFroms = EntityUtil.filterByDate(listConfigFroms);
        				if (!listConfigFroms.isEmpty()){
        					convertFrom = listConfigFroms.get(0);
        				} 
        				GenericValue convertTo = null;
        				List<GenericValue> listConfigTos = delegator.findList("ConfigPacking", EntityCondition.makeCondition(UtilMisc.toMap("productId", productId, "uomFromId", uomToId, "uomToId", basicUomId)), null, null, null, false);
        				listConfigTos = EntityUtil.filterByDate(listConfigTos);
        				if (!listConfigTos.isEmpty()){
  						...
  						if(UtilValidate.isNotEmpty(convertFrom) && UtilValidate.isNotEmpty(convertTo)){
  							BigDecimal quanConvertFrom = convertFrom.getBigDecimal("quantityConvert");
  							BigDecimal quanConvertTo= convertTo.getBigDecimal("quantityConvert");
  							if(quanConvertFrom != null && quanConvertTo != null && quanConvertTo.compareTo(BigDecimal.ZERO) != 0){
  								BigDecimal quantityConvert = quanConvertFrom.divide(quanConvertTo, 3, RoundingMode.HALF_UP);
  								BigDecimal newQuantity = UomWorker.customConvertUom(productId, uomFromId, basicUomId, quantity, "convert", delegator);
  								BigDecimal newQuantityATP = UomWorker.customConvertUom(productId, uomFromId, basicUomId, quantityATP, "convert", delegator);
  								quantityConverted = quantityConvert.multiply(quantity);
  								quantityATPConverted = quantityConvert.multiply(quantityATP);
  								this.quantity = newQuantity;
  								this.setAttribute("alternativeQuantity", quantityConverted);
  								this.setAttribute("atpQuantity", newQuantityATP);
  								this.setAttribute("alternativeAtpQuantity", quantityATPConverted);
  							}
      	...
      	this.setAttribute("quantityUomId", uomToId);
      	this.updatePrice(dispatcher, cart);
    }
    ...
    public boolean equals(String productId, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId,
        ...
        // TODOCHANGE
        if ((this.attributes == null && UtilValidate.isNotEmpty(attributes)) || (UtilValidate.isNotEmpty(this.attributes) && attributes == null) ||
                (this.attributes != null && attributes != null && (!(this.attributes.get("quantityUomId").equals(attributes.get("quantityUomId")))))) {
            return false;
        }
