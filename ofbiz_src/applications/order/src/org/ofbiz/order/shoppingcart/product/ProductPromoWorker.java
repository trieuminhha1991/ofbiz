...
public class ProductPromoWorker {
    ...
    public static List<GenericValue> getProductStorePromotions(ShoppingCart cart, Timestamp nowTimestamp, LocalDispatcher dispatcher) {
        ...
        try {
        	// TODOCHANGE change get promoProgram list apply from use ProductStorePromoAppl entity to ProductStorePromoApplFilter entity
            // loop through promotions and get a list of all of the rules...
        	List<GenericValue> productStorePromoApplsList = null;
        	Object resourceHandler = null;
        	try {
        		ModelReader reader = delegator.getModelReader();
        		resourceHandler = reader.getEntityResourceHandler("ProductStorePromoApplFilter");
        	} catch (Exception e) {
        		Debug.log("ProductStorePromoApplFilter view entity is not exists, run on ProductStorePromoAppl entity");
        	}
        	if (resourceHandler == null) {
        		// Old code
        		productStorePromoApplsList = productStore.getRelated("ProductStorePromoAppl", null, UtilMisc.toList("sequenceNum"), true);
        		productStorePromoApplsList = EntityUtil.filterByDate(productStorePromoApplsList, nowTimestamp);
        	} else {
        		// New code
        		productStorePromoApplsList = delegator.findList("ProductStorePromoApplFilter", 
        				EntityCondition.makeCondition(EntityCondition.makeCondition("productStoreId", productStore.get("productStoreId")), 
        						EntityOperator.AND, EntityUtil.getFilterByDateExpr()), null, UtilMisc.toList("sequenceNum"), null, false);
        	}
            ...
            while (prodCatalogPromoAppls != null && prodCatalogPromoAppls.hasNext()) {
                ...
                // TODOCHANGE add code: check customer's role type in role types of promotion? (Only check for promotion has promotion type != null)
                List<String> roleTypePromoApplies = EntityUtil.getFieldListFromEntityList(
                		delegator.findList("ProductPromoRoleTypeAppl", EntityCondition.makeCondition(
                				EntityCondition.makeCondition("productPromoId", prodCatalogPromoAppl.get("productPromoId")), EntityOperator.AND, 
                				EntityUtil.getFilterByDateExpr()), UtilMisc.toSet("roleTypeId"), null, null, false), "roleTypeId", true);
                if (UtilValidate.isNotEmpty(roleTypePromoApplies)) {
                	List<GenericValue> partyRoles = delegator.findList("PartyRole", 
                			EntityCondition.makeCondition(EntityCondition.makeCondition("partyId", cart.getOrderPartyId()), EntityOperator.AND, 
                					EntityCondition.makeCondition("roleTypeId", EntityOperator.IN, roleTypePromoApplies)), null, null, null, false);
    ...
    public static List<GenericValue> getSupplierPromotions(ShoppingCart cart, Timestamp nowTimestamp, LocalDispatcher dispatcher) {
    	List<GenericValue> productPromoList = FastList.newInstance();
    	
    	Delegator delegator = cart.getDelegator();
    	
    	String supplierId = cart.getOrderPartyId();
    	GenericValue supplier = null;
    	try {
    		supplier = delegator.findOne("PartyGroup", UtilMisc.toMap("partyId", supplierId), true);
    	} catch (GenericEntityException e) {
    		Debug.logError(e, "Error looking up supplier with id " + supplierId, module);
    	}
    	if (supplier == null) {
    		Debug.logWarning(UtilProperties.getMessage(resource_error,"OrderNoSupplierFoundWithIdNotDoingPromotions", UtilMisc.toMap("supplierId", supplierId), cart.getLocale()), module);
    		return productPromoList;
    	}
    	
    	try {
    		// TODOCHANGE change get promoProgram list apply from SupplierPromoAppl to SupplierPromoApplFilter
    		// loop through promotions and get a list of all of the rules...
    		List<GenericValue> supplierPromoApplsList = null;
    		Object resourceHandler = null;
    		try {
    			ModelReader reader = delegator.getModelReader();
    			resourceHandler = reader.getEntityResourceHandler("SupplierPromoApplFilter");
    		} catch (Exception e) {
    			Debug.log("SupplierPromoApplFilter view entity is not exists, run on SupplierPromoAppl entity");
    		}
    		if (resourceHandler == null) {
    			// Old code
    			supplierPromoApplsList = supplier.getRelated("SupplierPromoAppl", null, UtilMisc.toList("sequenceNum"), true);
    		} else {
    			// New code
    			supplierPromoApplsList = delegator.findByAnd("SupplierPromoApplFilter", UtilMisc.toMap("partyId", supplier.get("partyId")), UtilMisc.toList("sequenceNum"), false);
    		}
    		
    		supplierPromoApplsList = EntityUtil.filterByDate(supplierPromoApplsList, nowTimestamp);
    		
    		if (UtilValidate.isEmpty(supplierPromoApplsList)) {
    			if (Debug.verboseOn()) Debug.logVerbose("Not doing promotions, none applied to supplier with ID " + supplierId, module);
    		}
    		
    		Iterator<GenericValue> prodCatalogPromoAppls = UtilMisc.toIterator(supplierPromoApplsList);
    		while (prodCatalogPromoAppls != null && prodCatalogPromoAppls.hasNext()) {
    			GenericValue prodCatalogPromoAppl = prodCatalogPromoAppls.next();
    			if (UtilValidate.isNotEmpty(prodCatalogPromoAppl.getString("manualOnly")) && "Y".equals(prodCatalogPromoAppl.getString("manualOnly"))) {
    				// manual only promotions are not automatically evaluated (they must be explicitly selected by the user)
    				if (Debug.verboseOn()) Debug.logVerbose("Skipping promotion with id [" + prodCatalogPromoAppl.getString("productPromoId") + "] because it is applied to the supplierId with ID " + supplierId + " as a manual only promotion.", module);
    				continue;
    			}
    ...
    public static List<GenericValue> getAgreementPromotions(ShoppingCart cart, Timestamp nowTimestamp, LocalDispatcher dispatcher) {
        ...
        try {
            agreementItems = delegator.findByAnd("AgreementItem", UtilMisc.toMap("agreementId", agreementId, "agreementItemTypeId", "AGREEMENT_PRICING_PR", "currencyUomId", cart.getCurrency()), null, true);
            // TODOCHANGE comment code: get all agreement item in this agreement
        ...
        try {
            if (productPromoList == null) {
                if (cart.getOrderType().equals("SALES_ORDER")) {
                	...
                	// TODOCHANGE add code: get list agreement promotion for this shopping cart
                	List<GenericValue> agreePromoList = ProductPromoWorker.getAgreementPromotions(cart, nowTimestamp, dispatcher);
                	if (UtilValidate.isNotEmpty(agreePromoList)) {
                		productPromoList.addAll(agreePromoList);
                	}
                } else if (cart.getOrderType().equals("PURCHASE_ORDER")){
                	productPromoList = ProductPromoWorker.getSupplierPromotions(cart, nowTimestamp, dispatcher);
                	// TODOCHANGE add code: get list agreement promotion for this shopping cart
                	List<GenericValue> agreePromoList = ProductPromoWorker.getAgreementPromotions(cart, nowTimestamp, dispatcher);
                	if (UtilValidate.isNotEmpty(agreePromoList)) {
                		productPromoList.addAll(agreePromoList);
            ...
            /* TODOCHANGE 20181212 for the block comment above. Don't need sort the promotions
            ...
            // END change 20181212 */
            // TODOCHANGE 20181212 for the block comment above. Don't need sort the promotions
            runProductPromos(productPromoList, cart, delegator, dispatcher, nowTimestamp, false);
    ...
    protected static void runProductPromos(List<GenericValue> productPromoList, ShoppingCart cart, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp, boolean isolatedTestRun) throws GeneralException {
        ...
        if (cart.items().size() < 1) return; // TODOCHANGE 20181212 skip if cart don't have any item yet
    ...
    protected static boolean runProductPromoRules(ShoppingCart cart, boolean cartChanged, Long useLimit, boolean requireCode, String productPromoCodeId, Long codeUseLimit, long maxUseLimit,
        ...
        while ((useLimit == null || useLimit.longValue() > cart.getProductPromoUseCount(productPromoId)) &&
            ...
            Map<GenericPK, String> statusUsePerCondActual = new HashMap<GenericPK, String>(); // TODOCHANGE manual promotion
            Iterator<GenericValue> promoRulesIter = productPromoRules.iterator();
            while (promoRulesIter != null && promoRulesIter.hasNext()) {
                GenericValue productPromoRule = promoRulesIter.next();
                ...
                if (performActions) {
                	...
                	Set<GenericPK> promoActionsSelected = cart.getAttribute("promoActionSelected");
                	boolean isCheckManual = false;
                	if (UtilValidate.isNotEmpty(promoActionsSelected)) isCheckManual = true;
                	// end change
                    ...
                    while (productPromoActionIter != null && productPromoActionIter.hasNext()) {
                        GenericValue productPromoAction = productPromoActionIter.next();
                        GenericPK productPromoActionPK = productPromoAction.getPrimaryKey(); // TODOCHANGE manual promotion
                        boolean isActionPerformAction = true; // TODOCHANGE manual promotion
                        String statusUseAction = "Y"; // TODOCHANGE manual promotion
                        boolean isChangableStatusUseAction = false; // TODOCHANGE manual promotion
                        try {
                        	if (productPromoAction.containsKey("operatorEnumId") && "PPAOP_OR".equals(productPromoAction.getString("operatorEnumId"))) {
                        		isChangableStatusUseAction = true;
                        	}
                        	if (isChangableStatusUseAction) {
                        		// action is PPAOP_OR
                        		if (isCheckManual) {
                        			// user is check action
                            		if (!promoActionsSelected.contains(productPromoActionPK)) isActionPerformAction = false;
                        		} else {
                        			// not apply even if user not choose
                        			isActionPerformAction = false;
                        		}
                        	}
                            if (isActionPerformAction) {
                                ...
                                if (actionChangedCart) {
                                    ...
                                    if (isChangableStatusUseAction) statusUseAction = "Y";
                                }
                            } else {
                            	if (isChangableStatusUseAction) {
                            		promoUsed = true;
                            ...
                            statusUsePerCondActual.put(productPromoActionPK, statusUseAction);
            ...
            if (promoUsed) {
                BigDecimal totalDiscount = totalDiscountAmount;
                if ("SALES_ORDER".equals(cart.getOrderType())) {
                    int invoiceTypeDecimals = UtilNumber.getBigDecimalScale("invoice.SALES_INVOICE.decimals");
                    totalDiscount = totalDiscount.setScale(invoiceTypeDecimals, rounding);
                } else if ("PURCHASE_ORDER".equals(cart.getOrderType())) {
                    int invoiceTypeDecimals = UtilNumber.getBigDecimalScale("invoice.PURCHASE_INVOICE.decimals");
                    totalDiscount = totalDiscount.setScale(invoiceTypeDecimals, rounding);
                }
            	// TODOCHANGE manual promotion
                cart.addProductPromoUse(productPromo.getString("productPromoId"), productPromoCodeId, totalDiscount, quantityLeftInActions, statusUsePerCondActual);
            } else {
    ...
    protected static boolean checkCondition(GenericValue productPromoCond, ShoppingCart cart, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp) throws GenericEntityException {
        ...
        if ("PPIP_SERVICE".equals(inputParamEnumId)) {
            ...
            while (amountNeeded.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                ...
                if (passedItemConds && !cartItem.getIsPromo() &&
                    ...
                    BigDecimal itemPrice = cartItem.getBasePrice();
                	if (productPromoCond.containsKey("usePriceWithTax") && "Y".equals(productPromoCond.getString("usePriceWithTax"))) {
                		...
                		for (ShoppingCart.CartShipInfo csi : csis) {
                            ...
                            for (GenericValue taxAdj : itemInfo.itemTaxAdj) {
                            	sumTax = sumTax.add(taxAdj.getBigDecimal("amount"));
                            }
                		}
                		itemPrice = itemPrice.add(sumTax.divide(cartItem.getQuantity(), 3, RoundingMode.HALF_UP));
                	}
                	
                    // old BigDecimal basePrice = cartItem.getBasePrice();
                    // get a rough price, round it up to an integer
                    BigDecimal quantityNeeded = amountNeeded.divide(itemPrice, generalRounding).setScale(0, BigDecimal.ROUND_CEILING);
                    ...
                    amountNeeded = amountNeeded.subtract(quantity.multiply(itemPrice));
                }
            }
        ...
        */
            ...
            // does nothing on order level, only checked on item level, so ignore by always considering passed
            return true;
        } else if ("PPIP_EACH_ORDER_TOTA".equals(inputParamEnumId)) {
            ...
            boolean isRun = false;
            boolean usePriceWithTax = false;
            if (productPromoCond.containsKey("usePriceWithTax") && "Y".equals(productPromoCond.getString("usePriceWithTax"))) {
            	usePriceWithTax = true;
            }
            BigDecimal totalAvailabel = null;
            if (UtilValidate.isNotEmpty(productIds)) {
            	isRun = true;
            	totalAvailabel = BigDecimal.ZERO;
            	List<ShoppingCartItem> lineOrderedByBasePriceList = cart.getLineListOrderedByBasePrice(false);
                Iterator<ShoppingCartItem> lineOrderedByBasePriceIter = lineOrderedByBasePriceList.iterator();
                while (amountNeeded.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                    ShoppingCartItem cartItem = lineOrderedByBasePriceIter.next();
                    ...
                    if (passedItemConds && !cartItem.getIsPromo() &&
                		...
                		totalAvailabel = totalAvailabel.add(itemSubTotal);
                    }
                }
            } else {
            	List<EntityCondition> listConds = FastList.newInstance();
            	EntityCondition cond1 = EntityCondition.makeCondition(EntityCondition.makeCondition("productPromoRuleId", "_NA_"), 
            			EntityOperator.AND, EntityCondition.makeCondition("productPromoCondSeqId", "_NA_"));
            	EntityCondition cond2 = EntityCondition.makeCondition(EntityCondition.makeCondition("productPromoRuleId", productPromoCond.get("productPromoRuleId")), 
            			EntityOperator.AND, EntityCondition.makeCondition("productPromoCondSeqId", productPromoCond.get("productPromoCondSeqId")));
            	listConds.add(EntityCondition.makeCondition("productPromoId", productPromoCond.get("productPromoId")));
            	listConds.add(EntityCondition.makeCondition(cond1, EntityOperator.OR, cond2));
            	
            	List<GenericValue> productPromoCategoriesAll = delegator.findList("ProductPromoCategory", EntityCondition.makeCondition(listConds, EntityOperator.AND), null, null, null, true);
            	if (UtilValidate.isEmpty(productPromoCategoriesAll)) {
            		List<GenericValue> productPromoProductsAll = delegator.findList("ProductPromoProduct", EntityCondition.makeCondition(listConds, EntityOperator.AND), null, null, null, true);
                    if (UtilValidate.isEmpty(productPromoProductsAll)) {
                    	isRun = true;
                    }
            	}
            }
            ...
            if (isRun) {
            	// reduce amount still needed to qualify for promo (amountNeeded)
        		amount = cart.addPromAmountCandidateUse(amountNeeded, productPromoCond, false, totalAvailabel, usePriceWithTax);
            }
            
            // get pro-rated amount based on discount
            amountNeeded = amountNeeded.subtract(amount);
            ...
            if (amountNeeded.compareTo(BigDecimal.ZERO) > 0) {
                // failed, reset the entire rule, ie including all other conditions that might have been done before
                cart.resetPromoRuleUse(productPromoCond.getString("productPromoId"), productPromoCond.getString("productPromoRuleId"), amount);
                compareBase = Integer.valueOf(-1);
            } else {
                // we got it, the conditions are in place...
                compareBase = Integer.valueOf(0);
        ...
        }  else if ("PPIP_EACH_PROD_QUANT".equals(inputParamEnumId)) {
        	// TODOCHANGE new promotion condition
            ...
            if (UtilValidate.isNotEmpty(productIds)) {
	            ...
	            while (quantityNeeded.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
	                ...
	                if (passedItemConds && !cartItem.getIsPromo() &&
	                        (productIds.contains(cartItem.getProductId()) || (parentProductId != null && productIds.contains(parentProductId))) &&
	                        (product == null || !"N".equals(product.getString("includeInPromotions")))) {
	                	totalAvailabel = totalAvailabel.add(cartItem.getQuantity());
	                }
	            }
            } else {
            	List<EntityCondition> listConds = FastList.newInstance();
            ...
            if (isRun) {
            	// reduce quantity still needed to qualify for promo (quantityNeeded)
                quantity = cart.addPromoQuantityCandidateUse(quantityNeeded, productPromoCond, false, totalAvailabel);
            }
            quantityNeeded = quantityNeeded.subtract(quantity);

            // if quantityNeeded > 0 then the promo condition failed, so remove candidate promo uses and increment the promoQuantityUsed to restore it
            if (quantityNeeded.compareTo(BigDecimal.ZERO) > 0) {
                // failed, reset the entire rule, ie including all other conditions that might have been done before
                cart.resetPromoRuleUse(productPromoCond.getString("productPromoId"), productPromoCond.getString("productPromoRuleId"), quantity);
                compareBase = Integer.valueOf(-1);
            } else {
                // we got it, the conditions are in place...
                compareBase = Integer.valueOf(0);
        ...
        }  else if ("PPIP_PROD_QUANT_TOTA".equals(inputParamEnumId)) {
        	// TODOCHANGE new promotion condition
            ...
            if (UtilValidate.isNotEmpty(condValue)) {
                ...
                for (ShoppingCartItem cartItem : lineOrderedByBasePriceList) {
                    ...
                    if (passedItemConds && !cartItem.getIsPromo() &&
                        ...
                        // just count the entire quantity of the item
                    	if (cartItem.getQuantity() != null) amountAvailable = amountAvailable.add(cartItem.getQuantity());
                    }
                }
        ...
        if (compareBase != null) {
            ...
            if ("PPC_EQ".equals(operatorEnumId)) {
                ...
                if (compare >= 0) return true;
            } else if ("PPC_NIN".equals(operatorEnumId)) {
            	// TODOCHANGE process exception promotion
    ...
    protected static boolean checkConditionForItem(GenericValue productPromoCond, ShoppingCart cart, ShoppingCartItem cartItem, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp) throws GenericEntityException {
        ...
        // TODOCHANGE check divide by ZERO
        if (BigDecimal.ZERO.compareTo(listPrice) != 0) {
        	percentOff = amountOff.divide(listPrice, 2, BigDecimal.ROUND_HALF_UP).multiply(BigDecimal.valueOf(100L));
        }
    ...
    public static void performAction(ActionResultInfo actionResultInfo, GenericValue productPromoAction, ShoppingCart cart, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp) throws GenericEntityException, CartItemModifyException {
        ...
        if ("PROMO_SERVICE".equals(productPromoActionEnumId)) {
            ...
            Map<GenericPK, BigDecimal> countTimesUsedPerPromoCandidate = cart.getCountTimesUsedPerPromoCandidate();
            BigDecimal numberTimesApply = null;
            if (UtilValidate.isNotEmpty(countTimesUsedPerPromoCandidate)) {
                for (Map.Entry<GenericPK, BigDecimal> item : countTimesUsedPerPromoCandidate.entrySet()) {
                	BigDecimal itemValue = item.getValue();
                	GenericPK promoCondPK = item.getKey();
                	if (promoCondPK.getString("productPromoId").equals(productPromoAction.getString("productPromoId")) 
                			&& promoCondPK.getString("productPromoRuleId").equals(productPromoAction.getString("productPromoRuleId"))) {
                		if (numberTimesApply == null) {
                    		numberTimesApply = itemValue;
                    	} else {
                    		if (numberTimesApply.compareTo(itemValue) > 0) {
                    			numberTimesApply = itemValue;
                    		}
                    	}
                	}
            ...
            if (numberTimesApply == null) numberTimesApply = BigDecimal.ONE;
            ...
            if (!allowMultipleGwp && itemLoc != null) {
                ...
                if (productPromoAction.get("quantity") != null) {
                    ...
                    if (numberTimesApply.compareTo(BigDecimal.ONE) > 0) {
                    	quantity = quantity.multiply(numberTimesApply);
                    }
                } else {
                ...
                if (UtilValidate.isNotEmpty(productId)) {
                    ...
                    if ("Y".equals(product.getString("isVirtual"))) {
                    	...
                    	if ((!productPromoAction.containsKey("isCheckInv") || !"N".equals(productPromoAction.getString("isCheckInv"))) &&
                    			orderTypeId.equals("SALES_ORDER")){ // TODOCHANGE isCheckInv promo action
                ...
                while (optionProductIdIter.hasNext()) {
                    ...
                    GenericValue optionProduct = delegator.findOne("Product", UtilMisc.toMap("productId", optionProductId), true); // TODOCHANGE promotion virtual product
                    try {
                    	if (optionProduct != null && "Y".equals(optionProduct.getString("isVirtual"))) {
                    		// TODOCHANGE promotion virtual product
                            List<GenericValue> productAssocs = EntityUtil.filterByDate(optionProduct.getRelated("MainProductAssoc", UtilMisc.toMap("productAssocTypeId", "PRODUCT_VARIANT"), UtilMisc.toList("sequenceNum"), true));
                            if (productAssocs != null) {
                            	for (GenericValue productAssoc : productAssocs) {
                                    optionProductIds.add(productAssoc.getString("productIdTo"));
                                }
                            }
                            optionProductIds.remove(optionProductId);
                ...
                try {
                    ...
                    if ("PURCHASE_ORDER".equals(cart.getOrderType())) {
                    	try {
                    		GenericValue supplierProduct = cart.getSupplierProduct(productId, null, dispatcher);
                    		Map<String, Object> attributes = UtilMisc.toMap("quantityUomId", product.getString("quantityUomId"));
							gwpItem = ShoppingCartItem.makePurchaseOrderItem(null, product.getString("productId"), null, quantity, null, attributes, prodCatalogId, null, null, null, dispatcher, cart, supplierProduct, null, null, null, Boolean.FALSE);
                ...
                if("PURCHASE_ORDER".equals(cart.getOrderType())) {
                    gwpItem.setAlternativeUnitPrice(BigDecimal.ZERO);
                } else {
            ...
            // VietTB solution 
        } else if ("PROMO_TRADE_DISCOUNT".equals(productPromoActionEnumId)) {
            BigDecimal percentage = (productPromoAction.get("amount") == null ? BigDecimal.ZERO : (productPromoAction.getBigDecimal("amount").movePointLeft(2))).negate();
            BigDecimal amount = BigDecimal.ZERO;
            
            BigDecimal itemsTotal = BigDecimal.ZERO;
            List<ShoppingCartItem> carlLines ;
            carlLines = cart.getCartLines();
            for (ShoppingCartItem cartItem : carlLines) {
                GenericValue product = cartItem.getProduct();
                ...
                itemsTotal = cartItem.getNewItemSubTotal().multiply(percentage);
                amount = amount.add(itemsTotal);
                if (itemsTotal.compareTo(BigDecimal.ZERO) != 0) {
                	doOrderItemPromoTradeAction(productPromoAction, cartItem, itemsTotal, "amount", delegator);
                   // process for item
            }
           }
...
//                doOrderPromoAction(productPromoAction, cart, amount, "amount", delegator);
    ...
    // Create by VietTB
    public static void doOrderPromoTradeAction(GenericValue productPromoAction, ShoppingCart cart, BigDecimal amount, String amountField, Delegator delegator) {
        // round the amount before setting to make sure we don't get funny numbers in there
        amount = amount.setScale(decimals, rounding);
        GenericValue orderAdjustment = delegator.makeValue("OrderAdjustment",
                UtilMisc.toMap("orderAdjustmentTypeId", "DISCOUNT_ADJUSTMENT", amountField, amount,
                        "productPromoId", productPromoAction.get("productPromoId"),
                        "productPromoRuleId", productPromoAction.get("productPromoRuleId"),
                        "productPromoActionSeqId", productPromoAction.get("productPromoActionSeqId"),
                        "description", getProductPromoDescription((String) productPromoAction.get("productPromoId"), delegator)));
...
//        cartItem.addAdjustment(orderAdjustment);
    ...
    public static void doOrderItemPromoTradeAction(GenericValue productPromoAction, ShoppingCartItem cartItem, BigDecimal amount, String amountField, Delegator delegator) {
        // round the amount before setting to make sure we don't get funny numbers in there
        // only round to 3 places, we need more specific amounts in adjustments so that they add up cleaner as part of the item subtotal, which will then be rounded
        amount = amount.setScale(3, rounding);
        GenericValue orderAdjustment = delegator.makeValue("OrderAdjustment",
