...
public class ShoppingCartServices {
    ...
    public static Map<String, Object>loadCartFromOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        Boolean includeCancelQuantity = Boolean.TRUE.equals(context.get("includeCancelQuantity")); //TODOCHANGE process cancelQuantity
        ...
        // TODOCHANGE set default shipping date
        if (orderHeader.containsKey("estimatedDeliveryDate") && UtilValidate.isNotEmpty(orderHeader.get("estimatedDeliveryDate"))) cart.setDefaultItemDeliveryDate(String.valueOf(orderHeader.getTimestamp("estimatedDeliveryDate")));
        if (orderHeader.containsKey("shipBeforeDate") && UtilValidate.isNotEmpty(orderHeader.get("shipBeforeDate"))) cart.setDefaultShipBeforeDate(orderHeader.getTimestamp("shipBeforeDate"));
        if (orderHeader.containsKey("shipAfterDate") && UtilValidate.isNotEmpty(orderHeader.get("shipAfterDate"))) cart.setDefaultShipAfterDate(orderHeader.getTimestamp("shipAfterDate"));
        // end new
        ...
        if (UtilValidate.isNotEmpty(orderItems)) {
            ...
            for (GenericValue item : orderItems) {
                ...
                if (!includePromoItems && item.get("isPromo") != null && "Y".equals(item.getString("isPromo")) && !"PRODPROMO_ORDER_ITEM".equals(item.getString("orderItemTypeId"))) {
                    continue;
                }
                ...
                // TODOCHANGE process alternativeQuantity
                BigDecimal alternativeQuantity = item.getBigDecimal("alternativeQuantity");
                String quantityUomId = item.getString("quantityUomId");
                if (alternativeQuantity == null || UtilValidate.isEmpty(quantityUomId)) {
                	alternativeQuantity = quantity;
                }
                // TODOCHANGE process cancelQuantity
                if (includeCancelQuantity) {
                	BigDecimal cancelQuantity = item.getBigDecimal("cancelQuantity");
                	if (cancelQuantity != null) {
                		// recalculate alternative quantity
                		if (quantity.compareTo(BigDecimal.ZERO) != 0) {
                			BigDecimal alternativeCancelQty = cancelQuantity.multiply(alternativeQuantity).divide(quantity, 2, RoundingMode.HALF_UP);
                			alternativeQuantity = alternativeQuantity.subtract(alternativeCancelQty).setScale(2, RoundingMode.HALF_UP);
                		}
...
// change by thangnv - always modified
...
//                }
                ...
                if (item.get("productId") == null) {
                    ...
                    try {
                        ...
                        itemIndex = cart.addItemToEnd(productId, amount, alternativeQuantity, unitPrice, reservStart, reservLength, reservPersons,accommodationMapId,accommodationSpotId, null, attributes, prodCatalogId, configWrapper, item.getString("orderItemTypeId"), dispatcher, null, unitPrice == null ? null : false, skipInventoryChecks, skipProductChecks);
                ...
                // change to add alternative price by thangnv
                try {
					...
					if (UtilValidate.isNotEmpty(requireAmount) && "Y".equals(requireAmount)) {
						cartItem.setAlternativeUnitPrice(unitPrice);
						cartItem.setAttribute("alternativeUnitPrice", unitPrice);
					} else {
						if (UtilValidate.isNotEmpty(quantityUomId) && UtilValidate.isNotEmpty(baseQuantityUomId)) {
							BigDecimal convert = ProductUtil.getConvertPackingNumber(delegator, productId, quantityUomId, baseQuantityUomId);
							BigDecimal alternativeUnitPrice = unitPrice.multiply(convert);
							cartItem.setAlternativeUnitPrice(alternativeUnitPrice);
							cartItem.setAttribute("alternativeUnitPrice", alternativeUnitPrice);
						}
					}
				} catch (GenericEntityException e1) {
					return ServiceUtil.returnError("OLBIUS: product not found! " + e1.toString());
				}
                // attach addition item information
                cartItem.setAttribute("alternativeQuantity", alternativeQuantity);
                ...
                //TODOCHANGE add attributes (not yet add itemDesiredDeliveryDate)
                //cartItem.setAttribute("quantityUomId", item.getString("quantityUomId"));
                //cartItem.setAttribute("alternativeQuantity", item.getBigDecimal("alternativeQuantity"));
                //cartItem.setAttribute("alternativeUnitPrice", item.getBigDecimal("alternativeUnitPrice"));
                //cartItem.setAttribute("expireDate", item.getTimestamp("expireDate"));
                //defaultItemDeliveryDate = item.getString("estimatedDeliveryDate");
            ...
            if (defaultItemDeliveryDate != null) {
            	cart.setDefaultItemDeliveryDate(defaultItemDeliveryDate);
            }
