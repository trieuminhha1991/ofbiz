<script type="text/javascript" id="itemSummaryReportGrid">
		var config = {
                title: ' ${uiLabelMap.ReportItemSummaryReport}',
                sortable: true,
                service: 'salesOrder',
                columns: [
                    {text: "${uiLabelMap.BPOSProductCategoryId}", datafield:'categoryId', cellsalign: 'left', type: 'string', hidden: true},
                    {text: "${uiLabelMap.BPOSCategory}", datafield:'categoryName', cellsalign: 'left', type: 'string', width: 200},
                    	...
                    	olapcellrenderer: function(olap){
                   			return function (row, column, value) {
       	     					...
       	     					if (data && data.productId){
       	     						return '<div><a style = "margin-left: 10px" target="_blank" href=' + 'salesDetailReport?productId=' + data.productId + '>' +  data.productId + '</a>' + '</div>'
           						}   
                   	   		}
                   		}
                    },
        ...
        var itemSummaryGrid = OLBIUS.oLapGrid('itemSummaryReportGrid', config, configPopup, 'executeGetSalesOrderReport', true);
        var sortField = '';
		var sortOption = '';
        itemSummaryGrid.funcUpdate(function (oLap) {
            oLap.update({
                'fromDate': oLap.val('from_date_1'),
                'thruDate': oLap.val('thru_date_1'),
                'categoryId': '${parameters.categoryId?if_exists}',
                'sortField': sortField,
        ...
        itemSummaryGrid.init(function () {
        	itemSummaryGrid.runAjax();
        });
        
        var grid = itemSummaryGrid.getGrid();
