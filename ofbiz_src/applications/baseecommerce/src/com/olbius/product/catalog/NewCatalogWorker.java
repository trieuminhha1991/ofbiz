package com.olbius.product.catalog;
 ...
 */
public class NewCatalogWorker {

    public static final String module = NewCatalogWorker.class.getName();

    private NewCatalogWorker () {}
    ...
    public static List<GenericValue> getStoreCatalogs(Delegator delegator, String productStoreId) {
        try {
		return EntityUtil.filterByDate(delegator.findList("StoreCatalogCategory", EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, productStoreId),
				UtilMisc.toSet("prodCatalogId", "catalogName", "description", "headerLogo", "sequenceNum"), UtilMisc.toList("sequenceNum"), null, true), true);
        } catch (GenericEntityException e) {
    ...
    public static String getCurrentCatalogId(ServletRequest request) {
        ...
        // if no parameter, try from attribute
        if (prodCatalogId == null) {
		prodCatalogId = (String) request.getAttribute("CURRENT_CATALOG_ID");
        }
    ...
    public static String getCatalogBestSellingCategoryId(ServletRequest request) {
        return getCatalogBestSellingCategoryId(request, getCurrentCatalogId(request));
    }

    public static String getCatalogBestSellingCategoryId(ServletRequest request, String prodCatalogId) {
        if (prodCatalogId == null || prodCatalogId.length() <= 0) return null;
    ...
    public static String getCatalogMostPopular(ServletRequest request) {
        return getCatalogMostPopular(request, getCurrentCatalogId(request));
    }

    public static String getCatalogMostPopular(ServletRequest request, String prodCatalogId) {
	if (prodCatalogId == null || prodCatalogId.length() <= 0) return null;

        List<GenericValue> prodCatalogCategories = getProdCatalogCategories(request, prodCatalogId, "PCCT_MOST_POPULAR");
    ...
    public static String getCatalogWhatNew(ServletRequest request) {
        return getCatalogWhatNew(request, getCurrentCatalogId(request));
    }

    public static String getCatalogWhatNew(ServletRequest request, String prodCatalogId) {
	if (prodCatalogId == null || prodCatalogId.length() <= 0) return null;

        List<GenericValue> prodCatalogCategories = getProdCatalogCategories(request, prodCatalogId, "PCCT_WHATS_NEW");
