 ...
 */
public class NewCategoryWorker {

	public static final String module = NewCategoryWorker.class.getName();

	private NewCategoryWorker() {
	}
	...
	public static String getCatalogTopCategory(ServletRequest request,
		...
		if (topCatName == null) {
			topCatName = (String) httpRequest.getSession().getAttribute(
					"CATALOG_TOP_CATEGORY");
			if (topCatName != null)
				fromSession = true;
		}
		...
		if (!fromSession) {
			if (Debug.infoOn())
				Debug.logInfo(
						"[CategoryWorker.getCatalogTopCategory] Setting new top category: "
								+ topCatName, module);
			httpRequest.getSession().setAttribute("CATALOG_TOP_CATEGORY",
					topCatName);
		}
		return topCatName;
	}
	...
	public static void getCategoriesWithNoParent(ServletRequest request,
		...
		try {
			Collection<GenericValue> allCategories = delegator.findList(
					"ProductCategory", null, null, null, null, false);

			for (GenericValue curCat : allCategories) {
				Collection<GenericValue> parentCats = curCat.getRelated(
						"CurrentProductCategoryRollup", null, null, true);
	...
	public static void getRelatedCategories(ServletRequest request,
			String attributeName, boolean limitView) {
		Map<String, Object> requestParameters = UtilHttp
				.getParameterMap((HttpServletRequest) request);
		String requestId = null;

		requestId = UtilFormatOut.checkNull(
		...
		if (requestId.equals(""))
			return;
		if (Debug.infoOn())
			Debug.logInfo("[CategoryWorker.getRelatedCategories] RequestID: "
					+ requestId, module);
	...
	public static List<GenericValue> getRelatedCategoriesRet(
			ServletRequest request, String attributeName, String parentId,
			boolean limitView) {
	...
	public static List<GenericValue> getRelatedCategoriesRet(
			ServletRequest request, String attributeName, String parentId,
			boolean limitView, boolean excludeEmpty) {
		return getRelatedCategoriesRet(request, attributeName, parentId,
				limitView, excludeEmpty, false);
	}

	public static List<GenericValue> getRelatedCategoriesRet(
			ServletRequest request, String attributeName, String parentId,
			boolean limitView, boolean excludeEmpty, boolean recursive) {
		Delegator delegator = (Delegator) request.getAttribute("delegator");

		return getRelatedCategoriesRet(delegator, attributeName, parentId,
				limitView, excludeEmpty, false);
	}

	public static List<GenericValue> getRelatedCategoriesRet(
			Delegator delegator, String attributeName, String parentId,
			boolean limitView, boolean excludeEmpty, boolean recursive) {
		List<GenericValue> categories = FastList.newInstance();

		if (Debug.verboseOn())
			Debug.logVerbose("[CategoryWorker.getRelatedCategories] ParentID: "
					+ parentId, module);
	...
	public static long categoryMemberCount(GenericValue category) {
		...
		long count = 0;
		try {
			count = delegator.findCountByCondition(
					"ProductCategoryMember",
					buildCountCondition("productCategoryId",
							category.getString("productCategoryId")), null,
					null);
		} catch (GenericEntityException e) {
	...
	// category dont has child but it contain in another category
	public static long categoryRollupCountChildren(GenericValue category) {
		if (category == null)
			return 0;
		Delegator delegator = category.getDelegator();
		long count = 0;
	...
	/* Get all children category of category */
	public static List<Map<String, Object>> getChildrenCategory(GenericValue category) {
		...
		try {
			eli = delegator.find("ProductCategoryRollupAndChild", buildCountCondition("parentProductCategoryId", category.getString("productCategoryId")), null, null, UtilMisc.toList("sequenceNum"), null);
			GenericValue trans;
			while((trans = eli.next()) != null){
				Map<String, Object> ctx = FastMap.newInstance();
				ctx.put("productCategoryId", trans.getString("productCategoryId"));
				ctx.put("categoryName", trans.getString("categoryName"));
				arr.add(ctx);
			}
		} catch (GenericEntityException e) {
			Debug.logError(e, module);
	...
	public static void insertionSort(Map<String, Object> obj,
			List<Map<String, Object>> input) {
		int key = (Integer) obj.get("no");
		int i = 0;
		int size = input.size() ;
		if (size == 0) {
			input.add(obj);
		} else {
			for (Map<String, Object> cur : input) {
				int curNo = (Integer) cur.get("no");
				if (key > curNo) {
					input.add(i, obj);
				}else if (i == (size - 1)) {
					input.add(obj);
	...
	private static EntityCondition buildCountCondition(String fieldName,
			String fieldValue) {
		List<EntityCondition> orCondList = FastList.newInstance();
		orCondList.add(EntityCondition.makeCondition("thruDate",
				EntityOperator.GREATER_THAN, UtilDateTime.nowTimestamp()));
		orCondList.add(EntityCondition.makeCondition("thruDate",
				EntityOperator.EQUALS, null));
		EntityCondition orCond = EntityCondition.makeCondition(orCondList,
				EntityOperator.OR);

		List<EntityCondition> andCondList = FastList.newInstance();
		andCondList.add(EntityCondition.makeCondition("fromDate",
				EntityOperator.LESS_THAN, UtilDateTime.nowTimestamp()));
		andCondList.add(EntityCondition.makeCondition(fieldName,
				EntityOperator.EQUALS, fieldValue));
		andCondList.add(orCond);
		EntityCondition andCond = EntityCondition.makeCondition(andCondList,
				EntityOperator.AND);
	...
	public static List<String> adjustTrail(List<String> origTrail,
		...
		if (UtilValidate.isEmpty(previousCategoryId)) {
			if (trail.contains(currentCategoryId)) {
				...
				// if (Debug.infoOn())
				// Debug.logInfo("[CategoryWorker.setTrail] Starting new list, added TOP and previousCategory: "
		...
		if (!trail.contains(previousCategoryId)) {
			...
			// if (Debug.infoOn())
			// Debug.logInfo("[CategoryWorker.setTrail] previousCategory (" +
			// previousCategoryId +
		...
		if (Debug.verboseOn())
			Debug.logVerbose(
					"[CategoryWorker.setTrail] Continuing list: Added currentCategory: "
	...
	public static List<String> getTrail(ServletRequest request) {
		...
		List<String> crumb = UtilGenerics.checkList(session
				.getAttribute("_BREAD_CRUMB_TRAIL_"));
		return crumb;
	}

	public static List<String> setTrail(ServletRequest request,
			List<String> crumb) {
		HttpSession session = ((HttpServletRequest) request).getSession();
	...
	public static boolean isProductInCategory(Delegator delegator,
			String productId, String productCategoryId)
		...
		if (UtilValidate.isEmpty(productCategoryMembers)) {
			...
			List<GenericValue> productAssocs = ProductWorker
					.getVariantVirtualAssocs(product);
	...
	public static List<GenericValue> filterProductsInCategory(
			Delegator delegator, List<GenericValue> valueObjects,
			String productCategoryId) throws GenericEntityException {
		return filterProductsInCategory(delegator, valueObjects,
				productCategoryId, "productId");
	}

	public static List<GenericValue> filterProductsInCategory(
			Delegator delegator, List<GenericValue> valueObjects,
			String productCategoryId, String productIdFieldName)
			throws GenericEntityException {
	...
	public static void getCategoryContentWrappers(
			Map<String, CategoryContentWrapper> catContentWrappers,
			List<GenericValue> categoryList, HttpServletRequest request)
			throws GenericEntityException {
		...
		for (GenericValue cat : categoryList) {
			...
			CategoryContentWrapper catContentWrapper = new CategoryContentWrapper(
					cat, request);
			catContentWrappers.put(productCategoryId, catContentWrapper);
			List<GenericValue> subCat = getRelatedCategoriesRet(request,
					"subCatList", productCategoryId, true);
			if (subCat != null) {
	...
	public static Map getCategoryTrail(DispatchContext dctx, Map context) {
		...
		while (UtilValidate.isNotEmpty(parentProductCategoryId)) {
			...
			try {
				...
				rolllupConds.add(EntityCondition.makeCondition(
						"productCategoryId", parentProductCategoryId));
				rolllupConds.add(EntityUtil.getFilterByDateExpr());
				List<GenericValue> productCategoryRollups = delegator.findList(
						"ProductCategoryRollup",
				...
				if (UtilValidate.isNotEmpty(productCategoryRollups)) {
					...
					for (GenericValue productCategoryRollup : productCategoryRollups) {
						String trailCategoryId = productCategoryRollup
								.getString("parentProductCategoryId");
				...
				Map<String, String> messageMap = UtilMisc.toMap("errMessage",
						". Cannot generate trail from product category. ");
				String errMsg = UtilProperties.getMessage("CommonUiLabels",
						"CommonDatabaseProblem", messageMap,
	...
	public static GenericValue getCategoryDetail(ServletRequest request,
			String strCatId) {
		Delegator delegator = (Delegator) request.getAttribute("delegator");
		Map map = UtilMisc.toMap("productCategoryId", strCatId);
		try {
			return delegator.findOne("ProductCategory", map, true);
		} catch (GenericEntityException e) {
			Debug.logError(e, module);
		}
		return null;
	}
	public static List<Map<String, Object>> getCategoryProductHot(Delegator delegator, String productCategoryId, String type, int size, int page){
		List<Map<String, Object>> res = FastList.newInstance();
		if(UtilValidate.isNotEmpty(productCategoryId)){
			String productCategoryId_BSL = productCategoryId.toUpperCase() + "_" + type;
			int start = size * page;
			EntityListIterator products = null;
			try {
				EntityFindOptions opts = new EntityFindOptions();
				opts.setResultSetType(EntityFindOptions.TYPE_SCROLL_SENSITIVE);
				products = delegator.find("ProductCategoryMemberAndPriceFilter", EntityCondition.makeCondition(
											UtilMisc.toList(
											EntityCondition.makeCondition("productCategoryId", productCategoryId_BSL),
											EntityCondition.makeCondition("productPricePurposeId", "PURCHASE")
											)), null, null, null, opts);
				List<GenericValue> tmp = products.getPartialList(start, size);
				for (GenericValue e : tmp) {
					Map<String, Object> o = FastMap.newInstance();
					o.put("productId", e.getString("productId"));
					o.put("productName", e.getString("productName"));
					o.put("originalImageUrl", e.getString("originalImageUrl"));
					o.put("largeImageUrl", e.getString("largeImageUrl"));
					o.put("smallImageUrl", e.getString("smallImageUrl"));
					o.put("detailImageUrl", e.getString("detailImageUrl"));
					o.put("quantityUomId", e.getString("quantityUomId"));
					o.put("productCategoryId", productCategoryId);
					BigDecimal pr = e.getBigDecimal("price");
					o.put("default_price", pr);
					List<GenericValue> listPrice = delegator.findList("ProductPrice", EntityCondition.makeCondition(
											UtilMisc.toList(
											EntityCondition.makeCondition("productId", e.getString("productId")),
											EntityCondition.makeCondition("productPriceTypeId", "LIST_PRICE"),
											EntityUtil.getFilterByDateExpr()
											)), null, UtilMisc.toList("-fromDate"), null, false);
					if(UtilValidate.isNotEmpty(listPrice)){
						GenericValue fk = EntityUtil.getFirst(listPrice);
						o.put("list_price", fk.get("price"));
						float fl = fk.getBigDecimal("price").floatValue();
						float tmpr = fl - pr.floatValue();
						if(tmpr > 0){
							float discount = tmpr/fl*100;
	...
	public static List<Map<String, Object>> getCategoryProductTop(Delegator delegator, String productCategoryId, int size, int page) throws GenericEntityException{
		return getCategoryProductHot(delegator, productCategoryId, "BSL", size, page);
	}
	public static List<Map<String, Object>> getCategoryProductLatest(Delegator delegator, String productCategoryId, int size, int page) throws GenericEntityException{
		return getCategoryProductHot(delegator, productCategoryId, "NEW", size, page);
	}
}
