...
public class NewCategoryService {
	...
	public static final String module = NewCategoryService.class.getName();
	public static final String resourceError = "ProductErrorUiLabels";

	private NewCategoryService() {
	}

	public static Map<String, Object> getCategoryMembers(DispatchContext dctx,
		...
		try {
			productCategory = delegator.findOne("ProductCategory",
					UtilMisc.toMap("productCategoryId", categoryId), true);
			members = EntityUtil.filterByDate(productCategory.getRelated(
					"ProductCategoryMember", null,
					UtilMisc.toList("sequenceNum"), true), true);
			if (Debug.verboseOn())
				Debug.logVerbose("Category: " + productCategory
						+ " Member Size: " + members.size() + " Members: "
						+ members, module);
		} catch (GenericEntityException e) {
			Debug.logError(e,
					"Problem reading product categories: " + e.getMessage(),
					module);
	...
	public static Map<String, Object> getPreviousNextProducts(
			DispatchContext dctx, Map<String, ? extends Object> context) {
		Delegator delegator = dctx.getDelegator();
		String categoryId = (String) context.get("categoryId");
		String productId = (String) context.get("productId");
		boolean activeOnly = (context.get("activeOnly") != null ? ((Boolean) context
				.get("activeOnly")).booleanValue() : true);
		Integer index = (Integer) context.get("index");
		Timestamp introductionDateLimit = (Timestamp) context
				.get("introductionDateLimit");
		Timestamp releaseDateLimit = (Timestamp) context
				.get("releaseDateLimit");
		Locale locale = (Locale) context.get("locale");

		if (index == null && productId == null) {
			return ServiceUtil.returnFailure(UtilProperties.getMessage(
					resourceError,
					"categoryservices.problems_getting_next_products", locale));
		}

		List<String> orderByFields = UtilGenerics.checkList(context
				.get("orderByFields"));
		if (orderByFields == null)
			orderByFields = FastList.newInstance();
		String entityName = getCategoryFindEntityName(delegator, orderByFields,
				introductionDateLimit, releaseDateLimit);
		...
		try {
			...
			Debug.logInfo(
					e,
					"Error finding previous/next product info: " + e.toString(),
					module);
			return ServiceUtil.returnFailure(UtilProperties.getMessage(
					resourceError, "categoryservices.error_find_next_products",
					UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
		if (activeOnly) {
			productCategoryMembers = EntityUtil.filterByDate(
					productCategoryMembers, true);
		}
		...
		if (!filterConditions.isEmpty()) {
			productCategoryMembers = EntityUtil.filterByCondition(
					productCategoryMembers, EntityCondition.makeCondition(
							filterConditions, EntityOperator.AND));
		}
		...
		if (index == null) {
			...
			return ServiceUtil.returnFailure(UtilProperties
		...
		if (index.intValue() - 1 >= 0
				&& index.intValue() - 1 < productCategoryMembers.size()) {
			previous = productCategoryMembers.get(index.intValue() - 1)
					.getString("productId");
			result.put("previousProductId", previous);
		} else {
			previous = productCategoryMembers.get(
					productCategoryMembers.size() - 1).getString("productId");
			result.put("previousProductId", previous);
		}

		if (index.intValue() + 1 < productCategoryMembers.size()) {
			next = productCategoryMembers.get(index.intValue() + 1).getString(
					"productId");
	...
	public static String getCategoryFindEntityName(Delegator delegator,
		...
		String entityName = introductionDateLimit == null
				&& releaseDateLimit == null ? "ProductCategoryMember"
		...
		ModelEntity productCategoryMemberModel = delegator
				.getModelEntity("ProductCategoryMember");
	...
	public static Map<String, Object> getProductCategoryAndLimitedMembers(DispatchContext dctx, Map<String, ? extends Object> context) throws GenericEntityException {
		Delegator delegator = dctx.getDelegator();
		...
		if (productCategory != null) {
			try {
				...
				if (UtilValidate.isNotEmpty(brand)) {
					tmpCond.add(CategoryUtils.filterProductByBrand(delegator, brand));
				}
	...
	/* filter category product by price */
	public static Map<String, Object> getProductCategoryAndLimitedMembersByPrice(
			DispatchContext dctx, Map<String, ? extends Object> context) {
		Delegator delegator = dctx.getDelegator();
		String productCategoryId = (String) context.get("productCategoryId");
		BigDecimal lowPrice = new BigDecimal((String) context.get("lowPrice"));
		BigDecimal highPrice = new BigDecimal((String) context.get("highPrice"));
		...
		if (orderByFields == null)
			orderByFields = FastList.newInstance();
		String entityName = "ProductCategoryMemberAndPrice";
		String prodCatalogId = (String) context.get("prodCatalogId");

		boolean activeOnly = (context.get("activeOnly") == null || ((Boolean) context
				.get("activeOnly")).booleanValue());

		// checkViewAllow defaults to false, must be set to true and pass the
		// prodCatalogId to enable
		boolean checkViewAllow = (prodCatalogId != null
				&& context.get("checkViewAllow") != null && ((Boolean) context
				.get("checkViewAllow")).booleanValue());

		String viewProductCategoryId = null;
		if (checkViewAllow) {
			viewProductCategoryId = CatalogWorker
		...
		try {
			viewIndex = Integer
					.valueOf((String) context.get("viewIndexString"))
					.intValue();
		} catch (Exception e) {
			viewIndex = 0;
		}
		...
		int viewSize = defaultViewSize;
		try {
			viewSize = Integer.valueOf((String) context.get("viewSizeString"))
					.intValue();
		} catch (Exception e) {
			viewSize = defaultViewSize;
		}
		...
		try {
			productCategory = delegator.findOne("ProductCategory",
					...
					true);
			childProductCategory = delegator.findList("ProductCategoryRollup", EntityCondition.makeCondition("parentProductCategoryId", productCategoryId), null, null, null, true);
			childProductCategory = EntityUtil.filterByCondition(childProductCategory, EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr()));
		} catch (GenericEntityException e) {
			Debug.logWarning(e.getMessage(), module);
			productCategory = null;
		...
		if (productCategory != null) {
			try {
				...
				EntityFindOptions opts = new EntityFindOptions();
				opts.setMaxRows(viewSize);
				opts.setFetchSize(viewIndex);
				List<EntityCondition> exprs = FastList.newInstance();
				if(UtilValidate.isEmpty(childProductCategory)){
					exprs.add(EntityCondition.makeCondition("productCategoryId",
					...
					exprs.add(EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("productCategoryId",
							EntityOperator.EQUALS, productCategoryId), EntityCondition.makeCondition("productCategoryId", EntityOperator.IN, tmp)), EntityOperator.OR));
				}
				if (highPrice != BigDecimal.ZERO) {
					exprs.add(EntityCondition.makeCondition(
							"priceProductPriceTypeId", EntityOperator.EQUALS,
							"LIST_PRICE"));
					exprs.add(EntityCondition.makeCondition("pricePrice",
				...
				if (introductionDateLimit != null) {
					exprs.add(EntityCondition.makeCondition(EntityCondition
							.makeCondition("introductionDate",
				...
				productCategoryMembers = delegator.findList(entityName,
						EntityCondition
								.makeCondition(exprs, EntityOperator.AND),
						null, orderByFields, opts, false);
				if (activeOnly) {
					productCategoryMembers = EntityUtil.filterByDate(
							productCategoryMembers, true);
				}
				...
				if (UtilValidate.isNotEmpty(viewProductCategoryId)) {
					productCategoryMembers = CategoryWorker
							.filterProductsInCategory(delegator,
	...
	public static void getChildCategoryTree(HttpServletRequest request,
		...
		List<String> sortList = org.ofbiz.base.util.UtilMisc.toList(
				"sequenceNum", "title");
			...
			if (UtilValidate.isNotEmpty(category)) {
				if (isCatalog.equals("true") && isCategoryType.equals("false")) {
					CategoryWorker.getRelatedCategories(request,
							"ChildCatalogList", CatalogWorker
									.getCatalogTopCategoryId(request,
											productCategoryId), true);
					childOfCats = EntityUtil
							.filterByDate((List<GenericValue>) request
									.getAttribute("ChildCatalogList"));

				} else if (isCatalog.equals("false")
						&& isCategoryType.equals("false")) {
					childOfCats = EntityUtil.filterByDate(delegator.findByAnd(
							"ProductCategoryRollupAndChild", UtilMisc.toMap(
									"parentProductCategoryId",
									productCategoryId), null, false));
					childOfCats = EntityUtil.filterByCondition(childOfCats, EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr()));
				} else {
					childOfCats = EntityUtil.filterByDate(delegator.findByAnd(
							"ProdCatalogCategory",
				...
				if (UtilValidate.isNotEmpty(childOfCats)) {
					...
					for (GenericValue childOfCat : childOfCats) {
						...
						childList = EntityUtil.filterByDate(delegator
								.findByAnd("ProductCategoryRollup",
										UtilMisc.toMap(
												"parentProductCategoryId",
												catId), null, false));
						childList = EntityUtil.filterByCondition(childList, EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr()));
						// Get the chosen category information for the
						// categoryContentWrapper
						GenericValue cate = delegator.findOne(
								"ProductCategory",
						...
						String title = null;
						if (UtilValidate.isNotEmpty(categoryContentWrapper
								.get(catNameField))) {
						...
						josonMap.put("sequenceNum",
								childOfCat.get("sequenceNum"));
						josonMap.put("title", title);

						categoryList.add(josonMap);
					}
					List<Map<Object, Object>> sortedCategoryList = UtilMisc
							.sortMaps(categoryList, sortList);
	...
	public static void toJsonObjectList(List attrList,
			HttpServletResponse response) {
	...
	public static Map<String, Object> getProductsInCategory(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        String productCategoryId = (String) context.get("productCategoryId");
        Timestamp introductionDateLimit = (Timestamp) context.get("introductionDateLimit");
        Timestamp releaseDateLimit = (Timestamp) context.get("releaseDateLimit");
        ...
        if (productCategory != null) {
            try {
                if (useCacheForMembers) {
                    ...
                    productCategoryMembers = delegator.findList(entityName, EntityCondition.makeCondition(tmpCond), null, orderByFields, null, true);
                    if (activeOnly) {
                    ...
                    if(UtilValidate.isEmpty(childProductCategory)){
						mainCondList.add(EntityCondition.makeCondition("productCategoryId",
										EntityOperator.EQUALS, productCategoryId));
					}else{
						List<String> tmp = FastList.newInstance();
						for(GenericValue e : childProductCategory){
						...
						mainCondList.add(EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("productCategoryId",
								EntityOperator.EQUALS, productCategoryId), EntityCondition.makeCondition("productCategoryId", EntityOperator.IN, tmp)), EntityOperator.OR));
					}
                    if (activeOnly) {
                    ...
                    // using list iterator
                    pli = delegator.find(entityName, mainCond, null, null, orderByFields, findOpts);
