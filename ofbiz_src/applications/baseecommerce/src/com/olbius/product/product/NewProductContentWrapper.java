 ...
 */
public class NewProductContentWrapper implements ContentWrapper {

    public static final String module = NewProductContentWrapper.class.getName();
    ...
    public static NewProductContentWrapper makeProductContentWrapper(GenericValue product, HttpServletRequest request, String webSiteId) {
        return new NewProductContentWrapper(product, request, webSiteId);
    }
    ...
    public NewProductContentWrapper(LocalDispatcher dispatcher, GenericValue product, Locale locale, String mimeTypeId) {
    ...
    public NewProductContentWrapper(GenericValue product, HttpServletRequest request, String webSiteId) {
        this.dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        this.product = product;
        this.locale = UtilHttp.getLocale(request);
        this.mimeTypeId = "text/html";
        NewProductContentWrapper.webSiteId = webSiteId;
    }

    public StringUtil.StringWrapper get(String productContentTypeId) {
        if (this.product == null) {
            Debug.logWarning("Tried to get ProductContent for type [" + productContentTypeId + "] but the product field in the NewProductContentWrapper is null", module);
            return null;
        }
    ...
    public static void getProductContentAsText(String productId, GenericValue product, String productContentTypeId, Locale locale, String mimeTypeId, String partyId, String roleTypeId, Delegator delegator, LocalDispatcher dispatcher, Writer outWriter) throws GeneralException, IOException {
        ...
        List<GenericValue> productContentList = delegator.findByAnd("ProductAndContent", UtilMisc.toMap("productId", productId, "productContentTypeId", productContentTypeId, "webSiteId", webSiteId), UtilMisc.toList("-fromDate"), true);
        ...
        if (UtilValidate.isEmpty(productContentList) && ("Y".equals(product.getString("isVariant")))) {
            ...
            if (UtilValidate.isNotEmpty(parent)) {
                productContentList = delegator.findByAnd("ProductAndContent", UtilMisc.toMap("productId", parent.get("productId"), "productContentTypeId", productContentTypeId, "webSiteId", webSiteId), UtilMisc.toList("-fromDate"), true);
                productContentList = EntityUtil.filterByDate(productContentList);
            }
        }
