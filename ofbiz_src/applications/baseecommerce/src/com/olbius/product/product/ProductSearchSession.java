...
import com.olbius.product.product.ProductSearch.CategoryConstraint;
import com.olbius.product.product.ProductSearch.FeatureConstraint;
import com.olbius.product.product.ProductSearch.KeywordConstraint;
import com.olbius.product.product.ProductSearch.ProductSearchConstraint;
import com.olbius.product.product.ProductSearch.ProductSearchContext;
import com.olbius.product.product.ProductSearch.ResultSortOrder;
import com.olbius.product.product.ProductSearch.SortKeywordRelevancy;
/**
 * Utility class with methods to prepare and perform ProductSearch operations in
 * the content of an HttpSession
 */
public class ProductSearchSession {
	...
	public static class ProductSearchOptions implements java.io.Serializable {
		...
		public static List<ProductSearchConstraint> getConstraintList(
				HttpSession session) {
			return getProductSearchOptions(session).constraintList;
		}

		public static void addConstraint(
				ProductSearchConstraint productSearchConstraint,
				HttpSession session) {
			ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
			if (productSearchOptions.constraintList == null) {
				productSearchOptions.constraintList = FastList.newInstance();
			}
			if (!productSearchOptions.constraintList
					.contains(productSearchConstraint)) {
				productSearchOptions.constraintList
						.add(productSearchConstraint);
		...
		public static ResultSortOrder getResultSortOrder(
		...
		public static void setResultSortOrder(ResultSortOrder resultSortOrder,
				HttpSession session) {
			ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
		...
		public void setViewIndex(String viewIndexStr) {
			...
			try {
				...
				Debug.logError(e, "Error in formatting of VIEW_INDEX ["
						+ viewIndexStr + "], setting to 20", module);
		...
		public void setViewSize(String viewSizeStr) {
			...
			try {
				...
				Debug.logError(e, "Error in formatting of VIEW_SIZE ["
						+ viewSizeStr + "], setting to 20", module);
		...
		public void setTopProductCategoryId(String topProductCategoryId) {
			if (this.topProductCategoryId != null
					&& topProductCategoryId != null) {
		...
		public List<String> searchGetConstraintStrings(boolean detailed,
				Delegator delegator, Locale locale) {
			List<ProductSearchConstraint> productSearchConstraintList = this
					.getConstraintList();
			...
			for (ProductSearchConstraint productSearchConstraint : productSearchConstraintList) {
				if (productSearchConstraint == null)
					continue;
				String constraintString = productSearchConstraint
						.prettyPrintConstraint(delegator, detailed, locale);
	...
	public static ProductSearchOptions getProductSearchOptions(
			HttpSession session) {
		ProductSearchOptions productSearchOptions = (ProductSearchOptions) session
				.getAttribute("_PRODUCT_SEARCH_OPTIONS_CURRENT_");
		if (productSearchOptions == null) {
			productSearchOptions = new ProductSearchOptions();
			session.setAttribute("_PRODUCT_SEARCH_OPTIONS_CURRENT_",
	...
	public static void checkSaveSearchOptionsHistory(HttpSession session) {
		ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
		// if the options have changed since the last search, add it to the
		// beginning of the search options history
		if (productSearchOptions.changed) {
			List<ProductSearchOptions> optionsHistoryList = getSearchOptionsHistoryList(session);
			optionsHistoryList.add(0, new ProductSearchOptions(
					productSearchOptions));
			productSearchOptions.changed = false;
		}
	}

	public static List<ProductSearchOptions> getSearchOptionsHistoryList(
			HttpSession session) {
		List<ProductSearchOptions> optionsHistoryList = UtilGenerics
				.checkList(session
						.getAttribute("_PRODUCT_SEARCH_OPTIONS_HISTORY_"));
		if (optionsHistoryList == null) {
			optionsHistoryList = FastList.newInstance();
			session.setAttribute("_PRODUCT_SEARCH_OPTIONS_HISTORY_",
					optionsHistoryList);
		}
	...
	public static void setCurrentSearchFromHistory(int index,
			boolean removeOld, HttpSession session) {
		...
		if (index < searchOptionsHistoryList.size()) {
			ProductSearchOptions productSearchOptions = searchOptionsHistoryList
					.get(index);
			if (removeOld) {
			...
			if (productSearchOptions != null) {
				session.setAttribute("_PRODUCT_SEARCH_OPTIONS_CURRENT_",
						new ProductSearchOptions(productSearchOptions));
			}
		} else {
			throw new IllegalArgumentException(
					"Could not set current search options to history index ["
							+ index + "], only ["
							+ searchOptionsHistoryList.size()
							+ "] entries in the history list.");
		}
	}

	public static String clearSearchOptionsHistoryList(
	...
	public static String setCurrentSearchFromHistory(
			HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		String searchHistoryIndexStr = request
				.getParameter("searchHistoryIndex");
		String removeOldStr = request.getParameter("removeOld");

		if (UtilValidate.isEmpty(searchHistoryIndexStr)) {
			request.setAttribute("_ERROR_MESSAGE_",
					"No search history index passed, cannot set current search to previous.");
			return "error";
		}
	...
	/**
	 * A ControlServlet event method used to check to see if there is an
	 * override for any of the current keywords in the search
	 */
	public static final String checkDoKeywordOverride(
			HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		...
		if (productStoreId != null) {
			...
			List<ProductSearchConstraint> constraintList = ProductSearchOptions
					.getConstraintList(session);
			if (constraintList != null) {
				for (ProductSearchConstraint constraint : constraintList) {
					if (constraint instanceof KeywordConstraint) {
						...
						Set<String> keywordSet = keywordConstraint
								.makeFullKeywordSet(delegator);
						if (keywordSet != null)
			...
			if (keywords.size() > 0) {
				...
				try {
					productStoreKeywordOvrdList = delegator.findByAnd(
							"ProductStoreKeywordOvrd",
							UtilMisc.toMap("productStoreId", productStoreId),
							UtilMisc.toList("-fromDate"), true);
					productStoreKeywordOvrdList = EntityUtil.filterByDate(
							productStoreKeywordOvrdList, true);
				} catch (GenericEntityException e) {
					Debug.logError(
							e,
							"Error reading ProductStoreKeywordOvrd list, not doing keyword override",
							module);
				}
				...
				if (UtilValidate.isNotEmpty(productStoreKeywordOvrdList)) {
					for (GenericValue productStoreKeywordOvrd : productStoreKeywordOvrdList) {
						String ovrdKeyword = productStoreKeywordOvrd
								.getString("keyword");
						if (keywords.contains(ovrdKeyword)) {
							String targetTypeEnumId = productStoreKeywordOvrd
									.getString("targetTypeEnumId");
							String target = productStoreKeywordOvrd
									.getString("target");
							ServletContext ctx = (ServletContext) request
									.getAttribute("servletContext");
							RequestHandler rh = (RequestHandler) ctx
							...
							if ("KOTT_PRODCAT".equals(targetTypeEnumId)) {
								...
								String requestName = "/product/~product_id="
										+ target;
								target = rh.makeLink(request, response,
										requestName, false, false, false);
							} else if ("KOTT_OFBURL".equals(targetTypeEnumId)) {
								target = rh.makeLink(request, response, target,
										false, false, false);
							} else if ("KOTT_AURL".equals(targetTypeEnumId)) {
								// do nothing, is absolute URL
							} else {
								Debug.logError(
										"The targetTypeEnumId [] is not recognized, not doing keyword override",
										module);
							...
							try {
								...
								Debug.logError(
										e,
										"Could not send redirect to: " + target,
										module);
								continue;
							}
						}
	...
	public static ArrayList<String> searchDo(HttpSession session,
			Delegator delegator, String prodCatalogId) {
		String visitId = VisitHandler.getVisitId(session);
		ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
		List<ProductSearchConstraint> productSearchConstraintList = productSearchOptions
				.getConstraintList();
		...
		ResultSortOrder resultSortOrder = productSearchOptions
				.getResultSortOrder();

		// if the search options have changed since the last search, put at the
		// beginning of the options history list
		checkSaveSearchOptionsHistory(session);

		return ProductSearch.searchProducts(productSearchConstraintList,
	...
	public static List<String> searchGetConstraintStrings(boolean detailed,
			HttpSession session, Delegator delegator) {
		Locale locale = UtilHttp.getLocale(session);
		ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
		return productSearchOptions.searchGetConstraintStrings(detailed,
				delegator, locale);
	}
	...
	public static String searchGetSortOrderString(boolean detailed,
		...
		ResultSortOrder resultSortOrder = ProductSearchOptions
				.getResultSortOrder(request);
		if (resultSortOrder == null)
			return "";
		return resultSortOrder.prettyPrintSortOrder(detailed, locale);
	}

	public static void searchSetSortOrder(ResultSortOrder resultSortOrder,
			HttpSession session) {
		ProductSearchOptions.setResultSortOrder(resultSortOrder, session);
	}

	public static void searchAddFeatureIdConstraints(
			Collection<List<String>> featureIds, Boolean exclude,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (UtilValidate.isEmpty(featureIds)) {
			return;
		}
		for (List<String> productFeatureId : featureIds) {
			for (String strTmp : productFeatureId) {
				searchAddConstraint(new FeatureConstraint(strTmp, exclude),
						session);
			}
		}
	}

	public static void searchAddConstraint(
			ProductSearchConstraint productSearchConstraint, HttpSession session) {
	...
	public static void processSearchParameters(Map<String, Object> parameters,
			HttpServletRequest request) {
		Delegator delegator = (Delegator) request.getAttribute("delegator");
		Boolean alreadyRun = (Boolean) request
				.getAttribute("processSearchParametersAlreadyRun");
		if (Boolean.TRUE.equals(alreadyRun)) {
			// even if already run, check the VIEW_SIZE and VIEW_INDEX again,
			// just for kicks
			ProductSearchOptions productSearchOptions = getProductSearchOptions(request
					.getSession());
			productSearchOptions.setViewIndex((String) parameters
					.get("VIEW_INDEX"));
			productSearchOptions.setViewSize((String) parameters
					.get("VIEW_SIZE"));
		...
		// clear search? by default yes, but if the clearSearch parameter is N
		// then don't
		...
		if (!"N".equals(clearSearchString)) {
			...
			String removeConstraint = (String) parameters
					.get("removeConstraint");
			if (UtilValidate.isNotEmpty(removeConstraint)) {
				try {
					searchRemoveConstraint(Integer.parseInt(removeConstraint),
							session);
					constraintsChanged = true;
				} catch (Exception e) {
					Debug.logError(e, "Error removing constraint ["
							+ removeConstraint + "]", module);
				}
			}
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("PRIORITIZE_CATEGORY_ID"))) {
			prioritizeCategoryId = (String) parameters
					.get("PRIORITIZE_CATEGORY_ID");
		...
		if (UtilValidate.isNotEmpty(prioritizeCategoryId)) {
			ProductSearchOptions.setTopProductCategoryId(prioritizeCategoryId,
					session);
			constraintsChanged = true;
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("SEARCH_CATEGORY_ID"))) {
			String searchCategoryId = (String) parameters
					.get("SEARCH_CATEGORY_ID");
			String searchSubCategories = (String) parameters
					.get("SEARCH_SUB_CATEGORIES");
			String searchCategoryExc = (String) parameters
					.get("SEARCH_CATEGORY_EXC");
			Boolean exclude = UtilValidate.isEmpty(searchCategoryExc) ? null
					: Boolean.valueOf(!"N".equals(searchCategoryExc));
			searchAddConstraint(
					new ProductSearch.CategoryConstraint(searchCategoryId,
							!"N".equals(searchSubCategories), exclude), session);
			constraintsChanged = true;
		}
		...
		for (int catNum = 1; catNum < 10; catNum++) {
			if (UtilValidate.isNotEmpty(parameters.get("SEARCH_CATEGORY_ID"
					+ catNum))) {
				String searchCategoryId = (String) parameters
						.get("SEARCH_CATEGORY_ID" + catNum);
				String searchSubCategories = (String) parameters
						.get("SEARCH_SUB_CATEGORIES" + catNum);
				String searchCategoryExc = (String) parameters
						.get("SEARCH_CATEGORY_EXC" + catNum);
				Boolean exclude = UtilValidate.isEmpty(searchCategoryExc) ? null
						: Boolean.valueOf(!"N".equals(searchCategoryExc));
				searchAddConstraint(new ProductSearch.CategoryConstraint(
						searchCategoryId, !"N".equals(searchSubCategories),
						exclude), session);
				constraintsChanged = true;
			}
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("SEARCH_CATALOG_ID"))) {
			String searchCatalogId = (String) parameters
					.get("SEARCH_CATALOG_ID");
			if (searchCatalogId != null
					&& !searchCatalogId.equalsIgnoreCase("")) {
				String topCategory = NewCatalogWorker.getCatalogTopCategoryId(
						request, searchCatalogId);
				if (UtilValidate.isEmpty(topCategory)) {
					topCategory = NewCatalogWorker.getCatalogTopEbayCategoryId(
							request, searchCatalogId);
				}
				List<GenericValue> categories = CategoryWorker
						.getRelatedCategoriesRet(request, "topLevelList",
								topCategory, true, false, true);
				searchAddConstraint(new ProductSearch.CatalogConstraint(
						searchCatalogId, categories), session);
				constraintsChanged = true;
			}
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("SEARCH_STRING"))) {
			...
			boolean anyPrefixSuffix = !"N".equals(parameters
					.get("SEARCH_ANYPRESUF"));
			searchAddConstraint(new ProductSearch.KeywordConstraint(
					keywordString, anyPrefixSuffix, anyPrefixSuffix, null,
		...
		if (UtilValidate.isNotEmpty(parameters.get("SEARCH_PRODUCT_NAME"))) {
			...
			searchAddConstraint(new ProductSearch.ProductFieldConstraint(
		...
		if (UtilValidate
				.isNotEmpty(parameters.get("SEARCH_INTERNAL_PROD_NAME"))) {
			String internalName = (String) parameters
					.get("SEARCH_INTERNAL_PROD_NAME");
			searchAddConstraint(new ProductSearch.ProductFieldConstraint(
					internalName, "internalName"), session);
			constraintsChanged = true;
		}

		for (int kwNum = 1; kwNum < 10; kwNum++) {
			if (UtilValidate
					.isNotEmpty(parameters.get("SEARCH_STRING" + kwNum))) {
				String keywordString = (String) parameters.get("SEARCH_STRING"
						+ kwNum);
				String searchOperator = (String) parameters
						.get("SEARCH_OPERATOR" + kwNum);
				// defaults to true/Y, ie anything but N is true/Y
				boolean anyPrefixSuffix = !"N".equals(parameters
						.get("SEARCH_ANYPRESUF" + kwNum));
				searchAddConstraint(new ProductSearch.KeywordConstraint(
						keywordString, anyPrefixSuffix, anyPrefixSuffix, null,
						"AND".equals(searchOperator)), session);
				constraintsChanged = true;
		...
		for (String parameterName : parameters.keySet()) {
			if (parameterName.startsWith("SEARCH_FEAT")
				...
				String productFeatureId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureId)) {
					String paramNameExt = parameterName.substring("SEARCH_FEAT"
							.length());
					String searchCategoryExc = (String) parameters
							.get("SEARCH_FEAT_EXC" + paramNameExt);
					Boolean exclude = UtilValidate.isEmpty(searchCategoryExc) ? null
							: Boolean.valueOf(!"N".equals(searchCategoryExc));
					// Debug.logInfo("parameterName=" + parameterName +
					// ", paramNameExt=" + paramNameExt + ", searchCategoryExc="
					// + searchCategoryExc + ", exclude=" + exclude, module);
					searchAddConstraint(new ProductSearch.FeatureConstraint(
							productFeatureId, exclude), session);
					constraintsChanged = true;
				}
			}
			...
			if (parameterName.startsWith("S_PFI")) {
				String productFeatureId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureId)) {
					String paramNameExt = parameterName.substring("S_PFI"
							.length());
			...
			// if product features category were selected add a constraint for
			// each
			if (parameterName.startsWith("SEARCH_PROD_FEAT_CAT")
					&& !parameterName.startsWith("SEARCH_PROD_FEAT_CAT_EXC")) {
				String productFeatureCategoryId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureCategoryId)) {
					String paramNameExt = parameterName
							.substring("SEARCH_PROD_FEAT_CAT".length());
					String searchProdFeatureCategoryExc = (String) parameters
							.get("SEARCH_PROD_FEAT_CAT_EXC" + paramNameExt);
					Boolean exclude = UtilValidate
							.isEmpty(searchProdFeatureCategoryExc) ? null
							: Boolean.valueOf(!"N"
									.equals(searchProdFeatureCategoryExc));
					searchAddConstraint(
							new ProductSearch.FeatureCategoryConstraint(
									productFeatureCategoryId, exclude), session);
					constraintsChanged = true;
				}
			}
			...
			if (parameterName.startsWith("S_FCI")) {
				String productFeatureCategoryId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureCategoryId)) {
					String paramNameExt = parameterName.substring("S_FCI"
							.length());
					String searchProdFeatureCategoryExc = (String) parameters
							.get("S_FCX" + paramNameExt);
					Boolean exclude = UtilValidate
							.isEmpty(searchProdFeatureCategoryExc) ? null
							: Boolean.valueOf(!"N"
									.equals(searchProdFeatureCategoryExc));
			...
			if (parameterName.startsWith("SEARCH_PROD_FEAT_GRP")
					&& !parameterName.startsWith("SEARCH_PROD_FEAT_GRP_EXC")) {
				String productFeatureGroupId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureGroupId)) {
					String paramNameExt = parameterName
							.substring("SEARCH_PROD_FEAT_GRP".length());
					String searchProdFeatureGroupExc = (String) parameters
							.get("SEARCH_PROD_FEAT_GRP_EXC" + paramNameExt);
					Boolean exclude = UtilValidate
							.isEmpty(searchProdFeatureGroupExc) ? null
							: Boolean.valueOf(!"N"
									.equals(searchProdFeatureGroupExc));
					searchAddConstraint(
							new ProductSearch.FeatureGroupConstraint(
									productFeatureGroupId, exclude), session);
					constraintsChanged = true;
				}
			}
			...
			if (parameterName.startsWith("S_FGI")) {
				String productFeatureGroupId = (String) parameters
						.get(parameterName);
				if (UtilValidate.isNotEmpty(productFeatureGroupId)) {
					String paramNameExt = parameterName.substring("S_FGI"
							.length());
					String searchProdFeatureGroupExc = (String) parameters
							.get("S_FGX" + paramNameExt);
					Boolean exclude = UtilValidate
							.isEmpty(searchProdFeatureGroupExc) ? null
							: Boolean.valueOf(!"N"
									.equals(searchProdFeatureGroupExc));
		...
		Map<String, List<String>> featureIdByType = ParametricSearch
				.makeFeatureIdByTypeMap(parameters);
		...
		// Filter by pricing
		if (UtilValidate.isNotEmpty(parameters.get("MINPRICE"))
				|| UtilValidate.isNotEmpty(parameters.get("MAXPRICE"))) {
			BigDecimal minPrice = new BigDecimal((String) parameters.get("MINPRICE"));
			BigDecimal maxPrice = new BigDecimal((String) parameters.get("MAXPRICE"));
			searchAddConstraint(new ProductSearch.ListPriceRangeConstraint(minPrice,maxPrice,
					"USD"), session);
			constraintsChanged = true;
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("SEARCH_SUPPLIER_ID"))
			...
			String supplierPartyId = (String) parameters
					.get("SEARCH_SUPPLIER_ID");
			if (UtilValidate.isEmpty(supplierPartyId))
				supplierPartyId = (String) parameters.get("S_SUP");
			searchAddConstraint(new ProductSearch.SupplierConstraint(
					supplierPartyId), session);
			constraintsChanged = true;
		}
		...
		if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_LOW"))
			...
			if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_LOW"))) {
				try {
					listPriceLow = new BigDecimal(
							(String) parameters.get("LIST_PRICE_LOW"));
			...
			if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_HIGH"))) {
				try {
					listPriceHigh = new BigDecimal(
							(String) parameters.get("LIST_PRICE_HIGH"));
				} catch (NumberFormatException e) {
					Debug.logError("Error parsing LIST_PRICE_HIGH parameter ["
							+ (String) parameters.get("LIST_PRICE_HIGH")
							+ "]: " + e.toString(), module);
				}
			}
			searchAddConstraint(new ProductSearch.ListPriceRangeConstraint(
					listPriceLow, listPriceHigh, listPriceCurrency), session);
			constraintsChanged = true;
		}
		if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_RANGE"))
				|| UtilValidate.isNotEmpty(parameters.get("S_LPR"))) {
			String listPriceRangeStr = (String) parameters
					.get("LIST_PRICE_RANGE");
			...
			if (underscoreIndex >= 0) {
				...
				listPriceHighStr = listPriceRangeStr
						.substring(listPriceRangeStr.indexOf("_") + 1);
			} else {
			...
			if (UtilValidate.isNotEmpty(listPriceLowStr)) {
				try {
					...
					Debug.logError(
							"Error parsing low part of LIST_PRICE_RANGE parameter ["
									+ listPriceLowStr + "]: " + e.toString(),
							module);
				}
			}
			if (UtilValidate.isNotEmpty(listPriceHighStr)) {
				try {
					...
					Debug.logError(
							"Error parsing high part of LIST_PRICE_RANGE parameter ["
									+ listPriceHighStr + "]: " + e.toString(),
							module);
				}
			}
			searchAddConstraint(new ProductSearch.ListPriceRangeConstraint(
		...
		if (productStore != null
				&& !"N".equals(productStore
						.getString("prodSearchExcludeVariants"))) {
			searchAddConstraint(new ProductSearch.ExcludeVariantsConstraint(),
					session);
			// not consider this a change for now, shouldn't change often:
			// constraintsChanged = true;
		}

		if ("true".equalsIgnoreCase((String) parameters
				.get("AVAILABILITY_FILTER"))) {
			searchAddConstraint(new ProductSearch.AvailabilityDateConstraint(),
					session);
			constraintsChanged = true;
		}

		if (UtilValidate.isNotEmpty(parameters
				.get("SEARCH_GOOD_IDENTIFICATION_TYPE"))
				|| UtilValidate.isNotEmpty(parameters
						.get("SEARCH_GOOD_IDENTIFICATION_VALUE"))) {
			String include = (String) parameters
					.get("SEARCH_GOOD_IDENTIFICATION_INCL");
			...
			searchAddConstraint(
					new ProductSearch.GoodIdentificationConstraint(
							(String) parameters
									.get("SEARCH_GOOD_IDENTIFICATION_TYPE"),
							(String) parameters
									.get("SEARCH_GOOD_IDENTIFICATION_VALUE"),
		...
		String prodCatalogId = NewCatalogWorker.getCurrentCatalogId(request);
		String viewProductCategoryId = NewCatalogWorker
				.getCatalogViewAllowCategoryId(delegator, prodCatalogId);
		if (UtilValidate.isNotEmpty(viewProductCategoryId)) {
			ProductSearchConstraint viewAllowConstraint = new CategoryConstraint(
					viewProductCategoryId, true, null);
		...
		if (sortOrder != null) {
			if (sortOrder.equals("SortKeywordRelevancy")
				...
				String fieldName = sortOrder.substring("SortProductField:"
						.length());
				searchSetSortOrder(new ProductSearch.SortProductField(
						fieldName, ascending), session);
				...
				String priceTypeId = sortOrder.substring("SortProductPrice:"
						.length());
				searchSetSortOrder(new ProductSearch.SortProductPrice(
						priceTypeId, ascending), session);
				...
				String featureId = sortOrder.substring("SortProductFeature:"
						.length());
				searchSetSortOrder(new ProductSearch.SortProductFeature(
						featureId, ascending), session);
		...
		productSearchOptions
				.setViewIndex((String) parameters.get("VIEW_INDEX"));
		productSearchOptions.setViewSize((String) parameters.get("VIEW_SIZE"));
		productSearchOptions.setPaging((String) parameters.get("PAGING"));
	}

	@SuppressWarnings("unchecked")
	public static Map<String, Object> getProductSearchResult(
			HttpServletRequest request, Delegator delegator,
		...
		String addOnTopProdCategoryId = productSearchOptions
		...
		Integer previousViewSizeInteger = productSearchOptions
				.getPreviousViewSize();
		...
		// if noConditionFind to Y then find without conditions otherwise search
		// according to constraints.
		if ("Y".equals(noConditionFind)
				|| UtilValidate.isNotEmpty(productSearchConstraintList)) {
			// if the search options have changed since the last search, put at
			// the beginning of the options history list
			checkSaveSearchOptionsHistory(session);

			int addOnTopTotalListSize = 0;
			int addOnTopListSize = 0;
			List<GenericValue> addOnTopProductCategoryMembers = FastList
					.newInstance();
			if (UtilValidate.isNotEmpty(addOnTopProdCategoryId)) {
				// always include the members of the addOnTopProdCategoryId
				Timestamp now = UtilDateTime.nowTimestamp();
				List<EntityCondition> addOnTopProdCondList = FastList
						.newInstance();
				addOnTopProdCondList.add(EntityCondition.makeCondition(
						EntityCondition.makeCondition("thruDate",
								EntityOperator.EQUALS, null),
						EntityOperator.OR, EntityCondition.makeCondition(
								"thruDate", EntityOperator.GREATER_THAN, now)));
				addOnTopProdCondList.add(EntityCondition.makeCondition(
						"fromDate", EntityOperator.LESS_THAN, now));
				addOnTopProdCondList.add(EntityCondition.makeCondition(
						"productCategoryId", EntityOperator.EQUALS,
						addOnTopProdCategoryId));
				...
				try {
					pli = delegator.find("ProductCategoryMember",
							EntityCondition.makeCondition(addOnTopProdCondList,
									EntityOperator.AND), null, UtilMisc.toSet(
									"productId", "sequenceNum"), UtilMisc
									.toList("sequenceNum"), findOpts);
					addOnTopProductCategoryMembers = pli.getPartialList(
							lowIndex, viewSize);
					...
					for (GenericValue alwaysAddProductCategoryMember : addOnTopProductCategoryMembers) {
						productIds.add(alwaysAddProductCategoryMember
								.getString("productId"));
					}
					addOnTopTotalListSize = pli.getResultsTotalSize();
			...
			if (UtilValidate.isNotEmpty(productSearchConstraintList)) {
				productSearchContext
						.addProductSearchConstraints(productSearchConstraintList);
			}
			...
			if (UtilValidate.isNotEmpty(keywordTypeIds)) {
				...
				productSearchContext.keywordTypeIds = UtilMisc
						.toList("KWT_KEYWORD");
			}
		...
		List<String> searchConstraintStrings = searchGetConstraintStrings(
				false, session, delegator);
	...
	public static String makeSearchParametersString(
		...
		for (ProductSearchConstraint psc : constraintList) {
			if (psc instanceof ProductSearch.CategoryConstraint) {
				...
				if (cc.exclude != null) {
					...
					searchParamString.append(categoriesCount);
					searchParamString.append("=");
					searchParamString.append(cc.exclude.booleanValue() ? "Y"
							: "N");
				}
				 ...
				 * until later } else if (psc instanceof
				...
				if (pfcc.exclude != null) {
					...
					searchParamString.append(featureCategoriesCount);
					searchParamString.append("=");
					searchParamString.append(pfcc.exclude.booleanValue() ? "Y"
							: "N");
				}
				...
				if (pfgc.exclude != null) {
					...
					searchParamString.append(featureGroupsCount);
					searchParamString.append("=");
					searchParamString.append(pfgc.exclude.booleanValue() ? "Y"
							: "N");
				}
				...
				searchParamString.append("=");
				searchParamString.append(UtilHttp
						.encodeBlanks(kc.keywordsString));
				...
				searchParamString.append(keywordsCount);
				searchParamString.append("=");
				searchParamString.append(kc.anyPrefix | kc.anySuffix ? "Y"
						: "N");
		...
		String topProductCategoryId = productSearchOptions
				.getTopProductCategoryId();
	 ...
	 * productFeatureId's of input productFeatureTypeId, the constraint being
	 * applied on current ProductSearchConstraint list in session.
	 *
	 * @param productFeatureTypeId
	 *            The productFeatureTypeId, productFeatureId's of which should
	 *            be considered.
	 * @param session
	 *            Current session.
	 * @param delegator
	...
	public static List<Map<String, String>> listCountByFeatureForType(
			String productFeatureTypeId, HttpSession session,
			Delegator delegator) {
		String visitId = VisitHandler.getVisitId(session);
		...
		entityConditionList.add(EntityCondition.makeCondition(EntityCondition
				.makeCondition("pfacThruDate", EntityOperator.EQUALS, null),
				EntityOperator.OR, EntityCondition.makeCondition(
						"pfacThruDate", EntityOperator.GREATER_THAN,
		...
		entityConditionList.add(EntityCondition.makeCondition(
				"pfcProductFeatureTypeId", EntityOperator.EQUALS,
		...
		try {
			eli = delegator.findListIteratorByCondition(dynamicViewEntity,
					whereCondition, null, fieldsToSelect,
					productSearchContext.orderByList, efo);
		} catch (GenericEntityException e) {
		...
		while ((searchResult = eli.next()) != null) {
			featureCountList.add(UtilMisc.<String, String> toMap(
					"productFeatureId",
					(String) searchResult.get("pfacProductFeatureId"),
					"productFeatureTypeId",
		...
		if (eli != null) {
			try {
				...
				Debug.logError(e,
						"Error closing ProductSearch EntityListIterator");
			}
		}
	 ...
	 * constraint being applied on current ProductSearchConstraint list in
	 * session.
	 *
	 * @param priceLow
	 *            The low price.
	...
	public static long getCountForListPriceRange(BigDecimal priceLow,
		...
		dynamicViewEntity.addAlias("PPC", "ppcPrice", "price", null, null,
				null, null);
		dynamicViewEntity.addAlias("PPC", "priceRangeCount", "productId", null,
				null, null, "count-distinct");
		dynamicViewEntity.addViewLink("PROD", "PPC", Boolean.FALSE,
				ModelKeyMap.makeKeyMapList("productId"));
		fieldsToSelect.add("priceRangeCount");
		entityConditionList.add(EntityCondition.makeCondition(EntityCondition
				.makeCondition("ppcThruDate", EntityOperator.EQUALS, null),
				EntityOperator.OR, EntityCondition.makeCondition("ppcThruDate",
		...
		entityConditionList.add(EntityCondition.makeCondition(
				"ppcProductPriceTypeId", EntityOperator.EQUALS, "LIST_PRICE"));
	...
	/**
	 * This method returns count of products in a given category (including all
	 * sub categories), the constraint being applied on current
	 * ProductSearchConstraint list in session.
	 *
	...
	public static long getCountForProductCategory(String productCategoryId,
		...
		entityConditionList.add(EntityCondition.makeCondition(EntityCondition
				.makeCondition("pcmcThruDate", EntityOperator.EQUALS, null),
				EntityOperator.OR, EntityCondition.makeCondition(
						"pcmcThruDate", EntityOperator.GREATER_THAN,
		...
		ProductSearch.getAllSubCategoryIds(productCategoryId,
				productCategoryIdSet, delegator,
				productSearchContext.nowTimestamp);
		entityConditionList.add(EntityCondition.makeCondition(
				"pcmcProductCategoryId", EntityOperator.IN,
