...
public class ProductSearch {
    ...
    public static class ProductSearchContext {
        ...
        public void finishCategoryAndFeatureConstraints() {
            ...
            if (includeFeatureIds.size() > 0) {
                for (String includeFeatureId: includeFeatureIds) {
                    ...
                    if(includeFeatureIds.size() > 1){
			Set<String> tmpSet = FastSet.newInstance();
			String[] strArrStrings = includeFeatureIds.toArray(new String[includeFeatureIds.size()]);
			for(int tmpint = 0; tmpint < strArrStrings.length;tmpint++){
				tmpSet.add(strArrStrings[tmpint]);
				this.index++;
                        }
			incExcCondList.add(EntityCondition.makeCondition(featurePrefix + "ProductFeatureId", EntityOperator.IN, tmpSet));
                    }else{
        ...
        public ArrayList<String> makeProductIdList(EntityListIterator eli) {
            ...
            try {
                ...
                if (searchResult != null) {
                    this.totalResults = eli.getResultsTotalSize();
                }
    ...
    public static class SupplierConstraint extends ProductSearchConstraint {
        ...
        public void addConstraint(ProductSearchContext productSearchContext) {
            ...
            productSearchContext.dynamicViewEntity.addMemberEntity(entityAlias, "SFSupplierProduct");
