...
searchCategoryId = parameters.category_id;
if (!searchCategoryId) {
    searchCategoryId = context.productCategoryId;
}
if (searchCategoryId) {
    currentSearchCategory = delegator.findOne("ProductCategory", [productCategoryId: searchCategoryId], true);
    CategoryWorker.getRelatedCategories(request, "subCategoryList", searchCategoryId, true);
...
if (productCategoryId)  {
   context.productCategory = delegator.findOne("ProductCategory", [productCategoryId: productCategoryId], true);
...
prodCatalogId = NewCatalogWorker.getCurrentCatalogId(request);
...
if (searchCategoryId) {
    productCategoryRollups = delegator.findByAnd("ProductCategoryRollup", [productCategoryId: searchCategoryId], null, true);
...
colorFeatureType = delegator.findOne("ProductFeatureType", [productFeatureTypeId: "COLOR"], true);
if (colors) {
    colors.each { color ->
