...
String buildNext(Map map, List order, String current, String prefix, Map featureTypes) {
	...
	strFunction = "listFT" + current + prefix + "();";
...
if (userLogin) {
	...
	allShoppingLists = delegator.findList("ShoppingList", condition, null, ["listName"], null, true);
...
if (product) {
	...
	context.title = product.get("productName");
	...
	if (categoryId) {
		...
		if (ServiceUtil.isError(catNextPreviousResult)) {
			request.setAttribute("errorMessageList", [
				ServiceUtil.getErrorMessage(catNextPreviousResult)
			]);
			return;
		}
...
//	reviews = product.getRelated("ProductReview", reviewByAnd, ["-postedDateTime"], true);
				...
				if (variantTree && imageMap) {
					...
					if (variantTree) {
						...
						variantTree.each { key, value ->
							...
							contentWrapper = new NewProductContentWrapper(imageMap[key], request, webSiteId);
					...
					if (variants) {
						...
						variants.each { variantAssoc ->
							variant = variantAssoc.getRelatedOne("AssocProduct", true);
							...
							if(virtualVariants){
								virtualVariants.each { virtualAssoc ->
									virtual = virtualAssoc.getRelatedOne("MainProduct", true);
					...
					jsBuf.append("</script>");
					jsBuf.append("<script language=\"JavaScript\" type=\"text/javascript\">");
					jsBuf.append("	ja2 = jQuery.noConflict();");
					jsBuf.append("	ja2(document).ready(function() {");
					jsBuf.append(strFunction);
					jsBuf.append("	});");
					jsBuf.append("</script>");
	...
	if (isMarketingPackage) {
		...
		facilities = delegator.findList("ProductFacility", EntityCondition.makeCondition([productId : product.productId]), null, null, null, true)
		if(facilities) {
	...
	commonProductFeatureAndAppls = delegator.findByAnd("ProductFeatureAndAppl", [productId : productId, productFeatureTypeId : commonProductFeatureId], [
		"sequenceNum",
		"defaultSequenceNum"
	], true);
	...
	productImageList = [];
	productContentAndInfoImageManamentList = delegator.findByAnd("ProductContentAndInfo", ["productId": productId, productContentTypeId : "IMAGE", "statusId" : "IM_APPROVED", "drIsPublic" : "Y"], ["sequenceNum"], true);
	if(productContentAndInfoImageManamentList) {
		productContentAndInfoImageManamentList.each { productContentAndInfoImageManament ->
			contentAssocThumbList = delegator.findByAnd("ContentAssoc", [contentId : productContentAndInfoImageManament.contentId, contentAssocTypeId : "IMAGE_THUMBNAIL"], null, true);
			contentAssocThumb = EntityUtil.getFirst(contentAssocThumbList);
			if(contentAssocThumb) {
				imageContentThumb = delegator.findOne("Content", [contentId : contentAssocThumb.contentIdTo], true);
				if(imageContentThumb) {
					productImageThumb = delegator.findOne("ContentDataResourceView", [contentId : imageContentThumb.contentId, drDataResourceId : imageContentThumb.dataResourceId], true);
					productImageMap = [:];
	...
	// get product tags
	productKeywords = delegator.findByAnd("ProductKeyword", ["productId": productId, "keywordTypeId" : "KWT_TAG", "statusId" : "KW_APPROVED"], null, true);
	keywordMap = [:];
	if (productKeywords) {
		for (productKeyword in productKeywords) {
			keywordConds = [
				EntityCondition.makeCondition("keyword", EntityOperator.EQUALS, productKeyword.keyword),
...
// get supplier info
context.listSup = EntityUtil.filterByDate(delegator.findByAnd("SFSupplierProduct", ["productId": productId], null, true));