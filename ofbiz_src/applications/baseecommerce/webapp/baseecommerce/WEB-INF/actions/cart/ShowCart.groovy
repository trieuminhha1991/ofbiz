...
context.orderItems = shoppingCart.items();
...
allgiftWraps = delegator.findByAnd("ProductFeature", [productFeatureTypeId : "GIFT_WRAP"], ["defaultSequenceNum"], true);
...
if (userLogin) {
	...
	inputLoy = [userLogin : userLogin, cart: shoppingCart];
	loyaltyPoint = dispatcher.runSync("getLoyaltyPointCustomer", inputLoy);
	context.totalPointOrder = loyaltyPoint.totalPointOrder;
	context.totalPointCustomer = loyaltyPoint.totalPointCustomer;
}
...
context.contentPathPrefix = NewCatalogWorker.getContentPathPrefix(request);
...
if(shoppingCartItems) {
    shoppingCartItems.each { shoppingCartItem ->
        ...
        productCategoryMembers = delegator.findList("ProductCategoryMember", EntityCondition.makeCondition("productId", EntityOperator.EQUALS, parentProductId), null, null, null, true);
...
context.curPage = (String) request.getAttribute("checkoutpage");
