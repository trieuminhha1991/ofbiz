...
<#if productPromoId?exists && productPromo?exists>
        ...
        <div class="screenlet-body">
            <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
                ...
                <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                    <td><a href="<@ofbizUrl>EditProductStore?productStoreId=${productStorePromoAppl.productStoreId}</@ofbizUrl>" class="btn btn-mini btn-primary"><#if productStore?exists>${(productStore.storeName)?if_exists}</#if>[${productStorePromoAppl.productStoreId}]</a></td>
                    ...
                    <td align="center">
                       <form method="post" action="<@ofbizUrl>promo_deleteProductStorePromoAppl</@ofbizUrl>">
                           ...
                           <input type="hidden" name="productPromoId" value="${productStorePromoAppl.productPromoId}" />
                           <input type="hidden" name="fromDate" value="${productStorePromoAppl.fromDate}" />
                           <button class="btn btn-mini btn-danger" type="submit"><i class="icon-ok"></i>${uiLabelMap.CommonDelete}</button>
                       </form>
                    </td>
                </tr>
    ...
    <div class="screenlet">
        <div class="screenlet-title-bar">
            <h3 class="header smaller lighter blue">${uiLabelMap.ProductAddStorePromo}</h3>
        </div>
        <div class="screenlet-body">
