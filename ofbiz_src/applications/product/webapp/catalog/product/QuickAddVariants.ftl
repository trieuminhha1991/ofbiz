...
<#if (product.isVirtual)?if_exists != "Y">
    <div class="alert alert-warning">
			...
			<strong>${uiLabelMap.CommonFrameAlert1}! </strong>
			${uiLabelMap.ProductWarningProductNotVirtual} <br>
		</div>
</#if>
<#if featureTypes?has_content && (featureTypes.size() > 0)>
        ...
        <tr class="header-row">
            ...
            <td align="right"><b>${uiLabelMap.CommonAll}<label><input type="checkbox" name="selectAll" value="${uiLabelMap.CommonY}" onclick="javascript:clickAll(this);" /><span class="lbl"></span></label></b></td>
        </tr>
            ...
            <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                ...
                <td>
                    ...
                    <#list existingVariantProductIds as existingVariantProductId>
                        <a href="<@ofbizUrl>EditProduct?productId=${existingVariantProductId}</@ofbizUrl>" class="btn btn-mini btn-primary">${existingVariantProductId}</a>
                    </#list>
                    </div>
                </td>
                <td align="right">
                  <label><input type="checkbox" name="_rowSubmit_o_${rowCount}" value="Y" onclick="javascript:setProductVariantId(this, '${defaultVariantProductId}', 'productVariantId_o_${rowCount}');" /><span class="lbl"></span></label>
                </td>
            </tr>
        ...
        <tr>
            ...
            <td colspan="${columns}" align="center">
                ...
                <button type="submit" class="btn btn-small btn-primary"><i class="icon-ok"></i>${uiLabelMap.CommonCreate}</button>
            </td>
        </tr>
        </table>
    </form>
<#else>
    <div class="alert alert-warning">
			<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>
			<strong>${uiLabelMap.CommonFrameAlert1}! </strong>
			${uiLabelMap.ProductNoSelectableFeaturesFound} <br>
		</div>
</#if>
    ...
    <table cellspacing="0" class="basic-table">
        ...
        <tr>
            <td>
                ...
                <button type="submit" class="btn btn-small btn-primary"><i class="icon-ok"></i>${uiLabelMap.ProductVariantAdd}</button>
            </td>
        </tr>
    </table>
</form>