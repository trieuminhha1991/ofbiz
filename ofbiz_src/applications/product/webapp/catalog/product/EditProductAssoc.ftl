...
<div class="widget-box transparent no-bottom-border">
    <div class="widget-header">
        <h3 class="smaller lighter blue">${uiLabelMap.PageTitleEditProductAssociations}</h3>
    ...
    <div class="screenlet-body">
        <form action="<@ofbizUrl>UpdateProductAssoc</@ofbizUrl>" method="post" style="margin: 0;" name="editProductAssocForm" id="editProductAssocForm">
            ...
            <#if productId?exists && productIdTo?exists && productAssocTypeId?exists && fromDate?exists>
                ...
                <td>
                    <div>
                        ...
                        <span class="tooltipob">${uiLabelMap.CommonSetNowEmpty}</span>
                    </div>
                </td>
                </tr>
            <#else>
        ...
        <tr>
            <td  align="right">${uiLabelMap.ProductSequenceNum}</td>
            <td>&nbsp;</td>
            <td><input type="text" name="SEQUENCE_NUM" <#if useValues>value="${(productAssoc.sequenceNum)?if_exists}"<#else>value="${(request.getParameter("SEQUENCE_NUM"))?if_exists}"</#if> size="5" maxlength="10" /></td>
        </tr>
        <tr>
            <td  align="right">${uiLabelMap.ProductReason}</td>
            <td>&nbsp;</td>
            <td><input type="text" name="REASON" <#if useValues>value="${(productAssoc.reason)?if_exists}"<#else>value="${(request.getParameter("REASON"))?if_exists}"</#if> size="60" maxlength="255" /></td>
        </tr>
        <tr>
            <td align="right">${uiLabelMap.ProductInstruction}</td>
            <td>&nbsp;</td>
            <td><input type="text" name="INSTRUCTION" <#if useValues>value="${(productAssoc.instruction)?if_exists}"<#else>value="${(request.getParameter("INSTRUCTION"))?if_exists}"</#if> size="60" maxlength="255" /></td>
        </tr>

        <tr>
            <td align="right">${uiLabelMap.ProductQuantity}</td>
            <td>&nbsp;</td>
            <td><input type="text" name="QUANTITY" <#if useValues>value="${(productAssoc.quantity)?if_exists}"<#else>value="${(request.getParameter("QUANTITY"))?if_exists}"</#if> size="10" maxlength="15" /></td>
        </tr>

        <tr>
            <td colspan="2">&nbsp;</td>
            <td><button type="submit" onclick="editProductAssocForm.submit();" class="btn btn-small btn-primary"><i class="icon-ok"></i> <#if isCreate>${uiLabelMap.CommonCreate}<#else>${uiLabelMap.CommonUpdate}</#if> </button></td>
        </tr>
        </table>
        </form>
    </div>
...
<div class="screenlet">
    <div class="screenlet-title-bar">
        <h3 class="header smaller lighter blue">${uiLabelMap.ProductAssociationsFromProduct}</h3>
    </div>
    <div class="screenlet-body">
        <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
            ...
            <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                <td><a href="<@ofbizUrl>EditProduct?productId=${(assocFromProduct.productIdTo)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">${(assocFromProduct.productIdTo)?if_exists}</a></td>
                <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProduct?productId=${(assocFromProduct.productIdTo)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">${(listToProduct.internalName)?if_exists}</a></#if>&nbsp;</td>
                ...
                <td>
                <a href="<@ofbizUrl>UpdateProductAssoc?UPDATE_MODE=DELETE&amp;productId=${productId}&amp;PRODUCT_ID=${productId}&amp;PRODUCT_ID_TO=${(assocFromProduct.productIdTo)?if_exists}&amp;PRODUCT_ASSOC_TYPE_ID=${(assocFromProduct.productAssocTypeId)?if_exists}&amp;FROM_DATE=${(assocFromProduct.fromDate)?if_exists}&amp;useValues=true</@ofbizUrl>" class="btn btn-mini btn-danger">
                ${uiLabelMap.CommonDelete}</a>
                </td>
                <td>
                <a href="<@ofbizUrl>EditProductAssoc?productId=${productId}&amp;PRODUCT_ID=${productId}&amp;PRODUCT_ID_TO=${(assocFromProduct.productIdTo)?if_exists}&amp;PRODUCT_ASSOC_TYPE_ID=${(assocFromProduct.productAssocTypeId)?if_exists}&amp;FROM_DATE=${(assocFromProduct.fromDate)?if_exists}&amp;useValues=true</@ofbizUrl>" class="btn btn-mini btn-primary">
...
<div class="screenlet">
    <div class="screenlet-title-bar">
        <h3 class="header smaller lighter blue">${uiLabelMap.ProductAssociationsToProduct}</h3>
    </div>
    <div class="screenlet-body">
        <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
            <tr class="header-row">
            ...
            <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                <td><a href="<@ofbizUrl>EditProduct?productId=${(assocToProduct.productId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">${(assocToProduct.productId)?if_exists}</a></td>
                <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProduct?productId=${(assocToProduct.productId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">${(listToProduct.internalName)?if_exists}</a></#if></td>
...
<br />
<span class="tooltipob">${uiLabelMap.CommonNote} : ${uiLabelMap.ProductHighlightedExplanation}</span>
