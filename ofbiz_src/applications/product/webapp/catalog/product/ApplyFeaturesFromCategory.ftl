...
<a href="<@ofbizUrl>EditFeature?productFeatureCategoryId=${productFeatureCategoryId?if_exists}</@ofbizUrl>" class="btn btn-mini btn-info">${uiLabelMap.ProductCreateNewFeature}</a>
<#elseif productFeatureGroup?exists>
<a href="<@ofbizUrl>EditFeatureGroupAppls?productFeatureGroupId=${productFeatureGroup.productFeatureGroupId?if_exists}</@ofbizUrl>" class="btn btn-mini btn-info">${uiLabelMap.CommonEdit} ${productFeatureGroup.description?if_exists}</a>
</#if>
<#if productId?has_content>
    <a href="<@ofbizUrl>EditProduct?productId=${productId}</@ofbizUrl>" class="btn btn-mini btn-info">${uiLabelMap.ProductReturnToEditProduct}</a>
    <a href="<@ofbizUrl>EditProductFeatures?productId=${productId}</@ofbizUrl>" class="btn btn-mini btn-info">${uiLabelMap.ProductReturnToEditProductFeatures}</a>
</#if>
...
<table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
<form method="post" action="<@ofbizUrl>ApplyFeaturesToProduct</@ofbizUrl>" name="selectAllForm" id="selectAllForm">
  ...
  <tr class="header-row">
    ...
    <td><b>${uiLabelMap.CommonAll}<label><input type="checkbox" name="selectAll" value="${uiLabelMap.CommonY}" onclick="javascript:toggleAll(this, 'selectAllForm');highlightAllRows(this, 'productFeatureId_tableRow_', 'selectAllForm');" /><span class="lbl"></span></label></td>
  </tr>
<#assign rowCount = 0>
<#assign rowClass = "2">
<#if (listSize > 0)>
<#list productFeatures as productFeature>
  <#assign curProductFeatureType = productFeature.getRelatedOne("ProductFeatureType", true)>
    <tr id="productFeatureId_tableRow_${rowCount}"  valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
        ...
        <td><a href="<@ofbizUrl>EditFeature?productFeatureId=${productFeature.productFeatureId}</@ofbizUrl>" class="btn btn-mini btn-info">${productFeature.productFeatureId}</a></td>
        ...
        <td align="right">
            <label><input type="checkbox" name="_rowSubmit_o_${rowCount}" value="Y" onclick="javascript:checkToggle(this, 'selectAllForm');highlightRow(this,'productFeatureId_tableRow_${rowCount}');" /><span class="lbl"></span></label>
        </td>
    </tr>
...
<tr><td colspan="9" align="center"><button onclick="selectAllForm.submit();" class="btn btn-mini btn-info" type="submit"><i class="icon-ok"></i> ${uiLabelMap.CommonApply}</button></td></tr>
</#if>
