...
<#if productId?exists && product?exists>
  <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
    ...
    <#list productRoles as productRole>
      <#assign line = line + 1>
      <#assign curRoleType = productRole.getRelatedOne("RoleType", true)>
      <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
        <td><a href="/partymgr/control/viewprofile?partyId=${(productRole.partyId)?if_exists}" target="_blank" class="btn btn-mini btn-primary">${(productRole.partyId)?if_exists}</a></td>
        <td>${(curRoleType.get("description",locale))?if_exists}</td>
        <form method="post" action="<@ofbizUrl>updatePartyToProduct</@ofbizUrl>" name="lineForm${line}" id="updatePartyToProduct">
          ...
          <td><button onclick="updatePartyToProduct.submit();" type="submit" class="btn btn-mini btn-primary"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button></td>
        </form>

        <td align="center">
          <form action="<@ofbizUrl>removePartyFromProduct</@ofbizUrl>" method="post" id="removePartyFromProduct">
             <input type="hidden" name="partyId" value="${(productRole.partyId)?if_exists}" />
             <input type="hidden" name="productId" value="${(productRole.productId)?if_exists}" />
             <input type="hidden" name="roleTypeId" value="${(productRole.roleTypeId)?if_exists}" />
             <input type="hidden" name="fromDate" value="${(productRole.fromDate)?if_exists}" />
             <button type="submit" class="btn btn-mini btn-danger" onclick="removePartyFromProduct.submit();" ><i class="icon-trash"></i>${uiLabelMap.CommonDelete}</button>
          </form>
        </td>
      </tr>
