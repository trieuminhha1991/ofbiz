...
<div class="widget-box transparent no-bottom-border">
  <div class="widget-header">
    <h4>${uiLabelMap.PageTitleEditProductFeatures}</h4>
  ...
  <div class="screenlet-body">
    <form method="post" action="<@ofbizUrl>UpdateFeatureToProductApplication</@ofbizUrl>" name="selectAllForm" id="selectAllForm">
      ...
      <table cellspacing="0" class="table table-striped table-bordered table-hover dataTable">
        <tr class="header-row">
          ...
          <td><b>${uiLabelMap.CommonAll}<label><input type="checkbox" name="selectAll" value="${uiLabelMap.CommonY}" onclick="javascript:toggleAll(this, 'selectAllForm');highlightAllRows(this, 'productFeatureId_tableRow_', 'selectAllForm');" /><span class="lbl"></span></label></b></td>
        </tr>
  <#assign rowClass = "2">
  <#list productFeatureAndAppls as productFeatureAndAppl>
    ...
    <#assign curProductFeatureCategory = (productFeatureAndAppl.getRelatedOne("ProductFeatureCategory", true)?if_exists)>
        <tr id="productFeatureId_tableRow_${productFeatureAndAppl_index}" valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
          ...
          <a href="<@ofbizUrl>EditFeature?productFeatureId=${(productFeatureAndAppl.productFeatureId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">
          ...
          <td><a href="<@ofbizUrl>EditFeatureCategoryFeatures?productFeatureCategoryId=${(productFeatureAndAppl.productFeatureCategoryId)?if_exists}&amp;productId=${(productFeatureAndAppl.productId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary">
    ...
    <#if (productFeatureAndAppl.productFeatureApplTypeId)?exists>
      ...
      <#list productFeatureApplTypes as productFeatureApplType>
          ...
          <td align="right">
          	<label><input type="checkbox" name="_rowSubmit_o_${productFeatureAndAppl_index}" value="Y" onclick="javascript:checkToggle(this, 'selectAllForm');highlightRow(this,'productFeatureId_tableRow_${productFeatureAndAppl_index}');" /><span class="lbl"></span></label>
          </td>
          <td>
            <a href="javascript:document.RemoveFeatureFromProduct_o_${productFeatureAndAppl_index}.submit()" class="btn btn-mini btn-danger">${uiLabelMap.CommonDelete}</a>
          </td>
        </tr>
      ...
      <#assign rowClass = "2">
        ...
        <tr>
          <td colspan="8" align="center">
            ...
            <button onclick="selectAllForm.submit();" type="submit" class="btn btn-primary btn-mini"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button>
          </td>
        </tr>
      </table>
    </form>
...
<div class="screenlet">
  <div class="screenlet-title-bar">
    <h3 class="header smaller lighter blue">${uiLabelMap.ProductAddProductFeatureFromCategory}</h3>
  </div>
  <div class="screenlet-body">
    <form method="post" action="<@ofbizUrl>ApplyFeaturesFromCategory</@ofbizUrl>" style='margin: 0;' id="ApplyFeaturesFromCategoryob">
      <input type="hidden" name="productId" value="${productId}"/>
  ...
  <#list productFeatureApplTypes as productFeatureApplType>
    ...
    <#if (productFeatureApplType.productFeatureApplTypeId?exists && product?exists && product.isVirtual == 'N' && productFeatureApplType.productFeatureApplTypeId?if_exists =="STANDARD_FEATURE")>selected="selected"</#if>
      ...
      <button onclick="ApplyFeaturesFromCategoryob.submit();" type="submit" class="btn btn-primary btn-mini"><i class="icon-ok"></i>${uiLabelMap.CommonAdd}</button>
    </form>
  </div>
</div>
<div class="screenlet">
  <div class="screenlet-title-bar">
    <h3 class="header smaller lighter blue">${uiLabelMap.ProductAddProductFeatureTypeId}</h3>
  </div>
  <div class="screenlet-body">
    <form method="post" action="<@ofbizUrl>ApplyFeatureToProductFromTypeAndCode</@ofbizUrl>" name='addFeatureByTypeIdCode'>
      <input type="hidden" name="productId" value="${productId}"/>
      <span>${uiLabelMap.ProductFeatureType}: </span>
  ...
  <#list productFeatureTypes as productFeatureType>
      ...
      <span>${uiLabelMap.CommonIdCode}: </span><input type="text" size='10' name='idCode' value=''/>
      <br />
      <span>${uiLabelMap.ProductFeatureApplicationType}: </span>
      <select name='productFeatureApplTypeId' size="1">
  <#list productFeatureApplTypes as productFeatureApplType>
    ...
    <#if (productFeatureApplType.productFeatureApplTypeId?exists && product?exists && product.isVirtual == 'N' && productFeatureApplType.productFeatureApplTypeId =="STANDARD_FEATURE")>selected="selected"</#if>
      ...
      <button onclick="addFeatureByTypeIdCode.submit();" type="submit" class="btn btn-primary btn-mini"><i class="icon-ok"></i>${uiLabelMap.CommonAdd}</button>
    </form>
  </div>
</div>
<div class="screenlet">
  <div class="screenlet-title-bar">
    <h3 class="header smaller lighter blue">${uiLabelMap.ProductAddProductFeatureID}</h3>
  </div>
  <div class="screenlet-body">
    <form method="post" action="<@ofbizUrl>ApplyFeatureToProduct</@ofbizUrl>" name="addFeatureById" id="addFeatureById">
      <input type="hidden" name="productId" value="${productId}"/>
  ...
  <#list productFeatureApplTypes as productFeatureApplType>
    ...
    <#if (productFeatureApplType.productFeatureApplTypeId?exists && product?exists && product.isVirtual == 'N' && productFeatureApplType.productFeatureApplTypeId =="STANDARD_FEATURE")>selected="selected"</#if>
      ...
      <br />
      <span>${uiLabelMap.CommonFrom} : </span><@htmlTemplate.renderDateTimeField name="fromDate" event="" action="" className="" alert="" title="Format: yyyy-MM-dd HH:mm:ss.SSS" value="" size="25" maxlength="30" id="fromDate2" dateType="date" shortDateInput=false timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName=""/>
      <span>${uiLabelMap.CommonThru} : </span><@htmlTemplate.renderDateTimeField name="thruDate" event="" action="" className="" alert="" title="Format: yyyy-MM-dd HH:mm:ss.SSS" value="" size="25" maxlength="30" id="thruDate2" dateType="date" shortDateInput=false timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName=""/>
      <span>${uiLabelMap.CommonSequence} : </span><input type="text" size="5" name="sequenceNum"/>
      <button onclick="addFeatureById.submit();" type="submit" class="btn btn-primary btn-mini"><i class="icon-ok"></i>${uiLabelMap.CommonAdd}</button>
    </form>
  </div>
</div>
</#if>