...
<#if productCategoryId?exists && productCategory?exists>
    <div class="screenlet">
        <div class="screenlet-title-bar">
            <h3 class="header smaller lighter blue">${uiLabelMap.PageTitleEditCategoryParties}</h3>
        </div>
        <div class="screenlet-body">
            ...
            <td><a href="/partymgr/control/viewprofile?party_id=${(productCategoryRole.partyId)?if_exists}" target="_blank" class="btn btn-mini btn-info">${(productCategoryRole.partyId)?if_exists}</a></td>
            ...
            <td align="center">
                <form method="post" action="<@ofbizUrl>updatePartyToCategory</@ofbizUrl>" name="lineForm_update${line}">
                    ...
                    <button type="submit" class="btn btn-small btn-info margin-top-nav-10"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button>
                </form>
            </td>
            <td align="center">
                <form method="post" action="<@ofbizUrl>removePartyFromCategory</@ofbizUrl>" name="lineForm_delete${line}">
                    ...
                    <input type="hidden" name="roleTypeId" value="${(productCategoryRole.roleTypeId)?if_exists}" />
                    <input type="hidden" name="fromDate" value="${(productCategoryRole.getTimestamp("fromDate"))?if_exists}" />
                    <a href="javascript:document.lineForm_delete${line}.submit()" class="btn btn-mini btn-danger margin-top11">${uiLabelMap.CommonDelete}</a>
                </form>
            </td>
            </tr>
    ...
    <div class="screenlet">
        <div class="screenlet-title-bar">
            <h3 class="header smaller lighter blue">${uiLabelMap.ProductAssociatePartyToCategory}</h3>
        </div>
        <div class="screenlet-body">
                ...
                <tr>
                    <td>
                        <form method="post" action="<@ofbizUrl>addPartyToCategory</@ofbizUrl>" style="margin: 0;" name="addNewForm">
                            ...
                            <button type="submit" class="btn btn-mini btn-small btn-info"><i class="icon-ok"></i>${uiLabelMap.CommonAdd}</button>
                        </form>
                    </td>
                </tr>
            </table>
