...
<#if productCategoryId?exists && productCategory?exists>
  <div class="screenlet">
    <div class="screenlet-title-bar">
      <h3 class="header smaller lighter blue">${uiLabelMap.PageTitleEditCategoryProductCatalogs}</h3>
    </div>
        <div class="screenlet-body">
            ...
            <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                <td><a href="<@ofbizUrl>EditProdCatalog?prodCatalogId=${(prodCatalogCategory.prodCatalogId)?if_exists}</@ofbizUrl>" class="btn btn-mini btn-primary"><#if prodCatalog?exists>${(prodCatalog.catalogName)?if_exists}</#if> [${(prodCatalogCategory.prodCatalogId)?if_exists}]</a></td>
                <td>
                ...
                <td align="center">
                    <form method="post" action="<@ofbizUrl>category_updateProductCategoryToProdCatalog</@ofbizUrl>" name="lineForm_update${line}" id="category_updateProductCategoryToProdCatalog">
                        ...
                        <button class="btn btn-primary btn-mini" onclick="category_updateProductCategoryToProdCatalog.submit();" type="submit"><i class="icon-ok"></i>${uiLabelMap.CommonUpdate}</button>
                    </form>
                </td>
                <td align="center">
                  <form method="post" action="<@ofbizUrl>category_removeProductCategoryFromProdCatalog</@ofbizUrl>" name="lineForm_delete${line}">
                    ...
                    <input type="hidden" name="prodCatalogCategoryTypeId" value="${prodCatalogCategory.prodCatalogCategoryTypeId}"/>
                    <input type="hidden" name="fromDate" value="${(prodCatalogCategory.fromDate)?if_exists}"/>
                    <a href="javascript:document.lineForm_delete${line}.submit()" class="btn btn-mini btn-danger margin-top2">${uiLabelMap.CommonDelete}</a>
                  </form>
                </td>
            </tr>
    ...
    <div class="screenlet">
        <div class="screenlet-title-bar">
            <h3 class="header smaller lighter blue">${uiLabelMap.ProductAddCatalogProductCategory}</h3>
        </div>
        <div class="screenlet-body">
            <table cellspacing="0" class="basic-table">
                <tr><td>
                    <form method="post" action="<@ofbizUrl>category_addProductCategoryToProdCatalog</@ofbizUrl>" style="margin: 0;" name="addNewForm" id="category_addProductCategoryToProdCatalog">
                        ...
                        <button class="btn btn-primary btn-mini" onclick="category_addProductCategoryToProdCatalog.submit();" type="submit"><i class="icon-ok"></i>${uiLabelMap.CommonAdd}</button>
                    </form>
                </td></tr>
            </table>
        </div>
