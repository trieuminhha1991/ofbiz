...
physicalInventoryId = request.getParameter("physicalInventoryId");
context.physicalInventoryId = physicalInventoryId;
damagedStatus = request.getParameter("damagedStatus");
inventoryItemId = request.getParameter("inventoryItemId");
xferQty = request.getParameter("xferQty");
inventoryTransfer = null;
if(damagedStatus){
	inventoryDamagedStatus = delegator.findOne("StatusItem",[statusId:damagedStatus], false);
	if (inventoryDamagedStatus) {
		context.inventoryDamagedStatus = inventoryDamagedStatus;
	}
...
if (inventoryItemId) {
    ...
    if (inventoryItem) {
		//change status inventoryItem to damage if request is transfer damaged product -- by datnv
		if(damagedStatus){
			inventoryItem.statusId = damagedStatus;
//			delegator.store(inventoryItem);
			context.xferQty = xferQty;
		}
		//END change status inventoryItem to damage if request is transfer damaged product -- by datnv
