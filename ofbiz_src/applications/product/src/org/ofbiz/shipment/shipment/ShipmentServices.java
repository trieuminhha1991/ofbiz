...
public class ShipmentServices {
    ...
    public static Map<String, Object> updatePurchaseShipmentFromReceipt(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            for (GenericValue item: shipmentAndItems) {
                ...
                shippedCountMap.put(item.getString("productId"), quantity.setScale(decimals,rounding));
            }
            ...
            for (GenericValue item: shipmentReceipts) {
                ...
                BigDecimal quantityQualityAssurance = item.getBigDecimal("quantityQualityAssurance") != null ? item.getBigDecimal("quantityQualityAssurance") : BigDecimal.ZERO;
                BigDecimal quantityRejected = item.getBigDecimal("quantityRejected");
                receivedQuantity = receivedQuantity.add(quantityQualityAssurance); 
                receivedQuantity = receivedQuantity.add(quantityRejected); 
    			...
    			if (UtilValidate.isNotEmpty(requireAmount) && "Y".equals(requireAmount)) {
    				BigDecimal amountAccepted = item.getBigDecimal("amountAccepted");
    				if (UtilValidate.isNotEmpty(item.getString("orderId"))) {
    					...
    					quantity = amountAccepted.divide(selectedAmount, decimals, rounding);
    				} else if (UtilValidate.isNotEmpty(item.getString("returnId")))	{
    					GenericValue returnItem = delegator.findOne("ReturnItem", false, UtilMisc.toMap("returnId", item.getString("returnId"), "returnItemSeqId", item.getString("returnItemSeqId")));
    					BigDecimal returnAmount = returnItem.getBigDecimal("returnAmount");
    					quantity = amountAccepted.divide(returnAmount, decimals, rounding);
    				} else if (UtilValidate.isNotEmpty(item.getString("transferId"))) {
    					GenericValue transferItem = delegator.findOne("TransferItem", false, UtilMisc.toMap("transferId", item.getString("transferId"), "transferItemSeqId", item.getString("transferItemSeqId")));
    					BigDecimal amount = transferItem.getBigDecimal("amount");
    					quantity = amountAccepted.divide(amount, decimals, rounding);
    				} else if (UtilValidate.isNotEmpty(item.getString("requirementId"))) {
    					GenericValue requirementItem = delegator.findOne("RequirementItem", false, UtilMisc.toMap("requirementId", item.getString("requirementId"), "reqItemSeqId", item.getString("reqItemSeqId")));
    					BigDecimal weight = requirementItem.getBigDecimal("weight");
    					quantity = amountAccepted.divide(weight, decimals, rounding);    					
    				}
    			}
    			
                receivedCountMap.put(item.getString("productId"), quantity.setScale(decimals, rounding));
            }
            
            for(String key: receivedCountMap.keySet())
            {    
            	// so sanh so luong da nhan (trong shipmentReceipt) voi so luong da chuyen (trong shipmenItem voi status SHIPPED)
            	if (shippedCountMap.containsKey(key) && receivedCountMap.containsKey(key)){
            		if (receivedCountMap.get(key).compareTo(shippedCountMap.get(key)) > 0 )
                	{
                		// gan so luong da nhan trong shipmentReceipt bang so luong da chuyen
                		 receivedCountMap.put(key,shippedCountMap.get(key));
                	}
            	}
            }
            ...
            if (!shippedCountMap.equals(receivedCountMap)) {
            	// tra ve success khi so luong received < so luong shipped
                return ServiceUtil.returnSuccess();
            }

            // now update the shipment (update trang thai da nhan neu nhu con so received lon hon con so Shipped)
    ...
    public static Map<String, Object> createInventoryTransfersForTransferShipment(DispatchContext dctx, Map<String, Object> context) {
    	Delegator delegator = dctx.getDelegator();
    	LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
    	...
    	try {
        	...
        	List<GenericValue> itemIssuances = delegator.findList("ItemIssuance", EntityCondition.makeCondition(UtilMisc.toMap("shipmentId", shipmentId)), null, null, null, false);
        	for (GenericValue itemIssuance : itemIssuances) {
        		
        		List<GenericValue> inventoryTransfers = delegator.findList("InventoryTransfer", EntityCondition.makeCondition(UtilMisc.toMap("itemIssuanceId", itemIssuance.get("itemIssuanceId"))), null, null, null, false); 
        		for (GenericValue inventoryTransfer : inventoryTransfers) {
        			Map<String, Object> tmpResult = dispatcher.runSync("completeInventoryTransfer", UtilMisc.toMap("inventoryTransferId", inventoryTransfer.get("inventoryTransferId"), "userLogin", userLogin));
        			if (ServiceUtil.isError(tmpResult)) {
        				return tmpResult;
        			}
        		}
    ...
    @SuppressWarnings("unchecked")
	public static Map<String, Object> getShipmentCosts(DispatchContext ctx, Map<String, ? extends Object> context){
    	Delegator delegator = ctx.getDelegator();
    	Map<String, Object> successResult = ServiceUtil.returnSuccess();
    	List<EntityCondition> listAllConditions = (List<EntityCondition>) context.get("listAllConditions");
    	List<String> listSortFields = (List<String>) context.get("listSortFields");
    	...
    	try {
    		listItems = delegator.findList("", EntityCondition.makeCondition(listAllConditions), null, listSortFields, opts, false);
    	} catch (GenericEntityException e) {
			String errMsg = "Fatal error calling getShipmentCosts service: " + e.toString();
			Debug.logError(e, errMsg, module);
		}
