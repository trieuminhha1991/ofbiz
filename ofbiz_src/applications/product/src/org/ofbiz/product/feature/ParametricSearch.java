...
public class ParametricSearch {
	...
	// UtilCache.createUtilCache("custom.FeaturePerTypeByCategory", 0, 600000,
	// true);
	 ...
	 * -> ProductFeature. Returns a Map of Lists of ProductFeature GenericValue
	 * objects organized by productFeatureTypeId.
	 */
	public static Map<String, List<GenericValue>> makeCategoryFeatureLists(
			String productCategoryId, Delegator delegator) {
		return makeCategoryFeatureLists(productCategoryId, delegator,
				DEFAULT_PER_TYPE_MAX_SIZE);
	}

	public static Map<String, List<GenericValue>> makeCategoryFeatureLists(
			String productCategoryId, Delegator delegator, int perTypeMaxSize) {
		Map<String, Map<String, GenericValue>> productFeaturesByTypeMap = FastMap
				.newInstance();
		try {
			List<GenericValue> productFeatureCategoryAppls = delegator
					.findByAnd("ProductFeatureCategoryAppl", UtilMisc.toMap(
							"productCategoryId", productCategoryId), null, true);
			productFeatureCategoryAppls = EntityUtil.filterByDate(
					productFeatureCategoryAppls, true);
			if (productFeatureCategoryAppls != null) {
				for (GenericValue productFeatureCategoryAppl : productFeatureCategoryAppls) {
					List<GenericValue> productFeatures = delegator.findByAnd(
							"ProductFeature", UtilMisc.toMap(
									"productFeatureCategoryId",
									productFeatureCategoryAppl
											.get("productFeatureCategoryId")),
							null, true);
					for (GenericValue productFeature : productFeatures) {
						String productFeatureTypeId = productFeature
								.getString("productFeatureTypeId");
						Map<String, GenericValue> featuresByType = productFeaturesByTypeMap
								.get(productFeatureTypeId);
						if (featuresByType == null) {
							featuresByType = FastMap.newInstance();
							productFeaturesByTypeMap.put(productFeatureTypeId,
									featuresByType);
						}
						if (featuresByType.size() < perTypeMaxSize) {
							featuresByType.put(productFeature
									.getString("productFeatureId"),
									productFeature);
						}
					}
				}
			...
			Debug.logError(e,
					"Error getting feature categories associated with the category with ID: "
							+ productCategoryId, module);
		}

		try {
			List<GenericValue> productFeatureCatGrpAppls = delegator.findByAnd(
					"ProductFeatureCatGrpAppl",
					UtilMisc.toMap("productCategoryId", productCategoryId),
					null, true);
			productFeatureCatGrpAppls = EntityUtil.filterByDate(
					productFeatureCatGrpAppls, true);
			if (productFeatureCatGrpAppls != null) {
				for (GenericValue productFeatureCatGrpAppl : productFeatureCatGrpAppls) {
					List<GenericValue> productFeatureGroupAppls = delegator
							.findByAnd(
					...
					for (GenericValue productFeatureGroupAppl : productFeatureGroupAppls) {
						GenericValue productFeature = delegator
								.findOne("ProductFeature", UtilMisc.toMap(
										"productFeatureId",
										productFeatureGroupAppl
												.get("productFeatureId")), true);
			...
			Debug.logError(e,
					"Error getting feature groups associated with the category with ID: "
							+ productCategoryId, module);
		}

		// now before returning, order the features in each list by description
		Map<String, List<GenericValue>> productFeaturesByTypeMapSorted = FastMap
				.newInstance();
		for (Map.Entry<String, Map<String, GenericValue>> entry : productFeaturesByTypeMap
				.entrySet()) {
			List<GenericValue> sortedFeatures = EntityUtil.orderBy(entry
					.getValue().values(), UtilMisc.toList("description",
					"defaultSequenceNum"));
	...
	public static Map<String, List<GenericValue>> getAllFeaturesByType(
			Delegator delegator) {
		return getAllFeaturesByType(delegator, DEFAULT_PER_TYPE_MAX_SIZE);
	}

	public static Map<String, List<GenericValue>> getAllFeaturesByType(
			Delegator delegator, int perTypeMaxSize) {
		Map<String, List<GenericValue>> productFeaturesByTypeMap = FastMap
				.newInstance();
		try {
			Set<String> typesWithOverflowMessages = FastSet.newInstance();
			EntityListIterator productFeatureEli = delegator.find(
					"ProductFeature", null, null, null,
					UtilMisc.toList("description"), null);
			GenericValue productFeature = null;
			while ((productFeature = productFeatureEli.next()) != null) {
				String productFeatureTypeId = productFeature
						.getString("productFeatureTypeId");
				List<GenericValue> featuresByType = productFeaturesByTypeMap
						.get(productFeatureTypeId);
				if (featuresByType == null) {
				...
				if (featuresByType.size() > perTypeMaxSize) {
					if (!typesWithOverflowMessages
							.contains(productFeatureTypeId)) {
						typesWithOverflowMessages.add(productFeatureTypeId);
						// TODO: uh oh, how do we pass this message back? no
						// biggie for now
					}
				} else {
	...
	public static Map<String, List<String>> makeFeatureIdByTypeMap(
			ServletRequest request) {
		Map<String, Object> parameters = UtilHttp
				.getParameterMap((HttpServletRequest) request);
		return makeFeatureIdByTypeMap(parameters);
	}

	/**
	 * Handles parameters coming in prefixed with "pft_" where the text in the
	 * key following the prefix is a productFeatureTypeId and the value is a
	 * productFeatureId; meant to be used with drop-downs and such
	 */
	public static Map<String, List<String>> makeFeatureIdByTypeMap(
			Map<String, Object> parameters) {
		Map<String, List<String>> featureIdByType = FastMap.newInstance();
		if (parameters == null)
		...
		for (Map.Entry<String, Object> entry : parameters.entrySet()) {
			...
			if (parameterName.startsWith("pft_")) {
				...
				if (parameterName.contains("_#")) {
					productFeatureTypeId = parameterName.substring(4,
							parameterName.length() - 3);
				} else if (parameterName.contains("__")) {
					continue;
				} else {
					productFeatureTypeId = parameterName.substring(4);
				}
				...
				if (UtilValidate.isNotEmpty(productFeatureId)) {
					List<String> tmpList = featureIdByType.get(productFeatureTypeId);
					if(tmpList == null){
						tmpList = new ArrayList<String>();
					}
					tmpList.add(productFeatureId);
					featureIdByType.put(productFeatureTypeId, tmpList);
				}
			}
		}
	...
	/**
	 * Handles parameters coming in prefixed with "SEARCH_FEAT" where the
	 * parameter value is a productFeatureId; meant to be used with text entry
	 * boxes or check-boxes and such
	 */
	...
	public static String makeFeatureIdByTypeString(
			Map<String, String> featureIdByType) {
	...
	/**
	 * Handles parameters coming in prefixed with "SEARCH_PROD_FEAT_CAT" where
	 * the parameter value is a productFeatureCategoryId; meant to be used with
	 * text entry boxes or check-boxes and such
	 **/
	public static List<String> makeProductFeatureCategoryIdListFromPrefixed(
			Map<String, Object> parameters) {
