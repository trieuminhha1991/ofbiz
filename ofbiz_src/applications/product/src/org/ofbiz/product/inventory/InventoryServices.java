...
public class InventoryServices {
    ...
    public static Map<String, Object> prepareInventoryTransfer(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String damagedStatus = (String)context.get("damagedStatus");
        GenericValue inventoryItem = null;
        GenericValue newItem = null;
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Locale locale = (Locale) context.get("locale");
        String stt = "INV_NS_DAMAGED";
        ...
        try {
            ...
            if (inventoryType.equals("NON_SERIAL_INV_ITEM")) {
                ...
                // Change by thangnv to transfer quantity greater than ATP
...
//                	if (atp.compareTo(qoh) < 0) {
                        ...
                        //edit by datnv update damaged status for inventoryitem
                        if(damagedStatus != null && damagedStatus.equals(stt)){
                        	newItem.set("statusId", damagedStatus);
                        	GenericValue invItemVar = delegator.findOne("InventoryItemVariance", UtilMisc.toMap("physicalInventoryId",physicalInventoryId,"inventoryItemId", inventoryItemId), false);
                        	invItemVar.put("status","1");
                        	delegator.store(invItemVar);
                        	
                        }
                        //end edit by datnv update damaged status for inventoryitem
...
//                    }
                	...
                	BigDecimal reserved = qoh.subtract(atp);
                	BigDecimal remain = qoh.subtract(xferQty);
                	if (remain.compareTo(reserved) >=0 ){
                		GenericValue newInvItem = GenericValue.create(inventoryItem);
                    	newInvItem.set("quantityOnHandTotal", BigDecimal.ZERO);
                    	newInvItem.set("availableToPromiseTotal", BigDecimal.ZERO);
                    	delegator.createSetNextSeqId(newInvItem);
                    	results.put("inventoryItemId", newInvItem.get("inventoryItemId"));
                    	
                    	Map<String, Object> createNewDetailMap = UtilMisc.toMap("availableToPromiseDiff", xferQty, "quantityOnHandDiff", xferQty,
                                "inventoryItemId", newInvItem.get("inventoryItemId"), "userLogin", userLogin);
                        Map<String, Object> createUpdateDetailMap = UtilMisc.toMap("availableToPromiseDiff", BigDecimal.ZERO, "quantityOnHandDiff", xferQty.negate(),
                                "inventoryItemId", inventoryItem.get("inventoryItemId"), "userLogin", userLogin);
                        try {
                            Map<String, Object> resultNew = dctx.getDispatcher().runSync("createInventoryItemDetail", createNewDetailMap);
                            if (ServiceUtil.isError(resultNew)) {
                    	...
                    	Map<String, Object> createNewDetailMap = UtilMisc.toMap("availableToPromiseDiff", xferQty, "quantityOnHandDiff", xferQty,
                                "inventoryItemId", newInvItem.get("inventoryItemId"), "userLogin", userLogin);
                        Map<String, Object> createUpdateDetailMap = UtilMisc.toMap("availableToPromiseDiff", xferQty.negate(), "quantityOnHandDiff", xferQty.negate(),
                                "inventoryItemId", inventoryItem.get("inventoryItemId"), "userLogin", userLogin);
            ...
            if (inventoryType.equals("NON_SERIAL_INV_ITEM")) {
//            	if (xferQty.compareTo(inventoryItem.getBigDecimal("availableToPromiseTotal")) <= 0){
...
//            	}
            ...
            results.put("damagedStatus", damagedStatus);
    ...
    public static Map<String, Object> completeInventoryTransfer(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        Map<String, Object> rs = new FastMap<String, Object>();
        ...
        try {
            ...
            inv = inventoryItem.getString("inventoryItemId");
        ...
        inventoryTransfer.set("statusId", "IXF_COMPLETED");
...
//        if(damagedStatus != null && damagedStatus.equals(stt)){
//        	try{
//        	GenericValue invItem = delegator.findOne("InventoryItem", UtilMisc.toMap("inventoryItemId",inv), false);
//        	invItem.put("availableToPromiseTotal", BigDecimal.ZERO);
...
//        
//        //end edit by datnv
        rs.put("inventoryTransferId",inventoryTransferId);
        ...
        for (GenericValue orderItem: orderItems) {
            ...
            for (GenericValue facility: facilities) {
                ...
                try {
                    ...
                    // TODOCHANGE expireDate
                    if (UtilValidate.isNotEmpty(orderItem.get("expireDate"))) {
                    	invResult = dispatcher.runSync("getInventoryAvailableByFacility", UtilMisc.toMap("productId", productId, "facilityId", facility.getString("facilityId"), "expireDate", orderItem.getTimestamp("expireDate")));
                    } else {
    ...
    /*HUNGNC START EDIT*/
    public static Map<String,Object> UpdateInventoryItemLocation(DispatchContext ctx,
    ...
    public static Map<String,Object> addInventoryItemLocation(DispatchContext ctx,
    		Map<String, Object> context) throws GenericEntityException {
        	Map<String, Object> result = new FastMap<String, Object>();
        	Delegator delegator = ctx.getDelegator();
        	String inventoryItemId = (String)context.get("inventoryItemId");
    ...
    public static Map<String,Object> updateInventoryItemLocationByInventoryItemId(DispatchContext ctx,
    		Map<String, Object> context) throws GenericEntityException {
        	Map<String, Object> result = new FastMap<String, Object>();
        	Delegator delegator = ctx.getDelegator();
        	GenericValue inventoryItemLocation = delegator.makeValue("InventoryItemLocation");
        	...
        	/*if (inventoryItemId == null){
        		inventoryItemId = delegator.getNextSeqId("InventoryItemLocation");
        	}
        	if (facilityId == null){
        		facilityId = delegator.getNextSeqId("InventoryItemLocation");
        	}*/
        	String locationSeqId = (String)context.get("locationSeqId");
        	/*if (locationSeqId == null){
        		locationSeqId = delegator.getNextSeqId("InventoryItemLocation");
        	}*/
        	String quantity = (String)context.get("quantity");
        	String uomId = (String)context.get("uomId");
    ...
    public static Map<String,Object> processPhysicalStockMoveProduct(DispatchContext ctx,
    		Map<String, Object> context) throws GenericEntityException {
        	Map<String, Object> result = new FastMap<String, Object>();
        	Delegator delegator = ctx.getDelegator();
        	String uomId = (String)context.get("uomId");
        	String facilityId = (String)context.get("facilityId");
        	String locationSeqIdFrom = (String)context.get("locationSeqId");
        	String locationSeqIdTo = (String)context.get("targetLocationSeqId");
        	int quantity = Integer.parseInt((String) context.get("quantityMoved"));
        	GenericValue inventoryItemLocationTo = delegator.findOne("InventoryItemLocation", UtilMisc.toMap("facilityId", facilityId, "locationSeqId", locationSeqIdTo), false);
        	GenericValue inventoryItemLocationFrom = delegator.findOne("InventoryItemLocation", UtilMisc.toMap("facilityId", facilityId, "locationSeqId", locationSeqIdFrom), false);
        	
        		String facilityIdData = (String) inventoryItemLocationTo.get("facilityId");
        		String locationSeqIdToData = (String) inventoryItemLocationTo.get("locationSeqId");
        		String uomIdToData = (String) inventoryItemLocationTo.get("uomId");
        		int quantityToData = Integer.parseInt((String) inventoryItemLocationTo.get("quantity")) ;
        		if(facilityId.equals(facilityIdData) && locationSeqIdTo.equals(locationSeqIdToData) && uomId.equals(uomIdToData))
        		{
        			int quantityMoved = quantity + quantityToData;
        			String quantityCast = Integer.toString(quantityMoved);
        			inventoryItemLocationTo.put("quantity", quantityCast);
        		}
        		String locationSeqIdFromData = (String) inventoryItemLocationFrom.get("locationSeqId");
        		String facilityIdFromData = (String ) inventoryItemLocationFrom.get("facilityId");
        		String uomIdFromData = (String) inventoryItemLocationTo.get("uomId");
        		int quantityFromData = Integer.parseInt((String) inventoryItemLocationFrom.get("quantity")) ;
    			if(locationSeqIdFrom.equals(locationSeqIdFromData) && facilityId.equals(facilityIdFromData) && quantity <= quantityFromData && uomId.equals(uomIdFromData)){
    				int quantityFrom = quantityFromData - quantity;
    				String quantityCastFrom = Integer.toString(quantityFrom);
    				inventoryItemLocationFrom.put("quantity", quantityCastFrom);
    			}
        		inventoryItemLocationTo.store();
        		inventoryItemLocationFrom.store();
        	result.put("facilityId", facilityId);
        	/*result.put("inventoryItemId", inventoryItemId);*/
        	return result;
    	}
