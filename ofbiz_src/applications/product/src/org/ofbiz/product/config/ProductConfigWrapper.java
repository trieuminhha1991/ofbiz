...
public class ProductConfigWrapper implements Serializable {
    ...
    protected String quantityUomId; //TODOCHANGE product AGGREGATED
	...
	//TODOCHANGE product AGGREGATED
    public ProductConfigWrapper(Delegator delegator, LocalDispatcher dispatcher, String productId, String productStoreId, String catalogId, String webSiteId, String currencyUomId, Locale locale, GenericValue autoUserLogin, String quantityUomId) throws Exception {
    	init(delegator, dispatcher, productId, productStoreId, catalogId, webSiteId, currencyUomId, locale, autoUserLogin, quantityUomId);
    }
    ...
    public ProductConfigWrapper(ProductConfigWrapper pcw) {
        ...
        this.quantityUomId = pcw.quantityUomId;
    }
    ...
    private void init(Delegator delegator, LocalDispatcher dispatcher, String productId, String productStoreId, String catalogId, String webSiteId, String currencyUomId, Locale locale, GenericValue autoUserLogin) throws Exception {
    	init(delegator, dispatcher, productId, productStoreId, catalogId, webSiteId, currencyUomId, locale, autoUserLogin, null);
    }
    private void init(Delegator delegator, LocalDispatcher dispatcher, String productId, String productStoreId, String catalogId, String webSiteId, String currencyUomId, Locale locale, GenericValue autoUserLogin, String quantityUomId) throws Exception {
        product = delegator.findOne("Product", UtilMisc.toMap("productId", productId), false);
        if (product == null || !product.getString("productTypeId").equals("AGGREGATED") && !product.getString("productTypeId").equals("AGGREGATED_SERVICE") && !product.getString("productTypeId").equals("AGGR_DIGSERV")) {
        ...
        Map<String, Object> priceMap = calculateProductPrice(dispatcher, priceContext, quantityUomId);
    ...
    protected Map<String, Object> calculateProductPrice(LocalDispatcher localDispatcher, Map<String, Object> priceContext, String quantityUomId) throws GenericServiceException {
    	Map<String, Object> priceMap = null;
        if (this.quantityUomId == null) {
        	priceMap = localDispatcher.runSync("calculateProductPrice", priceContext);
        } else {
        	// TODOCHANGE product AGGREGATED
        	priceContext.put("quantityUomId", quantityUomId);
        	priceMap = localDispatcher.runSync("calculateProductPriceCustom", priceContext);
        }
        return priceMap;
    }
    ...
    public BigDecimal getTotalListPrice() {
        ...
        // TODOCHANGE check new field userOnlyManualPrice
        if (!product.containsKey("useOnlyManualPrice") || product.get("useOnlyManualPrice") == null || "N".equals(product.getString("useOnlyManualPrice"))) {
    ...
    public class ConfigOption implements java.io.Serializable {
        ...
        public ConfigOption(Delegator delegator, LocalDispatcher dispatcher, GenericValue option, ConfigItem configItem, String catalogId, String webSiteId, String currencyUomId, GenericValue autoUserLogin) throws Exception {
            ...
            for (GenericValue oneComponent: componentList) {
                ...
                Map<String, Object> priceMap = calculateProductPrice(dispatcher, fieldMap, null);
                ...
                if (validPriceFound.booleanValue()) {
                    ...
                    Map<String, Object> purchasePriceResultMap = calculateProductPrice(dispatcher, fieldMap, null);
        ...
        public void recalculateOptionPrice(ProductConfigWrapper pcw) throws Exception {
            ...
            for (GenericValue oneComponent: componentList) {
                ...
                Map<String, Object> priceMap = calculateProductPrice(pcw.getDispatcher(), fieldMap, null);
                BigDecimal componentListPrice = (BigDecimal) priceMap.get("listPrice");
                BigDecimal componentPrice = (BigDecimal) priceMap.get("price");
                Boolean validPriceFound = (Boolean)priceMap.get("validPriceFound");
                BigDecimal mult = BigDecimal.ONE;
                ...
                if (validPriceFound.booleanValue()) {
                    ...
                    fieldMap.put("productPricePurposeId", "PURCHASE");
                    Map<String, Object> purchasePriceResultMap = calculateProductPrice(pcw.getDispatcher(), fieldMap, null);
                    BigDecimal purchaseListPrice = (BigDecimal) purchasePriceResultMap.get("listPrice");
                    BigDecimal purchasePrice = (BigDecimal) purchasePriceResultMap.get("price");
                    if (purchaseListPrice != null) {
                        listPrice = purchaseListPrice;
