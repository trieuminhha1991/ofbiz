...
public class ProductWorker {
    ...
    public static List<GenericValue> getAggregatedAssocs(Delegator delegator, String  aggregatedProductId) throws GenericEntityException {
        ...
        if (UtilValidate.isNotEmpty(aggregatedProduct) && ("AGGREGATED".equals(aggregatedProduct.getString("productTypeId")) || "AGGREGATED_SERVICE".equals(aggregatedProduct.getString("productTypeId")) || "AGGR_DIGSERV".equals(aggregatedProduct.getString("productTypeId")))) {
...
//              Debug.logInfo("=====product found:" + productId + " and features: " + selectedFeatures);
    ...
    public static boolean isAggregateDigitalService(Delegator delegator, String aggregatedProductId) {
        try {
            GenericValue aggregatedProduct = delegator.findOne("Product", UtilMisc.toMap("productId", aggregatedProductId), true);
            if (UtilValidate.isNotEmpty(aggregatedProduct) && "AGGR_DIGSERV".equals(aggregatedProduct.getString("productTypeId"))) {
                return true;
            }
        } catch (GenericEntityException e) {
            Debug.logWarning(e.getMessage(), module);
