...
public class JackrabbitScaleImage {
	...
	public static Map<String, Object> scaleImageInAllSize(Map<String, ? extends Object> context, String path, Session session) throws IllegalArgumentException, ImagingOpException, IOException, JDOMException, RepositoryException {

		com.olbius.jackrabbit.api.Node olbiusNode = new NodeImpl(session);
		...
		if (resultXMLMap.containsKey("responseMessage") && resultXMLMap.get("responseMessage").equals("success")) {
			...
			String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.unable_to_parse", locale) + " : ImageProperties.xml";
		...
		if (resultBufImgMap.containsKey("responseMessage") && resultBufImgMap.get("responseMessage").equals("success")) {
			...
			if (imgHeight == 0.0 || imgWidth == 0.0) {
				String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.one_current_image_dimension_is_null", locale)
			...
			for (Map.Entry<String, Map<String, String>> entry : imgPropertyMap.entrySet()) {
				...
				if (resultScaleImgMap.containsKey("responseMessage") && resultScaleImgMap.get("responseMessage").equals("success")) {
					...
					try {
						...
						imageUrl = olbiusNode.createFile(node, sizeType + "." + imgExtension, (String) resultBufImgMap.get("mimeType"), is, false);
					} catch (IllegalArgumentException e) {
						String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.one_parameter_is_null", locale) + e.toString();
						Debug.logError(errMsg, module);
						result.put("errorMessage", errMsg);
						return result;
					} catch (IOException e) {
						String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.error_occurs_during_writing", locale)
								+ e.toString();
						Debug.logError(errMsg, module);
						result.put("errorMessage", errMsg);
						return result;
					...
					if (ScaleImage.sizeTypeList.contains(sizeType)) {
						if(isPublic.equals("Y")) {
							imgUrlMap.put(sizeType, Constant.getUrl() + session.getWorkspace().getName() + imageUrl);
						}
						if(isPublic.equals("N")) {
							imgUrlMap.put(sizeType, com.olbius.jackrabbit.api.Node.WEBDAV_URL + session.getWorkspace().getName() + imageUrl);
						}
			...
			String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.unable_to_scale_original_image", locale) + " : "
					+ node.getPath();
			Debug.logError(errMsg, module);
			result.put("errorMessage", errMsg);
	...
	public static Map<String, Object> scaleImageManageInAllSize(Map<String, ? extends Object> context, String filenameToUse, String viewType,
			String viewNumber, String imageType) throws IllegalArgumentException, ImagingOpException, IOException, JDOMException {
		...
		if (viewType.toLowerCase().contains("main")) {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(ScaleImage.resource, "ProductImageViewType", UtilMisc.toMap("viewType", type),
					locale));
		}
		...
		if (fileLocation.lastIndexOf("/") != -1) {
			filePathPrefix = fileLocation.substring(0, fileLocation.lastIndexOf("/") + 1); // adding
																							// 1
																							// to
																							// include
																							// the
			...
			for (String sizeType : sizeTypeList) {
				...
				if (resultScaleImgMap.containsKey("responseMessage") && resultScaleImgMap.get("responseMessage").equals("success")) {
					...
					if (viewType.toLowerCase().contains("main")) {
						...
						newFileLocation = addFilenameExpander.expandString(UtilMisc.toMap("location", "products", "id", id, "viewtype", viewType,
								"sizetype", sizeType));
					}
					String newFilePathPrefix = "";
					if (newFileLocation.lastIndexOf("/") != -1) {
						newFilePathPrefix = newFileLocation.substring(0, newFileLocation.lastIndexOf("/") + 1); // adding
																												// 1
																												// to
																												// include
																												// the
					...
					if (!targetDir.exists()) {
						...
						if (!created) {
							String errMsg = UtilProperties.getMessage(ScaleImage.resource, "ScaleImage.unable_to_create_target_directory", locale)
									+ " - " + targetDirectory;
