...
function formatnumber(num){
	...
	<div>
	<div class='titleText'>
		<span>${uiLabelMap.BCRMCallsThisMonth} </span>
	</div>
	<div class="valueTotal">
		<span class="calls-content"></span>
		<script type="text/javascript">
			$( document ).ready(function() {
				$(function(){
					$.ajax({url: 'getCallsNumber',
					    type: 'post',
					    async: false,
					    success: function(data) {
					    	var listDatafield = data.listCallsValue;
					    	if(listDatafield.length < 0 || listDatafield[0] == null){
					    		$(".calls-content").html("0");
					    	} else{
					    		$(".calls-content").html(formatnumber(listDatafield[0]));	
					    	}
					    },
					    error: function(data) {
					    	alert('Error !!');
