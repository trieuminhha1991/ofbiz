if (typeof (PrototypeCustomization) == "undefined") {
	var PrototypeCustomization = (function() {
		var init = function() {
			PrototypeCustomization.init = function() {
			};
			...
			myStringlibrary.prototype = {
				replaceAll : function(str1, str2, ignore) {
					return this.replace(new RegExp(str1.replace(
							/([\/\,\!\\\^\$\{\}\[\]\(\)\.\*\+\?\|\<\>\-\&])/g,
							"\\$&"), (ignore ? "gi" : "g")),
							(typeof (str2) == "string") ? str2.replace(/\$/g,
									"$$$$") : str2);
				},
				toInt : function() {
				...
				toTimeStamp : function() {
					...
					if (date == "") {
						...
						var timeStamp = splDate[2] + '-' + splDate[1] + '-'
								+ splDate[0];
						return timeStamp;
					}
				},
