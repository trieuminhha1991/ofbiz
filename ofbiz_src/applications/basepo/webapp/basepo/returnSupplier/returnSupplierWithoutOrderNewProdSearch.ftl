			...
			<div class="row-fluid">
				<div class="span12 form-window-content-custom">
					...
					<div class='row-fluid margin-bottom0'>
						...
						<div class='span3 align-right'><label>${uiLabelMap.BSAverageCost} (T):</label></div>
						<div class='span3'><span id="wn_prodsearch_returnPriceSug" class="red"></span></div>
					</div>
...
<script type="text/javascript" src="/aceadmin/jqw/jqwidgets/ett/olb.combobox.search.remote.js"></script>
<script type="text/javascript">
	$(function(){
		OlbReturnNewItemsProdSearch.init();
	});
	var OlbReturnNewItemsProdSearch = (function(){
		var itemProductTodo;
		var validatorVAL;
		...
		var initElement = function(){
			jOlbUtil.windowPopup.create($("#popupQuotItemsProdSearchAdd"), {width: 520, height: 420, cancelButton: $("#wn_prodsearch_alterCancel")});
		...
		var initValidateForm = function(){
			var extendRules = [
				{input: '#wn_prodsearch_listPriceVAT, #wn_prodsearch_listPrice', message: validFieldRequire, action: 'valueChanged', 
				...
				{input: '#wn_prodsearch_returnQuantity', message: validFieldRequire, action: 'valueChanged', 
					rule: function(input, commit){
		...
		function getInfoProductAndAddToGrid(rowData) {
			...
			if (!facilityId) {
				OlbCore.alert.info("${uiLabelMap.BPSelectFacilityBeforeSearchProduct}");
				return false;
			}
			
			$.ajax({
				type: 'POST',
				url: "getInfoProductAddToReturnSupplier",
				data: {
		...
		function addItemFromPopupToGrid(){
			...
			OlbReturnAddProductItems.addItemsToGridPopup([itemProductTodo]);
			closeWindowAdd();
			productSearchCBBS.focusSearch();
		}
		function openWindowAdd(){
			if (itemProductTodo && itemProductTodo.productId != null) {
				...
				$("#wn_prodsearch_returnQuantityStr").html("(" + itemProductTodo.quantityUomId + ")");
				...
				var unitPriceBef = itemProductTodo.unitPriceBef;
		 		...
		 		if (itemProductTodo.taxPercentage) taxPercentage = itemProductTodo.taxPercentage;
   				var unitPriceVATValue = formatcurrency(Math.round(unitPriceVAT + (unitPriceVAT * taxPercentage)/100), itemProductTodo.currencyUomId);
   				
				var returnPriceSug = itemProductTodo.returnPriceSug;
   				var returnPriceSugValue = formatcurrency(returnPriceSug, currencyUomId);
   				if (typeof returnPriceSug == "undefined") returnPriceSug = null;
   				
				$("#wn_prodsearch_unitPriceBef").html(unitPriceBefValue);
				$("#wn_prodsearch_unitPriceVAT").html(unitPriceVATValue);
				$("#wn_prodsearch_returnPriceSug").html(returnPriceSugValue);
				$("#wn_prodsearch_listPrice").jqxNumberInput("setDecimal", returnPriceSug);
			} else {
				...
				$("#wn_prodsearch_returnPriceSug").html("");
			...
			$("#wn_prodsearch_returnQuantity").jqxNumberInput('focus');
		}
