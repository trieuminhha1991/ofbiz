...
<#assign columnlist = "
			{text: '${uiLabelMap.BSQuotationId}', dataField: 'productQuotationId', width: 140, cellClassName: cellClassjqxQuotation, 
    			cellsrenderer: function(row, colum, value) {
                	return \"<span><a href='viewQuotationPO?productQuotationId=\" + value + \"'>\" + value + \"</a></span>\";
                }
          	}, 
			{text: '${uiLabelMap.BSQuotationName}', dataField: 'quotationName', minwidth: 120, cellClassName: cellClassjqxQuotation},
			{text: '${uiLabelMap.BSCurrencyUomId}', dataField: 'currencyUomId', width: 100, cellClassName: cellClassjqxQuotation}, 
...
<#if hasOlbPermission("MODULE", "PRODQUOTATIONPO_NEW", "")>
	<#assign tmpCreateUrl = "icon-plus open-sans@${uiLabelMap.wgaddnew}@newQuotationPO"/>
</#if>
<#--
...
<@jqGrid id="jqxgridQuotation" url="jqxGeneralServicer?sname=JQListProductQuotationPO" columnlist=columnlist dataField=dataField
		        ...
		        switch(tmpId) {
		    		case contextMenuItemId + "_viewdetailnewtab": {
		    			if (rowData) {
							...
							var url = 'viewQuotationPO?productQuotationId=' + productQuotationId;
		    		...
		    		case contextMenuItemId + "_createnew": {
			        	var win = window.open("newQuotationPO", "_self");
		        		window.focus();
		        		break;
		    		};
