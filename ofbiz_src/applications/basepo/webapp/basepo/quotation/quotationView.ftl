...
<#if hasOlbPermission("MODULE", "PRODQUOTATIONPO_APPROVE", "")>
		...
		<div class="row-fluid">
			...
			<div class="span6">
				...
				<#if partyIdApply?exists>
					<div class="row-fluid">
						<div class="div-inline-block">
							<label>${uiLabelMap.BSSupplier}:</label>
						</div>
						<div class="div-inline-block">
							<#assign partyApply = delegator.findOne("PartyFullNameDetailSimple", {"partyId": partyIdApply}, false)!/>
							<span><#if partyApply?exists>${partyApply.fullName?if_exists} (${partyApply.partyCode?if_exists})<#else>${partyIdApply}</#if></span>
						</div>
					</div>
				</#if>
				<#if partyGroupIdApply?exists>
					<div class="row-fluid">
						...
						<div class="div-inline-block">
							...
							<span><#if partyGroupApply?exists>${partyGroupApply.fullName?if_exists} (${partyGroupApply.partyCode?if_exists})<#else>${partyGroupIdApply}</#if></span>
						</div>
					</div>
				</#if>
					...
					<table class="table table-striped table-bordered table-hover">
						<thead>
							<tr>
								...
								<th rowspan="2" class="center" style="width: 6%">${uiLabelMap.BSTax}</th>
								<th rowspan="2" class="center" style="width: 8%">${uiLabelMap.BSUom}</th>
								<th rowspan="2" class="center" style="width: 10%">${uiLabelMap.BSPacking}</th>
						...
						<#list listProductQuotationRuleData as quotationRule>
				        	<tr>
				        		...
				        		<td><#if quotationRule.taxPercentage?exists>${quotationRule.taxPercentage} %</#if></td>
				        		...
				        		<td class="align-right">
				        			<#if quotationRule.priceToDistNormalAfterVAT?exists>
		        						<@ofbizCurrency amount=quotationRule.priceToDistNormalAfterVAT isoCode=productQuotation.currencyUomId/>
		            				</#if>
				        		</td>
		            			<#--
		            			<td class="align-right">
