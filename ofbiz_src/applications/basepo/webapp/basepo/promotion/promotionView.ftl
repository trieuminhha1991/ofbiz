...
<#if hasOlbPermission("MODULE", "PROMOPO", "APPROVE")>
	...
	<div class="widget-main">
		<div class="row-fluid">
			<div class="form-horizontal basic-custom-form form-decrease-padding" id="updateQuotation" name="updateQuotation" style="display: block;">
				<div class="row margin_left_10 row-desc">
					...
					<div class="span6">
						...
						<div class="control-group">
							<label class="control-label-desc">${uiLabelMap.POSupplier}:</label>
							<div class="controls-desc">
								<#if supplierPromoAppl?exists>
									<ul class="unstyled spaced2" style="margin: 0 0 0 15px;">
										<#list supplierPromoAppl as itemSupplierAppl>
											<#assign supplier = itemSupplierAppl.getRelatedOne("PartyGroup", true)/>
											<li style="margin-bottom: 0; margin-top:0">
												<i class="icon-plus green"></i>
												<#if supplier.groupName?exists>${supplier.groupName}<#else>${supplier.partyId}</#if>
											</li>
											<#if itemSupplierAppl_index &gt; 1 && supplierPromoAppl?size &gt; 2>
												<a href="javascript:void(0)" id="showSupplierViewMore">${uiLabelMap.BSViewMore} (${supplierPromoAppl?size - 3})</a>
												<#break/>
											</#if>
										</#list>
									</ul>
									<div style="display:none" id="supplierViewMore">
										<ul class="unstyled spaced2" style="margin: 0 0 0 15px;">
											<#list supplierPromoAppl as itemSupplierAppl>
												<#assign supplier = itemSupplierAppl.getRelatedOne("PartyGroup", true)/>
												<li style="margin-bottom: 0; margin-top:0">
			...
			<div style="text-align:right">
				<#if hasOlbPermission("MODULE", "PROMOPO", "APPROVE") && (productPromo.statusId?exists && "PROMO_CANCELLED" != productPromo.statusId)>
						...
						<div class="row-fluid container-approve">
								...
								<#if currentStatusId?exists && currentStatusId == "PROMO_CREATED">
									<#if hasApproved>
										<span class="widget-toolbar none-content">
											...
											<form name="PromoAccept" method="post" action="<@ofbizUrl>changePromoStatusPO</@ofbizUrl>">
											...
											<form name="PromoCancel" method="post" action="<@ofbizUrl>changePromoStatusPO</@ofbizUrl>">
							...
							<div class="span6" style="padding-top: 5px">
								<div class="row-fluid">
									...
									<div class="span9">
										<form name="updatePromoThruDate" id="updatePromoThruDate" method="POST" action="<@ofbizUrl>updatePromoThruDatePO</@ofbizUrl>" style="float:left">
											<input type="hidden" name="productPromoId" value="${productPromo.productPromoId?if_exists}" />
					...
					<#else>
						<span style="color:#D7432E">${uiLabelMap.BSThisPromotionHasExpired}</span>
					</#if>
				</#if>
			</div>
		</div>	
...
<#if hasOlbPermission("MODULE", "PROMOPO", "APPROVE") && (productPromo.statusId?exists && "PROMO_CANCELLED" != productPromo.statusId)>
	<#if (promotionPO.thruDate?exists && promotionPO.thruDate &gt; nowTimestamp) || !(promotionPO.thruDate?exists)>
