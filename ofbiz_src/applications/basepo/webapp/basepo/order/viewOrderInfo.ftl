...
<script type="text/javascript">
	var priorityData = [<#if listPriority?exists><#list listPriority as priorityItem>{
		enumId: "${priorityItem.enumId}",
		enumCode: "${priorityItem.enumCode?if_exists}",
		description: "${StringUtil.wrapString(priorityItem.get("description", locale))}"
	},</#list></#if>];
</script>
	...
	<div class="row-fluid">
						...
						<div class="profile-info-value">
							<div id="spanOrderReservationPriority">
								...
								<#if orderHeader.statusId != "ORDER_CANCELLED">
									<a href="javascript:orderCommon.openEditPriority();" style="margin-left:5px"><i class="fa fa-pencil-square"></i></a>
								</#if>
							</div>
							<form id="setOrderReservationPriority" name="setOrderReservationPriority" method="post" action="<@ofbizUrl>setOrderReservationPriority</@ofbizUrl>" style="display:none">
								...
								<a href="javascript:orderCommon.cancelEditPriority();" id="btnCancelSavePriority" data-rel="tooltip" title="${uiLabelMap.DACancel}" data-placement="bottom" class="btn btn-mini"><i class="icon-remove"></i></a>
							</form>
						</div>
					</div>
				</#if>
					...
					<div class="profile-info-row" id="viewed">
						...
						<div class="profile-info-value">
							...
							<form id="orderUnViewed" action="" style="display:inline-block">
								<a href="javascript:orderCommon.markOrderUnViewed();" data-rel="tooltip" title="${uiLabelMap.DAUnMark}" data-placement="bottom">
		...
		<div class="span5">
			...
			<#if displayParty?has_content || orderContactMechValueMaps?has_content>
				<div class="profile-user-info profile-user-info-striped" style="margin-left:0;margin-right:0">
					<div class="profile-info-row">
						<div class="profile-info-name">${uiLabelMap.Supplier}</div>
						<div class="profile-info-value">
		...
		<div class="span12">
			<#if orderHeader?has_content>
				<div id="order-notes-container">
					...
					${screens.render("component://basepo/widget/OrderScreens.xml#OrderNoteAjax")}
				</div>
			</#if>
		</div>
	</div><!--.row-fluid-->
