...
var OlbQuotationTotal = (function(){
	...
	var updateGridProdItems = function(){
		var itemsStr = "";
		var supplier = OlbQuotationInfo.getObj().partyDDB.getValue();
		var currencyUomId = OlbQuotationInfo.getObj().currencyUomIdCBB.getValue();
		if (supplier) {
			itemsStr += "&supplierId=" + supplier;
		}
		if (currencyUomId) {
			itemsStr += "&currencyUomId=" + currencyUomId;
		}
		OlbGridUtil.updateSource($('#jqxgridQuotItemsProdAdd'), "jqxGeneralServicer?sname=JQGetListQuotaProductBySupplier" + itemsStr);
	};
	...
	var finishCreateQuotation = function(){
		...
		dataMap.productQuotationModuleTypeId = "PURCHASE_QUOTATION";
	...
	var processResultCreateQuotation = function(data){
		jOlbUtil.processResultDataAjax(data, "default", function(data){
		        	...
		        	if (OlbCore.isNotEmpty(productQuotationId)) {
		        		window.location.href = "viewQuotationPO?productQuotationId=" + productQuotationId;
		        	}
				}
		);
	};
