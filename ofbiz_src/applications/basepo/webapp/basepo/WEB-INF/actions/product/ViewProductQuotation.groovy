				...
				if (quotationRule.productId) {
					...
					if (product != null) {
						...
						if (quotationRule.listPrice != null) {
							...
							BigDecimal listPriceVAT = quotationRule.getBigDecimal("listPriceVAT");
							if (listPriceVAT) {
								priceToDistNormalAfterVAT = Math.round(listPriceVAT.multiply(BigDecimal.valueOf(100)));
								priceToDistNormalAfterVAT = priceToDistNormalAfterVAT.divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
							} else {
								priceToDistNormalAfterVAT = listPriceVAT;
							}
