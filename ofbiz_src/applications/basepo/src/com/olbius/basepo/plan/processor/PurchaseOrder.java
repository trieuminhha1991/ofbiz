...
public class PurchaseOrder {
	...
	public PurchaseOrder(SQLProcessor processor, Delegator delegator, Date fromDate, Date thruDate, Object product,
	...
	private OlbiusQuery init() {
		...
		condition.and(Condition.makeEQ("pof.party_to_dim_id", getParty(organizationId)));
		condition.and(Condition.makeEQ("pof.product_dim_id", getProduct(product)));
		condition.and(Condition.make("sd.status_id", Condition.NOT_EQ, "ITEM_CANCELLED"));
		condition.and(Condition.make("sd.status_id", Condition.NOT_EQ, "ITEM_REJECTED"));
		query.select(new Sum("pof.quantity"), "quantity").from("purchase_order_fact", "pof")
				.join(Join.INNER_JOIN, "date_dimension", "dd", "dd.dimension_id = pof.order_date_dim_id")
				.join(Join.INNER_JOIN, "status_dimension", "sd", "dd.dimension_id = pof.status_dim_id")
				.where(condition);
		return query;
	}
