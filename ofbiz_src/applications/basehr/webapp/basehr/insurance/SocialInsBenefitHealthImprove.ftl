...
<#include "script/SocialInsBenefitHealthImproveScript.ftl"/>
<#assign datafield = "[{name: 'partyId', type: 'string'},
					   ...
					   {name: 'dayLeaveConcentrate', type: 'number'},
					   {name: 'dayLeaveFamily', type: 'number'},
...
<#assign columnlist = "{datafield: 'emplLeaveId', hidden: true},
						...
						{text: '${StringUtil.wrapString(uiLabelMap.InsuranceBenefitType)}', datafield: 'benefitTypeId', filterType : 'checkedlist',
							width: '15%', sortable: false,columntype: 'dropdownlist',
							cellsrenderer: function (row, column, value) {
							...
							createeditor: function (row, column, editor) {
								...
								editor.jqxDropDownList({source: dataAdapter, displayMember: 'description', valueMember: 'benefitTypeId', 
									height: 25, width: 70, dropDownWidth: 300,
								});
						...
						{text: '${StringUtil.wrapString(uiLabelMap.InsuranceBenefitCondition)}', datafield: 'statusConditionBenefit', width: 150},
						{text: '${StringUtil.wrapString(uiLabelMap.InsuranceParticipatePeriod)}', datafield: 'insuranceParticipatePeriod', width: 150},
						{text: '${StringUtil.wrapString(uiLabelMap.DayLeaveConcentrate)}', cellsalign: 'right', datafield: 'dayLeaveConcentrate', width: 130, 
							editable: true, filterType:'number', columntype: 'number', columngroup: 'dayLeaveInPeriod'},
						{text: '${StringUtil.wrapString(uiLabelMap.DayLeaveFamily)}', cellsalign: 'right', datafield: 'dayLeaveFamily', width: 130, editable: true, 
								filterType:'number', columntype: 'number', columngroup: 'dayLeaveInPeriod'},
						{text: '${StringUtil.wrapString(uiLabelMap.AllowanceAmountInPeriod)}', cellsalign: 'right', datafield: 'allowanceAmount', width: 170, 
							filterType :'number', columngroup: 'nbrOfProposal',
							cellsrenderer: function (row, column, value) {
								if(value){
...
<#assign columngroups = "{text: '${StringUtil.wrapString(uiLabelMap.YearOfBirth)}', align: 'center', name: 'partyBirthDate'},
		 {text: '${StringUtil.wrapString(uiLabelMap.NumberOfProposal)}', name: 'nbrOfProposal', align: 'center'},
		 {text: '${StringUtil.wrapString(uiLabelMap.InsDayLeaveInPeriod)}', name: 'dayLeaveInPeriod', align: 'center', parentgroup: 'nbrOfProposal'},
 "/>	
</script>
...
<div class="widget-box transparent no-bottom-border">
	<div class="widget-header">
		<h4>${uiLabelMap.InsBenefitHealthImprovementTitle}</h4>
...
<script type="text/javascript" src="/hrresources/js/insurance/SocialInsBenefitHealthImprovement.js"></script>
<script type="text/javascript" src="/hrresources/js/insurance/SocialInsBenefitHealthImprovEvent.js"></script>