var approve_kpi = (function(){
	...
	var initJqxWindow = function(){
		createJqxWindow($('#pushResultKPIWindow'), 555, 554, initJqxEditor);
	};
	/*var initJqxValidator = function(){
		$('#pushResultKPIWindow').jqxValidator({
			rules: [
	...
	var initJqxInput = function(){
		...
		$('#kpiActual').jqxNumberInput({width : '97%', height : '20px', disabled : false, max: 999999999999, digits: 11});
	};
	...
	var initJqxWindowEvent = function(){
		$('#pushResultKPIWindow').on('close', function(event){
			Grid.clearForm($(this));
            $('#kpiActual').jqxNumberInput('clear');
			/*$('#rejectKPI').jqxRadioButton({checked : false});
			$('#approveKPI').jqxRadioButton({checked : false});*/
			
		});
		$('#pushResultKPIWindow').on('open', function(event){
			$('#kpiComment').jqxEditor('val', _comment);
		});
	};
	
	var initBtnEvent = function(){
		$('#cancelResultKPIType').click(function(){
			$('#pushResultKPIWindow').jqxWindow('close');
		});
		
		$('#saveResultKPIType').click(function(){
			/*var valid = $("#pushResultKPIWindow").jqxValidator('validate');
			if(!valid){
				return;
			}*/
	...
	var getDataAppr = function(){
		...
		data.kpiActual = $('#kpiActual').jqxNumberInput('val');
	...
	var setWindowPushResultData = function(data){
	...
	return{
		init : init,
        setWindowPushResultData : setWindowPushResultData,
