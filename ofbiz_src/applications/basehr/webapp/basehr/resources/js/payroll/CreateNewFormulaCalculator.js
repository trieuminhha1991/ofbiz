    ...
    switch(gubun)
	    {
	        case 1:
	            if(col.length > 0){
	                if(last_gubun == gubun || last_gubun == "2" || last_gubun == "3" || (last_gubun == "8" && col[col.length-1] == ")")){
	                    alert (uiLabelMap.PayrollFormulaNotValid + "\n\n" + uiLabelMap.PayrollPleaseReviewTheFormula); 
	                    $(oSelitem).focus();
	                    return;
	                }
	            }
	        ...
	        case 4: 
	            if(col.length > 0){
	                ...
	                if(chkok < 1){
	                    alert (uiLabelMap.PleaseSelectOperator);
	                    $(oSelitem).focus();
	                    return;
	                }
	        ...
	        case 5: 
	            if(col.length > 0){
	                if(last_gubun == "2" || last_gubun == "4" || last_gubun == "5" || last_gubun == "6" || last_gubun == "7" || (last_gubun == "8" && col[col.length-1] == "(") ){
	                    ...
	                    if(arleft != 0 ){
	                        alert(uiLabelMap.PayrollFormulaNotValid + "\n\n" + uiLabelMap.PleaseCheckParentheses); 
	                        $(oSelitem).focus();
	                        return;
	                    }
	                }
	        ...
	        case 6: 
	            if(col.length == 0){
	                if(colval != "-" ){
	                    alert (uiLableMap.ActionCannotSelectedFirst); 
	                    $(oSelitem).focus();
	                    return;
	                }
	            }else{
	        ...
	        case 8: 
	            if(col.length == 0 && colval == "("){}
	                ...
	                if(arleft < 0 || (arleft == 0 && colval == ")") ){
	                    alert(uiLabelMap.PleaseSelectedItemFirst); 
	                    $(oSelitem).focus();
	                    return;
	                }

	                if(col[col.length -1] == "(" && colval == ")"){
	                    alert (uiLabelMap.CannotOnlyEnterParentheses); 
	                    $(oSelitem).focus();
	                    return;
	                }
	                ...
	                if( !( (colval == "(" && (last_gubun == 4 || last_gubun == 5 || last_gubun == 6 || last_gubun == 7 || col[col.length-1] == "(")) || (colval == ")" && (last_gubun == 1 || last_gubun == 3 || col[col.length-1] == ")") ) ) ){
	                    alert(uiLabelMap.PayrollFormulaNotValid + "\n\n" + uiLabelMap.PleaseCheckBrackets); 
	                    $(oSelitem).focus();
	                    return false;
	                }
	             }
