package com.olbius.basehr.payroll.util;
...
public class PayrollUtil {
	...
	public static String getActualValueByPeriod(GenericValue timekeepingSummaryParty, PeriodEnum period, String value) throws ScriptException{
		String retValue = value;
		Double workdayActual = timekeepingSummaryParty.getDouble("workdayActual");
		Double workdayStandard = timekeepingSummaryParty.getDouble("workdayStandard");
		String workdayActualStr = String.valueOf(workdayActual);
		String workdayStandardStr = String.valueOf(workdayStandard);
		switch (period){
			case YEARLY:
				retValue = PayrollUtil.evaluateStringExpression(value + "/ 12" + "*" + workdayActualStr + "/" + workdayStandardStr);
				break;
			case QUARTERLY:
				retValue = PayrollUtil.evaluateStringExpression(value + "/ 3" + "*" + workdayActualStr + "/" + workdayStandardStr);
				break;
			case MONTHLY:
				retValue = PayrollUtil.evaluateStringExpression(value + "*" + workdayActualStr + "/" + workdayStandardStr);
				break;
			case WEEKLY:
				retValue = PayrollUtil.evaluateStringExpression(value + "* 4" + "*" + workdayActualStr + "/" + workdayStandardStr);
				break;
			case DAILY:
				retValue = PayrollUtil.evaluateStringExpression(value + "*" + workdayActualStr);
				break;
			case NA:
				retValue = value;
				break;
	...
	//maybe delete
		...
		public static String getActualValueByPeriod(DispatchContext dctx, String emplTimesheetId, String partyId, Timestamp fromDate,
			...
			if(PeriodEnum.DAILY.equals(period)){
				dayLeaveMap = TimekeepingUtils.getEmplDayLeaveByTimekeeper(dctx, emplTimesheetId, fromDate, thruDate, partyId);
				float totalDayEmplLeave = (Float)dayLeaveMap.get("totalDayLeave");
				Map<Double, Double> leavePaidAndRate = (Map<Double, Double>)dayLeaveMap.get("leavePaidAndRate");
				...
				float totalDayEmplLeavePaid = 0f;
				for(Map.Entry<Double, Double> entry: leavePaidAndRate.entrySet()){
					totalDayEmplLeavePaid += entry.getKey() * entry.getValue();
				}
				...
				if(thruDate.after(endCycle)){
					...
					while(tempStartCycle.before(thruDate)){
						...
						dayLeaveMap = TimekeepingUtils.getEmplDayLeaveByTimekeeper(dctx, emplTimesheetId, tempFromDate, tempThrudate, partyId);
						float tempDayEmplLeave = (Float)dayLeaveMap.get("totalDayLeave");
						Map<Double, Double> tempLeavePaidAndRate = (Map<Double, Double>)dayLeaveMap.get("leavePaidAndRate");
						float tempDayEmplLeaveApproved = 0f;
						for(Map.Entry<Double, Double> entry: tempLeavePaidAndRate.entrySet()){
							tempDayEmplLeaveApproved += entry.getKey() * entry.getValue();
						}
	...
	public static Double getTotalValuePartyPayrollFormula(Delegator delegator,
		...
		for(GenericValue formula: listFormula){
			...
			BigDecimal value = tempGv.getBigDecimal("value");
			Double tempValue = value.doubleValue();
			retValue += tempValue;
	...
	@SuppressWarnings("unchecked")
	public static Map<String, Object> getSalaryCalculateParty(DispatchContext dctx, Delegator delegator, String orgId, 
			String payrollTableId, Timestamp fromDate, Timestamp thruDate, Locale locale, TimeZone timeZone,
		...
		for(GenericValue paramPosType: paramPosTypes){
			...
			if(!addressInExclude && addressInInclude){
				...
				for(Map<String, Timestamp> entry: fromThruDateMapList){
					...
					GenericValue tempGv = delegator.makeValue("PayrollParamPositionTypeAndParameters");
		...
		for(GenericValue emplPositionTypeRate: emplPositionTypeRateList){
			...
			if(!addressInExclude && addressInInclude){
				...
				for(Map<String, Timestamp> entry: fromThruDateMapList){
					...
					//if(tempFromDate)
					tempGv.set("emplPositionTypeRateId", emplPositionTypeRate.getString("emplPositionTypeRateId"));
	...
	public static boolean isPayrolTableEditable(GenericValue userLogin, Delegator delegator, String payrollTableId) throws GenericEntityException{
		...
		List<String> partyAccList = SecurityUtil.getPartiesByRolesWithCurrentOrg(userLogin, PropertiesUtil.ACC_MANAGER_ROLE, delegator);
		if(statusId.equals("PYRLL_TABLE_CREATED") || statusId.equals("PYRLL_TABLE_CALC") || statusId.equals("PYRLL_TABLE_REJECT")){
			return true;
		}
		if(partyAccList.contains(userLogin.getString("partyId")) && "PYRLL_WAIT_APPR".equals(statusId)){
			return true;
		}
		return false;
	}
	public static List<GenericValue> getListPartyApprovalPayrollTable(Delegator delegator, GenericValue userLogin, String payrollTableId) throws GenericEntityException{
		List<String> listAccParty = SecurityUtil.getPartiesByRolesWithCurrentOrg(userLogin, PropertiesUtil.ACC_MANAGER_ROLE, delegator);
		EntityCondition cond = EntityCondition.makeCondition(EntityCondition.makeCondition("payrollTableId", payrollTableId),
															 EntityJoinOperator.AND,
															 EntityCondition.makeCondition("partyId", EntityJoinOperator.IN, listAccParty));
		List<GenericValue> retList = delegator.findList("PayrollTableRecordStatusAndUserLogin", cond, null, UtilMisc.toList("-statusDatetime"), null, false);
		return retList;
	}
	
	public static List<String> getListPeriodTypeOfAllowance(Delegator delegator) throws GenericEntityException{
		List<String> periodTypeList = FastList.newInstance();
		List<GenericValue> allowanceParamList = delegator.findByAnd("PayrollParameters", UtilMisc.toMap("paramCharacteristicId", "PHU_CAP"), null, false);
		periodTypeList = EntityUtil.getFieldListFromEntityList(allowanceParamList, "periodTypeId", true);
