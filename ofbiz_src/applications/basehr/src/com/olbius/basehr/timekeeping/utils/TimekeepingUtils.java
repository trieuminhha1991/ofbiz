package com.olbius.basehr.timekeeping.utils;
...
public class TimekeepingUtils {
	...
	public static Map<String, Object> getEmplDayLeaveByTimekeeper(DispatchContext dctx, String emplTimesheetId, Timestamp fromDate, Timestamp thruDate, String partyId) throws GenericEntityException, GenericServiceException{
		...
		Timestamp fromDateNotCalcTs = UtilDateTime.getDayStart(nowTimestamp, 1);
		Date fromDateNotCalc = new Date(fromDateNotCalcTs.getTime());
		...
		conditions.add(EntityCondition.makeCondition("dateAttendance", EntityJoinOperator.LESS_THAN_EQUAL_TO, nowDate));
		...
		if(endDate.compareTo(fromDateNotCalc) >= 0){
			if(startDate.compareTo(fromDateNotCalc) >= 0){
				fromDateNotCalc = startDate;
			}
			Map<String, Object> workTypeResult = null;
			Calendar cal = Calendar.getInstance();
			cal.setTime(fromDateNotCalc);
			while(fromDateNotCalc.compareTo(endDate) <= 0){
				workTypeResult = TimekeepingUtils.getWorkingShiftDayOfParty(dctx, partyId, fromDateNotCalc);
				Boolean isDayLeave = (Boolean)workTypeResult.get("isDayLeave");
				if(!isDayLeave){
					String workTypeId = (String)workTypeResult.get("workTypeId");
				...
				fromDateNotCalc = new Date(cal.getTimeInMillis());
			}
		}
		...
		for(GenericValue tempGv: listWorkOverTime){
			...
			if("LAM_THEM_NGAY_THUONG".equals(code)){
				if(!isDayLeave){
					...
					retValue += getActualHourWorkingOvertime(delegator, partyId, tempDate, startTime, endTime);
				}
	...
	public static Float getActualHourWorkingOvertime(Delegator delegator, String partyId, Date dateWork, Time actualStartTime, Time actualEndTime) throws GenericEntityException{
		Float actualOvertimeHours = 0f;
		List<GenericValue> workingShiftEmplInDateList = delegator.findByAnd("WSEmployeeAndWS", UtilMisc.toMap("partyId", partyId, "dateWork", dateWork), null, false);
		if(workingShiftEmplInDateList != null){
			GenericValue workingShiftEmplInDate = workingShiftEmplInDateList.get(0);
			Time shiftStartTime = workingShiftEmplInDate.getTime("shiftStartTime");
			Time shiftEndTime = workingShiftEmplInDate.getTime("shiftEndTime");
			Time shiftBreakStart = workingShiftEmplInDate.getTime("shiftBreakStart");
			Time shiftBreakEnd = workingShiftEmplInDate.getTime("shiftBreakEnd");
			if(actualStartTime != null && actualEndTime != null){
				actualStartTime = TimekeepingUtils.getActualTimeByStartEndShiftTime(shiftStartTime, shiftEndTime, actualStartTime);
				actualEndTime = TimekeepingUtils.getActualTimeByStartEndShiftTime(shiftStartTime, shiftEndTime, actualEndTime);
				shiftBreakStart = TimekeepingUtils.getActualTimeByStartEndShiftTime(shiftStartTime, shiftEndTime, shiftBreakStart);
				shiftBreakEnd = TimekeepingUtils.getActualTimeByStartEndShiftTime(shiftStartTime, shiftEndTime, shiftBreakEnd);
				if(actualStartTime.before(shiftStartTime)){
					actualStartTime = shiftStartTime;
				}
				if(shiftBreakEnd != null && shiftBreakStart != null){
					if(actualStartTime.before(shiftBreakStart) && actualEndTime.after(shiftBreakEnd)){
						actualOvertimeHours += (float)(shiftBreakStart.getTime() - actualStartTime.getTime())/(1000 * 3600);
						actualOvertimeHours += (float)(actualEndTime.getTime() - shiftBreakEnd.getTime())/(1000 * 3600);
					}else{
						if(actualStartTime.after(shiftBreakStart) && actualStartTime.before(shiftBreakEnd)){
							actualStartTime = shiftBreakEnd;
						}
			...
			if(actualStartTime != null && actualEndTime != null){
				if(actualEndTime.before(actualStartTime)){
					actualEndTime = new Time(actualEndTime.getTime() + DateUtil.ONE_DAY_MILLIS);
				}
				actualOvertimeHours += (float)(actualEndTime.getTime() - actualStartTime.getTime())/(1000 * 3600);
			}
		}
	...
	public static Time getActualTimeByStartEndShiftTime(Time shiftStartTime, Time shiftEndTime, Time time) {
		if(shiftEndTime == null || shiftStartTime == null || time == null){
			return null;
		}
		Calendar cal = Calendar.getInstance();
		Calendar shiftStartTimeCal = Calendar.getInstance();
		Calendar shiftEndTimeCal = Calendar.getInstance();
		shiftStartTimeCal.setTime(shiftStartTime);
		shiftEndTimeCal.setTime(shiftEndTime);
		shiftStartTimeCal.set(Calendar.YEAR, cal.get(Calendar.YEAR));
		shiftStartTimeCal.set(Calendar.DATE, cal.get(Calendar.DATE));
		shiftStartTimeCal.set(Calendar.MONTH, cal.get(Calendar.MONTH));
		shiftEndTimeCal.set(Calendar.YEAR, cal.get(Calendar.YEAR));
		shiftEndTimeCal.set(Calendar.DATE, cal.get(Calendar.DATE));
		shiftEndTimeCal.set(Calendar.MONTH, cal.get(Calendar.MONTH));
		if(shiftEndTime.compareTo(shiftStartTime) > 0){
			return time;
		}
		Calendar timeCal = Calendar.getInstance();
		timeCal.setTime(time);
		int timeHours = timeCal.get(Calendar.HOUR_OF_DAY);
		//int shiftStartTimeHour = shiftStartTimeCal.get(Calendar.HOUR_OF_DAY); 
		if(timeHours >= 12){
			return time;
		}
		time = new Time(time.getTime() + DateUtil.ONE_DAY_MILLIS);
		return time;
	}
	...
	public static Time getTimeStartCalcOTWS(Delegator delegator, String workingShiftId, Date date) throws GenericEntityException {
		...
		Time startOverTimeAfterShift = null;
		if("ALL_SHIFT".equals(workTypeId) || "SECOND_HALF_SHIFT".equals(workTypeId)){
			startOverTimeAfterShift = workingShift.getTime("startOverTimeAfterShift");
			if(startOverTimeAfterShift == null){
				startOverTimeAfterShift = shiftEndTime;
			}
		}else if("FIRST_HALF_SHIFT".equals(workTypeId)){
			startOverTimeAfterShift = shiftBreakEnd;
			if(startOverTimeAfterShift == null){
				startOverTimeAfterShift = DateUtil.getMiddleTimeBetweenTwoTime(shiftStartTime, shiftEndTime);
			}
		}
		if(startOverTimeAfterShift == null){
			startOverTimeAfterShift = shiftEndTime;
	...
	public static Time getTimeEndCalcOTWS(Delegator delegator, String workingShiftId, Date date) throws GenericEntityException {
		GenericValue workingShift = delegator.findOne("WorkingShift", UtilMisc.toMap("workingShiftId", workingShiftId), false);
		String dayOfWeek = DateUtil.getDayName(date);
		GenericValue workingShiftDayWeek = delegator.findOne("WorkingShiftDayWeek", UtilMisc.toMap("workingShiftId", workingShiftId, "dayOfWeek", dayOfWeek), false); 
		String workTypeId = null;
		...
		Time shiftEndTime = workingShift.getTime("shiftEndTime");
		Time shiftStartTime = workingShift.getTime("shiftStartTime");
		Time endOverTimeAfterShift = null;
		
		if("ALL_SHIFT".equals(workTypeId) || "FIRST_HALF_SHIFT".equals(workTypeId)){
			endOverTimeAfterShift = workingShift.getTime("endOverTimeAfterShift");
			if(endOverTimeAfterShift == null){
				endOverTimeAfterShift = shiftStartTime;
			}
		}else if("SECOND_HALF_SHIFT".equals(workTypeId)){
			endOverTimeAfterShift = shiftBreakStart;
			if(endOverTimeAfterShift == null){
				endOverTimeAfterShift = DateUtil.getMiddleTimeBetweenTwoTime(shiftStartTime, shiftEndTime);
			}
		}
	...
	public static void editWorkOverTimeRegis(Delegator delegator, LocalDispatcher dispatcher,
			Time startTime, Time endTime, GenericValue userLogin, boolean checkBeforeUpdate, Date date, String partyId, String emplTimesheetId) throws GenericEntityException, GenericServiceException {
		...
		if(UtilValidate.isNotEmpty(updateWorkOverRegisList)){
			for(GenericValue updateTemp: updateWorkOverRegisList){
				dispatcher.runSync("updateEmplWorkovertime", UtilMisc.toMap("actualStartTime", String.valueOf(startTime.getTime()), 
						...
						"userLogin", userLogin, "checkBeforeUpdate", false, "emplTimesheetId", emplTimesheetId));
			}
		}else{
	...
	public static Map<String, Object> checkEmplTimesheetValid(Delegator delegator, String customTimePeriodId, String partyId, String userLoginId) throws GenericEntityException {
		String rootPartyId = PartyUtil.getRootOrganization(delegator, userLoginId);
		List<EntityCondition> conditions = FastList.newInstance();
		conditions.add(EntityCondition.makeCondition("statusId", EntityJoinOperator.NOT_EQUAL, "EMPL_TS_DELETED"));
		conditions.add(EntityCondition.makeCondition("customTimePeriodId", customTimePeriodId));
		conditions.add(EntityCondition.makeCondition("partyId", partyId));
		List<GenericValue> checkEmplTimesheet = delegator.findList("EmplTimesheets", EntityCondition.makeCondition(conditions), null, null, null, false);
		Map<String, Object> retMap = FastMap.newInstance();		
		if(UtilValidate.isNotEmpty(checkEmplTimesheet)){
			...
			if(!partyId.equals(rootPartyId)){
				GenericValue parentParty = PartyUtil.getParentOrgOfDepartmentCurr(delegator, partyId);
				if(parentParty != null){
					String parentPartyId = parentParty.getString("partyIdFrom"); 
					return checkEmplTimesheetValid(delegator, customTimePeriodId, parentPartyId, userLoginId);
				}
			}
		}
		return retMap;
	...
	public static void deleteEmplAttendanceTrackerExists(Delegator delegator, String partyId,
			java.sql.Date fromDate, java.sql.Date thruDate, String orgId) throws GenericEntityException {
		if(fromDate != null && thruDate != null){
			...
			conds.add(EntityCondition.makeCondition("dateAttendance", EntityJoinOperator.LESS_THAN_EQUAL_TO, thruDate));
			conds.add(EntityCondition.makeCondition("dateAttendance", EntityJoinOperator.GREATER_THAN_EQUAL_TO, fromDate));
			List<GenericValue> emplAttendanceTrackerList = delegator.findList("EmplAttendanceTracker", 
					EntityCondition.makeCondition(conds), null, null, null, false);
			for(GenericValue tempGv: emplAttendanceTrackerList){
				tempGv.remove();
			}
		}
	}
