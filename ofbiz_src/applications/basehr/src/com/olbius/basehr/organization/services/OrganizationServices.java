package com.olbius.basehr.organization.services;
...
public class OrganizationServices {
	...
	public static Map<String, Object> createOrgUnit(DispatchContext dctx, Map<String, Object> context){
		...
		try {
			List<GenericValue> checkEntt = delegator.findByAnd("Party", UtilMisc.toMap("partyCode", partyCode), null, false);
			if(UtilValidate.isNotEmpty(checkEntt)){
				return ServiceUtil.returnError(UtilProperties.getMessage("BaseHRDirectoryUiLabels", "PartyGroupExists", UtilMisc.toMap("partyId", partyCode), locale));
			}
	...
	public static Map<String, Object> getOrganizationUnit(DispatchContext dctx, Map<String, Object> context){
		...
		GenericValue userLogin = (GenericValue)context.get("userLogin");
		if(rootPartyId == null){
			rootPartyId = PartyUtil.getRootOrganization(delegator, userLogin.getString("userLoginId"));
		}
		try {
			...
			Map<String, Object> rootEmpl = OrganizationUtils.getEmplOfOrganization(dispatcher, delegator, userLogin, buildOrg, listReturn);
			int numberOfEmpl = (int)rootEmpl.get("numberOfEmpl");
			GenericValue partyGroup = delegator.findOne("PartyGroup", UtilMisc.toMap("partyId", rootPartyId), false);
			tempMap.put("partyId", rootPartyId);
			tempMap.put("partyCode", PartyUtil.getPartyCode(delegator, rootPartyId));
			tempMap.put("comments", partyGroup.get("comments"));
			tempMap.put("partyName", partyGroup.get("groupName"));
			tempMap.put("partyIdFrom", "-1");
			tempMap.put("totalEmployee", numberOfEmpl);
