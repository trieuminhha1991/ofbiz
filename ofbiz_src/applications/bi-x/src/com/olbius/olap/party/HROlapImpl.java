...
public class HROlapImpl extends AbstractOlap implements PartyOlap, HROlap{
	...
	public void personBirth(boolean gender) throws GenericDataSourceException,
		...
		String sql = "SELECT extract(year from age(birth_date_dimension.date_value)) AS _age, count(DISTINCT person_dim_id) AS _count, %GENDER% FROM person_relationship_fact"
				...
				+ " INNER JOIN date_dimension ON date_dim_id = date_dimension.dimension_id)"
				+ " INNER JOIN party_person_dimension ON person_dim_id = party_person_dimension.dimension_id"
		...
		if(gender) {
			...
			sql = sql.replaceAll("%AND_GENDER%", "AND gender is NOT NULL AND date_dimension.date_value is NOT NULL");
		...
		if(group != null && !group.isEmpty()) {
			sql = sql.replaceAll("%JOIN_GROUP%", "INNER JOIN party_group_dimension ON group_dim_id = party_group_dimension.dimension_id");
			sql = sql.replaceAll("%AND_GROUP%", "AND party_group_dimension.party_id = ?");
		} else {
	...
	public void member(String dateType, boolean cur, List<?> groups)
		...
		String sql = "SELECT count(DISTINCT person_dim_id) AS _count, %DATE_TYPE%, party_group_dimension.party_id FROM person_relationship_fact"
				+ " INNER JOIN date_dimension ON date_dim_id = date_dimension.dimension_id"
				+ " %AND_CUR%"
				+ " INNER JOIN party_group_dimension ON group_dim_id = party_group_dimension.dimension_id"
				+ " WHERE role_type_id = 'EMPLOYEE' %AND_GROUP% %AND_DATE%"
				+ " GROUP BY %DATE_TYPE%, party_group_dimension.party_id ORDER BY %DATE_TYPE%, party_group_dimension.party_id";
		
		if(group != null && !group.isEmpty()) {
			sql = sql.replaceAll("%AND_GROUP%", "AND party_group_dimension.party_id = ?");
		} else if(groups != null && !groups.isEmpty()) {
			String s = "AND party_group_dimension.party_id IN (";
	...
	public void personOlap(String dateType, boolean ft)
		...
		String sql = "SELECT count(DISTINCT person_dim_id) AS _count, %DATE_TYPE%, party_group_dimension.party_id FROM party_person_relationship"
	...
	public void timeTracker(String timeId) throws GenericDataSourceException,
		...
		if(group != null && !group.isEmpty()) {
			sql = sql.replaceAll("%JOIN_GROUP%", "INNER JOIN person_relationship_fact ON attendance_tracker_fact.party_dim_id = person_relationship_fact.person_dim_id"
					...
					+ " INNER JOIN party_group_dimension ON person_relationship_fact.group_dim_id = party_group_dimension.dimension_id");
			sql = sql.replaceAll("%AND_GROUP%", "AND party_group_dimension.party_id = ?");
		} else {
			sql = sql.replaceAll(" %JOIN_GROUP%", "");
			sql = sql.replaceAll(" %AND_GROUP%", "");
	...
	public void salaryStructure(String party_person) throws GenericDataSourceException,
        ...
        if(party_person != null && !party_person.isEmpty()) {
            sql = sql.replaceAll("%JOIN_PERSON%",
                    "INNER JOIN party_person_dimension as ppd "
            ...
            if (group != null && !group.isEmpty()) {
                sql = sql.replaceAll("%JOIN_GROUP%",
                        ...
                        + "INNER JOIN party_group_dimension AS pgd "
		...
		if (group != null && !group.isEmpty()) {
			sql = sql.replaceAll("%JOIN_GROUP%",
					"INNER JOIN person_relationship_fact "
							...
							+ " INNER JOIN party_group_dimension "
							+ "ON person_relationship_fact.group_dim_id = party_group_dimension.dimension_id ");
