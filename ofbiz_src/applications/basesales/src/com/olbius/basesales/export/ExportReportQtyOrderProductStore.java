...
public class ExportReportQtyOrderProductStore extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_SO_LUONG_DON_THEO_CUA_HANG";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "BSPSQuantityOrderByProductStore", locale));
		setRunServiceName("olapQtyOrderProductStore");
		setModuleExport("SALES");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "BSPSQuantityOrderByProductStore", locale));
