...
public class ExportReportTorProductProdStore extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_DOANH_SO_SP_THEO_CUA_HANG";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "BSPSAbbTurnOverProductBySalesChannel", locale));
		setRunServiceName("olapTorProductProdStore");
		setModuleExport("SALES");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "BSPSAbbTurnOverProductBySalesChannel", locale));
		...
		addColumn(14, UtilProperties.getMessage(RESOURCE, "BSPrimaryCategory", locale), "category_name");
		addColumn(42, UtilProperties.getMessage(RESOURCE, "BSPSChannelName", locale), "store_name");
		addColumn(14, UtilProperties.getMessage(RESOURCE, "BSQuantity", locale), "total_quantity", ExportExcelStyle.STYLE_CELL_NUMBER);
