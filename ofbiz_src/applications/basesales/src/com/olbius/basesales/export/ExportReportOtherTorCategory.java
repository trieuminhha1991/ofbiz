...
public class ExportReportOtherTorCategory extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_TONG_QUAN_DANH_MUC";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage("BasePosUiLabels", "ReportDepartmentSummaryReport", locale));
		setRunServiceName("olapOtherTorCategory");
		setModuleExport("SALES");
		setDescriptionExport(UtilProperties.getMessage("BasePosUiLabels", "ReportDepartmentSummaryReport", locale));
		...
		addColumn(16, UtilProperties.getMessage(RESOURCE, "BSCategoryId", locale), "category_id");
		addColumn(36, UtilProperties.getMessage(RESOURCE, "BSCategoryName", locale), "category_name");
		addColumn(18, UtilProperties.getMessage(RESOURCE, "BSQuantity", locale), "total_quantity", ExportExcelStyle.STYLE_CELL_NUMBER);
		...
		addColumn(18, UtilProperties.getMessage(RESOURCE, "BSValueTotal", locale), "total_amount", ExportExcelStyle.STYLE_CELL_CURRENCY);
	}
