...
public class ExportReportOtherTorProduct extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_TONG_QUAN_SAN_PHAM";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage("BasePosUiLabels", "ReportItemSummaryReport", locale));
		setRunServiceName("olapOtherTorProduct");
		setModuleExport("SALES");
		setDescriptionExport(UtilProperties.getMessage("BasePosUiLabels", "ReportItemSummaryReport", locale));
		...
		addColumn(16, UtilProperties.getMessage(RESOURCE, "BSProductId", locale), "product_code");
		addColumn(36, UtilProperties.getMessage(RESOURCE, "BSProductName", locale), "product_name");
		addColumn(16, UtilProperties.getMessage(RESOURCE, "BSPrimaryCategory", locale), "category_name");
