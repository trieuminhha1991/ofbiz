...
public class ChartColTorLoyaltyCustomerReport extends OlbiusOlapService {
	...
	public void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		putParameter("fileName", "ChartColTorLoyaltyCustomerReport"); // cache the specific file
	...
	private OlbiusQuery init() {
		...
		query.select("PCGD.party_classification_group_id")
			.select("SUM(SOF.total_amount)", "total_amount")
			.from("sales_order_new_fact", "SOF")
			.join(Join.INNER_JOIN, "party_classification_fact", "PCF", "SOF.customer_dim_id = PCF.party_dim_id AND ((PCF.thru_date_dim_id = '-1' AND SOF.order_date_dim_id > PCF.from_date_dim_id) OR (SOF.order_date_dim_id BETWEEN PCF.from_date_dim_id AND PCF.thru_date_dim_id))")
			...
			.groupBy("PCGD.party_classification_group_id")
			.where(Condition.makeBetween("SOF.entry_date_dim_id", getSqlTime(getFromDate()), getSqlTime(getThruDate()))
						...
						.andEQ("PCGD.party_classification_type_id", "LOYALTY_POINT_CLASS")
	...
	public void prepareResultChart() {
		if (getOlapResult() instanceof OlapColumnChart) {
			...
			addXAxis("party_classification_group_id");
