...
public class SalesOlapMultiImplv2 extends OlbiusBuilder {
	...
	public SalesOlapMultiImplv2(Delegator delegator) {
		super(delegator);
	}
	...
	public void prepareResultChart() {
		if(getOlapResult() instanceof OlapColumnChart) {
			...
			List<Object> agencyId = (List<Object>) getParameter(AGENCY_ID);
			if(UtilValidate.isNotEmpty(agencyId)){
				addSeries("agency_id");
			} else {
	...
	private void initQuery() {
		...
		String flagSM = (String) getParameter(FLAGSM);
		List<Object> status = FastList.newInstance();
		String status2 = null;
		if("SM".equals(flagSM)){
			status = (List<Object>) getParameter(ORDER_STATUS);
			...
			status2 = (String) getParameter(ORDER_STATUS);
		}
		...
		if(UtilValidate.isNotEmpty(productStoreId) && productStoreId.toArray().length == 1 && "all".equals(productStoreId.get(0))){
			
		} else {
			condition.and(Condition.makeIn("enumeration_dimension.enum_id", productStoreId, !"all".equals(productStoreId)));
		}
		if(UtilValidate.isNotEmpty(flagSM)){
		...
		condition.andEQ("sales_order_fact.order_status", status2, status2 != null)
		.and("category_dimension.category_id not like '%TAX%'").and("product_category_relationship.thru_dim_date = -1")
		.andIn("sales_order_fact.order_status", status, "SM".equals(flagSM) && !status.isEmpty())
...
//		condition.and(Condition.make("sales_order_fact.return_id isnull"));
		if("ORDER_CANCELLED".equals(status)){
			condition.andEQ("sales_order_fact.order_status", status, status != null);
			condition.and("sales_order_fact.order_item_status = 'ITEM_CANCELLED'");
		} else {
			condition.and("sales_order_fact.order_item_status <> 'ITEM_CANCELLED'");
		}
		condition.andIn("pd.party_id", agencyId, UtilValidate.isNotEmpty(agencyId));
	}
