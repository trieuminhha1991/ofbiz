...
public class SynthesisTurnoverReportByStaffOlapImpl extends OlbiusBuilder{
	public static final String ORDER_STATUS = "ORDER_STATUS";
	public static final String CHANNEL = "CHANNEL";
	public static final String TOTALDES = "TOTALDES";
	public static final String QUANTITYDES = "QUANTITYDES";
	public static final String DESCRIPTION = "DESCRIPTION";
	
	private OlbiusQuery query;
	
	public SynthesisTurnoverReportByStaffOlapImpl(Delegator delegator) {
		super(delegator);
	}
	...
	private void initQuery() {
		...
		.select("sales_order_fact.call_center_party_dim_id")
		...
		.join(Join.INNER_JOIN, "party_dimension", null, "sales_order_fact.call_center_party_dim_id = party_dimension.dimension_id")
		...
		.groupBy("sales_order_fact.call_center_party_dim_id")
		...
		queryJoin1.select("party_dimension.party_id").select(" SUM(total) AS Total1").select(" SUM(total + tax) AS TotalVAT").select(" SUM(quantity) AS Quantity1").select("sales_order_fact.call_center_party_dim_id")
		.from("sales_order_fact").join(Join.INNER_JOIN, "date_dimension", "sales_order_fact.order_date_dim_id = date_dimension.dimension_id")
		.join(Join.INNER_JOIN, "party_dimension", "sales_order_fact.call_center_party_dim_id = party_dimension.dimension_id")
		...
		.select("TMP.call_center_party_dim_id").select("TMP.party_id").select("TMP.full_name")
		.select("JOIN1.Total1", "turnoverTotal").select("JOIN1.TotalVAT", "turnoverTotalVAT").select("JOIN1.Quantity1", "turnoverVolumeTotal")
		.join(Join.INNER_JOIN, queryJoin1, "JOIN1", "TMP.call_center_party_dim_id = JOIN1.call_center_party_dim_id")
		.where(Condition.make("TMP.party_id is not null"))
		.groupBy("TMP.call_center_party_dim_id").groupBy("TMP.party_id").groupBy("turnoverTotal").groupBy("turnoverTotalVAT").groupBy("turnoverVolumeTotal")
