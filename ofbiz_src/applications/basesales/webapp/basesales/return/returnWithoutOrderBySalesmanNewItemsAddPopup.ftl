...
<script type="text/javascript">
	$(function(){
		OlbReturnNewItemsProdAddPopBySalesman.init();
	});
	var OlbReturnNewItemsProdAddPopBySalesman = (function(){
		var productGRID;
		...
		var initElementComplex = function(){
			jOlbUtil.windowPopup.create($("#windowReturnItemsProdAdd"), {maxWidth: 1100, width: 1100, height: 580, cancelButton: $("#wn_returnItems_alterCancel")});
			...
			var configGridProduct = {
				...
				useUtilFunc: false,
				useUrl: true,
				url: 'jqxGeneralServicer?sname=JQGetListProductSellAll',
				groupable: false,
		...
		var initEvent = function(){
			if (OlbReturnNewItemsProdSearchBySalesman) {
				productGRID.bindingCompleteListener(function(){
					OlbReturnNewItemsProdSearchBySalesman.productToSearchFocus();
				}, true);
			}
		...
		var addItemsToGrid = function(listData) {
			OlbReturnAddProductItemsBySalesman.addItemsToGridPopup(listData);
