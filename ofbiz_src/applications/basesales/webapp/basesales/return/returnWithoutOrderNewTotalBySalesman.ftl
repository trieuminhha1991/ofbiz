...
<div class="row-fluid">
	<div>
		<div>
			<#include "returnWithoutOrderBySalesmanNewInfo.ftl"/>
		</div>
		<div class="form-window-content-custom">
			<#include "returnWithoutOrderBySalesmanNewItemsProd.ftl"/>
		</div>
	</div>
...
<script type="text/javascript">
	$(function(){
		OlbReturnWithoutOrderBySalesmanTotal.init();
	});
	
	var OlbReturnWithoutOrderBySalesmanTotal = (function(){
		var orderListDDB;
		...
		var initEvent = function(){
			...
			$("#alterSave").on("click", function(){
				if (!OlbReturnWithoutOrderBySalesmanInfo.getValidator().validate()) {
					return false;
				};
		...
		var finishCreateReturn = function(){
			var dataMap = OlbReturnWithoutOrderBySalesmanInfo.getValue();
			var listProd = OlbReturnAddProductItemsBySalesman.getListProductAll();
			...
			if (listProd.length <= 0) {
				...
				$.ajax({
					type: 'POST',
					url: 'createReturnWithoutOrderBySalesman',
					data: dataMap,
					beforeSend: function(){
