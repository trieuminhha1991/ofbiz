...
var filterPlaceHolder = "${StringUtil.wrapString(uiLabelMap.BSLookingFor)}";
	$(function(){
		...
		$.ajax({url: 'getStoreListColumn', type: 'post', async: false, success: function(data) {
	    	...
	    	for (var i = 0; i < listDatafield.length; i++){
	    		...
	    		if (full_title) {
	    			var field2 = {text: full_title, align: 'center', name: listDatafield[i].product_id};
		    		...
		    		var field4 = {text: '${StringUtil.wrapString(uiLabelMap.FormFieldTitle_percentage)}', datafield:listDatafield[i].product_id+"_p", width: '12%', columngroup: listDatafield[i].product_id, filterable: false,
    ...
    var testGrid = OLBIUS.oLapGrid('salesOut', config, configPopup, 'evaluateESOGrid', true);
