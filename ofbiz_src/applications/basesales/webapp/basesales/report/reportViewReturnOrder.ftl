...
$(function(){
	...
	var config = {
	    title: '${StringUtil.wrapString(uiLabelMap.BSReturnOrderReport)}',
	    service: 'salesOrder',
	    columns: column,
	    columnsheight: 75,
	};
	...
	var configPopup = [
	    ...
	    },
	    {
	        ...
	        params : [{
	            id : 'orderStatus',
	            label : '${StringUtil.wrapString(uiLabelMap.DAStatus)}',
	            data : statusItemData,
	            index: 0
	        }]
	    },
    ...
    var testGrid = OLBIUS.oLapGrid('test', config, configPopup, 'evaluateReturnOrderGrid', true);
    ...
    testGrid.init(function () {
    	...
    	if(dataAll.length != 0){
        	...
        	window.location.href = "exportReturnOrderReportToExcel?fromDate=" + dateFrom + "&thruDate=" + thruFrom + "&orderStatus=" + orderStatus  + "&channel=" + channel + "&fromDate2=" + dateFrom2 + "&thruDate2=" + thruFrom2;
    	}else{
