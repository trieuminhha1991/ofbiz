<#if !isCustomer?exists><#assign isCustomer = false/></#if>
...
<div id="orderinfo-tab" class="tab-pane<#if activeTab?exists && activeTab == "orderinfo-tab"> active</#if>">
	<div class="row-fluid">
			...
			<div class="profile-user-info profile-user-info-striped" style="margin-left:0;margin-right:0">
				...
				<#if !isCustomer>
					...
					<#if productStore?has_content>
						<div class="profile-info-row">
							<div class="profile-info-name">${uiLabelMap.BSSalesChannel}</div>
							<div class="profile-info-value">
							...
							<div class="profile-info-value">
										...
										<#if orderHeader.priority?exists && listPriority?exists>
			                    			<#list listPriority as priorityItem>
			                    				<#if orderHeader.priority == priorityItem.enumId>
			                    					${priorityItem.description?if_exists}
			                    				</#if>
			                    			</#list>
										<#else>
								...
								<form id="setOrderReservationPriority" name="setOrderReservationPriority" method="post" action="<@ofbizUrl>setOrderReservationPriority</@ofbizUrl>" style="display:none">
			                    	...
			                    	<select name="priority" style="margin-bottom:0; font-size:13px;padding: 2px 6px;height: 26px;">
			                    		<#if listPriority?exists>
			                    			<#list listPriority as priorityItem>
			                    				<option value="${priorityItem.enumId}" <#if (orderHeader.priority)?if_exists == priorityItem.enumId>selected="selected" </#if>>${StringUtil.wrapString(priorityItem.description?default(""))}</option>
			                    			</#list>
			                    		</#if>
			                    	</select>
		...
		<div class="span5">
			...
			<#if displayParty?has_content || orderContactMechValueMaps?has_content || consignee?has_content>
				<div class="profile-user-info profile-user-info-striped" style="margin-left:0;margin-right:0">
					<div class="profile-info-row">
						...
						<div class="profile-info-value">
		              		...
		              		<#if partyId?exists>
		                		...
		                		<#if orderHeader.salesChannelEnumId != "POS_SALES_CHANNEL">
                					<div style="display:inline-block;">
			                			<#if !isCustomer>
			                			<a href="<@ofbizUrl>listOrder?partyId=${partyId}</@ofbizUrl>">
			                   				<i class="fa-search-plus"></i>${uiLabelMap.BSListOrder}
			                   			</a>&nbsp;&nbsp;&nbsp;&nbsp;
			                   			</#if>
										<#if hasOlbEntityPermission("SALESORDER", "CREATE")>
			                				...
			                				<a href="<@ofbizUrl>copySalesOrder?orderId=${orderId?if_exists}</@ofbizUrl>">
			                					<i class="fa-plus-circle"></i>
			                					${uiLabelMap.BSCopyOrder}
			                				</a>
			                			</#if>
                					</div>
                					<#--
							...
							<#if hasOlbPermission("MODULE", "RETURN_ORDER_NEW", "") && orderHeader.statusId == "ORDER_COMPLETED">
					          	<#if returnableItems?has_content>
							        ...
							        <div style="display:inline-block;">
						            	...
						            	<a href="newReturnOrder?orderId=${orderId}"><i class="fa fa-arrow-circle-left"></i>${uiLabelMap.BSCreateReturn}</a>
					            	</div>
					          	</#if>
					        </#if>
						</div>
                    ...
                    <div class="profile-info-row">
                        <div class="profile-info-name">${uiLabelMap.BSPartyReceive}</div>
                        <div class="profile-info-value">
							<#if consignee?has_content>
								<#assign consigneeNameResult = dispatcher.runSync("getPartyNameForDate", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", consignee.partyId, "compareDate", orderHeader.orderDate, "userLogin", userLogin))/>
								${consigneeNameResult.fullName?default("[${uiLabelMap.OrderPartyNameNotFound}]")}
							</#if>
		              		<#if partyId?exists>
		                		&nbsp;(${partyId})&nbsp;&nbsp;&nbsp;<#--<a href="${customerDetailLink?if_exists}${partyId}${externalKeyParam}" target="partymgr">${partyId}</a>-->
								<div style="clear:both;"></div>
								<#if "_NA_" != partyId>
									<#if consigneePhones?has_content>
										${uiLabelMap.BSPhoneShipping}:
										<#list consigneePhones as consigneePhone>
											${consigneePhone.contactNumber?string}<#if consigneePhone_has_next>, </#if>
										</#list>
									</#if>
								</#if>
							</#if>
			...
			<#if listLoyaltyPoint?has_content>
				<h4 class="smaller green margin-top20" style="display:inline-block">
					${uiLabelMap.BSInformationLoyaltyPointOfCustomer} <i class="fa fa-gift pink" aria-hidden="true"></i>
				</h4>
				<div class="profile-user-info profile-user-info-striped" style="margin-left:0;margin-right:0">
					<div class="profile-info-row">
						<div class="profile-info-name">${uiLabelMap.BSLoyaltyPoint}</div>
						<div class="profile-info-value">
							<table cellspacing="0" cellpadding="1" border="0" class="table table-bordered" style="margin-bottom:0">
								<tr>
									<td width="70%">${uiLabelMap.BSPSSalesChannel}</td>
									<td width="30%">${uiLabelMap.BSPoint}</td>
								</tr>
								<#list listLoyaltyPoint as loyaltyPoint>
									<tr>
										<td>
										<#if loyaltyPoint.productStoreId?exists>
											<#assign productStoreTmp = delegator.findOne("ProductStore", {"productStoreId": loyaltyPoint.productStoreId}, false)!>
											${productStoreTmp?if_exists.storeName?if_exists}
										</#if>
										</td>
										<td>
										<#if loyaltyPoint.point?exists>
											${Static["org.ofbiz.base.util.UtilFormatOut"].formatDecimalNumber(loyaltyPoint.point, "#,###.##", locale)}
										</#if>
										</td>
									</tr>
								</#list>
					...
					<div class="profile-info-row">
						<div class="profile-info-name">${uiLabelMap.BSRating}</div>
						<div class="profile-info-value">
							<span class="pink" style="font-weight: bold; font-size:16px">
							<#if partyClassification?exists>
								<#assign partyClassificationGroup = delegator.findOne("PartyClassificationGroup", {"partyClassificationGroupId": partyClassification.partyClassificationGroupId}, false)!>
								${partyClassificationGroup?if_exists.description?if_exists}
							</#if>&nbsp;
							</span>
						</div>
					</div>
		...
		<div class="span12">
			<#if orderHeader?has_content>
		    	<div id="order-notes-container">
		    		...
		    		${screens.render("component://basesales/widget/OrderScreens.xml#OrderNoteAjax")}
		    	</div>
			</#if>
		</div>
	</div><!--.row-fluid-->
