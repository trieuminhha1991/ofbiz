...
<div id="olbiusSynTorSalesEmpl"></div>
<script type="text/javascript">
    $(function() {
        var config = {
            title: "${StringUtil.wrapString(uiLabelMap.BSTurnoverSalesEmployee)}<#if viewPartner?exists && viewPartner == "Y"> (NPP)<#elseif viewPartner?exists && viewPartner == "A"> (DTM)</#if>",
            ...
            id: "olbiusSynTorSalesEmpl",
            olap: "olapSynTorSalesEmpl",
            sortable: true,
            filterable: true,
            showfilterrow: true,
            showstatusbar: true,
