<div id="alterpopupWindowCustomerMTAddItem" style="display:none">
	<div>${uiLabelMap.BSAddMemberIntoMT}</div>
...
<script type="text/javascript">
	$(function(){
		OlbCustomerMTAddItem.init();
	});
	var OlbCustomerMTAddItem = (function(){
		var validatorVAL;
		var memberDDB;
		...
		var initElement = function(){
			jOlbUtil.windowPopup.create($("#alterpopupWindowCustomerMTAddItem"), {width: 500, height: 200, cancelButton: $("#wn_additem_alterCancel")});
					...
					$.ajax({
						...
						success: function(data){
							jOlbUtil.processResultDataAjax(data, function(data, errorMessage){
							        	...
							        	$('#alterpopupWindowCustomerMTAddItem').jqxWindow('hide');
		       							$('#alterpopupWindowCustomerMTAddItem').jqxWindow('close');
									}
							);
						},
						error: function(data){
		...
		var initValidator = function(){
			...
			validatorVAL = new OlbValidator($("#alterpopupWindowCustomerMTAddItem"), mapRules);
		};
		return {
			init: init
		};
