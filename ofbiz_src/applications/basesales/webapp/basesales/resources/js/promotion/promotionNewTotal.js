...
var OlbPromoNewTotal = (function(){
	...
	var initEvent = function(){
		$('#fuelux-wizard').ace_wizard().on('change' , function(e, info){
			...
			var tabRules = $('[id^="tab-rule"]').length;
			if (tabRules > 0) {
				...
				jOlbUtil.alert.error(uiLabelMap.BSHaveNotAnyRuleInPromotionYet);
			}
	...
	var finishCreatePromo = function(){
		...
		var isHasCondSpecial = true;
		var isContinue = false;
		var isHasCondProduct = false;
		$('[id^="inputParamEnumId"]').each(function(i, obj) {
		    var condEnumId = $(obj).jqxComboBox('val');
			if ("PPIP_EACH_ORDER_TOTA" == condEnumId
					|| "PPIP_EACH_PROD_QUANT" == condEnumId
					|| "PPIP_PRODUCT_AMOUNT" == condEnumId
					|| "PPIP_PRODUCT_QUANT" == condEnumId) {
				isHasCondProduct = true;
				return false;
			}
		});
		if (isHasCondProduct) {
			isHasCondSpecial = false;
		}
		//if (dataStr.indexOf("PPIP_ORDER_TOTAL") > 0 || dataStr.indexOf("PPIP_PRODUCT_TOTAL") > 0 || dataStr.indexOf("PPIP_PROD_QUANT_TOTA") > 0) {
		//	isHasCondSpecial = true;
		//}
		
		if (isHasCondSpecial) {
			jOlbUtil.confirm.dialog(uiLabelMap.BSThisIsSpecialPromotionSetLimitEqualOne, 
				function() {
            	...
            	jOlbUtil.processResultDataAjax(data, 'default', 
    				function(data){
    		        	...
    		        	if (OlbElementUtil.isNotEmpty(productPromoId)) {
    		        		window.location.href = "viewPromotion?productPromoId=" + productPromoId;
    		        	}
    				}
        		);
            },
