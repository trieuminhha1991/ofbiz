var addSalesFCSheetDetailObj = (function(){
	var _workBook = null;
	var _optFS = "|OLBIUS|";
	var _optRS = "\n";
	var _csv;
	...
	var initWindow = function(){
		createJqxWindow($("#UploadFileSalesFCEdited"), 700, 550);
	};
	var openWindow = function(){
		openJqxWindow($("#UploadFileSalesFCEdited"));
		$("#numberLineTitle").val(6);
	};
	var initSourceFileGrid = function(){
		var datafield = [];
		var columns = [];
		var grid = $("#sourceFileSalesFCGrid");
		var config = {
				url: '',
				showtoolbar : false,
				autoheight: true,
	...
	var initJoinColumnDataGrid = function(){
		...
		var customcontrol = "fa fa-columns open-sans@" + uiLabelMap.ColumnMapAuto + "@javascript: void(0);@addSalesFCSheetDetailObj.autoMapColumn()";
		var customcontrol2 = "fa fa-refresh open-sans@" + uiLabelMap.BSCommonReset + "@javascript: void(0);@addSalesFCSheetDetailObj.resetJoinColumnGridData()";
	...
	var getJoinColumnGridLocaldata = function(){
		var localdata = [];
		localdata.push({fieldValueInSys: 'productCode', fieldNameInSys: uiLabelMap.BSProductId, fileType: "string"});
		localdata.push({fieldValueInSys: 'features', fieldNameInSys: uiLabelMap.BSFeature, fileType: "string"});
		for(var i = 0; i < listPeriod.length; i ++){
			var row = {fieldValueInSys: listPeriod[i].customTimePeriodId, fieldNameInSys: listPeriod[i].periodName, fileType: "string"};
			localdata.push(row);
		}
		return localdata;
	};
	...
	var initSimpleInput = function(){
		$('#sourceFileSalesFCImport').ace_file_input({
	  		no_file:'No File ...',
	...
	var initEvent = function(){
		var xlf = document.getElementById('sourceFileSalesFCImport');
	...
	var updateSourceFileGrid = function(dataFieldData, columnRowData){
		...
		var source = $("#sourceFileSalesFCGrid").jqxGrid('source');
		source._source.datafields = datafield;
		source._source.localdata = localdata;
		$("#sourceFileSalesFCGrid").jqxGrid('columns', columns);
		$("#sourceFileSalesFCGrid").jqxGrid('source', source);
	}
	...
	var getEditorColumnArr = function(){
		var textData = [];
		var cols = $("#sourceFileSalesFCGrid").jqxGrid("columns");
	...
	var validate = function(){
		return $("#fileSalesFCExcelUpload").jqxValidator('validate');
	};
	var hideValidate = function(){
		$("#fileSalesFCExcelUpload").jqxValidator('hide');
	};
	var initJqxValidator = function(){
		$("#fileSalesFCExcelUpload").jqxValidator({
			rules: [
				...
				{input : '#sourceFileSalesFCImportContainer', message : uiLabelMap.FieldRequired, action: 'blur', 
					rule : function(input, commit){
						var value = $("#sourceFileSalesFCImport").val();
						if(!value){
							return false;
						}
						return true;
	...
	var resetData = function(){
		...
		_csv = null;
		_totalCols = 0;
		clearAceInputFile($("#sourceFileSalesFCImport"));
	...
	var refreshSourceFileGrid = function(){
		var source =  $("#sourceFileSalesFCGrid").jqxGrid('source');
		$("#sourceFileSalesFCGrid").jqxGrid('columns', []);
