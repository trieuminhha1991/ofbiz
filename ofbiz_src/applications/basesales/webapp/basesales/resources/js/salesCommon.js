...
if(typeof(jSalesCommon) == "undefined"){
var jSalesCommon = (function(){
	...
	var formatDate = function(value, charJoin) {
		if (!isNotEmpty(charJoin) || charJoin.length > 1) {
			charJoin = '/';
		}
		if (jSalesCommon.isNotEmpty(value)) {
			var dateStr = "";
			dateStr += addZero(value.getDate()) + charJoin;
			dateStr += addZero(value.getMonth()+1) + charJoin;
			dateStr += addZero(value.getFullYear());
			return dateStr;
		} else {
			return "";
	...
	var validElement = function(input, commit, nameValidor, config){
		...
		} else if ('validDeliveryDateNotNull' == nameValidor) { // only sales
		...
		} else if ('validCompareStartDateAndFinishDate' == nameValidor) { // only sales
			return validCompareStartDateAndFinishDate(input, commit);
		} else if ('validCompareDeliveryDateBetweenStartDateAndFinishDate' == nameValidor) { // only sales
		...
		} else if ('validComboBoxMultiNotNull' == nameValidor) { // only sales
	...
	var processConfig = function(config){
		...
		configCf.disabled = typeof(config.disabled) != 'undefined' ? config.disabled : null;
		...
		if (objType == ObjectTypeEnum.DROP_DOWN_LIST || objType == ObjectTypeEnum.COMBO_BOX) {
			...
			if (objType == ObjectTypeEnum.DROP_DOWN_BUTTON) {
				...
				configCf.source.createUrl = typeof(config.createUrl) != 'undefined' ? config.createUrl : null;
				configCf.source.addColumns = typeof(config.addColumns) != 'undefined' ? config.addColumns : null;
				configCf.source.removeUrl = typeof(config.removeUrl) != 'undefined' ? config.removeUrl : null;
				configCf.source.deleteColumns = typeof(config.deleteColumns) != 'undefined' ? config.deleteColumns : null;
				...
				configCf.parentKeyId = typeof(config.parentKeyId) != 'undefined' ? config.parentKeyId : null;
				configCf.altRows = typeof(config.altRows) != 'undefined' ? config.altRows : true;
			}
	...
	var initTreeGrid = function(treeGridObject, localData, config, selectArr){
	    ...
	    $(treeGridObject).jqxTreeGrid({
	    	source: dataAdapter,
          	...
          	ready: function () {
          		if (config.expandAll) {
          			$(treeGridObject).jqxTreeGrid('expandAll');
	...
	var initDropDownButton = function(dropDownButtonObject, dropDownGridObject, localData, config, selectArr){
		...
		config.pagesize = typeof(config.pagesize) != "undefined" ? config.pagesize : 10;
		config = processConfig(config);
		...
		} else if (config.gridType == "jqxTree") {
			return;
		} else {
			if (config.useUtilFunc) {
	...
	var processRowSelectedDropDownButton = function(dropDownButtonObject, dropDownGridObject, row, config){
		if(row){
			...
			jSalesCommon.setValueDropDownButtonOnly($(dropDownButtonObject), row[key], descValue + ' [' + row[key] + ']');
	...
	var dropDownButton = (function(){
		...
		var selectItem = function(dropDownButtonObject, dropDownGridObject, config, selectArr){
			...
			if (isNotEmpty(selectArr) && selectArr.length > 0) {
				...
				if (!isExists) {
					jSalesCommon.setValueDropDownButtonOnly($(dropDownButtonObject), selectArr[0], "[" + selectArr[0] + "]");
				}
			} else {
				if (!isExists) {
					if (isNotEmpty(config.displayValue)) {
						...
						isExists = true;
						jSalesCommon.setValueDropDownButtonOnly($(dropDownButtonObject), displayValue, displayLabel);
					}
				}
				if (!isExists) {
					// select item by index
	...
	var createNotificationContainer = function(idContainer, idNotification, template, config){
		...
		var autoClose = typeof(config.autoClose) != "undefined" ? config.autoClose : false;
