...
if (shoppingCart) {
	...
	if (productPromoUseInfoIter) {
		while (productPromoUseInfoIter.hasNext()) {
			...
			if (productPromoIdsUse.contains(productPromoUseInfo.productPromoId)) {
				...
				for (Map<String, Object> productPromoUseInfoMapExisted : productPromoUseInfos) {
					if (productPromoUseInfoMapExisted.productPromoId == productPromoUseInfo.productPromoId) {
						...
						// begin check product promo code
						String productPromoCodeId = productPromoUseInfo.productPromoCodeId ? productPromoUseInfo.productPromoCodeId : null;
						List<Map<String, Object>> listInfoProductPromoCode = productPromoUseInfoMapExisted.productPromoCodeInfos;
						def tmpTotalDiscount;
						def tmpQuantityLeft;
						boolean isProductCodeExist = false;
						for (Map<String, Object> productPromoCodeExist : listInfoProductPromoCode) {
							if (productPromoCodeExist.productPromoCodeId == productPromoCodeId) {
								// sum total discount amount
								if (productPromoUseInfo.totalDiscountAmount != 0) {
									tmpTotalDiscount = productPromoUseInfo.totalDiscountAmount;
									if (productPromoCodeExist.totalDiscountAmount != null) tmpTotalDiscount = tmpTotalDiscount + productPromoCodeExist.totalDiscountAmount;
									productPromoCodeExist.put("totalDiscountAmount", tmpTotalDiscount);
								}
								if (productPromoUseInfo.quantityLeftInActions > 0) {
									tmpQuantityLeft = productPromoUseInfo.quantityLeftInActions;
									if (productPromoCodeExist.quantityLeftInActions != null
										&& (productPromoUseInfo.quantityLeftInActions > productPromoCodeExist.quantityLeftInActions)){
										tmpQuantityLeft = productPromoCodeExist.quantityLeftInActions;
									}
									productPromoCodeExist.put("quantityLeftInActions", tmpQuantityLeft);
								}
								isProductCodeExist = true;
							}
						}
						if (!isProductCodeExist) {
							Map<String, Object> newProductPromoCodeInfo = FastMap.newInstance();
							newProductPromoCodeInfo.put("productPromoCodeId", productPromoCodeId);
							newProductPromoCodeInfo.put("totalDiscountAmount", productPromoUseInfo.totalDiscountAmount);
							newProductPromoCodeInfo.put("quantityLeftInActions", productPromoUseInfo.quantityLeftInActions);
							listInfoProductPromoCode.add(newProductPromoCodeInfo);
						}
						// end
			...
			String productPromoCodeId = productPromoUseInfo.productPromoCodeId ? productPromoUseInfo.productPromoCodeId : null;
			List<Map<String, Object>> productPromoCodeInfos = FastList.newInstance();
			Map<String, Object> productPromoCodeInfo = FastMap.newInstance();
			productPromoCodeInfo.put("productPromoCodeId", productPromoCodeId);
			productPromoCodeInfo.put("totalDiscountAmount", productPromoUseInfo.totalDiscountAmount);
			productPromoCodeInfo.put("quantityLeftInActions", productPromoUseInfo.quantityLeftInActions);
			productPromoCodeInfos.add(productPromoCodeInfo);
			productPromoUseInfoMap.put("productPromoCodeInfos", productPromoCodeInfos);
			// end
