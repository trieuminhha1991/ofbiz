...
context.toPartyId = returnHeader.toPartyId; // NEED
...
*/
if (orderId) {
	List<String> partyOrders = EntityUtil.getFieldListFromEntityList(delegator.findByAnd("OrderRole", [orderId : orderId, roleTypeId : "PLACING_CUSTOMER"], null, false), "partyId", true);
	context.partyOrders = partyOrders;
	if (UtilValidate.isNotEmpty(partyOrders)) {
		context.partyId = partyOrders.get(0);
	}
}
