...
var OlbPicklistBinList = (function(){
	...
	var initEvent = function(){
		$("#contextMenuPicklistBin").on('itemclick', function (event) {
			...
			switch (itemId) {
				...
				case "binCancel":
					...
					break;
				case "binEmployeePick":
					if (_selectedData.length == 1) {
						ChangeEmployee.open($("#jqxgridPicklistBin"), _selectedData[0].picklistBinId, "PICKING_PICKER", $("#jqxgridPicklistBin").jqxGrid("getcellvalue", rowIndexSelected, "partyPickId"));
					}
					break;
				case "binEmployeeCheck":
					if (_selectedData.length == 1) {
						ChangeEmployee.open($("#jqxgridPicklistBin"), _selectedData[0].picklistBinId, "PICKING_CHECKER", $("#jqxgridPicklistBin").jqxGrid("getcellvalue", rowIndexSelected, "partyCheckId"));
					}	
					break;
				default:
					break;
		...
		$("#contextMenuPicklistBin").on("shown", function () {
			if (_selectedData.length == 1) {
				var binStatusId = _selectedData[0].binStatusId;
				...
				$("#contextMenuPicklistBin").jqxMenu("disable", "binViewDetail", true);
				$("#contextMenuPicklistBin").jqxMenu("disable", "binEmployee", true);
			}
		});
