var CreatePicklist = (function () {
		...
		var initElements = function () {
			...
			var initFacilityDrDGrid = function (dropdown, grid, width) {
				...
				GridUtils.initDropDownButton({url: "JQGetShippingAddressFullCustomer", autorowheight: false, filterable: true, showfilterrow: true,
					width: 800, source: {pagesize: 5},
					clearOnClose: false, dropdown: {width: 200 }},
					datafields, columns, null, grid, dropdown, "contactMechId", "fullName");
			};
			...
			var initSalesOrderDrDGrid = function (dropdown, grid, width) {
				...
				[
					...
					{ text: multiLang.BSAbbOrderId, datafield: "purchaseOrderId", width: 100 },
		...
		var handleEvents = function () {
			...
			$("#txtPickingFacility").on("close", function (event) {
				...
				if (facilitySelected && contactMechSelected) {
					...
					if (adapter) {
						adapter.url = "jqxGeneralServicer?sname=JQGetListSalesOrderHeaderApproved&facilityId=" + facilitySelected + "&contactMechId=" + contactMechSelected;
						adapter._source.url = adapter.url;
