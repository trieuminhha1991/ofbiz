if(typeof(jLogCommon) == "undefined"){
	var jLogCommon = (function(){
		var uiLabelMap = {};
		...
		var formatFullDate = function(value) {
			if (jLogCommon.isNotEmpty(value)) {
				var dateStr = "";
		...
		var processRowSelectedDropDownButton = function(dropDownButtonObject, dropDownGridObject, row, config){
			if(row){
				...
				jLogCommon.setValueDropDownButtonOnly($(dropDownButtonObject), row[key], descValue + ' [' + row[key] + ']');
		...
		var dropDownButton = (function(){
			...
			var selectItem = function(dropDownButtonObject, dropDownGridObject, config, selectArr){
				...
				if (isNotEmpty(selectArr) && selectArr.length > 0) {
					...
					if (!isExists) {
						jLogCommon.setValueDropDownButtonOnly($(dropDownButtonObject), selectArr[0], "[" + selectArr[0] + "]");
					}
				} else {
					if (!isExists) {
						if (isNotEmpty(config.displayValue)) {
							...
							isExists = true;
							jLogCommon.setValueDropDownButtonOnly($(dropDownButtonObject), displayValue, displayLabel);
						}
					}
					if (!isExists) {
						// select item by index
