...
public class PayPalEvents {
    ...
    public static String callPayPal(HttpServletRequest request, HttpServletResponse response) {
		...
		if (orderId != null) {
			...
			try {
				...
				request.setAttribute("_ERROR_MESSAGE_", UtilProperties
						.getMessage(resourceErr,
								"payPalEvents.problemsGettingOrderHeader",
								locale));
				return "error";
			}

			// get the order total
			total = orderHeader.getBigDecimal("grandTotal")
					.toPlainString();
			currencyUom = orderHeader.getString("currencyUom");
		} else {
			// get the order header
			GenericValue invoiceHeader = null;
			try {
				invoiceHeader = delegator.findOne("Invoice",
						UtilMisc.toMap("invoiceId", invoiceId), false);
			} catch (GenericEntityException e) {
				Debug.logError(e, "Cannot get the invoice header for invoice: "
						+ invoiceId, module);
				request.setAttribute("_ERROR_MESSAGE_", UtilProperties
						.getMessage(resourceErr,
								"payPalEvents.problemsGettingInvoiceHeader",
								locale));
				return "error";
			}

			// get the order total
			total = InvoiceWorker.getInvoiceNotApplied(delegator,invoiceId).toPlainString();
			currencyUom = invoiceHeader.getString("currencyUomId");
		}
        ...
        if (UtilValidate.isNotEmpty(orderId)){
        	itemName = UtilProperties.getMessage(resource, "AccountingOrderNr", locale) + orderId + " " +
                                 (company != null ? UtilProperties.getMessage(commonResource, "CommonFrom", locale) + " "+ company : "");
        } else {
        	itemName = UtilProperties.getMessage(resource, "AccountingInvoiceNr", locale) + invoiceId + " " +
                    (company != null ? UtilProperties.getMessage(commonResource, "CommonFrom", locale) + " "+ company : "");
        }
        ...
        request.getSession().setAttribute("PAYPAL_INVOICE", invoiceId);
    ...
    public static String payPalIPN(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (orderHeader == null) {
			...
			if (invoiceHeader == null) {
				...
				Debug.logError("Cannot get the invoice header for invoice: "
						+ invoiceId, module);
				request.setAttribute("_ERROR_MESSAGE_", UtilProperties
						.getMessage(resourceErr,
								"payPalEvents.problemsGettingInvoiceHeader",
        ...
        try {
            ...
            if (okay) {
				...
				if (orderHeader != null) {
			        ...
			        paymentCtx.put("paymentMethodTypeId", "EXT_PAYPAL");
			        //paymentCtx.put("paymentMethodId", "EXT_PAYPAL");
			        // TODO check responseId
			        //paymentCtx.put("paymentGatewayResponseId", responseId);
			        paymentCtx.put("partyIdTo", invoiceHeader.get("partyIdFrom"));
			        paymentCtx.put("partyIdFrom", invoiceHeader.get("partyId"));
			        ...
			        paymentCtx.put("amount", InvoiceWorker.getInvoiceNotApplied(delegator,invoiceId));
			        paymentCtx.put("userLogin", userLogin);

			        String paymentId = null;
			        
		            Map<String, Object> payment = dispatcher.runSync("createPayment", paymentCtx);
					...
					if (okay) {
						...
						Map<String, Object> createPayApplResult = dispatcher
								.runSync("createPaymentApplication", appl);
