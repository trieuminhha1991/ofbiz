...
public class TaxAuthorityServices {
    ...
    public static Map<String, Object> rateProductTaxCalc(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        List<BigDecimal> itemAlternativeQuantityList = UtilGenerics.checkList(context.get("itemAlternativeQuantityList"));
        List<BigDecimal> itemAlternativePriceList = UtilGenerics.checkList(context.get("itemAlternativePriceList"));
        List<BigDecimal> itemAlternativeAmountList = UtilGenerics.checkList(context.get("itemAlternativeAmountList"));
        List<String> itemQuantityUomIdList = UtilGenerics.checkList(context.get("itemQuantityUomIdList"));
        ...
        // TODOCHANGE new list contain [product, adjustment]
        List<Map<String, Object>> listProductAndItemAdj = FastList.newInstance();
        ...
        for (int i = 0; i < itemProductList.size(); i++) {
            ...
            // TODOCHANGE quantityUomId
            BigDecimal itemAlternativeQuantity = itemAlternativeQuantityList != null ? itemAlternativeQuantityList.get(i) : null;
            BigDecimal itemAlternativePrice = itemAlternativePriceList != null ? itemAlternativePriceList.get(i) : null;
            BigDecimal itemAlternativeAmount = itemAlternativeAmountList != null ? itemAlternativeAmountList.get(i) : null;
            String itemQuantityUomId = itemQuantityUomIdList != null ? itemQuantityUomIdList.get(i) : "";

            List<GenericValue> taxList = null;
            if (shippingAddress != null) {
                taxList = getTaxAdjustmentsCustom(delegator, product, productStore, payToPartyId, billToPartyId, taxAuthoritySet, itemPrice, itemQuantity, itemAmount, shippingAmount, ZERO_BASE,
                        itemAlternativePrice, itemAlternativeQuantity, itemAlternativeAmount, itemQuantityUomId);
            }
            ...
            // TODOCHANGE tax
            Map<String, Object> productAndItemAdj = FastMap.newInstance();
            productAndItemAdj.put("product", product);
            productAndItemAdj.put("taxList", taxList);
            listProductAndItemAdj.add(productAndItemAdj);
        }
        ...
        if (orderPromotionsAmount != null && orderPromotionsAmount.compareTo(BigDecimal.ZERO) != 0) {
            List<GenericValue> taxList = getTaxAdjustments(delegator, null, productStore, payToPartyId, billToPartyId, taxAuthoritySet, ZERO_BASE, ZERO_BASE, ZERO_BASE, ZERO_BASE, orderPromotionsAmount, listProductAndItemAdj);
            orderAdjustments.addAll(taxList);
        }
    ...
    // VIETTB create
    public static Map<String, Object> rateProductTaxDivCalc(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        String productStoreId = (String) context.get("productStoreId");
        String facilityId = (String) context.get("facilityId");
        String payToPartyId = (String) context.get("payToPartyId");
        ...
        List<BigDecimal> itemShippingList = UtilGenerics.checkList(context.get("itemShippingList"));
        //Extend by VietTB
        List<GenericValue> allAdjustments = UtilGenerics.checkList(context.get("allAdjustments"));
        //TODOCHANGE quantityUomId
        List<BigDecimal> itemAlternativeQuantityList = UtilGenerics.checkList(context.get("itemAlternativeQuantityList"));
        List<BigDecimal> itemAlternativePriceList = UtilGenerics.checkList(context.get("itemAlternativePriceList"));
        List<BigDecimal> itemAlternativeAmountList = UtilGenerics.checkList(context.get("itemAlternativeAmountList"));
        List<String> itemQuantityUomIdList = UtilGenerics.checkList(context.get("itemQuantityUomIdList"));
        List<List<BigDecimal>> itemDivTaxList =UtilGenerics.checkList(context.get("itemDivTaxList"));
        BigDecimal cartSubTotal = (BigDecimal) context.get("cartSubTotal");
        Timestamp moment = (Timestamp) context.get("momentTime"); //TODOCHANGE new moment time
        ...
        GenericValue productStore = null;
        GenericValue facility = null;
    	List<GenericValue> listOrderAdjusment = new FastList<GenericValue>();
        try {
            if (productStoreId != null) {
                productStore = delegator.findOne("ProductStore", UtilMisc.toMap("productStoreId", productStoreId), false);
            }
        ...
        for (int i = 0; i < itemProductList.size(); i++) {
            ...
            BigDecimal itemAlternativeAmount = itemAlternativeAmountList != null ? itemAlternativeAmountList.get(i) : null;
            String itemQuantityUomId = itemQuantityUomIdList != null ? itemQuantityUomIdList.get(i) : "";
            List<BigDecimal> itemDivTax = itemDivTaxList.isEmpty() ? null : itemDivTaxList.get(i);
            List<GenericValue> taxList = null;
            if (shippingAddress != null) {
                taxList = getDivTaxAdjustmentsCustom(delegator, product, productStore, payToPartyId, billToPartyId, taxAuthoritySet, itemPrice, itemQuantity, itemAmount, shippingAmount, ZERO_BASE,
                        itemAlternativePrice, itemAlternativeQuantity, itemAlternativeAmount, itemQuantityUomId, itemDivTax, moment);
            }
            // this is an add and not an addAll because we want a List of Lists of GenericValues, one List of Adjustments per item
            itemAdjustments.add(taxList);
        ...
        Map<String, Object> resultAdjustment =  OrderReadHelper.calcOrderPromoAdjustmentsBdNotAmount(delegator, allAdjustments);
        BigDecimal orderPromoAmount = (BigDecimal) resultAdjustment.get("promoAdjTotal");

        if (orderPromotionsAmount != null && orderPromotionsAmount.compareTo(BigDecimal.ZERO) != 0) {
        	if (orderPromoAmount != null && orderPromoAmount.compareTo(BigDecimal.ZERO) != 0) {
        		List<GenericValue> taxList = getTaxAdjustments(delegator, null, productStore, payToPartyId, billToPartyId, taxAuthoritySet, ZERO_BASE, ZERO_BASE, ZERO_BASE, ZERO_BASE,
        				orderPromoAmount, listProductAndItemAdj, cartSubTotal);
        		orderAdjustments.addAll(taxList);
        	}

            List<GenericValue> taxList = getTaxAdjustments(delegator, null, productStore, payToPartyId, billToPartyId, taxAuthoritySet, ZERO_BASE, ZERO_BASE, ZERO_BASE, ZERO_BASE,
                    orderPromotionsAmount, listProductAndItemAdj, cartSubTotal);
            orderAdjustments.addAll(taxList);
        }
    ...
    // TODOCHANGE add a parameter listProductAndItemAdj
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore,
                                                        String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet,
                                                        BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount,
                                                        BigDecimal shippingAmount, BigDecimal orderPromotionsAmount, List<Map<String, Object>> listProductAndItemAdj) {
        return getTaxAdjustments(delegator, product, productStore,
                payToPartyId, billToPartyId, taxAuthoritySet,
                itemPrice, itemQuantity, itemAmount,
                shippingAmount, orderPromotionsAmount, listProductAndItemAdj, null);
    }
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore,
                                                        String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet,
                                                        BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount,
                                                        BigDecimal shippingAmount, BigDecimal orderPromotionsAmount, List<Map<String, Object>> listProductAndItemAdj, BigDecimal cartSubTotal) {
        Timestamp nowTimestamp = UtilDateTime.nowTimestamp();
        List<GenericValue> adjustments = FastList.newInstance();
        ...
        try {
            ...
            // TODOCHANGE refresh group by taxAuthorityRateSeqId
            BigDecimal sumAdjustment = BigDecimal.ZERO;
            BigDecimal sumOrder = BigDecimal.ZERO;
            boolean hasTaxZero = false;
            Map<String, Object> taxAuthorityRateSeqIdMap = FastMap.newInstance();
            for (Map<String, Object> productAndItemAdj : listProductAndItemAdj) {
                //GenericValue product1 = (GenericValue) productAndItemAdj.get("product");
                @SuppressWarnings("unchecked")
                List<GenericValue> itemAdj1 = (List<GenericValue>) productAndItemAdj.get("taxList");
                for (GenericValue itemAdj2 : itemAdj1) {
                    if (itemAdj2.get("taxAuthorityRateSeqId") != null && itemAdj2.get("amount") != null) {
                        String taxAuthorityRateSeqId2 = (String) itemAdj2.get("taxAuthorityRateSeqId");
                        BigDecimal sourcePercentage2 = itemAdj2.getBigDecimal("sourcePercentage");
                        BigDecimal amount = (BigDecimal) itemAdj2.get("amount");
                        BigDecimal amountOrigin = amount;
                        if (sourcePercentage2.compareTo(BigDecimal.ZERO) > 0) {
                            amountOrigin = (amount.divide(sourcePercentage2, 2 , RoundingMode.HALF_UP)).multiply(PERCENT_SCALE);
                            hasTaxZero = true;
                        }
                        sumOrder = sumOrder.add(amountOrigin);
                        sumAdjustment = sumAdjustment.add(amount);
                        if (taxAuthorityRateSeqIdMap.containsKey(taxAuthorityRateSeqId2)) {
                            amountOrigin = amountOrigin.add((BigDecimal) taxAuthorityRateSeqIdMap.get(taxAuthorityRateSeqId2));
                        }
                        taxAuthorityRateSeqIdMap.put(taxAuthorityRateSeqId2, amountOrigin);
                    }

                }
            }
            if (hasTaxZero && cartSubTotal != null) sumOrder = cartSubTotal;
            ...
            for (GenericValue taxAuthorityRateProduct : filteredList) {
                ...
                if (orderPromotionsAmount != null && taxAuthorityRateProduct != null && (taxAuthorityRateProduct.get("taxPromotions") == null || (taxAuthorityRateProduct.get("taxPromotions") != null && taxAuthorityRateProduct.getBoolean("taxPromotions").booleanValue()))) {
                    // TODOCHANGE filter by taxAuthorityRateSeqId
                    if (sumAdjustment.compareTo(BigDecimal.ZERO) > 0 && taxAuthorityRateSeqIdMap != null
                            && taxAuthorityRateSeqIdMap.get(taxAuthorityRateProduct.get("taxAuthorityRateSeqId")) != null) {
                        //BigDecimal disparityValue = orderPromotionsAmount.divide(sumAdjustment, 2, RoundingMode.HALF_UP);
                        BigDecimal orderPromotionsAmountItem = (BigDecimal) taxAuthorityRateSeqIdMap.get(taxAuthorityRateProduct.get("taxAuthorityRateSeqId"));
                        //taxable = taxable.add(orderPromotionsAmountItem.multiply(disparityValue));
                        taxable = orderPromotionsAmount.multiply(orderPromotionsAmountItem).divide(sumOrder, 2, RoundingMode.HALF_UP);
                    }
                    //Old taxable = taxable.add(orderPromotionsAmount);
                }
    ...
    // TODOCHANGE quantityUomId - new method advance many parameters
    private static List<GenericValue> getTaxAdjustmentsCustom(Delegator delegator, GenericValue product, GenericValue productStore,
                                                              String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet,
                                                              BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount,
                                                              BigDecimal shippingAmount, BigDecimal orderPromotionsAmount,
                                                              BigDecimal itemAlternativePrice, BigDecimal itemAlternativeQuantity, BigDecimal itemAlternativeAmount, String itemQuantityUomId) {
        BigDecimal valueNull = new BigDecimal(-1);
        if (UtilValidate.isNotEmpty(itemAlternativePrice) && itemAlternativePrice.compareTo(valueNull) == 0
                && UtilValidate.isNotEmpty(itemAlternativeQuantity) && itemAlternativeQuantity.compareTo(valueNull) == 0
                && UtilValidate.isNotEmpty(itemAlternativeAmount) && itemAlternativeAmount.compareTo(valueNull) == 0) {
            // Running normal
            return getTaxAdjustments(delegator, product, productStore, payToPartyId, billToPartyId, taxAuthoritySet,
                    itemPrice, itemQuantity, itemAmount, shippingAmount, orderPromotionsAmount);
        } else {
            // New process
            Timestamp nowTimestamp = UtilDateTime.nowTimestamp();
            List<GenericValue> adjustments = FastList.newInstance();
            ...
            try {
                ...
                // build the main condition clause
                List<EntityCondition> mainExprs = UtilMisc.toList(storeCond, taxAuthoritiesCond, productCategoryCond);
                List<EntityCondition> listMinItemPrice = FastList.newInstance();
                listMinItemPrice.add(EntityCondition.makeCondition("minItemPrice", EntityOperator.EQUALS, null));
                if (UtilValidate.isNotEmpty(itemAlternativePrice)) listMinItemPrice.add(EntityCondition.makeCondition("minItemPrice", EntityOperator.LESS_THAN_EQUAL_TO, itemAlternativePrice));
                else listMinItemPrice.add(EntityCondition.makeCondition("minItemPrice", EntityOperator.LESS_THAN_EQUAL_TO, itemPrice));
                mainExprs.add(EntityCondition.makeCondition(listMinItemPrice, EntityOperator.OR));

                List<EntityCondition> listMinPurchase = FastList.newInstance();
                listMinPurchase.add(EntityCondition.makeCondition("minPurchase", EntityOperator.EQUALS, null));
                if (UtilValidate.isNotEmpty(itemAlternativeAmount)) listMinPurchase.add(EntityCondition.makeCondition("minPurchase", EntityOperator.LESS_THAN_EQUAL_TO, itemAlternativeAmount));
                else listMinPurchase.add(EntityCondition.makeCondition("minPurchase", EntityOperator.LESS_THAN_EQUAL_TO, itemAmount));
                mainExprs.add(EntityCondition.makeCondition(listMinPurchase, EntityOperator.OR));
                ...
                for (GenericValue taxAuthorityRateProduct : filteredList) {
                    ...
                    if (product != null && (product.get("taxable") == null || (product.get("taxable") != null && product.getBoolean("taxable").booleanValue()))) {
                        if (UtilValidate.isNotEmpty(itemAlternativeAmount))
                            taxable = taxable.add(itemAlternativeAmount);
                        else taxable = taxable.add(itemAmount);
                        //taxable = taxable.add(itemAlternativeAmount);
                    }
                    if (shippingAmount != null && taxAuthorityRateProduct != null && (taxAuthorityRateProduct.get("taxShipping") == null || (taxAuthorityRateProduct.get("taxShipping") != null && taxAuthorityRateProduct.getBoolean("taxShipping").booleanValue()))) {
                        taxable = taxable.add(shippingAmount);
    ...
    /* TODOCHANGE quantityUomId - new method advance many parameters and Modify to div Tax by VietTB
       Call to new method has a moment time
    */
    @SuppressWarnings("unused")
    private static List<GenericValue> getDivTaxAdjustmentsCustom(Delegator delegator, GenericValue product, GenericValue productStore,
                                                                 String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet,
                                                                 BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount,
                                                                 BigDecimal shippingAmount, BigDecimal orderPromotionsAmount,
                                                                 BigDecimal itemAlternativePrice, BigDecimal itemAlternativeQuantity, BigDecimal itemAlternativeAmount, String itemQuantityUomId, List<BigDecimal> itemDivTaxList) {
        return getDivTaxAdjustmentsCustom(delegator, product, productStore, payToPartyId, billToPartyId, taxAuthoritySet, itemPrice,
                itemQuantity, itemAmount, shippingAmount, orderPromotionsAmount, itemAlternativePrice, itemAlternativeQuantity,
                itemAlternativeAmount, itemQuantityUomId, itemDivTaxList, null);
    }

    /* TODOCHANGE quantityUomId - new method advance many parameters and Modify to div Tax by VietTB
       Add new parameter moment time calculate tax
    */
    private static List<GenericValue> getDivTaxAdjustmentsCustom(Delegator delegator, GenericValue product, GenericValue productStore,
                                                                 String payToPartyId, String billToPartyId, Set<GenericValue> taxAuthoritySet,
                                                                 BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount,
                                                                 ...
                                                                 BigDecimal itemAlternativePrice, BigDecimal itemAlternativeQuantity, BigDecimal itemAlternativeAmount, String itemQuantityUomId, List<BigDecimal> itemDivTaxList,
                                                                 Timestamp moment) {
        if (moment == null) moment = UtilDateTime.nowTimestamp(); //TODOCHANGE new moment time
        ...
        if (UtilValidate.isNotEmpty(itemAlternativePrice) && itemAlternativePrice.compareTo(valueNull) == 0
            ...
            try {
                ...
                if (product != null) {
                    ...
                    List<GenericValue> pcmList = delegator.findList("ProductCategoryMember", productIdCond, UtilMisc.toSet("productCategoryId", "fromDate", "thruDate"), null, null, true);
                    pcmList = EntityUtil.filterByDate(pcmList, moment);
                    for (GenericValue pcm : pcmList) {
                        productCategoryIdSet.add(pcm.getString("productCategoryId"));
                    }

                    // NEW CODE
                    // if category tax is null, check category tax of parent product
                    if (productCategoryIdSet.size() == 0) {
                        if (virtualProductId != null) {
                            productIdCond = EntityCondition.makeCondition("productId", EntityOperator.EQUALS, virtualProductId);
                        }
                        pcmList = delegator.findList("ProductCategoryMember", productIdCond, UtilMisc.toSet("productCategoryId", "fromDate", "thruDate"), null, null, true);
                        pcmList = EntityUtil.filterByDate(pcmList, moment);
                        for (GenericValue pcm : pcmList) {
                            productCategoryIdSet.add(pcm.getString("productCategoryId"));
                        }
                ...
                if (UtilValidate.isNotEmpty(itemAlternativeAmount)) {
                    //edit by dunglv
                    //process return not order
                    if (UtilValidate.isNotEmpty(productStore) && productStore.getString("salesMethodChannelEnumId").equals("SMCHANNEL_POS")){
                        listMinPurchase.add(EntityCondition.makeCondition("minPurchase", EntityOperator.LESS_THAN_EQUAL_TO, itemAlternativeAmount.abs()));
                    } else {
                        listMinPurchase.add(EntityCondition.makeCondition("minPurchase", EntityOperator.LESS_THAN_EQUAL_TO, itemAlternativeAmount));
                    }
                    //end edit by dunglv
                }
                mainExprs.add(EntityCondition.makeCondition(listMinPurchase, EntityOperator.OR));
                mainExprs.add(EntityCondition.makeCondition("taxAuthorityRateTypeId", EntityOperator.NOT_EQUAL,"IMPORT_TAX"));
                mainExprs.add(EntityCondition.makeCondition("taxAuthorityRateTypeId", EntityOperator.NOT_EQUAL,"EXCISE_TAX"));
                ...
                // finally ready... do the rate query
                List<GenericValue> lookupList = delegator.findList("TaxAuthorityRateProduct", mainCondition, null, orderList, null, false);
                List<GenericValue> filteredList = EntityUtil.filterByDate(lookupList, moment);
                ...
                for (GenericValue taxAuthorityRateProduct : filteredList) {
                    ...
                    if (product != null && taxAuthPartyId != null && taxAuthGeoId != null) {
                        ...
                        Map<String, String> priceFindMap = UtilMisc.toMap("productId", product.getString("productId"),
                                ...
                                "productPricePurposeId", "PURCHASE");
                        List<GenericValue> productPriceList = delegator.findByAnd("ProductPrice", priceFindMap, UtilMisc.toList("-fromDate"), false);
                        productPriceList = EntityUtil.filterByDate(productPriceList, moment);
                        productPrice = (productPriceList != null && productPriceList.size() > 0) ? productPriceList.get(0): null;
                        //Debug.logInfo("=================== productId=" + product.getString("productId"), module);
                        //Debug.logInfo("=================== productPrice=" + productPrice, module);
                    ...
                    if (UtilValidate.isNotEmpty(billToPartyId) && UtilValidate.isNotEmpty(taxAuthGeoId)) {
                        ...
                        Set<String> billToPartyIdSet = FastSet.newInstance();
                        billToPartyIdSet.add(billToPartyId);
                        List<GenericValue> partyRelationshipList = EntityUtil.filterByDate(delegator.findByAnd("PartyRelationship", UtilMisc.toMap("partyIdTo", billToPartyId, "partyRelationshipTypeId", "GROUP_ROLLUP"), null, true), moment);
                        for (GenericValue partyRelationship : partyRelationshipList) {
                            billToPartyIdSet.add(partyRelationship.getString("partyIdFrom"));
                        }
                        handlePartyTaxExempt(taxAdjValue, billToPartyIdSet, taxAuthGeoId, taxAuthPartyId, taxAmount, moment, delegator);
                    } else {
                        Debug.logInfo("NOTE: A tax calculation was done without a billToPartyId or taxAuthGeoId, so no tax exemptions or tax IDs considered; billToPartyId=[" + billToPartyId + "] taxAuthGeoId=[" + taxAuthGeoId + "]", module);
                    }

                    adjustments.add(taxAdjValue);
                    for (int itemDTL = 0;  itemDTL < itemDivTaxList.size(); itemDTL++)
                    {
                        taxAdjValue = delegator.makeValue("OrderAdjustment");
                        taxAmount = (itemDivTaxList.get(itemDTL).multiply(taxRate)).divide(PERCENT_SCALE, salestaxCalcDecimals, salestaxRounding);
                        if (productPrice != null && "Y".equals(productPrice.getString("taxInPrice"))) {
                            // tax is in the price already, so we want the adjustment to be a VAT_TAX adjustment to be subtracted instead of a SALES_TAX adjustment to be added
                            taxAdjValue.set("orderAdjustmentTypeId", "VAT_TAX");
    ...
    public static Map<String, Object> logisticsGetTaxRateProductTaxCalc(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        String productStoreId = (String) context.get("productStoreId");
        String billToPartyId = (String) context.get("billToPartyId");
        String productId = (String) context.get("productId");
