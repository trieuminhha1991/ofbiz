...
public class PaymentWorker {
     ...
     * @return the applied total as BigDecimal in the currency of the payment
     */
    public static BigDecimal getPaymentTrueApplied(GenericValue payment, Boolean actual) {
        BigDecimal paymentApplied = BigDecimal.ZERO;
        List<GenericValue> paymentApplications = null;
        try {
            List<EntityExpr> cond = UtilMisc.toList(
            ...
            //XXX
            EntityCondition statusCond = EntityCondition.makeCondition("statusId", EntityOperator.IN, UtilMisc.toSet("PMNT_RECEIVED", "PMNT_CONFIRMED", "PMNT_SENT"));
            paymentApplications = payment.getDelegator().findList("PaymentAndApplication", EntityCondition.makeCondition(UtilMisc.toList(partyCond, statusCond), EntityJoinOperator.AND), null, UtilMisc.toList("invoiceId", "billingAccountId"), null, false);
            if (UtilValidate.isNotEmpty(paymentApplications)) {
                for (GenericValue paymentApplication : paymentApplications) {
                    BigDecimal amountApplied = paymentApplication.getBigDecimal("amountApplied");
                    // check currency invoice and if different convert amount applied for display
    ...
    public static BigDecimal getPaymentNotTrueApplied(GenericValue payment) {
        if (payment != null) { 
            return payment.getBigDecimal("amount").subtract(getPaymentTrueApplied(payment)).setScale(decimals,rounding);
        } 
        return BigDecimal.ZERO;
    }
    
    public static BigDecimal getPaymentTrueApplied(GenericValue payment) {
        return getPaymentTrueApplied(payment, false);
    }

    public static Boolean isAppliedInvoice(GenericValue payment) throws GenericEntityException {
        GenericValue paymentType = payment.getRelatedOne("PaymentType", false);
        if(UtilValidate.isEmpty(paymentType.get("isAppliedInvoice"))) return true;
        return paymentType.getBoolean("isAppliedInvoice");
    }
   ...
   /* public static BigDecimal getPaymentTrueApplied(GenericValue payment, Boolean actual) {
        ...
        List<GenericValue> paymentApplications = null;
        try {
            EntityConditionList<EntityExpr> invoiceCondition = EntityCondition.makeCondition(UtilMisc.toList(
                    EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "INVOICE_READY"),
                    EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "INVOICE_PAID"), 
                    EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "INVOICE_SENT"),
                    EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "INVOICE_RECEIVED")), EntityOperator.OR);
            ...
            EntityCondition listAllCond = EntityCondition.makeCondition(UtilMisc.toList(
            		partyCond, invoiceCondition), EntityOperator.AND);
            
            paymentApplications = payment.getDelegator().findList("InvoiceAndApplAndPayment", listAllCond, null, UtilMisc.toList("invoiceId", "billingAccountId"), null, false);
            if (UtilValidate.isNotEmpty(paymentApplications)) {
                for (GenericValue paymentApplication : paymentApplications) {
                    BigDecimal amountApplied = paymentApplication.getBigDecimal("amountApplied");
                    // check currency invoice and if different convert amount applied for display
