...
public class InvoiceWorker {
     ...
     */
    public static BigDecimal getPayrollInvoiceTotal(Delegator delegator, String invoiceId) throws GenericEntityException {
        GenericValue payrollInv = delegator.findOne("Invoice", UtilMisc.toMap("invoiceId", invoiceId), false);
    	return getPayrollInvoiceTotal(payrollInv);
    }
     ...
     */
      ...
      * @return the invoice total as BigDecimal
      */
      public static BigDecimal getInvoicePayrollTotal(GenericValue invoice) {
         return getInvoicePayrollTotal(invoice, Boolean.TRUE);
     }     
     
     /**
      * Method to return the total amount of an payroll invoice
      * @param invoice GenericValue object of the Invoice
      * @return the invoice total as BigDecimal
      */
     public static BigDecimal getPayrollInvoiceTotal(GenericValue invoice) {
         return getPayrollInvoiceTotal(invoice, Boolean.TRUE);
     }
     ...
     public static BigDecimal getInvoiceTotal(GenericValue invoice, Boolean actualCurrency) {
        ...
        if (invoiceItems != null) {
            for (GenericValue invoiceItem : invoiceItems) {
            	/*String strInvoiceItemType = "";
            	if(UtilValidate.isNotEmpty(invoiceItem.get("invoiceItemTypeId"))){
            		strInvoiceItemType = invoiceItem.get("invoiceItemTypeId").toString();
            	}
            	if (!strInvoiceItemType.startsWith("PAYROL_TUNS") && !strInvoiceItemType.startsWith("PAYROL_SOCS") && !strInvoiceItemType.startsWith("PAYROL_MDS") && !strInvoiceItemType.startsWith("PAYROL_MEDS"))*/
                if(invoice.getString("invoiceTypeId").equals("PAYROL_INVOICE")) {
                	if(invoiceItem.getBigDecimal("amount").compareTo(BigDecimal.ZERO) > 0) {
     ...
     public static BigDecimal getInvoicePayrollTotal(GenericValue invoice, Boolean actualCurrency) {
         BigDecimal invoiceTotal = ZERO;
         BigDecimal invoiceTaxTotal = ZERO;
         invoiceTaxTotal = InvoiceWorker.getInvoiceTaxTotal(invoice);
     ...
     /**
     *
     * Return the total amount of the payroll invoice (including tax) using the the invoice GenericValue as input.
     * with the ability to specify if the actual currency is required.
     * @param invoice GenericValue object of the Invoice
     * @param actualCurrency true: provide the actual currency of the invoice (could be different from the system currency)
     *                       false: if required convert the actual currency into the system currency.
     * @return Return the total amount of the invoice
     */
    public static BigDecimal getPayrollInvoiceTotal(GenericValue invoice, Boolean actualCurrency) {
    	BigDecimal invoiceTotal = ZERO;
        BigDecimal invoiceTaxTotal = ZERO;
        invoiceTaxTotal = InvoiceWorker.getInvoiceTaxTotal(invoice);

        List<GenericValue> invoiceItems = null;
    	if("PAYROL_INVOICE".equals(invoice.getString("invoiceTypeId"))) {
    		try {
    	           invoiceItems = invoice.getRelated("InvoiceItem", null, null, false);
    	           invoiceItems = EntityUtil.filterByAnd(
                   invoiceItems, UtilMisc.toList(
    ...
    public static BigDecimal getInvoiceNotTrueApplied(Delegator delegator, String invoiceId) {
        return InvoiceWorker.getInvoiceTotal(delegator, invoiceId).subtract(getInvoiceTrueApplied(delegator, invoiceId, UtilDateTime.nowTimestamp(), Boolean.TRUE));
    }
    /**
            ...
            if (UtilValidate.isNotEmpty(paymentApplications)) {
                ...
                if(invoice.getString("invoiceTypeId").equals("PAYROL_INVOICE")) {
                	for (GenericValue paymentApplication : paymentApplications) {
                        if(paymentApplication.getBigDecimal("amountApplied").compareTo(BigDecimal.ZERO) == 0) {
                        	conditions = EntityCondition.makeCondition(UtilMisc.toList(
                                    dateCondition,
                                    EntityCondition.makeCondition("paymentId", EntityOperator.EQUALS, paymentApplication.getString("paymentId")),
                                    EntityCondition.makeCondition("taxAuthGeoId", EntityOperator.NOT_EQUAL, null)),
                                    EntityOperator.AND);
                        	List<GenericValue> pp = delegator.findList("PaymentAndApplication", conditions, null, UtilMisc.toList("effectiveDate"), null, false);
                        	if(UtilValidate.isNotEmpty(pp)) {
                                paymentApplication = paymentApplications.get(0);
     ...
     * @return returns amount applied to invoice before an asOfDateTime
     */
    public static BigDecimal getInvoiceTrueApplied(Delegator delegator, String invoiceId, Timestamp asOfDateTime, Boolean actualCurrency) {
        if (delegator == null) {
            throw new IllegalArgumentException("Null delegator is not allowed in this method");
        }
        ...
        EntityConditionList<EntityCondition> conditions = EntityCondition.makeCondition(UtilMisc.toList(
                ...
                EntityCondition.makeCondition("invoiceId", EntityOperator.EQUALS, invoiceId),
                //XXX
                EntityCondition.makeCondition("statusId", EntityOperator.IN, UtilMisc.toSet("PMNT_RECEIVED", "PMNT_CONFIRMED", "PMNT_SENT"))
        		),
                EntityOperator.AND);
    ...
    public static Boolean isImportTrade(LocalDispatcher dispatcher, String organizationPartyId,String currencyUomId, GenericValue userLogin) throws GenericServiceException {
        if(UtilValidate.isEmpty(currencyUomId)) return false;
        ...
        if(UtilValidate.isEmpty(partyAccountingPreference)) return false;
        String baseCurrencyUomId = partyAccountingPreference.getString("baseCurrencyUomId");
        if(!baseCurrencyUomId.equals(currencyUomId)) return true;
        return false;
    }

    public static Long getDueDayFromParty(Delegator delegator, String partyId) throws GenericEntityException {
        List<EntityCondition> conds = FastList.newInstance();
        conds.add(EntityUtil.getFilterByDateExpr());
        conds.add(EntityCondition.makeCondition("partyId", partyId));
        GenericValue customerTimePayment = EntityUtil.getFirst(delegator.findList("CustomerTimePayment", EntityCondition.makeCondition(conds), null, null, null, false));
        if(UtilValidate.isEmpty(customerTimePayment)) return null;
        return customerTimePayment.getLong("dueDay");
    }

    public static Boolean isCostImport(Delegator delegator, String orderId, String productId) throws GenericEntityException {
        List<EntityCondition> conds = FastList.newInstance();
        conds.add(EntityCondition.makeCondition("orderId", orderId));
        conds.add(EntityCondition.makeCondition("productId", productId));
        List<GenericValue> productCosts = delegator.findList("CostProductInOrder", EntityCondition.makeCondition(conds), null, null, null, false);
        return UtilValidate.isNotEmpty(productCosts);
    }
}
