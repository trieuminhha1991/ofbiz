...
public class InvoiceServices {
    ...
    protected static final int DECIMALS = UtilNumber.getBigDecimalScale("invoice.decimals");
    protected static final int ROUNDING = UtilNumber.getBigDecimalRoundingMode("invoice.rounding");
    protected static final int TAX_DECIMALS = UtilNumber.getBigDecimalScale("salestax.calc.decimals");
    protected static final int TAX_ROUNDING = UtilNumber.getBigDecimalRoundingMode("salestax.rounding");
    protected static final int TAX_CALC_SCALE = UtilNumber.getBigDecimalScale("salestax.calc.decimals");
    protected static final int INVOICE_ITEM_SEQUENCE_ID_DIGITS = 5; // this is the number of digits used for invoiceItemSeqId: 00001, 00002...
    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        if (UtilValidate.isNotEmpty(context.get("conversionFactor"))) {
            conversionFactor = (BigDecimal) context.get("conversionFactor");
        }
        ...
        try {
            ...
            if (UtilValidate.isEmpty(invoiceId)) {
                ...
                createInvoiceContext.put("productStoreId", productStoreId);
                ...
                if("SALES_INVOICE".equals(invoiceType)) {
                    dueDay = InvoiceWorker.getDueDayFromParty(delegator, billToCustomerPartyId);
                }
                else if("PURCHASE_INVOICE".equals(invoiceType)){
                    dueDay = InvoiceWorker.getDueDayFromParty(delegator, billFromVendorPartyId);
                }
                if (UtilValidate.isNotEmpty(dueDay)) {
                    Timestamp dueDateInvoice = UtilDateTime.addDaysToTimestamp(UtilDateTime.nowTimestamp(), dueDay.intValue());
                    createInvoiceContext.put("dueDate", dueDateInvoice);
                }
                // start with INVOICE_IN_PROCESS, in the INVOICE_READY we can't change the invoice (or shouldn't be able to...)
                createInvoiceContext.put("statusId", "INVOICE_IN_PROCESS");
                createInvoiceContext.put("currencyUomId", orderHeader.getString("currencyUom"));
                createInvoiceContext.put("conversionFactor", conversionFactor);
            ...
            for (GenericValue currentValue : billItems) {
                ...
                if (itemIssuance != null) {                    
                    if (UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount"))) {
                    	billingQuantity = itemIssuance.getBigDecimal("weight");
                    } else billingQuantity = itemIssuance.getBigDecimal("quantity");
                    ...
                    if (UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount"))) {
                    	billingQuantity = shipmentReceipt.getBigDecimal("amountAccepted");
                    } else 
                    	billingQuantity = shipmentReceipt.getBigDecimal("quantityAccepted");
                	// Modify by VietTB
//                	quantityTotalReceipt = quantityTotalReceipt.add(shipmentReceipt.getBigDecimal("quantityRejected"));
//                	quantityTotalReceipt = quantityTotalReceipt.add(shipmentReceipt.getBigDecimal("quantityQualityAssurance"));
//                	quantityTotalReceipt = quantityTotalReceipt.subtract(shipmentReceipt.getBigDecimal("quantityExcess"));
//                    billingQuantity = quantityTotalReceipt;
                } else {
//                    BigDecimal orderedQuantity = OrderReadHelper.getOrderItemQuantity(orderItem);
                    BigDecimal orderedQuantity = OrderReadHelper.getOrderItemQuantityExtend(orderItem);
                ...
                // TODOCHANGE: invoice by alternative
                boolean isNewProcessOrder = false;
                if (UtilValidate.isNotEmpty(orderItem.getBigDecimal("alternativeUnitPrice")) && UtilValidate.isNotEmpty(orderItem.getBigDecimal("alternativeQuantity"))) {
                	isNewProcessOrder = true;
                }
                BigDecimal quantityConvert = BigDecimal.ONE;
                if (isNewProcessOrder) {
                	if (product != null) {
                		String quantityUomIdDefault = product.getString("quantityUomId");
                		String quantityUomIdOrder = orderItem.getString("quantityUomId");
                		if (UtilValidate.isNotEmpty(quantityUomIdOrder) && UtilValidate.isNotEmpty(quantityUomIdDefault)) {
                			Map<String, Object> resultValue = dispatcher.runSync("getConvertPackingNumber", UtilMisc.toMap("productId", orderItem.get("productId"), "uomFromId", quantityUomIdOrder, "uomToId", quantityUomIdDefault, "userLogin", userLogin));
                	...
                	if (billingQuantity != null) billingQuantity = billingQuantity.divide(quantityConvert, 2, RoundingMode.HALF_UP);
                }
                ...
                // Old: BigDecimal billingAmount = orderItem.getBigDecimal("unitPrice").setScale(invoiceTypeDecimals, ROUNDING);
                // TODOCHANGE: invoice by alternative
                /* screen 1
                ...
                if (UtilValidate.isNotEmpty(orderItem.getBigDecimal("alternativeUnitPrice")) && UtilValidate.isNotEmpty(orderItem.getBigDecimal("alternativeQuantity"))) {
                	...
                	billingAmount = orderItem.getBigDecimal("alternativeUnitPrice").divide(quantityConvert, 2, RoundingMode.HALF_UP).setScale(invoiceTypeDecimals, ROUNDING);
                } else {
                ...
                if (isNewProcessOrder) {
                	if (UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount"))) {
                    	billingAmount = orderItem.getBigDecimal("alternativeUnitPrice").divide(orderItem.getBigDecimal("selectedAmount"), 12, ROUNDING); // Need update here VIETTB
                    	billingAmount = billingAmount.setScale(invoiceTypeDecimals, ROUNDING);
                    } else               	
                    	billingAmount = orderItem.getBigDecimal("alternativeUnitPrice"); //.setScale(invoiceTypeDecimals, ROUNDING);                	
                } else {
                   	if (UtilValidate.isNotEmpty(product.get("requireAmount")) && "Y".equals(product.getString("requireAmount"))) {
                    	billingAmount = orderItem.getBigDecimal("alternativeUnitPrice").divide(orderItem.getBigDecimal("selectedAmount"), 12, ROUNDING); // Need update here VIETTB
                    	billingAmount = billingAmount.setScale(invoiceTypeDecimals, ROUNDING);
                ...
                createInvoiceItemContext.put("quantityUomId", orderItem.get("quantityUomId"));
                ...
                // Extended by VietTB
//                GenericValue objProduct = delegator.findOne("Product", false, UtilMisc.toMap("productId", orderItem.get("productId")));
...
//                	unitPrice = unitPrice.divide(amount, DECIMALS, ROUNDING);
//                    createInvoiceItemContext.put("alternativeQuantity", orderItem.getBigDecimal("selectedAmount"));
//                    createInvoiceItemContext.put("alternativeAmount", unitPrice);
//                }
                ...
                for (GenericValue adj : itemAdjustments) {
                    ...
                    BigDecimal originalOrderItemQuantity = OrderReadHelper.getOrderItemQuantityExtend(originalOrderItem);
                	...
                	if (amount.signum() != 0) {
                		// Edit for SGC by VIETTB
                    	if (!("PURCHASE_INVOICE".equals(invoiceType) && "SALES_TAX".equals(adj.getString("orderAdjustmentTypeId")))) {
	                        ...
	                        //GenericValue orderItemParent = delegator.findOne("OrderItem", UtilMisc.toMap("orderId", adj.getString("orderId"), "orderItemSeqId", adj.getString("orderItemSeqId")), false);
	                        if (!originalOrderItem.getString("orderItemTypeId").equals("PRODPROMO_ORDER_ITEM"))
	                        ...
	                        else createInvoiceItemAdjContext.put("invoiceItemTypeId","ITM_PROMPAY_ADJ");
            ...
            for (GenericValue payment : currentPayments) {
                ...
                // Edit by VietTB process for multi Shipment amountApplied from invoice not payment
                BigDecimal notAppliedPayment = PaymentWorker.getPaymentNotApplied(payment,true);
                BigDecimal notAppliedInvoice = InvoiceWorker.getInvoiceNotApplied(delegator, invoiceId);
                BigDecimal notApplied;
                if (notAppliedInvoice.compareTo(notAppliedPayment) < 0)
                	notApplied = notAppliedInvoice; 
                	else notApplied = notAppliedPayment;
                if (notApplied.signum() > 0) {
                    ...
                    // Create by VIETTB to automatic change status payment
//                    String autoApproveInvoice = productStore != null ? productStore.getString("autoApproveInvoice") : "Y";
//                    if (!"N".equals(autoApproveInvoice)) {
//                    	String nextStatusPaymentId = "SALES_INVOICE".equals(invoiceType) ? "PMNT_RECEIVED" : "PMNT_NOT_PAID";
//                    	 Map<String, Object> setPaymentStatusResult = dispatcher.runSync("setPaymentStatus", UtilMisc.<String, Object>toMap("paymentId", payment.get("paymentId"), "statusId", nextStatusPaymentId, "userLogin", userLogin));
//                         if (ServiceUtil.isError(setPaymentStatusResult)) {
//                             return ServiceUtil.returnError(UtilProperties.getMessage(resource,
//                                     "AccountingErrorCreatingInvoiceFromOrder", locale), null, null, setPaymentStatusResult);
//                         }
//                    }
...
//                    }
            ...
            String autoApproveInvoice = productStore != null ? productStore.getString("autoApproveInvoice") : "N";
            if (!"N".equals(autoApproveInvoice)) {
            	/**edit by kieuanhvu**/
        ...
        for (String orderId : orders) {
            ...
            for (GenericValue issue : billItems) {
                ...
                try {
                	...
                	objProduct = delegator.findOne("Product", false, UtilMisc.toMap("productId", orderItem.get("productId")));
                } catch (GenericEntityException e) {
                    Debug.logError(e, "Problem getting OrderItem/OrderItemBilling records " + issue, module);
                    return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                            "AccountingProblemGettingOrderItemOrderItemBilling",
                            UtilMisc.toMap("lookup", issue), locale));
                }                

                if (issue.getEntityName().equals("ShipmentReceipt")) {
                	// Modify by VietTB
                	if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                		BigDecimal totalIssueReceipt = issue.getBigDecimal("amountAccepted");                		
	                	totalIssueReceipt = totalIssueReceipt.add(issue.getBigDecimal("amountRejected") != null ? issue.getBigDecimal("amountRejected") : BigDecimal.ZERO);
	                    issueQty = totalIssueReceipt;
                	} else {
	                	BigDecimal totalIssueReceipt = issue.getBigDecimal("quantityAccepted");
	                	totalIssueReceipt = totalIssueReceipt.add(issue.getBigDecimal("quantityRejected"));
	                	totalIssueReceipt = totalIssueReceipt.add(issue.getBigDecimal("quantityQualityAssurance"));
	                    issueQty = totalIssueReceipt;
                    }
                } else {
                	if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                		issueQty = issue.getBigDecimal("weight");  // Need edit here with kg VietTB
                	} else
...
//                    GenericValue orderItem = null;
                    ...
                    try {
                        ...
                        //Extend by VietTB
                        if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                        	orderedQty = orderItem.getBigDecimal("selectedAmount").multiply(orderItem.getBigDecimal("quantity"));                        	
                        }  
                ...
                if (billAvail != null && billAvail.signum() == 1) { // this checks if billAvail is a positive non-zero number
                    if (issueQty != null && issueQty.compareTo(billAvail) >= 0) {
                        ...
                        if ("ShipmentReceipt".equals(issue.getEntityName())) {
                        	if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                        		issue.set("amountAccepted", billAvail);
                        	} else
                        		issue.set("quantityAccepted", billAvail);
                        } else {
                        	if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                        		issue.set("weight", billAvail);
                        	} else
            ...
            if (purchaseShipmentFound) {
                try {
                    ...
                    GenericValue delivery = EntityUtil.getFirst(delegator.findList("Delivery", EntityCondition.makeCondition("shipmentId", shipmentId), null, null, null, false) );
                    serviceContext.put("conversionFactor", delivery.getBigDecimal("conversionFactor"));
            ...
            try {
                result = dispatcher.runSync("createInvoiceForOrder", serviceContext);
            ...
            //Update invoice status from ready to paid if payment method type is EXT_OFFLINE
            //Modified
            try {
				List<GenericValue> listOrderPaymentPref = delegator.findByAnd("OrderPaymentPreference", UtilMisc.toMap("orderId", orderId, "paymentMethodTypeId", "EXT_OFFLINE"), null, false);
				List<String> prefIds = EntityUtil.getFieldListFromEntityList(listOrderPaymentPref, "orderPaymentPreferenceId", true);
				List<GenericValue> payments = delegator.findList("Payment", EntityCondition.makeCondition("paymentPreferenceId", EntityOperator.IN, prefIds), null, null, null, false);
				if(!listOrderPaymentPref.isEmpty() && result.get("invoiceId") != null && UtilValidate.isNotEmpty(payments)) {
					Map<String, Object> setInvStatusCtx = new HashMap<String, Object>();
					setInvStatusCtx.put("invoiceId", result.get("invoiceId"));
					setInvStatusCtx.put("statusId", "INVOICE_PAID");
					setInvStatusCtx.put("userLogin", context.get("userLogin"));
					dispatcher.runSync("setInvoiceStatus", setInvStatusCtx);
				}
            } catch (GenericEntityException e) {
    ...
    protected static String getInvoiceItemType(Delegator delegator, String key1, String key2, String invoiceTypeId, String defaultValue) {
    ...
    public static Map<String, Object> createInvoicesFromReturnShipment(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            for (Map.Entry<String, List<GenericValue>> entry : itemsShippedGroupedByReturn.entrySet()) {
                ...
                Map<String, Object> input = UtilMisc.toMap("returnId", returnId, "billItems", billItems, "userLogin", context.get("userLogin"),"locale", context.get("locale"));
    ...
    public static Map<String, Object> createInvoiceFromReturn(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            if (returnHeader.getString("returnHeaderTypeId").startsWith("CUSTOMER_")) {
                ...
                description = UtilProperties.getMessage(resource, "ReturnInvoiceForCustomerReturn",locale);
                description = description + " #" + returnId;
            } else {
                invoiceTypeId = "PURC_RTN_INVOICE";
                description = UtilProperties.getMessage(resource, "ReturnInvoiceForVendorReturn",locale);
                description = description + " #" + returnId;
            }
            ...
            if ("CUST_RTN_INVOICE".equals(invoiceTypeId))
            	input.put("invoiceDate", returnHeader.getTimestamp("entryDate"));
            else 
            ...
            for (GenericValue item : billItems) {
                ...
                if ( returnItem.getBigDecimal("alternativeQuantity") != null)
                	returnPrice = returnItem.getBigDecimal("alternativePrice");
                else returnPrice = returnItem.getBigDecimal("returnPrice");
                ...
                if (shipmentReceiptFound) {
                	if ( returnItem.getBigDecimal("alternativeQuantity") != null)
                		quantity = returnItem.getBigDecimal("alternativeQuantity");
                	else
                ...
                // Extended by VietTB
                GenericValue objProduct = delegator.findOne("Product", false, UtilMisc.toMap("productId", returnItem.get("productId")));
                ...
                if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                	input.put("amount", returnItem.getBigDecimal("returnPrice").multiply(returnItem.getBigDecimal("returnQuantity")).multiply(returnItem.getBigDecimal("returnAmount")));
                	input.put("alternativeQuantity", returnItem.getBigDecimal("returnAmount").multiply(returnItem.getBigDecimal("returnQuantity")));
                	input.put("alternativeAmount", returnItem.getBigDecimal("returnPrice"));
                }
                else 
                input.put("amount", returnPrice);                
                ...
                if (UtilValidate.isNotEmpty(objProduct.get("requireAmount")) && "Y".equals(objProduct.getString("requireAmount"))) {
                    actualAmount = returnPrice.multiply(quantity).multiply(returnItem.getBigDecimal("returnAmount")).setScale(DECIMALS, ROUNDING);
                    promisedAmount = returnPrice.multiply(quantity.add(cancelQuantity)).multiply(returnItem.getBigDecimal("returnAmount")).setScale(DECIMALS, ROUNDING);
                }
                ...
                for (GenericValue adjustment : adjustments) {
                    ...
                    if ( returnItem.getBigDecimal("alternativeQuantity") != null)                    
                    	ratio = quantity.divide(returnItem.getBigDecimal("alternativeQuantity"), 100, ROUNDING);
                    else 
                    	ratio = quantity.divide(returnItem.getBigDecimal("returnQuantity"), 100, ROUNDING);
                    ...
                    input.put("taxAuthorityRateSeqId", adjustment.get("taxAuthorityRateSeqId"));
                    input.put("userLogin", userLogin);
                    ...
                    // Process for SGC by VIETTB
                    if ( !("SRT_SALES_TAX_ADJ".equals(invoiceItemTypeId) && "PURC_RTN_INVOICE".equals(invoiceTypeId))) {
                    	serviceResults = dispatcher.runSync("createInvoiceItem", input);
                    	if (ServiceUtil.isError(serviceResults)) {
                    		return ServiceUtil.returnError(errorMsg, null, null, serviceResults);
                    	}
            ...
            for (GenericValue adjustment : adjustments) {
                ...
                GenericValue orderAdjustment = delegator.findOne("OrderAdjustment", UtilMisc.toMap("orderAdjustmentId", adjustment.get("orderAdjustmentId")), false);
                GenericValue invoiceItem = null;
                if (UtilValidate.isNotEmpty(orderAdjustment)) {
                	invoiceItem = delegator.findOne("OrderItem", UtilMisc.toMap("orderId", orderAdjustment.get("orderId"), "orderItemSeqId", orderAdjustment.get("orderItemSeqId")), false);
				}
                String productId = null;
                if(UtilValidate.isNotEmpty(invoiceItem))
                   productId = invoiceItem.get("productId") != null ? invoiceItem.getString("productId") : null;
            ...
            BigDecimal invoiceAmount = InvoiceWorker.getInvoiceTotal(delegator, invoiceId, true);
        	
            //create payment
            List<GenericValue> returnItems = null;
            try {
                if (returnHeader != null) {
                    returnItems = returnHeader.getRelated("ReturnItem", UtilMisc.toMap("returnId", returnId), null, false);
                }
            } catch (GenericEntityException e) {
                Debug.logError(e, "Problems looking up return information", module);
                return ServiceUtil.returnError(UtilProperties.getMessage(errorMsg,
            ...
            if (orderHeader != null)
            {
	        	...
	        	if (UtilValidate.isNotEmpty(salesMethodChannelEnumId) && !"SMCHANNEL_POS".equals(salesMethodChannelEnumId))
	        	{
	            	String paymentDescription = UtilProperties.getMessage(resource, "ReturnPaymentForCustomerReturn", locale);
		        	Map<String, Object> createPaymentMap = FastMap.newInstance();
		        	createPaymentMap.put("userLogin", userLogin);
		        	createPaymentMap.put("paymentMethodTypeId", "COMPANY_ACCOUNT");
		        	createPaymentMap.put("paymentTypeId", "CUSTOMER_REFUND");
		        	createPaymentMap.put("partyIdTo", returnHeader.get("fromPartyId"));
		        	createPaymentMap.put("partyIdFrom", returnHeader.get("toPartyId"));
		        	createPaymentMap.put("statusId", "PMNT_NOT_PAID");
		        	createPaymentMap.put("comments", paymentDescription);
		        	createPaymentMap.put("amount", invoiceAmount);
		        	createPaymentMap.put("currencyUomId", returnHeader.get("currencyUomId"));
		            ...
		            if (ServiceUtil.isError(createPayment)) {
		                return ServiceUtil.returnError(errorMsg, null, null, createPayment);
		            }    		
		    		...
		    		createPaymentApp = dispatcher.runSync("createPaymentApplication", paymentApplicationMap);
		            if (ServiceUtil.isError(createPaymentApp)) {
		                return ServiceUtil.returnError(errorMsg, null, null, createPaymentApp);
		            }     
	        	}
	        	else if (orderTypeId.equals("PURCHASE_ORDER"))
	        	{	
	        		// Process for SGC by VIETTB  not user	        		
//	            	String paymentDescription = UtilProperties.getMessage(resource, "ReturnPaymentForVendorReturn", locale);
//		        	Map<String, Object> createPaymentMap = FastMap.newInstance();
//		        	createPaymentMap.put("userLogin", userLogin);
//		        	createPaymentMap.put("paymentMethodTypeId", "COMPANY_ACCOUNT");
//		        	createPaymentMap.put("paymentTypeId", "VENDOR_REFUND");
//		        	createPaymentMap.put("partyIdTo", returnHeader.get("fromPartyId"));
//		        	createPaymentMap.put("partyIdFrom", returnHeader.get("toPartyId"));
//		        	createPaymentMap.put("statusId", "PMNT_NOT_PAID");
//		        	createPaymentMap.put("comments", paymentDescription);
...
//                if ("PMNT_CONFIRMED".equals(payment.get("statusId")) && UtilAccounting.isDisbursement(payment)) {
//                    continue; // keep
//                }
    ...
    protected static BigDecimal calcHeaderAdj(Delegator delegator, GenericValue adj, String invoiceTypeId, String invoiceId, String invoiceItemSeqId,
        ...
        if (adj.get("amount") != null) {
	        ...
	        if (amount.signum() != 0) {
	        	// VietTB update for SGC
	        	 if (!("PURCHASE_INVOICE".equals(invoiceTypeId) && "SALES_TAX".equals(adj.getString("orderAdjustmentTypeId")))) {   
	                Map<String, Object> createInvoiceItemContext = FastMap.newInstance();
	                createInvoiceItemContext.put("invoiceId", invoiceId);
	                createInvoiceItemContext.put("invoiceItemSeqId", invoiceItemSeqId);
    ...
    public static Map<String, Object> createOrderInvoiceReference(DispatchContext ctx, Map<String, Object> context) {
        Delegator delegator = ctx.getDelegator();
        String orderId = (String) context.get("orderId");
        String invoiceId = (String) context.get("invoiceId");
        Map<String, Object> result = ServiceUtil.returnSuccess();
        try {
            GenericValue orderInvoice = delegator.makeValue("OrderInvoiceReference");
            String orderInvoiceId = delegator.getNextSeqId("OrderInvoiceReference");
            orderInvoice.put("orderInvoiceId", orderInvoiceId);
            orderInvoice.put("orderId", orderId);
            orderInvoice.put("invoiceId", invoiceId);
            delegator.create(orderInvoice);
            result.put("orderInvoiceId", orderInvoiceId);
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        return result;
