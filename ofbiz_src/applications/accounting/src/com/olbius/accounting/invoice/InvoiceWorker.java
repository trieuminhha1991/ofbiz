package com.olbius.accounting.invoice;
...
public class InvoiceWorker {
    ...
    /** Method to get the order payment preference id  as a List of orderPaymentPreferenceIds.  These are identified in OrderPaymentPreference with orderId. */
    public static List<String> getOrderPaymentPreferenceIds(Delegator delegator, String orderId) throws GenericEntityException {
        List<String> orderPaymentPreferenceIds = FastList.newInstance();
        List<GenericValue> listOrderPaymentPreference = delegator.findByAnd("OrderPaymentPreference", UtilMisc.toMap("orderId", orderId), null, true);
        for (GenericValue orderPaymentPreference : listOrderPaymentPreference) {
        	orderPaymentPreferenceIds.add(orderPaymentPreference.getString("orderPaymentPreferenceId"));
        }
        return orderPaymentPreferenceIds;
    }
    ...
    public static BigDecimal getInvoiceTaxTotalByTaxRateSeq(GenericValue invoice) throws GenericEntityException {
        BigDecimal taxTotal = ZERO;
        Delegator delegator = invoice.getDelegator();
        List<EntityCondition> listCondition = FastList.newInstance();
        listCondition.add(EntityCondition.makeCondition("invoiceId", invoice.get("invoiceId")));
        listCondition.add(EntityCondition.makeCondition("taxAuthorityRateSeqId", EntityOperator.NOT_EQUAL, "NULL"));
        List<GenericValue> invoiceItems = delegator.findList("InvoiceItem", EntityCondition.makeCondition(listCondition), null, null, null, false);
        taxTotal = getTaxTotalForInvoiceItems(invoiceItems);
        return taxTotal;
    }
    ...
    public static Map<String, Set<String>> getInvoiceTaxAuthPartyAndGeos (GenericValue invoice) {
        ...
        try {
            ...
            EntityConditionList<EntityExpr> condition = EntityCondition.makeCondition(UtilMisc.toList(
                    ...
                    EntityOperator.AND);
            /*
             * lay ra tat ca cac invoice_item co TAX
             */
     ...
     * @param taxAuthGeoId
     * @return The invoice item list  for a given tax authority and geo location
     * @author VietTB
     */
    
    public static List<GenericValue> getInvoiceItemTaxAuthPartyAndGeos (GenericValue invoice, String taxAuthPartyId, String taxAuthGeoId) {      
        if (invoice == null)
           throw new IllegalArgumentException("Invoice cannot be null.");
        List<GenericValue> invoiceTaxItems = null;
        try {
            ...
            EntityConditionList<EntityExpr> condition = EntityCondition.makeCondition(UtilMisc.toList(
                    ...
                    EntityOperator.AND);            
            invoiceTaxItems = delegator.findList("InvoiceItem", condition, null, null, null, false);
            Debug.log(module + "::getInvoiceItemTaxAuthPartyAndGeos, list.sz = " + invoiceTaxItems.size());
        ...
        return invoiceTaxItems;
    }

    public static Boolean isFixedAsset(GenericValue invoiceItemTax) {
        if(invoiceItemTax == null) {
            throw new IllegalArgumentException("Invoice cannot be null.");
        }

        Delegator delegator = invoiceItemTax.getDelegator();
        EntityConditionList<EntityExpr> condition = EntityCondition.makeCondition(UtilMisc.toList(
                EntityCondition.makeCondition("invoiceId", invoiceItemTax.getString("invoiceId")),
                EntityCondition.makeCondition("invoiceItemSeqId", invoiceItemTax.getString("parentInvoiceItemSeqId"))),
                EntityOperator.AND);
        try {
            List<GenericValue> invoiceItemParents = delegator.findList("InvoiceItem", condition, null, null, null, false);
            GenericValue invoiceItemParent = EntityUtil.getFirst(invoiceItemParents);
            if(invoiceItemParent == null) {
                return false;
            }
            GenericValue invoiceItemType = delegator.findOne("InvoiceItemType", UtilMisc.toMap("invoiceItemTypeId", invoiceItemParent.get("invoiceItemTypeId")), false);
            if(invoiceItemType != null && isFixedAssetAccount((String) invoiceItemType.get("defaultGlAccountId"))) return true;
            else return false;
        } catch (GenericEntityException e) {
            Debug.logError(e, "Trouble getting InvoiceItem list", module);
            return false;
    ...
    public static String FIXED_ASSET_TYPE_1 = "211";
    public static String FIXED_ASSET_TYPE_2 = "212";
    public static String FIXED_ASSET_TYPE_3 = "213";

    public static Boolean isFixedAssetAccount(String glAccountId) {
        if(UtilValidate.isEmpty(glAccountId)) return false;
        if(glAccountId.startsWith(FIXED_ASSET_TYPE_1) || glAccountId.startsWith(FIXED_ASSET_TYPE_2) || glAccountId.startsWith(FIXED_ASSET_TYPE_3)) {
            return true;
        }
        return false;
    }
    ...
    public static Map<String, Object> jqGetInvoiceTotal(DispatchContext ctx, Map<String, Object> context) {
    	...
    	successResult.put("invoiceTotal", invoiceTotal);
     ...
     * Method to return the total amount of an invoice which is applied to a payment
     * @param invoice
     * @return the not applied payment total as BigDecimal
     */
    public static BigDecimal getInvoiceNotPayment(GenericValue invoice) {
        return InvoiceWorker.getInvoiceTotal(invoice, Boolean.TRUE).subtract(getInvoicePayment(invoice));
    }    
     ...
     * @return the applied total as BigDecimal
     */
    public static BigDecimal getInvoicePayment(GenericValue invoice) {
        return getInvoicePayment(invoice, UtilDateTime.nowTimestamp());
    }    
    
    public static BigDecimal getInvoicePayment(GenericValue invoice, Timestamp asOfDateTime) {
        return getInvoicePayment(invoice.getDelegator(), invoice.getString("invoiceId"), asOfDateTime, Boolean.TRUE);
    }
     ...
     * @return returns amount applied to invoice before an asOfDateTime
     */
    public static BigDecimal getInvoicePayment(Delegator delegator, String invoiceId, Timestamp asOfDateTime, Boolean actualCurrency) {
        if (delegator == null) {
            throw new IllegalArgumentException("Null delegator is not allowed in this method");
        }
        ...
        EntityConditionList<EntityExpr> applicationCondition = EntityCondition.makeCondition(UtilMisc.toList(
                ...
                EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "PMNT_CONFIRMED")), EntityOperator.OR);
      
        EntityConditionList<EntityCondition> conditions = EntityCondition.makeCondition(UtilMisc.toList(
                dateCondition, applicationCondition,
                EntityCondition.makeCondition("invoiceId", EntityOperator.EQUALS, invoiceId)),
                EntityOperator.AND);
    ...
    public static Boolean isExistsVoucher(GenericValue invoice) throws GenericEntityException {
        List<GenericValue> voucher = invoice.getRelated("VoucherInvoice", null, null, false);
        return UtilValidate.isNotEmpty(voucher);
    }
}
