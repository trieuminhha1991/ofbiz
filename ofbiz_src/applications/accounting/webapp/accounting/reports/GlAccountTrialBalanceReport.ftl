    ...
    <#if glAcctgTrialBalanceList?has_content>
        <div>
            <form name="glAccountTrialBalanceReport" id="glAccountTrialBalanceReport">
                <div>
                    <a href="<@ofbizUrl>GlAccountTrialBalanceReportPdf.pdf?organizationPartyId=${organizationPartyId}&amp;timePeriod=${parameters.timePeriod}&amp;isPosted=${parameters.isPosted}&amp;glAccountId=${parameters.glAccountId}</@ofbizUrl>" target="_BLANK" class="btn btn-small btn-info">${uiLabelMap.AccountingInvoicePDF}</a>
                </div>
                <h4 class="header smaller lighter blue" style="margin-left: 15px">${uiLabelMap.AccountingSubsidiaryLedger}</h4>
                <h4 class="header smaller lighter blue" style="margin-left: 15px">${uiLabelMap.FormFieldTitle_companyName} : ${(currentOrganization.groupName)!}</h4>
                <h4 class="header smaller lighter blue" style="margin-left: 15px">${uiLabelMap.AccountingTimePeriod} : <#if currentTimePeriod?has_content>${(currentTimePeriod.fromDate)!} ${uiLabelMap.CommonTo} ${(currentTimePeriod.thruDate)!}</#if></h4>
                <h4 class="header smaller lighter blue" style="margin-left: 15px">${uiLabelMap.AccountingGlAccountNameAndGlAccountCode} : ${(glAccount.accountCode)!} - ${(glAccount.accountName)!}</h4>
