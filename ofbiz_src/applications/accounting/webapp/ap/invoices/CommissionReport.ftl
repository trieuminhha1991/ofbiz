...
<#if commissionReportList?has_content>
  <div>
    <a href="<@ofbizUrl>CommissionReport.pdf?isSearch=Y&amp;productId=${parameters.productId!}&amp;partyId=${parameters.partyId!}&amp;fromDate=${parameters.fromDate!}&amp;thruDate=${parameters.thruDate!}</@ofbizUrl>" target="_BLANK" class="btn btn-info btn-mini">${uiLabelMap.AccountingInvoicePDF}</a>
  </div>
  <table class="table table-striped table-hover table-bordered dataTable" cellspacing="0">
    ...
    <#list commissionReportList as commissionReport>
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><a class="btn btn-info btn-mini" href="/catalog/control/EditProduct?productId=${commissionReport.productId!}">${commissionReport.productName!}</a></td>
        ...
        <td>
          ...
          <#if commissionReport.salesInvoiceIds?has_content>
            <#list commissionReport.salesInvoiceIds as salesInvoiceId>
              [<a class="btn btn-info btn-mini" href="/ap/control/invoiceOverview?invoiceId=${salesInvoiceId!}">${salesInvoiceId!}</a>]
            </#list>
          </#if>
        </td>
