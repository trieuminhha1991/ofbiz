...
<#if invoices?has_content >
  ...
  <form name="listPurchaseInvoices" id="listPurchaseInvoices" method="post" action="javascript:void(0);">
    <div class="margin-left10 margin-top10" align="left">
      ...
      <button class="btn btn-small btn-info margin-top-nav-10" id="submitButton" type="button" onclick="javascript:runAction();" disabled="disabled">
      <i class="icon-ok"></i>
      ${uiLabelMap.CommonRun}
      </button>
    </div>
    ...
    <div id="issueChecks" style="display: none;" align="left" class="margin-left10">
    ...
    <table class="table hover-bar table-bordered table-striped dataTable" cellspacing="0">
      <#-- Header Begins -->
      <tr class="header-row-2">
        ...
        <td>${uiLabelMap.CommonSelectAll}
        <div style="display: inline"><input type="checkbox" class="checkbox-opacity margin-top-nav-1" id="checkAllInvoices" name="checkAllInvoices" onchange="javascript:toggleInvoiceId(this);"/></div></td>
      </tr>
      <#-- Header Ends-->
      ...
      <#list invoices as invoice>
        ...
        <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
          ...
          <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
            <td><a class="btn btn-mini btn-info" href="<@ofbizUrl>invoiceOverview?invoiceId=${invoice.invoiceId?if_exists}</@ofbizUrl>">${invoice.get("invoiceId")}</a></td>
            ...
            <td><a class="btn btn-mini btn-info" href="/partymgr/control/viewprofile?partyId=${invoice.partyIdFrom}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyIdFrom, false)?if_exists} [${(invoice.partyIdFrom)?if_exists}] </a></td>
            <td><a class="btn btn-mini btn-info" href="/partymgr/control/viewprofile?partyId=${invoice.partyId}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyId, false)?if_exists} [${(invoice.partyId)?if_exists}]</a></td>
            ...
            <td align="right"><input type="checkbox" class="checkbox-opacity" id="invoiceId_${invoice_index}" name="invoiceIds" value="${invoice.invoiceId}" onclick="javascript:getInvoiceRunningTotal();"/></td>
          </tr>
...
<#else>
  <p class="alert alert-info">${uiLabelMap.AccountingNoInvoicesFound}</p>
</#if>
