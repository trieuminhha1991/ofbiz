  ...
  <form name="listSalesInvoices" id="listSalesInvoices" method="post">
    ...
    <table class="table table-striped table-hover table-bordered dataTable" cellspacing="0">
      ...
      <tr class="header-row-2">
        <td width="9%"><input type="checkbox" id="checkAllInvoices" class="checkbox-opacity" name="checkAllInvoices" onchange="javascript:toggleInvoiceId(this);"/> ${uiLabelMap.CommonSelectAll}</td>
      ...
      <#list invoices as invoice>
        ...
        <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
          <td>
          	<label>
				<input id="invoiceId_${invoice_index}" name="invoiceIds" type="checkbox" value="${invoice.invoiceId}" onclick="javascript:enableSubmitButton();"><span class="lbl"></span>
			</label>
          	</td>
          <td><a class="btn btn-primary btn-mini" href="<@ofbizUrl>invoiceOverview?invoiceId=${invoice.invoiceId}</@ofbizUrl>">${invoice.get("invoiceId")}</a></td>
