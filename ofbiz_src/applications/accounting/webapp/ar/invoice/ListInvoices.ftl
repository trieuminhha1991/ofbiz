...
<#if invoiceList?has_content && (parameters.noConditionFind)?if_exists == 'Y'>
  <div class="alert alert-info">
    <span class="p">${uiLabelMap.AccountingRunningTotalOutstanding} :</span>
    <span class="p" id="showInvoiceRunningTotal"></span>
  </div>
  <form name="listInvoices" id="listInvoices"  method="post" action="">
    <div align="right" style="padding: 10px">
      <select name="serviceName" id="serviceName" onchange="javascript:setServiceName(this);" style="margin-bottom: 0px;">
      ...
      <button id="submitButton" class="btn btn-info btn-small" style="margin-bottom: 0px;" type="button"  onclick="javascript:jQuery('#listInvoices').submit();" disabled="disabled" >
      	<i class="icon-ok"></i>
    ...
    <table class="table table-hover table-striped table-bordered dataTable" cellspacing="0">
      <thead>
        <tr class="header-row-2">
          ...
          <td align="right">${uiLabelMap.CommonSelectAll} <input style="opacity: 0.5 !important; position: relative !important;" type="checkbox" id="checkAllInvoices" name="checkAllInvoices" onchange="javascript:toggleInvoiceId(this);"/></td>
        </tr>
      </thead>
      <tbody>
        ...
        <#list invoiceList as invoice>
          ...
          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a class="" href="<@ofbizUrl>invoiceOverview?invoiceId=${invoice.invoiceId}</@ofbizUrl>">${invoice.get("invoiceId")}</a></td>
              <td>
              ...
              <td><a class="" href="/partymgr/control/viewprofile?partyId=${invoice.partyIdFrom}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyIdFrom, false)?if_exists} [${(invoice.partyIdFrom)?if_exists}] </a></td>
              <td><a class="" href="/partymgr/control/viewprofile?partyId=${invoice.partyId}">${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, invoice.partyId, false)?if_exists} [${(invoice.partyId)?if_exists}]</a></td>
              ...
              <td align="right"><input style="opacity: 0.5 !important; position: relative !important;" type="checkbox" id="invoiceId_${invoice_index}" name="invoiceIds" value="${invoice.invoiceId}" onclick="javascript:getInvoiceRunningTotal();"/></td>
            </tr>
...
<#else>
<div class="alert alert-info">
  <p>${uiLabelMap.AccountingNoInvoicesFound}</p>
</div>
</#if>
