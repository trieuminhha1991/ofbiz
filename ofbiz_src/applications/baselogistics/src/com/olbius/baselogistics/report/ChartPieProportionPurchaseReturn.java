...
public class ChartPieProportionPurchaseReturn extends AbstractOlap {
	...
	private void initQuery() {
		...
		String a = "PURCHASE_ORDER";
		String c = "VENDOR_RETURN";
		Object b = a;
		Object d = c;
		query = new OlbiusQuery(getSQLProcessor());
		query.select("iif.inventory_change_type");
		query.select(
		...
		query.from("inventory_item_fact", "iif")
				...
				.groupBy("iif.inventory_change_type")
				.where(Condition.makeBetween("dd.date_value", getSqlDate(fromDate), getSqlDate(thruDate))
						...
						.andIn("iif.inventory_change_type", UtilMisc.toList(b, d))
						);
	}
		...
		public Object resultQuery(OlapQuery query) {
			...
			try {
				...
				while (resultSet.next()) {
					String inventory_change_type = resultSet.getString("inventory_change_type");
