...
public class PercentageCustomerCompletedOrder extends OlbiusOlapService {
	...
	private OlbiusQuery init() {
		...
		// tong so order cua khach hang theo chu ki
		queryTotal
			.select("pt.party_id as party_id")
			...
			.from("order_status_fact", "osf")
				...
				.join(Join.INNER_JOIN, "party_dimension", "pt", "osf.party_to_dim_id = pt.dimension_id ")
				...
				.where(Condition
						...
						.andEQ("stt.status_id", "ORDER_CREATED")
						.andEQ("osf.order_type_id", "SALES_ORDER")
						.andEQ("cpn.party_id", company))
						.groupBy(dateType)
						.groupBy("osf.party_to_dim_id");
	
	
		// tong so order khach hang da hoan thanh theo chu ki
		queryTemp
		...
		query
			...
			.select("total.dateTime as dateTime")
				...
				.join(Join.INNER_JOIN, queryTemp, "temp", "total.dateTime = temp.dateTime and total.party_id = temp.party_id");
