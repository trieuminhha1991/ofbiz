...
public class AverageWorkEmployeeOrderExcel extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "[LOG]_BC_NHAN_VIEN_XU_LY_DON_HANG_BAN";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "BLAverageWorkEmployeeOrder", locale));
		setRunServiceName("olapAverageWorkEmployeeOrder");
		setModuleExport("LOG");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "BLAverageWorkEmployeeOrder", locale));
		...
		addColumn(24, UtilProperties.getMessage(RESOURCE, "BLStaffCode", locale), "party_code");
		addColumn(48, UtilProperties.getMessage(RESOURCE, "BLStaffName", locale), "party_name");
