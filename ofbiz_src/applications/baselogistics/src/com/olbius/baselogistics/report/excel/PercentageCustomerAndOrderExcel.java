...
public class PercentageCustomerAndOrderExcel extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "[LOG]_BC_KHACH_HANG_VA_DON_HANG_BAN";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "BLPercentageCustomerAndOrder", locale));
		setRunServiceName("olapPercentageCustomerAndOrder");
		setModuleExport("LOG");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "BLPercentageCustomerAndOrder", locale));
		...
		addColumn(24, UtilProperties.getMessage(RESOURCE, "BLCustomerCode", locale), "party_code");
		addColumn(48, UtilProperties.getMessage(RESOURCE, "BLCustomerName", locale), "party_name");
		addColumn(24, UtilProperties.getMessage(RESOURCE, "BLTotalOrderCustomer", locale), "order_num", ExportExcelStyle.STYLE_CELL_NUMBER);
		addColumn(24, UtilProperties.getMessage(RESOURCE, "BLTotalOrderPeriod", locale), "order_num_total", ExportExcelStyle.STYLE_CELL_NUMBER);
		addColumn(24, UtilProperties.getMessage(RESOURCE, "BLPercentageOrder", locale), "percentage_order", ExportExcelStyle.STYLE_CELL_DECIMAL);
	}
