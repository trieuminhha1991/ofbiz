...
public class ReceiveCustomerReturnExcel extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_NHAP_KHO_KH_TRA_LAI";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "BLReceivedCustomerReturnReport", locale));
		setRunServiceName("olapReceivedCustomerReturnReport");
		setModuleExport("LOG");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "BLReceivedCustomerReturnReport", locale));
		...
		addColumn(16, UtilProperties.getMessage(RESOURCE, "FacilityId", locale), "facility_id");
