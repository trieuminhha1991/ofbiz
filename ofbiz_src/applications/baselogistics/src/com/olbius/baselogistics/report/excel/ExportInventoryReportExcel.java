...
public class ExportInventoryReportExcel extends ExportExcelAbstract{
	...
	protected void prepareParameters(DispatchContext dctx, Map<String, Object> context) {
		...
		String fileName = "BC_NHAP_XUAT_TON";
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd_HHmmss");
		String dateTime = format.format(nowTimestamp);
		fileName += "_" + dateTime;
		setFileName(fileName);
		setHeaderName(UtilProperties.getMessage(RESOURCE, "InventoryReportTotal", locale));
		setRunServiceName("olapInventoryReport");
		setModuleExport("LOG");
		setDescriptionExport(UtilProperties.getMessage(RESOURCE, "InventoryReportTotal", locale));
		...
		addColumn(16, UtilProperties.getMessage(RESOURCE, "OpeningInventoryQuantity", locale), "inventoryP", ExportExcelStyle.STYLE_CELL_NUMBER);
		addColumn(16, UtilProperties.getMessage(RESOURCE, "BLTotalReceived", locale), "receive", ExportExcelStyle.STYLE_CELL_NUMBER);
		addColumn(16, UtilProperties.getMessage(RESOURCE, "BLTotalExported", locale), "export", ExportExcelStyle.STYLE_CELL_NUMBER);
		addColumn(16, UtilProperties.getMessage(RESOURCE, "EndingInventoryQuantity", locale), "inventory", ExportExcelStyle.STYLE_CELL_NUMBER);
	}
