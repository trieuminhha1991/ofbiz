<#if contactMechTypeIdCheck == "EMAIL_ADDRESS">
	...
	<script>
		...
		//Create emailAddress
		$("#emailAddress").jqxInput({width: 195, height: 20});
		...
		$('#alterpopupWindow').jqxValidator({
		    rules: [
						{ input: '#emailAddress', message: '${uiLabelMap.CheckIsEmptyInfo}', action: 'keyup, blur', rule: 'required' },
		...
		$("#alterCreateEmailAddress").click(function (){
			...
			if(validate != false){
				...
				if(contactMechPurposeTypeId == "" || infoString == ""){
					...
					$("#alterCreateEmailAddress").addClass("disabled");
					$("#alterExitEmailAddress").addClass("disabled");
					var request = $.ajax({
					...
					request.done(function(data) {
						...
						$("#alterCreateEmailAddress").removeClass("disabled");
						$("#alterExitEmailAddress").removeClass("disabled");
						if(data.responseMessage == "error"){
							$('#jqxNotificationCreateEmailAddressError').jqxNotification({ template: 'error'});
				            $("#notificationCreateEmailAddressError").text(data.errorMessage);
				            $("#jqxNotificationCreateEmailAddressError").jqxNotification("open");
				        }else{
				        	$('#container').empty();
				        	$('#emailAddress').val('');
...
<div id="jqxNotificationCreateInternetIPAddressError" >
	...
	<script>
		...
		//Create infoString
		$("#internetIPAddress").jqxInput({width: 195, height: 20, height: 25});
		...
		$("#alterCreateInternetIPAddress").click(function (){
			...
			if(contactMechTypeId == "" || infoString == ""){
				$("#notificationCreateInternetIPAddressError").text('${StringUtil.wrapString(uiLabelMap.CheckIsEmptyInfo)}');
