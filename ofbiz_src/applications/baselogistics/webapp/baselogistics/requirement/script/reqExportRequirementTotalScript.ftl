...
<script type="text/javascript">
	...
	if (curStatusId != 'REQ_APPROVED' && curStatusId != 'REQ_RECEIVED'){
	...
	<#if originFacility?has_content>
		...
		facilitySelected = {"facilityId": "${originFacility.facilityId?if_exists}", "facilityName": "${originFacility.facilityName?if_exists}"};
	</#if>
	...
	<#assign cond1 = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("ownerPartyId", company?if_exists)/> 
	<#assign cond2 = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("ownerPartyId", null)/>
	<#assign listCondsOw = []>
	<#assign listCondsOw = listCondsOw + [cond1]>
	<#assign listCondsOw = listCondsOw + [cond2]>
	<#assign listCondsOwOr = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition(listCondsOw, Static["org.ofbiz.entity.condition.EntityOperator"].OR)/>
	<#assign condReq = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("requirementId", requirementId)/>
	<#assign requirementItems = delegator.findList("RequirementItemDetailAndInventory", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition(Static["org.ofbiz.base.util.UtilMisc"].toList(condReq, listCondsOwOr)), null, null, null, false)/>
	...
	<#if requirementItems?has_content>
		<#list requirementItems as item>
		    ...
		    <#if item.requireAmount?has_content && item.requireAmount == 'Y'>
				var quantityTmp = "${item.weight?if_exists}";
		    ...
		    <#else>
			    <#if item.quantityOnHandTotal?has_content>
					...
					<#if item.quantityOnHandTotal &gt; item.quantity>
				...
				var requiredQuantityTmp = "${item.quantity?if_exists}";
				item.requiredQuantity = parseFloat(requiredQuantityTmp, null, 3);
		    </#if>
...
<script type="text/javascript" src="/logresources/js/requirement/reqExportRequirementTotal.js?v=1.1.1"></script>
