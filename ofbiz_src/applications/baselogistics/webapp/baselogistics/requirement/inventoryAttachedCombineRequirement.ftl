<#include "script/inventoryAttachedScript.ftl"/>
<div id="original-item">
	...
	<#assign columnlistTo="
					...
					{ text: '${uiLabelMap.Quantity}', dataField: 'quantity', columntype: 'numberinput', width: 150, editable: true,
						cellsrenderer: function(row, column, value){
							var data = $('#jqxgridItemLabelTo').jqxGrid('getrowdata', row);
							...
							if (value === undefined || value === null || value === ''){
								if (value === null || value === undefined || value === ''){
									uid = data.uid;
									$('#jqxgridItemLabelTo').jqxGrid('setcellvaluebyid', uid, 'quantity', data.quantityTmp);
					    ...
					    validation: function (cell, value) {
					    	var data = $('#jqxgridItemLabelTo').jqxGrid('getrowdata', cell.row);
					        if (value < 0){
					        	return { result: false, message: '${uiLabelMap.ValueMustBeGreateThanZero}'};
					        } else if (value > data.quantityTmp){
	...
	<@jqGrid filtersimplemode="true" filterable="true" showtoolbar="false" dataField=dataFieldTo columnlist=columnlistTo editable="true" editmode="click" 
		url="jqxGeneralServicer?sname=getInventoryLabelToRequirement&requirementId=${parameters.requirementId?if_exists}" id="jqxgridItemLabelTo"
			selectionmode="checkbox"
	/>
</div>
<script>
$("#jqxgridItemLabelTo").on('rowselect', function (event) {
	var rows = $("#jqxgridItemLabelTo").jqxGrid('getselectedrowindexes');
	for (var i = 0; i < requirementItemData.length; i ++){
		...
		for (var j = 0; j < rows.length; j ++){
			var data = $("#jqxgridItemLabelTo").jqxGrid('getrowdata', rows[j]);
