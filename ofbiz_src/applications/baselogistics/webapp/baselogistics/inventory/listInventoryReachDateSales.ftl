...
<#assign columnlist="
				...
				{ text: '${uiLabelMap.ATP}', datafield: 'availableToPromiseTotal', hidden: true, align: 'left', width: 100, cellsalign: 'right', columntype: 'numberinput', filtertype: 'number',
					cellsrenderer: function(row, column, value){
						if (value){
       						var desc = '';
       						var data = $('#jqxgirdProductInventoryDateReachSales').jqxGrid('getrowdata', row);
				...
				{ text: '${uiLabelMap.RemainingDay}', datafield: 'dateSaleNumber', align: 'left', width: 130, cellsalign: 'right', columntype: 'numberinput', filtertype: 'number', filterable: false,
					cellsrenderer: function(row, column, value){
						var listData = $('#jqxgirdProductInventoryDateReachSales').jqxGrid('getrowdata', row);
...
<@jqGrid filtersimplemode="true" filterable="true" dataField=dataField columnlist=columnlist editable="false" showtoolbar="true"
	id="jqxgirdProductInventoryDateReachSales" addrefresh="true" filterable="true" mouseRightMenu="true" contextMenuId="menuReachDateSales"
	url="jqxGeneralServicer?sname=JQGetListInventoryReachDateSales&productId=${parameters.productId?if_exists}" 
	customTitleProperties="BLListProductInventoryDateSales" />	
				
<div id='menuReachDateSales' style="display:none;">
	<ul>
...
<script>
	$(document).ready(function (){
		$("#menuReachDateSales").jqxMenu({ width: 220, autoOpenPopup: false, mode: 'popup', theme: theme});
	});
	$("#menuReachDateSales").on('itemclick', function (event) {
		var tmpStr = $.trim($(args).text());
		if(tmpStr == "${StringUtil.wrapString(uiLabelMap.BSRefresh)}"){
			$('#jqxgirdProductInventoryDateReachSales').jqxGrid('updatebounddata');
		}
	});
</script>