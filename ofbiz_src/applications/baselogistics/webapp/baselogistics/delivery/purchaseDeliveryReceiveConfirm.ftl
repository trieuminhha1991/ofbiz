		...
		<div class='row-fluid'>
			<div class="span6">
				<div class='row-fluid'>
					...
					<div class="span8">
						<div class="green-label" style="text-align: left;" id="orderIdCf" name="orderIdCf">${orderId?if_exists}</div>
			   		</div>
				</div>
				<div class='row-fluid'>
					<div class='span4 align-right'>
				...
				<table id="tableProduct" width="100%" border="0" cellpadding="0" class="table table-striped table-bordered table-hover dataTable">
					<thead>
						<tr valign="bottom" style="height: 40px">
							...
							<th width="8%" class="align-center <#if !hasOlbPermission("MODULE", "PO_PRICE", "VIEW")> hide </#if>"><span><b>${uiLabelMap.UnitPrice} (${orderHeader.currencyUom?if_exists})</b></span></th>
							<th width="10%" class="align-center <#if !hasOlbPermission("MODULE", "PO_PRICE", "VIEW")> hide </#if>"><span><b>${uiLabelMap.BPOTotal} </br></span></th>
						</tr>
					</thead>
					<tbody>
