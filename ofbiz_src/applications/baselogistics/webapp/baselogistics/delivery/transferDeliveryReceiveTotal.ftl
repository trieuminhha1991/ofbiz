<#include 'script/transferDeliveryReceiveTotalScript.ftl'/>
<h4 class="smaller lighter blue" style="margin: 5px 0px 10px 0px !important;font-weight:500;line-height:20px;font-size:18px;">
	${uiLabelMap.BLReceiveInventoryByTransferDelivery} - ${uiLabelMap.DeliveryTransferId}: ${deliveryId?if_exists}
</h4>
<div class="row-fluid">
	<div id="fuelux-wizard" class="row-fluid" data-target="#step-container">
		<ul class="wizard-steps wizard-steps-square">
			<li data-target="#step1" class="active">
				<span class="step">1. ${uiLabelMap.UpdateActualReceivedQuantity}</span>
			</li>
	...
	<div class="step-content row-fluid position-relative" id="step-container">
		...
		<div class="step-pane active" id="step1">
			<div style="position:relative">
				<div class="row-fluid">
					<div class="span12">
						<#include "transferDeliveryReceiveProduct.ftl"/>
					</div> 
				</div>
			</div>
		</div>
		...
		<div class="step-pane" id="step2">
			<#include "transferDeliveryReceiveConfirm.ftl"/>
		</div>
	</div><!--.step-content-->
