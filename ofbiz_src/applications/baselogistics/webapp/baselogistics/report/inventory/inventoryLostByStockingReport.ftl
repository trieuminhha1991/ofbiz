<div id="olbiusLostStockingReport"></div>
<script type="text/javascript">
    $(function() {
        var config = {
            title: "${StringUtil.wrapString(uiLabelMap.BLReportInventoryLostStocking)}",
            service: "facilityInventory",
            button: true,
            id: "olbiusLostStockingReport",
            olap: "olapLostStockingReport",
            sortable: true,
            filterable: true,
            showfilterrow: true,
            columns: [
            ...
            excel: function(obj){
				...
				if (!isExistData) {
					OlbCore.alert.error("${uiLabelMap.BLReportInventoryLostStocking}");
					return false;
				}
				var otherParam = "";
				if (obj._data) {
				...
				window.open("exportLostStockingReportExcel?" + otherParam, "_blank");
            },
            exportFileName: '[LOG]_BC_THAT_THOAT_KIEM_KE_' + (new Date()).formatDate("ddMMyyyy")
        };
