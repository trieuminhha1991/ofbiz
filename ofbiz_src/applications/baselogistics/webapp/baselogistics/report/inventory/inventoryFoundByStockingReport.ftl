<div id="olbiusFoundStockingReport"></div>
<script type="text/javascript">
    $(function() {
        var config = {
            title: "${StringUtil.wrapString(uiLabelMap.BLReportInventoryFoundStocking)}",
            service: "facilityInventory",
            button: true,
            id: "olbiusFoundStockingReport",
            olap: "olapFoundStockingReport",
            sortable: true,
            filterable: true,
            showfilterrow: true,
            columns: [
            ...
            excel: function(obj){
				...
				if (!isExistData) {
					OlbCore.alert.error("${uiLabelMap.BLReportInventoryFoundStocking}");
					return false;
				}
				var otherParam = "";
				if (obj._data) {
				...
				window.open("exportFoundStockingReportExcel?" + otherParam, "_blank");
            },
            exportFileName: '[LOG]_BC_PHAT_SINH_KIEM_KE_' + (new Date()).formatDate("ddMMyyyy")
        };
