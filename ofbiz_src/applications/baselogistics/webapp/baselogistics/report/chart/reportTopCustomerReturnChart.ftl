...
<div id="olbiusChartColTopCustomerReturn"></div>
<script type="text/javascript">
	$(function(){
		var configChartColTopCustomerReturn = {
			service: "facilityInventory",
            id: "olbiusChartColTopCustomerReturn",
            olap: "olapChartColTopCustomerReturn",
            
            chart: {
                type: 'column'
            },
            title: {
                text: '${StringUtil.wrapString(uiLabelMap.BLChartTopCustomerReturn)}',
                x: -20 //center
            },
            xAxis: {
                type: 'category',
            ...
            tooltip: {
                formatter: function () {
                    return '<b> SKU: ' + this.x + '</b>' + '<br/>' + '<b> ${StringUtil.wrapString(uiLabelMap.RemainingSubTotal)}: ' + this.y.toLocaleString(locale) + '</b>';
                }
            },
            plotOptions: {
	            series: {
		...
		OlbiusUtil.chart(configChartColTopCustomerReturn);
	});
</script>