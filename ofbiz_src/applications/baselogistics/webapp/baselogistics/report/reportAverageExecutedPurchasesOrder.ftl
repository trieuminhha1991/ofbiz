<div id="averageExecutedPurchasesOrder"></div>
<script type="text/javascript">
	$(function () {
		var config = {
			title: '${StringUtil.wrapString(uiLabelMap.BLAverageTimeExecutedOrderPurchases)}',
			button: true,
			service: 'orderStatus',
			id: 'averageExecutedPurchasesOrder',
			olap: 'olapAverageExecutedPurchasesOrder',
			sortable: true,
			filterable: true,
			showfilterrow: true,
			columns: [
				{ 
					...
					text: '${StringUtil.wrapString(uiLabelMap.BLSupplierCode)}',
					datafield: {name: 'party_code', type: 'string'}, width: 180,
					cellsrenderer: function (row, column, value) {
						return '<div class=\"text-left\">' + value + '</div>';
					}
				}, {
					text: '${StringUtil.wrapString(uiLabelMap.BLSupplierName)}',
					datafield: {name: 'party_name', type: 'string'}, width: 200,
					cellsrenderer: function (row, column, value) {
						return '<div class=\"text-left\">' + value + '</div>';
					}
				}, { 
					...
					datafield: {name: 'executed_time_total', type: 'string'},width: 200,
					cellsrenderer: function (row, column, value) {
						if (value){
							return '<div class=\"text-right\">' + formatnumber(value) + '</div>';
						} else {
					...
					datafield: {name: 'average_executed_time', type: 'string'}, width: 240,
					cellsrenderer: function (row, column, value) {
						if (value){
							return '<div class=\"text-right\">' + formatnumber(value) + '</div>';
						} else {
					...
					datafield: {name: 'average_executed_time_hour', type: 'string'}, width: 230,
					cellsrenderer: function (row, column, value) {
						if (value){
							return '<div class=\"text-right\">' + formatnumber(value) + '</div>';
						} else {
			...
			excel: function(obj){
				...
				window.open("exportAverageExecutedPurchasesOrderExcel?" + otherParam, "_blank");
            },
