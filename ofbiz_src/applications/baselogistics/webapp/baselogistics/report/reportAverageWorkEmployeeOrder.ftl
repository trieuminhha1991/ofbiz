<div id="averageWorkEmployeeOrder"></div>
<script type="text/javascript">
	$(function () {
		var config = {
			title: '${StringUtil.wrapString(uiLabelMap.BLAverageWorkEmployeeOrder)}',
			button: true,
			service: 'orderStatus',
			id: 'averageWorkEmployeeOrder',
			olap: 'olapAverageWorkEmployeeOrder',
			sortable: true,
			filterable: true,
			showfilterrow: true,
			columns: [
				{ 
					...
					text: '${StringUtil.wrapString(uiLabelMap.BLStaffCode)}',
					datafield: {name: 'party_code', type: 'string'}, width: 200,
					cellsrenderer: function (row, column, value) {
						return '<div class=\"text-left\">' + value + '</div>';
					}
				}, {
					text: '${StringUtil.wrapString(uiLabelMap.BLStaffName)}',
					...
					datafield: {name: 'order_num_total', type: 'string'}, 
					cellsrenderer: function (row, column, value) {
						if (value){
							return '<div class=\"text-right\">' + formatnumber(value) + '</div>';
						} else {
			...
			excel: function(obj){
				...
				window.open("exportAverageWorkEmployeeOrderExcel?" + otherParam, "_blank");
            },
            exportFileName: '[LOG]_BC_NHAN_VIEN_XU_LY_DON_HANG_BAN_' + (new Date()).formatDate("ddMMyyyy")
		};
