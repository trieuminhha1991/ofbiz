...
<script type="text/javascript">
	...
	var returnTypeId = '${returnHeader.returnHeaderTypeId?if_exists}';
	var curStatusId = '${returnHeader.statusId?if_exists}';
	if (curStatusId != 'SUP_RETURN_ACCEPTED'){
		window.location.href = "getDetailVendorReturn?returnId="+returnId;
	}
	...
	var facilityId = '${returnHeader.destinationFacilityId?if_exists}';
	...
	<#assign returnStatus = delegator.findList("StatusItem", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("statusTypeId", "PORDER_RETURN_STTS"), null, null, null, false) />
	...
	<#assign condRt = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("returnId", Static["org.ofbiz.entity.condition.EntityOperator"].EQUALS, returnId)>
	<#assign condStt = Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition("statusId", Static["org.ofbiz.entity.condition.EntityOperator"].EQUALS, 'SUP_RETURN_ACCEPTED')>
	
	<#assign returnItems = delegator.findList("ReturnItemDetailAndInventory", Static["org.ofbiz.entity.condition.EntityCondition"].makeCondition(Static["org.ofbiz.base.util.UtilMisc"].toList(condRt, condStt)), null, null, null, false)/>
		...
		<#list returnItems as item>
		    ...
		    <#if item.requireAmount?has_content && item.requireAmount == 'Y'>
				...
				<#if item.amountOnHandTotal?has_content>
					var qoh = "${item.amountOnHandTotal?if_exists}";
					<#if locale == 'vi'>
					...
					item.qoh = parseFloat(qoh, null, 3);
		    ...
		    <#else>
			    <#if item.quantityOnHandTotal?has_content>
					...
					item.qoh = parseFloat(qohTmp, null, 3);
					<#if item.quantityOnHandTotal &gt; item.returnQuantity>
						var quantityTmp = "${item.returnQuantity?if_exists}";
						item.quantity = parseFloat(quantityTmp, null, 3);
					<#else>
						var quantityTmp = "${item.quantityOnHandTotal?if_exists}";
						item.quantity = parseFloat(quantityTmp, null, 3);
					</#if>
			    <#else>
			    	item.qoh = 0;
				...
				var returnQuantityTmp = "${item.returnQuantity?if_exists}";
				item.returnQuantity = parseFloat(returnQuantityTmp, null, 3);
		    </#if>
	...
	returnHeader['estimatedBudget'] = '${returnHeader.estimatedBudget?if_exists}';
	...
	returnHeader['entryDate'] = '${returnHeader.entryDate?if_exists}';
...
<script type="text/javascript" src="/logresources/js/return/supReturnExportReturnTotal.js?v=1.1.1"></script>
