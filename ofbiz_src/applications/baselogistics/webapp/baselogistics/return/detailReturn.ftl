...
<#if returnHeader.returnHeaderTypeId == "CUSTOMER_RETURN">
<#assign columnlist= columnlist + "{ text: '${uiLabelMap.ReceivedNumber}', dataField: 'quantityOnHandDiff', width: 150, editable:false, filtertype: 'number',
	cellsrenderer: function (row, column, value){
		if (value){
			return '<span style=\"text-align: right\">' + formatnumber(value) + '</span>';
		} else {
...
<#elseif returnHeader.returnHeaderTypeId == "VENDOR_RETURN">
	...
	<#assign columnlist= columnlist + "{ text: '${uiLabelMap.OrderNumber}', dataField: 'actualDeliveredQuantity', width: 150, editable:false, filtertype: 'number',
	cellsrenderer: function (row, column, value){	
		var data = $('#jqxgridProductReturn').jqxGrid('getrowdata', row);
		if (data.requireAmount && data.requireAmount == 'Y') {
