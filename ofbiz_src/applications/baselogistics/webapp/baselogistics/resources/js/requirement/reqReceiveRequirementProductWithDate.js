...
var OlbReqProduct = (function() {
					...
					if (value != oldvalue){
						if (items && items.length > 0){
							...
							if (!check){
								...
								newItem["reqItemSeqId"] = rowData.reqItemSeqId;
	...
	var getColumns = function(grid){
		var columns = [
			...
			{ text: uiLabelMap.ActualReceivedQuantitySum, datafield: 'quantity', sortable: false,  width: '23%', editable: true, filterable: false, sortable: false, cellclassname: productGridCellclass,
              	cellsalign: 'right', columntype: 'numberinput',
				cellsrenderer: function(row, column, value) {
					var rowData = grid.jqxGrid('getrowdata', row);
			...
			{ text: uiLabelMap.Quantity, datafield: 'quantity', sortable: false,  width: '20%', editable: true, filterable: false, sortable: false, cellclassname: productGridCellclass,
				...
				cellsrenderer: function(row, column, value) {
					...
					for (var x in listProductSelected){
						if ( listProductSelected[x].productId == rowData.productId){
							if (listProductSelected[x].requireAmount && 'Y' == listProductSelected[x].requireAmount){
								check = true;
							}
							break;
						}
	...
	var generaterow = function (){
		  ...
		  if (productSelected){
			  ...
			  row['reqItemSeqId'] = productSelected.reqItemSeqId;
			  row['expireDate'] = null;
