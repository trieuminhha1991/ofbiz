$(function(){
	ReqEditTemplateObj.init();
});
var ReqEditTemplateObj = (function() {
	var btnClick = false;
	...
	var initEvents = function() {
		$('#fuelux-wizard').ace_wizard().on('change' , function(e, info){
			if(info.step == 1 && (info.direction == "next")) {
				...
				var resultValidate = !ReqEditInfoObj.getValidator().validate();
	...
	function showConfirmPage(){
		if (!requirementTypeId){
			$("#requirementTypeDT").text($("#requirementTypeId").text());
		}
		if (!reasonEnumId){
		...
		if ($("#tableProduct").length > 0){
			...
			for(var i in listProductSelected){
				if((listProductSelected[i].statusId != "REQ_CANCELLED") && (listProductSelected[i].statusId != "REQ_REJECTED")){
			...
			for (var i in finalListProductSelected){
				...
				if (!hideFacility){
					var newCell4 = newRow.insertCell(++count);
				...
				if (!hidePrice) {
					...
					if (data.unitCost && data.quantity){
						...
						if (value) {
							newText = document.createTextNode(formatcurrency(value));
						} 
					} else {
						newText = document.createTextNode(formatcurrency(0));
					}
			...
			if (!hidePrice) {
				if (totalValue){
					...
					var newTextTotal = document.createTextNode(formatcurrency(totalValue));
	...
	function finishEditRequirement(){
		var listProducts = [];
		if (listProductSelected != undefined && listProductSelected.length > 0){
			for (var i = 0; i < listProductSelected.length; i ++){
				...
				map['sequenceId'] = data.sequenceId;
	...
	var isAllItemQuantityNull = function(){
		var count = 0;
		for(var i = 0; i < listProductSelected.length; i++){
			if((listProductSelected[i].quantity == 0) || (listProductSelected[i].statusId == 'REQ_CANCELLED')){
				count++;
			}
		}
