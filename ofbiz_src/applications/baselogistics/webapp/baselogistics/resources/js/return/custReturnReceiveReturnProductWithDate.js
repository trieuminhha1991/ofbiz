...
var OlbReqProduct = (function() {
			...
			if (rowData){
				if (dataField == 'quantity'){
					if (value != oldvalue){
						...
						if (value >= 0 && value <= returnQuantity){
					...
					if (value != oldvalue){
						if (items && items.length > 0){
							...
							if (!check){
								...
								newItem["returnItemSeqId"] = rowData.returnItemSeqId;
	...
	var getColumns = function(grid){
		var columns = [
			...
			{editable: false, text: uiLabelMap.UnitSum, sortable: false, dataField: 'uomId', width: '7%', columntype: 'dropdownlist', filterable:false, cellclassname: productGridCellclass,
				cellsrenderer: function(row, column, value) {
			...
			{ text: uiLabelMap.RequiredNumberSum, datafield: 'returnQuantity', sortable: false,  width: '13%', editable: false, filterable: false, sortable: false, cellclassname: productGridCellclass,
			...
			{ text: uiLabelMap.BLReceivedNumberSum, datafield: 'receivedQuantity', sortable: false,  width: '13%', editable: false, filterable: false, sortable: false, cellclassname: productGridCellclass,
				cellsalign: 'right', columntype: 'numberinput',
				cellsrenderer: function(row, column, value) {
					if (value > 0) {
						return '<span class="align-right">' + formatnumber(value) +'</span>';
			...
			{ text: uiLabelMap.ActualReceivedQuantitySum, datafield: 'quantity', sortable: false,  width: '20%', editable: true, filterable: false, sortable: false, cellclassname: productGridCellclass,
              	cellsalign: 'right', columntype: 'numberinput',
				cellsrenderer: function(row, column, value) {
					var rowData = grid.jqxGrid('getrowdata', row);
					...
					var reqQty = rowData.returnQuantity;
					if (rowData.receivedQuantity > 0){
						reqQty = reqQty - rowData.receivedQuantity;
					}
					if (reqQty < value){
						return { result: false, message: uiLabelMap.BLCannotGreaterThanNotReceivedQuantity + ": " + formatnumber(value) + " > " + formatnumber(reqQty)};
					}
			...
			{ text: uiLabelMap.UnitPrice, hidden: hidePrice, datafield: 'returnPrice', sortable: false,  width: '15%', editable: false, filterable: false, sortable: false, cellclassname: productGridCellclass,
				cellsalign: 'right', columntype: 'numberinput',
				cellsrenderer: function(row, column, value) {
					if (value > 0) {
						if (locale && locale === 'vi' && typeof(value) === 'string'){
							value = data.returnPrice.toString().replace('.', '');
							value = value.replace(',', '.');
						}
						return '<span class="align-right">' + formatnumber(value) +'</span>';
					} else {
			...
			{ text: uiLabelMap.Quantity, datafield: 'quantity', sortable: false,  width: '20%', editable: true, filterable: false, sortable: false, cellclassname: productGridCellclass,
				...
				cellsrenderer: function(row, column, value) {
					...
					$.each(listProductSelected, function(i){
		   				var olb = listProductSelected[i];
		   				if (olb.productId == rowData.productId){
		   					returnQuantity = olb.returnQuantity;
		   					return false;
		   				}
		   			});
					if (typeof (returnQuantity) == 'string'){
						returnQuantity = parseFloat(returnQuantity);
					}
					if (returnQuantity < total){
						return { result: false, message: uiLabelMap.CannotGreaterRequiredNumber + ": " + formatnumber(total) + " > " + formatnumber(returnQuantity)};
					}
					return true;
				}, 
			},
	...
	var rendertoolbarInfo = function (toolbar){
        ...
        container.append('<a href="javascript:OlbReqProduct.deleteRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="red fa fa-times"></i></a>');
        container.append('<a href="javascript:OlbReqProduct.addRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="fa fa-plus"></i></a>');
        container.append('<a href="javascript:OlbReqProduct.addMultiRow()" data-rel="tooltip" data-placement="bottom" class="button-action"><i class="fa fa-plus-square"></i></a>');
	...
	var updateGridProductInfoData = function (originData, listInv){
		...
		var returnItemSeqId = originData.returnItemSeqId;
	...
	var generaterow = function (){
		  ...
		  if (productSelected){
			  ...
			  row['returnItemSeqId'] = productSelected.returnItemSeqId;
			  row['expireDate'] = null;
