$(function(){
	...
	OlbShipmentConfirm.init();
});
var OlbShipmentConfirm = (function(){
	var init = function(){
		initInputs();
		initElementComplex();
		initEvents();
	...
	var initEvents = function() {
		$("#jqxgridInvSelected").on("cellendedit", function (event) {
	    	var args = event.args;
	    	if (args.datafield == "quantity") {
	    		var rowBoundIndex = args.rowindex;
		    	var data = $("#jqxgridInvSelected").jqxGrid("getrowdata", rowBoundIndex);
		    	if (data && data.productId) {
			   		var oldValue = args.oldvalue;
			   		var newValue = args.value;
			   		if (isNaN(newValue)) {
			   			$('#jqxgridInvSelected').jqxGrid('setcellvalue', rowBoundIndex, 'quantity', oldValue);
			   			return false;
			   		}
			   		if (newValue > 0){
			   			var existed = false;
		    	...
		    	if (OlbCore.isNotEmpty(data) && OlbCore.isNotEmpty(data.productId) && OlbCore.isNotEmpty(data.quantity)) {
			   		...
			   		if (isNaN(quantity)) {
			   			$('#jqxgridInvSelected').jqxGrid('setcellvalue', rowBoundIndex, 'quantityUomId', oldValue);
			   			return false;
			   		}
			   		if (data.quantity > 0){
			   			var map = {};
