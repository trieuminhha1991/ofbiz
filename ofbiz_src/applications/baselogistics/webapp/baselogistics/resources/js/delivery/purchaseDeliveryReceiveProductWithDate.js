...
var PurDlvProduct = (function() {
					...
					if (value != oldvalue){
						if (items && items.length > 0){
							...
							if (!check){
								...
								newItem["deliveryItemSeqId"] = rowData.deliveryItemSeqId;
	...
	var getColumns = function(grid){
		var columns = [
			...
			{ text: uiLabelMap.UnitPrice, hidden: hidePrice, datafield: 'unitPrice', sortable: false,  width: '15%', editable: false, filterable: false, sortable: false, cellclassname: productGridCellclass,
				cellsalign: 'right', columntype: 'numberinput',
				cellsrenderer: function(row, column, value) {
					if (value >= 0) {
	...
	var updateGridProductInfoData = function (originData, listInv){
		...
		var deliveryItemSeqId = originData.deliveryItemSeqId;
	...
	var generaterow = function (){
		  ...
		  if (productSelected){
			  ...
			  row['requireAmount'] = productSelected.requireAmount;
			  row['expireDate'] = null;
