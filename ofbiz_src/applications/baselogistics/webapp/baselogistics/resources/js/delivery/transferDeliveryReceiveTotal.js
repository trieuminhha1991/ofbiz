...
var PurDlvTotalObj = (function() {
	...
	function showConfirmPage(){
		if ($("#tableProduct").length > 0){
			...
			for (var i in listProductSelected){
				...
				if (data.unitPrice && data.quantity >= 0){
					var u = data.unitPrice;
					if (locale && locale === 'vi' && typeof(u) === 'string'){
						u = data.unitPrice.toString().replace('.', '');
						u = u.replace(',', '.');
	...
	function finishReceiveDelivery(){
				...
				if (items){
					if (items.length > 0){
						for (var x in items){
							...
							if (myObj.quantity > 0 && (myObj.expireDate || myObj.datetimeManufactured || myObj.lotId)){
    	...
    	var url = "receiveProductFromTransferDelivery";
    	$.ajax({	
			 type: "POST",
			 url: url,
			 data: {
