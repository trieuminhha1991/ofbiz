...
var DlvTotalObj = (function() {
	...
	function showConfirmPage(){
		if ($("#tableProduct").length > 0){
			...
			for (var i in listProductSelected){
				...
				if (data.isPromo){
					if ("Y" == data.isPromo){
						newText = document.createTextNode(uiLabelMap.LogYes);
					} else {
						newText = document.createTextNode(uiLabelMap.LogNO);
					}
				} else {
				...
				if (data.promoQuantity > 0) {
					...
					promoTotal = promoTotal + data.promoQuantity;
				}
				...
				newCell9.className = 'align-right';
				if (data.unitPrice && total >= 0){
					if (promoTotal > 0) total = total - promoTotal;
					if (data.selectedAmount > 0){
						total = total/data.selectedAmount;
					}
	...
	function finishUpdateDelivery(){
		...
		if (listProductSelected != undefined && listProductSelected.length > 0){
			for (var i = 0; i < listProductSelected.length; i ++){
				...
				var map = {};
		   		...
		   		map['promoQuantity'] = data.promoQuantity;
    	...
    	var url = "updateDeliveredSalesDelivery";
    	$.ajax({	
			 type: "POST",
			 url: url,
			 data: {
