...
var DlvTotalObj = (function() {
	...
	function showConfirmPage(){
		if ($("#tableProduct").length > 0){
			...
			for (var i in listProductSelected){
				...
				if (data.promoQuantity > 0 && requireDate == 'N') {
	...
	function finishExportDelivery(){
				...
				if (items){
					if (items.length > 0){
						for (var x in items){
							...
							if ((myObj.quantity > 0 || myObj.promoQuantity > 0) && (myObj.expireDate || myObj.datetimeManufactured || myObj.lotId)){
		   		...
		   		map['quantity'] = parseFloat(parseFloat(data.quantity).toFixed(2));
    	...
    	var url = "exportProductFromTransferDelivery";
    	$.ajax({	
			 type: "POST",
			 url: url,
			 data: {
			 ...
			 success: function(data){
				window.location.href = "deliveryTransferDeliveryDetail?deliveryId="+deliveryId;
			 },
