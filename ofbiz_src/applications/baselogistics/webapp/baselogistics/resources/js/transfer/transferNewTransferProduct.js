...
var OlbReqProduct = (function() {
	...
	var getColumns = function(grid){
		var columns = [
			...
			{ text: uiLabelMap.BSAverageCost, datafield: 'unitCost', sortable: false, width: 120, editable: false, filterable: false, cellsalign: 'right', columntype: 'numberinput', cellclassname: productGridCellclass,
					...
					if (rowData.quantity > 0 && value != '' && value != null && value != undefined & value != 'null'){
						if (listProductSelected.length > 0){
							...
							$.each(listProductSelected, function(i){
				   				var olb = listProductSelected[i];
				   				if (olb.productId == rowData.productId ){
				   					listProductSelected[i].description = value;
				   					return false;
				   				}
				   			});
						}
