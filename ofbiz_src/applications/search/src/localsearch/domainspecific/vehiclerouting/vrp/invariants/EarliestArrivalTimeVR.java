...
public class EarliestArrivalTimeVR implements InvariantVR {
	...
	private void update(int k) {
		...
		for (Point x = s; x != XR.getTerminatingPointOfRoute(k); x = XR.next(x)) {
			Point nx = XR.next(x);
			// System.out.println(earliestArrivalTime.get(x) +
			// "  "+serviceDuration.get(x) + "   "+ awm.getDistance(x, nx));
			double tnx = earliestArrivalTime.get(x) + serviceDuration.get(x)
					+ awm.getDistance(x, nx);
			double tmp = tnx > earliestAllowedArrivalTime.get(nx) ? tnx
					: earliestAllowedArrivalTime.get(nx);
			earliestArrivalTime.put(nx, tmp);
			// System.out.println(x+"  "+nx+"  "+awm.getDistance(x,
			// nx)+"   "+tmp);
		}
	}
	...
	void updateFromPoint(Point x) {
		// System.out.println(name() + "::updateFromPoint(" + x.ID +
		// "), earliestArrivalTime = " + earliestArrivalTime.get(x));
		Point pX = XR.prev(x);
		Point p = pX;
		Point nP = x;
		...
		// System.out.println(name() + "::updateFromPoint(" + x.ID +
		// "), earliestArrivalTime = " + earliestArrivalTime.get(x) + ", p = " +
		// p.ID + ", nP = " + nP.ID);
		do {
			double curTime = earliestArrivalTime.get(p)
					+ serviceDuration.get(p);
			if (earliestAllowedArrivalTime.get(nP) > curTime
					+ awm.getDistance(p, nP)) {
				earliestArrivalTime.put(nP,
						1.0 * earliestAllowedArrivalTime.get(nP));
			} else {
				earliestArrivalTime.put(nP, curTime + awm.getDistance(p, nP));
			}
			// System.out.println(name() + "::updateFromPoint(" + x.ID +
			// "), set earliestArrivalTime(" + nP.ID + ") = " +
			// earliestArrivalTime.get(nP));
			p = nP;
			nP = XR.next(p);
		} while (!XR.isTerminatingPoint(p));
		/*
		 * System.out.println(name() + "::updateFromPoint(" + x.ID +
		 * "), after propagate: "); for(int k = 1; k <= XR.getNbRoutes(); k++){
		 * for(Point z = XR.startPoint(k); z != XR.endPoint(k); z = XR.next(z)){
		 * System.out.println("earliestArrivalTime(" + z.ID + ") = " +
		 * earliestArrivalTime.get(z)); } Point z = XR.endPoint(k);
		 * System.out.println("earliestArrivalTime(" + z.ID + ") = " +
		 * earliestArrivalTime.get(z)); }
		 */
	}
	...
	public void propagateKPointsMove(ArrayList<Point> x, ArrayList<Point> y) {
		// TODO Auto-generated method stub
		/*
		 * HashMap<Integer,Point> mp = new HashMap<Integer,Point>(); int i;
		 * for(i=0;i<x.size();++i) { Point p = x.get(i); if(y.get(i) ==
		 * CBLSVR.NULL_POINT) continue; int k = XR.route(p); int ind =
		 * XR.index(p); if(mp.containsKey(k)) { if(XR.index(mp.get(k)) > ind)
		 * mp.put(k, p); } else{ mp.put(k, p); } } Set<Integer> keyset =
		 * mp.keySet(); for(int k : keyset) { updateFromPoint(mp.get(k)); }
		 */
		// System.out.print(name() + "::propagateKPointsMove, XR = " +
		// XR.toString() + ", x = ");
