...
public class AccumulatedNodeWeightsOnPathVR implements IFunctionVR {
	...
	public AccumulatedNodeWeightsOnPathVR(AccumulatedWeightNodesVR accWN,
			Point v) {
		this.accWN = accWN;
		this.v = v;
	...
	private double calc(Point s, Point t) {
		...
		return XR.isBefore(s, t) ? accWN.getSumWeights(t)
				- accWN.getSumWeights(s) + accWN.getWeights(s) : accWN
				.getSumWeights(s)
				- accWN.getSumWeights(t)
				+ accWN.getWeights(t);
	}
	...
	private double evaluateMove(Point[] x, Point[] y, Point[] z) {
		...
		while (i < x.length || j < z.length) {
			if (j == z.length || (i < x.length && XR.isBefore(y[i], z[j]))) {
				...
				if (v == x[i]) {
					return d + accWN.getSumWeights(y[i])
	...
	public double evaluateTwoOptMoveOneRoute(Point x, Point y) {
		System.out.println(name()
				+ "::evaluateTwoOptMoveOneRoute, NOT IMPLEMENTED");
		System.exit(-1);
		return 0;
	}
		...
		if (XR.route(x) == XR.route(v)) {
			if (v == XR.getTerminatingPointOfRoute(XR.route(x))) {
				return accWN.getSumWeights(x)
						+ accWN.getSumWeights(y)
						- accWN.getWeights(XR.getStartingPointOfRoute(XR
								.route(y)))
						+ accWN.getWeights(XR.getTerminatingPointOfRoute(XR
								.route(x))) - accWN.getSumWeights(v);
			}
			if (XR.isBefore(x, v)) {
	...
	public double evaluateTwoOptMove5(Point x, Point y) {
		...
		if (XR.isBefore(x, v)) {
			return accWN.getSumWeights(y) + calc(XR.next(x), v)
					- accWN.getSumWeights(v);
		}
		if (XR.isBefore(y, v)) {
			return accWN.getSumWeights(x) + calc(XR.next(y), v)
					- accWN.getSumWeights(v);
		}
		return 0;
	}
		...
		if (XR.route(v) == XR.route(x)) {
			...
			return calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y)))
					- accWN.getWeights(XR.getTerminatingPointOfRoute(XR
							.route(y)))
					+ accWN.getWeights(XR.getStartingPointOfRoute(XR.route(x)))
					+ calc(x, v) - accWN.getSumWeights(v);
		}
		...
		if (XR.route(v) == XR.route(y)) {
			...
			return calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x)))
					- accWN.getWeights(XR.getTerminatingPointOfRoute(XR
							.route(x)))
					+ accWN.getWeights(XR.getStartingPointOfRoute(XR.route(y)))
					+ calc(y, v) - accWN.getSumWeights(v);
		}
		return 0;
	}
	...
	public double evaluateOrOptMove1(Point x1, Point x2, Point y) {
		...
		if (XR.route(x1) == XR.route(v)) {
			if (XR.isBefore(x2, v)) {
				return accWN.getSumWeights(XR.prev(x1)) + calc(XR.next(x2), v)
						- accWN.getSumWeights(v);
			}
			if (XR.isBefore(XR.prev(x1), v)) {
				return accWN.getSumWeights(y) + calc(x1, v)
						- accWN.getSumWeights(v);
			}
			return 0;
		}
	...
	public double evaluateOrOptMove2(Point x1, Point x2, Point y) {
		...
		if (XR.route(x1) == XR.route(v)) {
			...
			if (XR.isBefore(XR.prev(x1), v)) {
				return accWN.getSumWeights(y) + calc(x2, v)
						- accWN.getSumWeights(v);
			}
			return 0;
		}
	...
	public double evaluateThreeOptMove1(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			if (XR.isBefore(v, XR.next(z)) && XR.isBefore(y, v)) {
				return accWN.getSumWeights(x) + calc(v, z)
						- accWN.getSumWeights(v);
			}
			if (XR.isBefore(v, XR.next(y)) && XR.isBefore(x, v)) {
				return accWN.getSumWeights(x) + calc(XR.next(y), z)
						+ calc(XR.next(x), v) - accWN.getSumWeights(v);
			}
		}
		return 0;
	}
	...
	public double evaluateThreeOptMove2(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(y, v)) {
				double newV = accWN.getWeights(XR.getStartingPointOfRoute(XR
						.route(x)))
						- accWN.getWeights(XR.getTerminatingPointOfRoute(XR
								.route(x)));
				newV += calc(XR.next(z),
	...
	public double evaluateThreeOptMove3(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(y, v)) {
				return accWN.getSumWeights(x) + calc(XR.next(x), y)
						+ calc(v, z) - accWN.getSumWeights(v);
			}
			if (XR.isBefore(x, v)) {
				return accWN.getSumWeights(x) + calc(v, y)
						- accWN.getSumWeights(v);
			}
		}
		return 0;
	...
	public double evaluateThreeOptMove8(Point x, Point y, Point z) {
		// TODO Auto-generated method stub
		if (!XR.checkPerformThreeOptMove(x, y, z)) {
			System.out.println(name() + ":: Error evaluate three opt move8: "
					+ x + " " + y + " " + z + "\n" + XR.toString());
			System.exit(-1);
		}
		if (v == XR.getStartingPointOfRoute(XR.route(x))
	...
	public double evaluateCrossExchangeMove(Point x1, Point y1, Point x2,
		...
		if (!XR.checkPerformCrossExchangeMove(x1, y1, x2, y2)) {
			System.out.println(name() + ":: Error evaluateCrosExchangeMove: "
					+ x1 + " " + y1 + " " + x2 + " " + y2 + "\n"
					+ XR.toString());
			System.exit(-1);
		}
		if (XR.route(x1) == XR.route(v)) {
			if (XR.isBefore(y1, v)) {
				return accWN.getSumWeights(x1) + calc(XR.next(x2), y2)
						+ calc(XR.next(y1), v) - accWN.getSumWeights(v);
			}
			if (XR.isBefore(x1, v)) {
				return accWN.getSumWeights(x2) + calc(XR.next(x1), v)
						- accWN.getSumWeights(v);
			}
		}
		if (XR.route(x2) == XR.route(v)) {
			if (XR.isBefore(y2, v)) {
				return accWN.getSumWeights(x2) + calc(XR.next(x1), y1)
						+ calc(XR.next(y2), v) - accWN.getSumWeights(v);
			}
			if (XR.isBefore(x2, v)) {
				return accWN.getSumWeights(x1) + calc(XR.next(x2), v)
						- accWN.getSumWeights(v);
			}
		}
		return 0;
	...
	public double evaluateThreePointsMove(Point x1, Point x2, Point x3,
		...
		if (!XR.checkPerformThreePointsMove(x1, x2, x3, y1, y2, y3)) {
			System.out.println(name() + ":: Error evaluate ThreePointsMove: "
					+ x1 + " " + y1 + " " + x2 + " " + y2 + " " + x3 + " " + y3
					+ "\n" + XR.toString());
			System.exit(-1);
		}
	...
	public double evaluateFourPointsMove(Point x1, Point x2, Point x3,
		...
		if (!XR.checkPerformFourPointsMove(x1, x2, x3, x4, y1, y2, y3, y4)) {
			System.out.println(name() + ":: Error evaluate FourPointsMove: "
					+ x1 + " " + y1 + " " + x2 + " " + y2 + " " + x3 + " " + y3
					+ " " + x4 + " " + y4 + "\n" + XR.toString());
			System.exit(-1);
		}
	...
	public double evaluateAddRemovePoints(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(z)) {
			...
			if (XR.route(x) == XR.route(v)) {
				if (v == x) {
					return -(accWN.getSumWeights(XR.prev(x)) + accWN
							.getWeights(x));
				}
				if (XR.isBefore(x, v)) {
					return -accWN.getWeights(x);
