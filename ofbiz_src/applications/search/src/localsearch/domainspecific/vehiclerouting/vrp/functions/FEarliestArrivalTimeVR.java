...
public class FEarliestArrivalTimeVR implements IFunctionVR {
	...
	public void propagateTwoOptMoveOneRoute(Point x, Point y) {
		System.out.println(name()
				+ "::propagateTwoOptMoveOneRoute, NOT IMPLEMENTED");
		System.exit(-1);
	}
	...
	double calDeltaSegment(Point begin, Point end) {
		...
		Point v = begin;
		int delta = 0;
		double dt = eat.getEarliestArrivalTime(v) + eat.getServiceDuration(v);
	...
	public double evaluateOnePointMove(Point x, Point y) {
		...
		if (kx == ky) {
			...
			if (kx == kv) {
				...
				while (v != XR.getTerminatingPointOfRoute(kx)) {
					nv = XR.next(v);
					at = at + eat.getServiceDuration(v)
							+ eat.getTravelTime(v, nv);
					if (at < eat.getEarliestAllowedArrivalTime(nv))
						at = eat.getEarliestAllowedArrivalTime(nv);
					if (nv == calPoint)
						return at - earliestArrivalTime;
	...
	public double evaluateTwoOptMove1(Point x, Point y) {
		...
		delta += calDeltaSegment(XR.getTerminatingPointOfRoute(kx),
				XR.getTerminatingPointOfRoute(ky));
	...
	public double evaluateTwoOptMove3(Point x, Point y) {
		...
		delta += calDeltaSegment(x, XR.getStartingPointOfRoute(ky));
		delta += calDeltaSegment(XR.getTerminatingPointOfRoute(ky),
				XR.getTerminatingPointOfRoute(kx));
	...
	public double evaluateThreeOptMove2(Point x, Point y, Point z) {
		...
		int delta = 0;
		delta += calDeltaSegment(XR.getTerminatingPointOfRoute(k),
				XR.getStartingPointOfRoute(k));
		return delta;
	}
	...
	private double evaluateDeltaRoute(int k, Set<Point> out,
			ArrayList<Point> in, ArrayList<Point> preIn) {
