...
public class TotalCostVR implements IFunctionVR {
	...
	public void propagateTwoOptMoveOneRoute(Point x, Point y){
		if(!XR.checkPerformTwoOptMoveOneRoute(x, y)){
			System.out.println(name() + "::propagateTwoOptMoveOneRoute, check failed");
			System.exit(-1);
		}
		int k = XR.oldRoute(x);
	...
	public void propagateTwoOptOneRouteMove(Point x, Point y) {
		// x is before y on route k (k = 1,2,...)
	...
	public double evaluateTwoOptMoveOneRoute(Point x, Point y) {
		if(!XR.checkPerformTwoOptMoveOneRoute(x, y)){
			System.out.println(name() + "::evaluateTwoOptMoveOneRoute, check failed");
			System.exit(-1);
		}
		//int idx = getIndex(x);
		//int idy = getIndex(y);
