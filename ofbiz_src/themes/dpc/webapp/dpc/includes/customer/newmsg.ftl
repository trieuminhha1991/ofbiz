...
<div class="screenlet">
    <div class="screenlet-title-bar">
        <div class="marginbottom5">
            <#if showMessageLinks?default("false")?upper_case == "TRUE">
                <a class="button" href="<@ofbizUrl>messagelist</@ofbizUrl>" class="submenutextright">${uiLabelMap.EcommerceViewList}</a>
            </#if>
        </div>
        <h2>${pageHeader}</h2>
    </div>
    <div >
      <form name="contactus" method="post" action="<@ofbizUrl>${submitRequest}</@ofbizUrl>" style="margin: 0;">
        ...
        <table width="100%" border='0' cellspacing='2' cellpadding='0' class='boxbottom'>
          <tr>
            <td colspan="3">&nbsp;</td>
          </tr>
          <tr>
            ...
            <td><div>&nbsp;${sessionAttributes.autoName?if_exists} [${userLogin.partyId}] (${uiLabelMap.CommonNotYou}?&nbsp;<a class="linkcolor" href="<@ofbizUrl>autoLogout</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonClickHere}</a>)</div></td>
          </tr>
          ...
          <tr>
            <td colspan="2">&nbsp;</td>
            <td><input type="submit" class="button" value="${uiLabelMap.CommonSend}"/></td>
          </tr>
        </table>
      </form>
    </div>
