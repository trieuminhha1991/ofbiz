    ...
    <#assign partyNameTo = "${uiLabelMap.CommonNA}">
              ...
              <tr>
                ...
                <td align="right">
                  ...
                  <a class="button" href="javascript:document.ecomm_read_mess${index}.submit()">${uiLabelMap.EcommerceRead}</a>

                  <#if isSentMessage>
                  <form method="post" action="<@ofbizUrl>newmessage</@ofbizUrl>" name="ecomm_sent_mess${index}">
                    <input name="communicationEventId" value="${communicationEvent.communicationEventId}" type="hidden"/>
                  </form>
                  <a class="button" href="javascript:document.ecomm_sent_mess${index}.submit()">${uiLabelMap.PartyReply}</a>
                  </#if>
                </td>
              </tr>
</#macro>
    ...
    <div class="screenlet-title-bar">
        <div class="boxlink">
            <#if parameters.showSent?if_exists == "true">
              <a href="<@ofbizUrl>messagelist</@ofbizUrl>" class="button">${uiLabelMap.EcommerceViewReceivedOnly}</a>
            <#else>
              <a href="<@ofbizUrl>messagelist?showSent=true</@ofbizUrl>" class="button">${uiLabelMap.EcommerceViewSent}</a>
            </#if>
        </div>
        <br />
        <h2>${uiLabelMap.CommonMessages}</h2>
    </div>
    <div class="cartdiv">
