    ...
    <div class="screenlet-title-bar">
        <div class="marginbottom5">
            <#if (communicationEvent.partyIdFrom?if_exists != (userLogin.partyId)?if_exists)>
              <a class="button" href="<@ofbizUrl>newmessage?communicationEventId=${communicationEvent.communicationEventId}</@ofbizUrl>" class="submenutext">${uiLabelMap.PartyReply}</a>
            </#if>
            <a class="button" href="<@ofbizUrl>messagelist</@ofbizUrl>" class="submenutextright">${uiLabelMap.EcommerceViewList}</a>
        </div>
        <h2>${uiLabelMap.EcommerceReadMessage}</h2>
    </div>
    <div class="cartdiv">
