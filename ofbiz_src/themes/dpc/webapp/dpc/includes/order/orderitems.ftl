...
<hr />
  ...
  <#if maySelectItems?default("N") == "Y" && roleTypeId?if_exists == "PLACING_CUSTOMER">
      ...
      <a href="javascript:document.addCommonToCartForm.add_all.value='true';document.addCommonToCartForm.submit()" class="button">${uiLabelMap.OrderAddAllToCart}</a>
      <a href="javascript:document.addCommonToCartForm.add_all.value='false';document.addCommonToCartForm.submit()" class="button">${uiLabelMap.OrderAddCheckedToCart}</a>
      <a href="<@ofbizUrl fullPath="true">createShoppingListFromOrder?orderId=${orderHeader.orderId}&amp;frequency=6&amp;intervalNumber=1&amp;shoppingListTypeId=SLT_AUTO_REODR</@ofbizUrl>" class="button">${uiLabelMap.OrderSendMeThisEveryMonth}</a>
  </#if>
	...
	<div class="checkout-product">
		  <table class="margin10">
		    ...
		    <tfoot>
			    ...
			    <tr>
			      <td class="tableboldtext" colspan="7">${uiLabelMap.OrderGrandTotal}:</td>
			      <td>
		    ...
		    <tbody>
			    <#list orderItems as orderItem>
			      ...
			      <#if maySelectItems?default("N") == "Y" && (orderHeader.statusId != "ORDER_SENT" && orderItem.statusId != "ITEM_COMPLETED" && orderItem.statusId != "ITEM_CANCELLED" && pickedQty == 0)>
			        <tr>
			          <td colspan="7"><div class="divinline">${uiLabelMap.OrderReturnReason}</div>
			            ...
			            <div class="divinline">${uiLabelMap.CommonComments}</div>
			            <input class="inputBox" type="text" name="icm_${orderItem.orderItemSeqId}" value="" size="30" maxlength="60"/>
			          </td>
			          <td colspan="4"><a href="javascript:document.addCommonToCartForm.action='<@ofbizUrl>cancelOrderItem</@ofbizUrl>';document.addCommonToCartForm.submit()" class="button">${uiLabelMap.CommonCancel}</a>
