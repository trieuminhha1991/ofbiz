...
( function(a) {
		...
		a.extend(a.jqx._jqxWindow.prototype, {
			...
			_fixWindowZIndex : function(m) {
				var e = a.data(document.body, "jqxwindows-list") || [], n = this.zIndex, h;
				if (!this.isModal) {
			...
			_validateMinSize : function() {
				...
				var h = this._header.children(0).outerWidth(true), e = this._header.children(1).outerWidth(true), g = h + e;
			...
			_centerElementInParent : function(e, m, h, f) {
				...
				if (h.indexOf("y") >= 0) {
					var g = e.outerHeight(true), k = m.height(), i = (Math.max(0, k - g)) / 2;
					e.css(f + "top", i + "px")
				}
				if (h.indexOf("x") >= 0) {
			...
			_addDragDropHandlers : function() {
				if (this.draggable) {
					...
					this.addHandler(this._header, this._getEvent("mousedown"), function(i) {
			...
			_dropHandler : function(g, k) {
				...
				if (g._isDragging && !g.isResizing && !g._resizeDirection) {
					var e = parseInt(g.host.css("left"), 10), l = parseInt(g.host.css("top"), 10), i = (g._isTouchDevice) ? 0 : k.pageX, h = (g._isTouchDevice) ? 0 : k.pageY;
			...
			_dragHandler : function(p, h) {
				if (p._isDragging && !p.isResizing && !p._resizeDirection) {
					...
					var k = n.left, i = n.top, g = k - p._mousePosition.x, f = i - p._mousePosition.y, e = p._windowPosition.x + g, q = p._windowPosition.y + f;
					p.move(e, q, h);
					h.preventDefault();
					return false
				}
			...
			collapse : function(g) {
				if (!this.collapsed && !this.host.is(":animated")) {
					...
					var e = this, h = this._header.outerHeight(true), i = parseInt(this._header.css("border-bottom-width"), 10), f = parseInt(this._header.css("margin-bottom"), 10), g = !isNaN(parseInt(g)) ? g : this.collapseAnimationDuration;
					if (!isNaN(i)) {
						h -= 2 * i
					}
					if (!isNaN(f)) {
			...
			expand : function(f) {
				if (this.collapsed && !this.host.is(":animated")) {
					var e = this, f = !isNaN(parseInt(f)) ? f : this.collapseAnimationDuration;
					this.host.animate({
			...
			closeAll : function(h) {
				var h = true;
				var g = a.data(document.body, "jqxwindows-list"), f = g.length, e = a.data(document.body, "jqxwindow-modal") || [];
				while (f) {
					f -= 1;
			...
			bringToFront : function() {
				...
				var l = f[f.length - 1], k = parseInt(l.css("z-index"), 10), g = this._indexOf(this.host, f);
			...
			move : function(q, p, e, h) {
				var g = 0, f = 0, n, k, i, q = parseInt(q, 10), p = parseInt(p, 10);
				if (a.jqx.browser.msie) {
		...
		var a = ( function(c) {
				return {
					...
					_validateHeight : function(f, k, i) {
						var l = 0, e = 2, d = false, h = f, g = this._getParent();
					...
					_validateWidth : function(k, i, h) {
						var l = 0, e = 2, d = false, g = k, f = this._getParent();
					...
					_resizeRight : function(e) {
						var f = e.element.lastWidth + e.x, d = e.self._validateResize(f, 0, "horizontal", e.element, "right");
						if (!d.result) {
							f = d.fix
						}
					...
					_resizeLeft : function(f) {
						var g = f.element.lastWidth - f.x, e = f.self._validateResize(g, 0, "horizontal", f.element, "left"), d = f.element.x + f.x;
						if (!e.result) {
					...
					_resizeBottom : function(f) {
						var e = f.element.lastHeight + f.y, d = f.self._validateResize(0, e, "vertical", f.element, "bottom");
						if (!d.result) {
							e = d.fix
						}
					...
					_resizeTop : function(f) {
						var e = f.element.lastHeight - f.y, d = f.self._validateResize(0, e, "vertical", f.element, "top"), g = f.element.y + f.y;
						if (!d.result) {
					...
					_raiseResizeEvent : function(f) {
						...
						g.args = d;
						if (f == 0) {
							var e = this._resizeEvents[2], h = c.Event(e);
							h.args = d;
