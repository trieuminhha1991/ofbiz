...
var GridClass = function() {
		...
		var initGrid = function(config, datafields, columns, formatData, grid) {
			...
			grid.jqxGrid({
				...
				showstatusbar : typeof(config.showstatusbar) != 'undefined' ? config.showstatusbar : true,
		...
		var initSource = function(allconfig, url, datafields, grid) {
			...
			if(typeof(allconfig.virtualmode) != undefined && allconfig.virtualmode == false){
		...
		var deleteRow = function(grid, url, data, commit, callback) {
			sendRequest(grid, url, data, commit, wgdeletesuccess, callback);
		};
		...
		var renderMessage = function(id, message, template) {
			...
			if(!containernotify.length){
				var str = "<div id='jqxNotification"+id+"'><div id='notificationContent"+id+"'></div></div>";
				$('#' + id).prepend(str);
		...
		createAddRowButton = function(grid, container, label, popup){
			...
			grid.jqxGrid('addRow', null, popup.data, "first");
                    grid.jqxGrid('clearSelection');
...
//            	fix config to hidden column by hoangominh
		...
		var initGridNotificationContainer = function(grid){
			var id = grid.attr('id');
			var tmpWidth = grid.jqxGrid('width');
...
var formatcurrency = function(num, uom){
    ...
    return((formatted ? formatted : "0") + ((parts) ? decimalseparator + parts[1].substr(0, 2) : "") + " " + currencysymbol);
};
var formatnumber = function(num, uom){
    if(num == null){
        return "";
    }
    decimalseparator = ",";
