...
(function(b) {
    ...
    b.extend(b.jqx._jqxDataTable.prototype, {
        ...
        createInstance: function(e) {
            ...
            if (l.element.nodeName.toLowerCase() == "table") {
            ...
            if (e !== true) {
                ...
                if (p) {
                    ...
                    for (var w = 0; w < p.length; w++) {
                        var f = p[w].editor;
                        var c = p[w].column;
                        var t = k._geteditorvalue(c, f, j, q);
                        if (c.validation) {
                            f.removeClass(k.toThemeProperty("jqx-grid-validation-label"));
                            var o = c.datafield;
                            try {
                                var s = c.validation({
                                    value: t,
                                ...
                                var l = k.gridlocalization.validationstring;
                                if (s.message != undefined) {
                                    l = s.message
                                }
                                var m = typeof s == "boolean" ? s : s.result;
                                if (!m) {
                                    if (s.showmessage == undefined || s.showmessage == true) {
                                        k._showvalidationpopup(z, o, l, f)
                                    }
                                    d = false
                                }
                            } catch (u) {
                                k._showvalidationpopup(z, o, k.gridlocalization.validationstring, f);
                                d = false
                            }
                        }
                    }
                    if (d) {
                        ...
                        for (var w = 0; w < p.length; w++) {
                            ...
                            if (k.editorsCache) {
                                ...
                                ;
                                var B = "customeditor_" + c.displayfield + "_" + E;
                                var A = "templateeditor_" + c.displayfield;
                                if (k.editorsCache[B]) {
                                    var D = b(k.editorsCache[B]).data();
                                    y(D);
                                    k.editorsCache[B].detach()
                                }
                                if (k.editorsCache[A]) {
                                    k.editorsCache[A].detach();
                                    var D = b(k.editorsCache[A]).data();
                                    y(D)
                                }
                            }
                            if (t != r) {
                ...
                k.updaterowbykey(k.editKey, h, false)
            }
            if (k.editorsCache) {
                for (var f in k.editorsCache) {
                    var D = b(k.editorsCache[f]).data();
                    if (D && D.jqxWidget) {
                        var v = D.jqxWidget.element.className;
                        if (v.indexOf("dropdownlist") >= 0 || v.indexOf("datetimeinput") >= 0 || v.indexOf("combobox") >= 0 || v.indexOf("menu") >= 0) {
                            if (D.jqxWidget.isOpened) {
                                var x = D.jqxWidget.isOpened();
                                if (x) {
                                    D.jqxWidget.close()
                                }
                            }
                        }
                        b(k.editorsCache[f]).detach()
                    }
                }
            }
            k.beginupdate();
            var C = k.getrowdisplayindex(h);
            if (k.editSettings.editSingleCell) {
                var h = k.rowsByKey[E];
                var t = null ;
                var g = null ;
                if (h) {
                    t = h[k.editColumn.datafield];
                    g = h[k.editColumn.displayfield]
                }
                k._raiseEvent("cellEndEdit", {
                    value: t,
                    displayValue: g,
                    key: E,
                    index: C,
                    dataField: k.editColumn.datafield,
                    displayField: k.editColumn.displayfield,
                    boundIndex: j,
                    row: k.rowsByKey[E]
                })
            }
            k._raiseEvent("rowEndEdit", {
                key: E,
                index: C,
                boundIndex: j,
                row: k.rowsByKey[E]
        ...
        updaterowbykey: function(h, j, f) {
            if (h != undefined && j != undefined) {
                ...
                var d = function(n, k, s) {
                    if (n._loading) {
                        throw new Error("jqxDataTable: " + n.loadingerrormessage);
                        return false
                    }
                    var q = false;
                    if (!b.isArray(k)) {
                        ...
                        var r = function(u) {
                            if (!u) {
                                ...
                                for (var t = 0; t < n.columns.records.length; t++) {
                                    ...
                                    if (n.groups.length > 0 && u.originalRecord) {
        ...
        showdetailsbykey: function(k) {
            ...
            if (l) {
                ...
                if (e) {
                    ...
                    if (!this.rtl) {
                        g.removeClass(this.toThemeProperty("jqx-grid-group-collapse"));
                        g.addClass(this.toThemeProperty("jqx-grid-group-expand"));
                        g.removeClass(this.toThemeProperty("jqx-icon-arrow-right"));
                        g.addClass(this.toThemeProperty("jqx-icon-arrow-down"))
                    } else {
                        g.removeClass(this.toThemeProperty("jqx-grid-group-collapse-rtl"));
                        g.addClass(this.toThemeProperty("jqx-grid-group-expand-rtl"));
                        g.removeClass(this.toThemeProperty("jqx-icon-arrow-left"));
                        g.addClass(this.toThemeProperty("jqx-icon-arrow-down"))
                    }
                    e.next().show();
                    var i = h.next();
        ...
        _renderrows: function() {
            ...
            var al = b.jqx.browser.msie && b.jqx.browser.version < 8;
            if (al) {
                this.host.attr("hideFocus", "true")
            }
            var O = Z.groups.length;
            var ag = new Array();
            ...
            if (Z.groups && Z.groups.length > 0) {
                var M = this.pageable ? this.dataview.pagesize * this.dataview.pagenum : 0;
                var u = Z.source.getGroupedRecords(Z.groups, "records", "label", null , "data", null , "parent", s, M);
                var v = function(j, w) {
                    for (var aq = 0; aq < j.length; aq++) {
                        var c = j[aq];
                        w.push(c);
                        if (c.records && c.records.length > 0) {
                            var ar = v(c.records, new Array());
            ...
            var ag = s;
            this.renderedRecords = ag;
            var Y = ag.length;
            var S = this.tableZIndex;
            var am = 0;
            var X = 0;
            if (al) {
                for (var aj = 0; aj < H; aj++) {
                    var W = this.columns.records[aj];
                    var e = W.width;
                    if (e < W.minwidth) {
                        e = W.minwidth
                    }
                    ...
                    var l = h.find("td");
                    am = 1 + parseInt(l.css("padding-left")) + parseInt(l.css("padding-right"));
                    h.remove();
                    break
                }
            }
            var ai = this.rtl ? " " + this.toTP("jqx-grid-table-rtl") : "";
            var y = "<table cellspacing='0' cellpadding='0' class='" + this.toTP("jqx-grid-table") + ai + "' id='table" + this.element.id + "'><colgroup>";
            var an = "<table cellspacing='0' cellpadding='0' class='" + this.toTP("jqx-grid-table") + ai + "' id='pinnedtable" + this.element.id + "'><colgroup>";
            var F = 0;
            var r = H;
            for (var aj = 0; aj < H; aj++) {
                var W = this.columns.records[aj];
                ...
                if (al) {
                    ...
                    if (W.rowdetailscolumn) {
                        var aa = aa + 4;
                        an += "<col style='max-width: " + aa + "px; width: " + aa + "px;'>";
                        y += "<col style='max-width: " + aa + "px; width: " + aa + "px;'>"
                    } else {
                        an += "<col style='max-width: " + aa + "px; width: " + aa + "px;'>";
                        y += "<col style='max-width: " + aa + "px; width: " + aa + "px;'>"
                    }
                } else {
                    an += "<col style='max-width: " + e + "px; width: " + e + "px;'>";
            ...
            if (Y === 0) {
                ...
                if (this.filterable) {
                    var ae = this.filter.find(".filterrow");
                    var A = this.filter.find(".filterrow-hidden");
                    var z = 1;
                    if (A.length > 0) {
                        z = 0
                    }
                    k -= this.filterheight + this.filterheight * ae.length * z
                }
                if (this.showstatusbar) {
                ...
                var h = '<td colspan="' + this.columns.records.length + '" role="gridcell" style="border: none; min-height: ' + k + "px; height: " + k + "px; max-width:" + X + "px; width:" + X + "px;";
                ...
                y += n;
                an += n;
                this.table[0].style.width = X + 2 + "px";
                Q = X
            }
            var p = this.getRows();
            for (var ak = 0; ak < ag.length; ak++) {
                ...
                var n = '<tr data-key="' + I + '" role="row" id="row' + ak + this.element.id + '">';
                var B = '<tr data-key="' + I + '" role="row" id="row' + ak + this.element.id + '">';
                if (O > 0) {
                    if (D.level < O) {
                        var n = '<tr data-role="row-group" data-key="' + I + '" role="row" id="row' + ak + this.element.id + '">';
                        var B = '<tr data-role="row-group" data-key="' + I + '" role="row" id="row' + ak + this.element.id + '">'
                    }
                }
                var o = 0;
                var P = 0;
                for (var aj = 0; aj < H; aj++) {
                    var ac = this.columns.records[aj];
                    if (ac.pinned || (this.rtl && this.columns.records[H - 1].pinned)) {
                        f = true
                    }
                    var e = ac.width;
                    if (e < ac.minwidth) {
                    ...
                    if (this.sortcolumn === ac.displayfield) {
                        E += " " + this.toTP("jqx-grid-cell-sort")
                    }
                    if (this.altrows && ak % 2 != 0) {
                    ...
                    var ad = '<td role="gridcell" ' + L + ' style="pointer-events: none; visibility: hidden; border-color: transparent; max-width:' + e + "px; width:" + e + "px;";
                    if (aj == H - 1 && H == 1) {
                        h += "border-right-color: transparent;";
                        ad += "border-right-color: transparent;"
                    }
                    var V = false;
                    if (O > 0) {
                        if (D.level < O) {
                    ...
                    if (!V) {
                        if (ac.cellsalign != "left") {
                            if (ac.cellsalign === "right") {
                    ...
                    if (this.rowinfo[D.uid]) {
                        if (this.rowinfo[D.uid].selected && !ac.rowdetailscolumn) {
                    ...
                    if (O > 0 && al && V && aj >= O) {
                        ...
                        ad += "font-size: 1px; border-color: transparent; color: transparent;"
                    }
                    if (ac.pinned) {
                        h += "pointer-events: auto;";
                        ad += "pointer-events: auto;"
                    }
                    if (!V && ac.rowdetailscolumn) {
                        if (this.rowinfo[D.uid]) {
                            if (!this.rowinfo[D.uid].nodetails) {
                                if (this.rowinfo[D.uid].expanded) {
                                    ...
                                    E += " " + this.toTP("jqx-icon-arrow-down")
                                } else {
                                    if (!this.rtl) {
                                        E += " " + this.toTP("jqx-grid-group-collapse");
                                        E += " " + this.toTP("jqx-icon-arrow-right")
                                    } else {
                                        E += " " + this.toTP("jqx-grid-group-collapse-rtl");
                                        E += " " + this.toTP("jqx-icon-arrow-left")
                                    }
                                }
                            }
                        } else {
                    ...
                    var ah = Z._getcellvalue(ac, D);
                    var R = ac.cellsformat;
                    if (O > 0) {
                        if (D.level < O) {
                            ah = D.label;
                            R = Z.getColumn(Z.groups[D.level]).cellsformat
                        }
                    }
                    if (R != "") {
                        if (b.jqx.dataFormat) {
                            if (b.jqx.dataFormat.isDate(ah)) {
                                ah = b.jqx.dataFormat.formatdate(ah, R, Z.gridlocalization)
                            } else {
                                if (b.jqx.dataFormat.isNumber(ah) || (!isNaN(parseFloat(ah)) && isFinite(ah))) {
                                    ah = b.jqx.dataFormat.formatnumber(ah, R, Z.gridlocalization)
                                }
                            }
                        }
                    }
                    if (!V) {
                        if (ac.cellclassname != "" && ac.cellclassname) {
                            if (typeof ac.cellclassname == "string") {
                                E += " " + ac.cellclassname
                            } else {
                                var q = ac.cellclassname(ak, ac.datafield, ah, D);
                                if (q) {
                                    E += " " + q
                                }
                            }
                        }
                        if (ac.cellsrenderer != "" && ac.cellsrenderer) {
                            var d = p.indexOf(D);
                            ah = ac.cellsrenderer(d, ac.datafield, ah, D)
                        }
                    } else {
                        if (this.groupsrenderer) {
                            ah = this.groupsrenderer(ah, D, D.level)
                        }
                    }
                    h += '" class="' + E + '">';
                    h += ah;
                ...
                if (this.rowdetails) {
                    ...
                    if (this.rowinfo[D.uid]) {
                        if (this.rowinfo[D.uid].expanded) {
                            var af = '<tr data-role="row-details"><td valign="top" style="pointer-events: auto; overflow: hidden; min-height: ' + C + "px; max-height: " + C + "px; height: " + C + 'px; overflow: hidden; border-left: none; border-right: none;" colspan="' + this.columns.records.length + '" role="gridcell"'
                        } else {
                            var af = '<tr data-role="row-details" style="display: none;"><td valign="top" style="pointer-events: auto; overflow: hidden; min-height: ' + C + "px; max-height: " + C + "px; height: " + C + 'px; overflow: hidden; border-left: none; border-right: none;" colspan="' + this.columns.records.length + '" role="gridcell"'
                        }
                    } else {
                        var af = '<tr data-role="row-details" style="display: none;"><td valign="top" style="pointer-events: auto; overflow: hidden; min-height: ' + C + "px; max-height: " + C + "px; height: " + C + 'px; overflow: hidden; border-left: none; border-right: none;" colspan="' + this.columns.records.length + '" role="gridcell"'
                    }
                    ...
                    af += '" class="' + E + '"><div style="pointer-events: auto; overflow: hidden; min-height: ' + C + "px; max-height: " + C + "px; height: " + C + 'px;"><div data-role="details"></div></div></td></tr>';
                    y += af;
                    an += af
                }
            }
            ...
            if (f) {
                if (Z.WinJS) {
                    ...
                    Z.table[0].innerHTML = an + y
                }
                var T = this.table.find("#table" + this.element.id);
                var U = this.table.find("#pinnedtable" + this.element.id);
                U.css("float", "left");
                U.css("pointer-events", "none");
                T.css("float", "left");
                U[0].style.position = "absolute";
                T[0].style.position = "relative";
                T[0].style.zIndex = S - 10;
                U[0].style.zIndex = S + 10;
                this._table = T;
                ...
                if (Z.WinJS) {
                    ...
                    Z.table[0].innerHTML = y
                }
                var ab = this.table.find("#table" + this.element.id);
                this._table = ab;
                if (b.jqx.browser.msie && b.jqx.browser.version < 10) {
                    this._table[0].style.width = Q + "px"
                }
        ...
        setcolumnindex: function(e, i, j) {
            ...
            if (this.filterrow) {
                b(this.filterrow.children()[0]).children().detach();
            ...
            b.each(this.columns.records, function(o, r) {
                ...
                if (m.filterrow) {
                    var s = b('<div style="overflow: hidden; position: absolute; height: 100%;" class="' + k + '"></div>');
                    c.append(s);
                    s.css("left", f);
                    s.css("z-index", n + 1);
                    s.width(this.width);
                    s[0].left = f;
                    s.append(this._filterwidget);
        ...
        _handlecolumnsreorder: function() {
            ...
            this.addHandler(b(document), f, function(k) {
                if (g.resizing) {
                    return true
                }
        ...
        _rendercolumnheaders: function() {
            ...
            b.each(this.columns.records, function(J, H) {
                ...
                if (L.resizable && r.columnsresize && !I) {
                    var y = false;
                    var j = "mousemove";
                    if (r.isTouchDevice()) {
                        y = true;
