			...
			for (var t = 0; t < z; t++) {
				...
				case "list":
					...
					if (g.selectedIndex > 0) {
						var b = g.selectedValue;
						//g.getItem(g.selectedIndex);
						var p = b;
						var o = "equal";
						if (p === "") {
							o = "NULL"
		...
		_addfilterwidget : function(C, d, A) {
			...
			case "olbiusdropgrid":
				if (this.host.jqxDropDownButton) {
					...
					var t = a("<input placeHolder='" + this.gridlocalization.notificationfiltershortkey + "'  id='input" + tmpColName + "' style='height:21px;'/>");
					//<div id='jqxgrid" + C.datafield + "'></div>
		...
		_updatefilterpanel : function(r, o, D) {
			...
			if (this._hasdatefilter) {
				...
				if (D.filtertype == "date") {
					...
					var m = function(c) {
						var i = {
							calendar : r.gridlocalization,
							todayString : r.gridlocalization.todaystring,
							clearString : r.gridlocalization.clearstring
						};
						c.jqxDateTimeInput({
							disabled : r.disabled,
							localization : i,
							rtl : r.rtl,
							width : r._filterpanelwidth - 15,
							height : 20,
							value : null,
