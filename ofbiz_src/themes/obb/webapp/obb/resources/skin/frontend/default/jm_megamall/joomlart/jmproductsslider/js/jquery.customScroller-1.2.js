...
(function ($) {
	...
	$.fn.customScroller = function (options) {
		...
		if (this.length > 0) {
			...
			this.each(function (index, domElement) {
				...
				_that[_uniqueID].objOItem = $(".jm-slider-li", this);
							...
							if (this.id !== undefined && this.id.length > 0) {
								...
								if (_that[id] && $(_that[id].div).length > 0) {
									...
									if (_that[id].vertical === 1 && _that[id]._vscroll === false && _that[id].objContainer.height() > 0 && _that[id].objContent.outerHeight(true) > _that[id].objContainer.height()) {
										...
										$(_that[_uniqueID].div).addClass('has-vscrollbar');
									...
									if (_that[id].horizontal === 1 && _that[id]._oscroll === false && _that[id].objContainer.width() > 0 && _that[id].objContent.outerWidth(true) > _that[id].objContainer.width()) {
										...
										$(_that[_uniqueID].div).addClass('has-hscrollbar');

										// SET CONTAINER HEIGHT
										_that[id].objContainer.css('height', _that[id].objContainer.height() + 'px');
										...
										// ACTUAL TRACE POSITION right
										_that[id].posOrTrace = _that[id].objOTrace.offset().right;
									...
									if ($.browser.msie) {
										_that[id].objContent.css({
																   width: _that[id].objContent.width() + 'px'
																   });
									}
				...
				_that[_uniqueID].objOTrace.bind("mousedown", function (e) {
					//alert( e.pageX +', '+ e.pageY + " - " + _that[_uniqueID].posOTrace);
					objOCursorWidth = _that[_uniqueID].objOCursor.width();
					if((e.pageX - _that[_uniqueID].posOTrace - objOCursorWidth)>0){
						moveDimension = e.pageX - _that[_uniqueID].posOTrace - objOCursorWidth;
						//alert(e.pageX +"-"+ _that[_uniqueID].posOTrace + "-" + objOCursorWidth + "=" + moveRight);
					}
					//if(e.pageX < _that[_uniqueID].posOTrace){
					//	moveDimension2 =  _that[_uniqueID].posOTrace - e.pageX;
					//}
					cursorLeftPos = _that[_uniqueID].objOCursor.offset().left;
					//alert(cursorLeftPos+"-"+e.pageX);
					if(e.pageX < cursorLeftPos){
						moveDimension =  cursorLeftPos - e.pageX;
					}
					/*var increment;
					...
					if (spanX > (_that[_uniqueID].cursorWidth + parseInt(_that[_uniqueID].objOCursor.css('left'), 10))) {
						_clickMoveRight(_that[_uniqueID], moveDimension);
					} else if (spanX < parseInt(_that[_uniqueID].objOCursor.css('left'), 10)) {
						_clickMoveLeft(_that[_uniqueID], moveDimension);
					}

					return false;
				});

				_that[_uniqueID].objLeft.bind("mousedown", function (e) {
					$('span', this).addClass('hover');
				...
				_that[_uniqueID].objLeft.bind("mouseup", function (e) {
					$('span', this).removeClass('hover');
					window.clearTimeout(_delay);
					_stopMove();
					_clickLeft(_that[_uniqueID]);
					return false;
				});


				_that[_uniqueID].objRight.bind("mousedown", function (e) {
					$('span', this).addClass('hover');
				...
				_that[_uniqueID].objRight.bind("mouseup", function (e) {
					$('span', this).removeClass('hover');
					window.clearTimeout(_delay);
					//alert(_delay);
					_stopMove();
					_clickRight(_that[_uniqueID]);
					return false;
				});
	...
	var _clickRight = function (objCopy) {
		var itemWidth = objCopy.objOItem.width();
		var scrollMoveDim = parseInt((itemWidth*objCopy.traceOVoid)/(objCopy.contentWidth-objCopy.containerWidth),10);
		//var cursorLeft = scollMoveDim + objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;
		var dimRight = parseInt(objCopy.objOTrace.width() + objCopy.objOTrace.offset().left - objCopy.objOCursor.offset().left - objCopy.objOCursor.width(),10);
		var itemRight = objCopy.objContent.width()- objCopy.containerWidth - (objCopy.objContainer.offset().left - objCopy.objContent.offset().left)

		if(dimRight/scrollMoveDim > 0.1){
			if((dimRight%scrollMoveDim)/scrollMoveDim > 0.1){
				//cursorLeft = scollMoveDim + objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;
				var cursorLeft = parseInt(objCopy.objOCursor.css('left'),10)+parseInt(dimRight%scrollMoveDim,10);
				//objCopy.objOCursor.css('left', cursorLeft + 'px');
				 objCopy.objOCursor.animate({"left": cursorLeft}, "slow");
				if((itemRight%itemWidth)/itemWidth > 0.1){
					var delright = parseInt(objCopy.objContent.css('left'),10)-parseInt(itemRight%itemWidth);
				}else{
					var delright = parseInt(objCopy.objContent.css('left'),10)-itemWidth-parseInt(itemRight%itemWidth);
				}
				//objCopy.objContent.css('left', delright + 'px');
				objCopy.objContent.animate({"left": delright}, "slow");
			}else{
				var cursorLeft = parseInt(objCopy.objOCursor.css('left'),10)+parseInt(dimRight%scrollMoveDim)+scrollMoveDim;
				//objCopy.objOCursor.css('left', cursorLeft + 'px');
				objCopy.objOCursor.animate({"left": cursorLeft}, "slow");
				if((itemRight%itemWidth)/itemWidth > 0.1){
					var delright = parseInt(objCopy.objContent.css('left'),10)-parseInt(itemRight%itemWidth);
	...
	var _clickLeft = function (objCopy) {
		var itemWidth = objCopy.objOItem.width();
		var scollMoveDim = (itemWidth*objCopy.traceOVoid)/(objCopy.contentWidth-objCopy.containerWidth);
		var cursorLeft = -scollMoveDim + objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;

		var dimLeft =  objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;

		if(dimLeft/scollMoveDim>=1){
			if((dimLeft%scollMoveDim)/scollMoveDim>0.1){
				cursorLeft = -(dimLeft%scollMoveDim) + objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;
				//objCopy.objOCursor.css('left', cursorLeft + 'px');
				objCopy.objOCursor.animate({"left": cursorLeft}, "slow");
				var delRight = parseInt((cursorLeft*(objCopy.contentWidth-objCopy.containerWidth)/objCopy.traceOVoid),10);
				//objCopy.objContent.css('left', -delRight + 'px');
				objCopy.objContent.animate({"left": -delRight + 'px'}, "slow");
			}else{
				//objCopy.objOCursor.css('left', cursorLeft + 'px');
				objCopy.objOCursor.animate({"left": cursorLeft + 'px'}, "slow");
				var delRight = parseInt((cursorLeft*(objCopy.contentWidth-objCopy.containerWidth)/objCopy.traceOVoid),10);
				//objCopy.objContent.css('left', -delRight + 'px');
				objCopy.objContent.animate({"left": -delRight + 'px'}, "slow");
			}
		}
		if((dimLeft/scollMoveDim>0)&&(dimLeft/scollMoveDim<1)){
			cursorLeft = -dimLeft + objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left;
			//objCopy.objOCursor.css('left', cursorLeft + 'px');
			objCopy.objOCursor.animate({"left": cursorLeft + 'px'}, "slow");
			var delRight = parseInt((cursorLeft*(objCopy.contentWidth-objCopy.containerWidth)/objCopy.traceOVoid),10);
		    //objCopy.objContent.css('left', -delRight + 'px');
	...
	var _clickMoveRight = function (objCopy, wheelDelta) {
		var scrolling = parseInt(objCopy.objContent.css('left'), 10);
		var maxScroll = ((0 - objCopy.contentWidth) + objCopy.containerWidth);
		if (scrolling >= maxScroll) {
			//objCopy.objOCursor.css('left', wheelDelta + 'px');
			objCopy.objOCursor.animate({"left": wheelDelta + 'px'}, "slow");
		    var delRight = parseInt((wheelDelta*(objCopy.contentWidth-objCopy.containerWidth)/objCopy.traceOVoid),10);
		    //objCopy.objContent.css('left', -delRight + 'px');
			objCopy.objContent.animate({"left": -delRight + 'px'}, "slow");
		}
	};

	var _clickMoveLeft = function (objCopy, wheelDelta) {
		var scrolling = parseInt(objCopy.objContent.css('left'), 10);
		if (scrolling <= 0) {
			var cusorMoveLeft = objCopy.objOCursor.offset().left - objCopy.objOTrace.offset().left - wheelDelta;
			//objCopy.objOCursor.css('left', cusorMoveLeft + 'px');
			objCopy.objOCursor.animate({"left": cusorMoveLeft + 'px'}, "slow");
			var addLeft = parseInt((cusorMoveLeft*(objCopy.contentWidth-20-objCopy.containerWidth)/objCopy.traceOVoid),10);
		    //objCopy.objContent.css('left', -addLeft + 'px');
			objCopy.objContent.animate({"left": -addLeft + 'px'}, "slow");
		}
	};
	...
	$.fn.dragCursor = function (id, which) {
		...
		if (_that[id] && $(_that[id].div).length > 0) {
			...
			if (this.length > 0) {
				this.each(function (index, domElement) {
					...
					$(this).bind("mousedown touchstart", function (e) {
						...
						_lastMouseX = e.originalEvent.pageX;
						_lastMouseY = e.originalEvent.pageY;
			...
			var updatePosition = function (e) {
				var spanX = (e.originalEvent.pageX - _lastMouseX);
				var spanY = (e.originalEvent.pageY - _lastMouseY);
