jQuery.noConflict();// JavaScript Document
(function($){
   var defaults  = {
	  listitem:{},
	  btnCart:".btn-cart",
...
//						    		$(e.currentTarget).parent().parent().remove();
	   ...
	   initialize:function(){
			...
			productlink = $(bcart).siblings(".add-to-links").find(".link-compare").attr("qv");
			    bcartparent = $(bcart).parent();
			    if((productlink != null) && (productlink != undefined)){
				productid = productlink.substring(productlink.lastIndexOf("=") + 1,productlink.length);
			    }
			    if((productlink != null) && (productlink != undefined) && (productlink.indexOf("productmaindetail") > -1) && !bcartparent.children("#quickviewbox"+productid).length ){
				quickviewtag = $("<a/>",{
							"rel": "quickviewbox",
							"href": "quickview/index",
							"id": "quickviewbox"+productid,
							"title": uiLabel.BEQuickView
					  });
					  quickviewtag.attr("href",baseurl+"products/"+productid+"");
					  quickviewtag.append(' <button class="form-button jmquickview"><span>' + uiLabel.BEQuickView + '</span></button>');
					  $(bcart).after(quickviewtag);
				      quickviewtag.colorbox({current: uiLabel.BEProductNo + " {current} " + uiLabel.BEIn +" {total} " + uiLabel.BEItems,onComplete:$.proxy(function(){
...
j(window).load(function($) {
    j("ul.products-grid").jmquickview({});
    j("div.products-grid").jmquickview({});
    j("table.products-grid").jmquickview({});
    j("ol.products-list").jmquickview({});
 });