		...
		<h3>
		  ...
		  <ul>
		    ...
		    <li>
		      ...
		      <#if orderHeader?has_content>
		        ${localOrderReadHelper.getCurrentStatusString(locale)}
		      <#else>
		        ...
		        <li>
		          <#if paymentMethodType.paymentMethodTypeId == "EXT_OFFLINE">
		            ...
		            <#if orderHeader?has_content && paymentAddress?has_content>
		              ...
		              ${uiLabelMap.ObbBeSureToIncludeYourOrderNb}
		            </#if>
		          <#else>
        ...
        <#if shippingAddress?has_content>
          <li>
            <ul>
              ...
              <li>
                <#assign shippingWardGeo = (delegator.findOne("Geo", {"geoId", shippingAddress.wardGeoId?if_exists}, false))?if_exists />
                <#if shippingWardGeo?has_content>${shippingWardGeo.geoName?if_exists}</#if>
	              <#assign shippingDistrictGeo = (delegator.findOne("Geo", {"geoId", shippingAddress.districtGeoId?if_exists}, false))?if_exists />
	              <#if shippingDistrictGeo?has_content>${shippingDistrictGeo.geoName?if_exists}</#if>
	              <#assign shippingStateGeo = (delegator.findOne("Geo", {"geoId", shippingAddress.stateProvinceGeoId?if_exists}, false))?if_exists />
	              <#if shippingStateGeo?has_content>, ${shippingStateGeo.geoName?if_exists}</#if>
